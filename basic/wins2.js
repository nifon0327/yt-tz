function lib_doc_size(){
	...
	this.y=0;
	this.y2=bw.ie && document.body.offsetHeight-5||innerHeight-12||0; 
	if(bw.ns6) 
	this.x2-=15; 
	this.x50=this.x2/2;
	this.y50=this.y2/2;
...
function b_clipIt(tstop,rstop,bstop,lstop,step,fn,wh,www){
	...
	if(!this.clipactive)
		this.clip(clipval[0],clipval[1],clipval[2],clipval[3],(tstop-clipval[0])/totantstep,(rstop-clipval[1])/totantstep,(bstop-clipval[2])/totantstep,(lstop-clipval[3])/totantstep,totantstep,0, fn,wh,www);
		}
...
function b_clip(tcurr,rcurr,bcurr,lcurr,tperstep,rperstep,bperstep,lperstep,totantstep,antstep,fn,wh,www){
	...
	if(www)
		this.clipTo(tcurr,rcurr,bcurr,lcurr,1); 
	else 
  ...
  var ended=new Date()  
	...
	oWin.currwins=i; if(is){isFront=1; setWindows()}
		...
		for(i=2;i<arguments.length;i+=4){
			...
			oWin[j].checkscroll(); oWin[j].showIt(); 
			if(arguments[i+1] == 620){ //Ad win && arguments[i+1] == 580
				oWin[j].resize(arguments[i],630)
				oWin[j].oButtons.hideIt()
				oWin[j].oHead.hideIt()
				oWin[j].oResize.hideIt()
				oWin[j].css.backgroundColor="white"
				oWin[j].oWindow.moveIt(0,0)
				oWin[j].oUp.hideIt()
				oWin[j].oDown.hideIt()
			}
			j++
...
function setWindows(){
	...
	for(var i=0;i<wins;i++){
		...
		oWin[i].origh=(winpage.y2-50-(between*oWin.rows))/oWin.rows 
		oWin[i].origx=oWin[i].origw*(j)+(between*j+1) +5
		oWin[i].origy=oWin[i].origh*c+27+(between*c)  + 3  
