...
ChangeWtitle("$SubCompany 非bom配件退回仓库更新");//需处理
...
$backQty=$backQty-$thisQty;//(已经退回仓库)
...
$Parameter="Id,$Id,funFrom,$funFrom,From,$From,Pagination,$Pagination,Page,$Page,GoodsId,$GoodsId,oldQty,$Qty,ActionId,157";
//步骤5：//需处理
?>
	...
	<tr><td class="A0011">
      <table width="100%"  border="0" align="center" cellspacing="0">
		<tr>
			...
			<td valign="middle" scope="col" ><?php echo $GoodsName.$Info?><input type="hidden" id="SIdList" name="SIdList">
          <input  id="PropertySign" name="PropertySign" type="hidden" value="<?php echo $PropertySign?>"></td>
		</tr>
		<tr>
     ...
     <tr>
       ...
       <tr>
			...
			<td valign="middle" scope="col" ><input  id="thisthQty" name="thisthQty" type="text" value="<?php echo $thisQty?>" size="14" onblur="CheckNum(this,<?php echo $MaxQty?>)" ></td>
		</tr>
		<tr>
		  <td align="right" valign="top" height="30">单 &nbsp;&nbsp;&nbsp;据：</td>
	...
	<tr>
		...
		<td colspan="5" align="center" class="A0111" id="ShowInfo">	
                   ...
                   while($ListRows= mysql_fetch_array($sListResult)){
                          ...
                          $CheckReBackFixedResult=mysql_fetch_array(mysql_query("SELECT Id FROM $DataIn.nonbom8_rebackfixed WHERE BackId=$Id AND BarCode=$BarCode",$link_id));
                          $CheckReBackFixedId=$CheckReBackFixedResult["Id"];
                           if($CheckReBackFixedId!=""){
                            $Checked="checked";
                             }
                           else  $Checked="";

                          echo"<tr bgcolor='$theDefaultColor'><td  align='center'  width='50' class='A0101'><input name='backCode[]' type='checkbox' id='backCode$i' value='$BarCode' $disabledStr  $Checked></td>";
...
function  CheckForm(){
    ...
    if(chooseSign!=thisQty && PropertySign==1){//是固定资产
       ...
       if(Message==""){
		     document.form1.action="nonbom21_updated.php";
