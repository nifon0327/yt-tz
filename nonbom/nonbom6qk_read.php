...
if($From!="slist"){	//非查询：过滤采购、结付方式、供应商、月份
	...
	$EstateResult = mysql_query("SELECT B.Estate FROM $DataIn.nonbom11_qksheet B WHERE 1 GROUP BY B.Estate ORDER BY B.Estate ",$link_id);
	...
	$checkResult = mysql_query("SELECT G.BuyerId,F.Name 
		...
		WHERE G.BuyerId>0 GROUP BY G.BuyerId ORDER BY F.Name",$link_id);

	if($checkRow = mysql_fetch_array($checkResult)) {
		echo"<select name='BuyerId' id='BuyerId' onchange='document.form1.submit()'>";
	...
	$checkResult = mysql_query("SELECT C.CompanyId,C.Forshort 
		...
		WHERE  1 $SearchRows GROUP BY B.CompanyId ORDER BY C.Forshort",$link_id);
	if($checkRow = mysql_fetch_array($checkResult)){
...
if($myRow = mysql_fetch_array($myResult)){
	...
	do{
		...
		if($ALLAmount!=$Amount){
			$LockRemark="严重错误，请款结付金额($ALLAmount!=$Amount)！！请与管理员联系！"; //	
		}
		...
		if( $HavedAmount>$S_ALLAmount){
			$LockRemark="严重错误，请款总额大于总结付金额($HavedAmount>$S_ALLAmount)！！请与管理员联系！"; //
		}
		...
		$StuffListTB="
		   ...
		   if ($Login_P_Number==10868) $LockRemark="";
			switch($JFEstate){
				case "0":
