<script>
function ShowOrHideFixed(e,f,Order_Rows,BfId,RowId){
	...
	if (yy.indexOf("showtable")==-1){
		...
		//动态加入采购明细
		if(BfId!=""){			
			var url="../nonbom/nonbom22_ajax.php?BfId="+BfId+"&RowId="+RowId; 
...
$ColsNumber=10;
ChangeWtitle("$SubCompany 非bom配件个人领用报废记录审核");
$funFrom="nonbom22";
$From=$From==""?"m":$From;
$Th_Col="选项|60|序号|30|分类|80|编码|50|非bom配件名称|350|单位|40|报废数量|60|报废原因|250|附件|40|报废时间|70|状态|50|申请报废人|70";
...
$mySql="SELECT A.Id,A.GoodsId,A.Qty,B.GoodsName,B.Attached,B.Unit,C.TypeName,A.Date,A.Estate,A.Operator,A.Remark,E.Name AS BfName,A.Picture
...
LEFT JOIN $DataPublic.staffmain E ON E.Number=A.bfNumber
WHERE 1 $SearchRows  AND A.Estate=1 ORDER BY A.Date ASC";
...
if($myRow = mysql_fetch_array($myResult)){
	...
	do{
      ...
      $Picture=$myRow["Picture"];
       if($Picture!=""){
                 $Dir21=anmaIn("download/nonbom22/",$SinkOrder,$motherSTR);
      ...
      $CheckCodeRow=mysql_fetch_array(mysql_query("SELECT  Id FROM $DataIn.nonbom8_bffixed WHERE BfId='$Id'",$link_id));
