...
ChangeWtitle("$SubCompany 非bom领用配件盘点");//需处理
$fromWebPage=$funFrom."_read";		
$nowWebPage =$funFrom."_pd";	
...
$PropertyResult=mysql_query("SELECT Id FROM $DataPublic.nonbom4_goodsproperty WHERE GoodsId=$GoodsId AND Property=7",$link_id);  
 if($PropertyRow=mysql_fetch_array($PropertyResult)){
          $PropertySign=1;
          $Info="<span class='redB'>(属性为固定资产，必须选择已有的固定资产记录)</span>";
      }
else {
         $PropertySign=0;
...
$Parameter="Id,$Id,funFrom,$funFrom,From,$From,Pagination,$Pagination,Page,$Page,GoodsId,$GoodsId,ActionId,159";
//步骤5：//需处理
?>
	...
	<tr><td class="A0011">
      <table width="100%"  border="0" align="center" cellspacing="0" id="NoteTable" >
		<tr>
			...
			<td valign="middle" scope="col" ><?php echo $GoodsName?>---<span class="redB">(领用配件盘点处理)</span><input  id="PropertySign" name="PropertySign" type="hidden" value="<?php echo $PropertySign?>"></td>
		</tr>
		<tr>
     ...
     <tr>
       ...
       <tr>
			...
			<td valign="middle" scope="col" class="greenB"><?php echo $MaxQty;?><input  type="hidden" name="MaxQty" id="MaxQty" value="<?php echo $MaxQty;?>"></td>
		</tr>

       <tr>
			<td align="right" >丢弃或报废数量：</td>
			<td ><input  id="pdQty" name="pdQty" type="text" style="width: 150px;" onblur="CheckNum(this,<?php echo $MaxQty?>)"></td>
		</tr>

        <tr>
          <td  align="right" >盘点备注：</td>
          <td ><textarea name="Remark" rows="2" id="Remark" style="width: 300px;" dataType='Require' msg='未填写'></textarea></td>
        </tr>
	  </table>
</td></tr></table>

<table border="0" width="<?php  echo $tableWidth?>" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
 <tr><td  class="A0010">&nbsp;</td><td colspan="5" height="30" ><span class='blueB' >1.非BOM固定资产个人领用明细(盘点)</span></td><td  class="A0001">&nbsp;</td></tr>
	...
	<tr>
		...
		<td colspan="5" align="center" class="A0111" id="ShowInfo">	
                   ...
                   while($ListRows= mysql_fetch_array($sListResult)){
                          ...
                          echo"<tr bgcolor='$theDefaultColor'><td  align='center' height='20' width='50' class='A0101'><input name='pdCode[]' type='checkbox' id='pdCode$i' value='$BarCode'  $disabledStr ></td>";
...
function  CheckForm(){
      ...
      var PropertySign=document.getElementById("PropertySign").value;
       var thisQty=document.getElementById("pdQty").value;
