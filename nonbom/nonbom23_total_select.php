...
ChangeWtitle("$SubCompany 非BOM个人转出记录查询");			//需处理
...
<TR>
                  <TD align="right">转出人员
                    <input name="Field[]" type="hidden" id="Field[]" value="OutNumber">
                  </TD>
                  <TD align="center">
                    <select name="fun[]" id="fun[]" style="width: 60px;">
				  ...
				  <select name=value[] id="value[]" style="width: 380px;">
                    ...
                    <?php 
                        $CheckOutResult=mysql_query("SELECT M.Number,M.Name FROM $DataIn.nonbom8_turn  T 
                         LEFT JOIN $DataPublic.staffmain M ON M.Number=T.OutNumber WHERE M.Estate=1",$link_id);
...
<TR>
                  <TD align="right">接收人员
                    <input name="Field[]" type="hidden" id="Field[]" value="InNumber">
                  </TD>
                  <TD align="center">
                    <select name="fun[]" id="fun[]" style="width: 60px;">
                      <option value="=" selected>=</option>
				  ...
				  <select name=value[] id="value[]" style="width: 380px;">
				  	<option value="" selected>全部</option>
                    <?php 
                        $CheckInResult=mysql_query("SELECT M.Number,M.Name FROM $DataIn.nonbom8_turn  T 
                         LEFT JOIN $DataPublic.staffmain M ON M.Number=T.InNumber WHERE M.Estate=1",$link_id);
                       while($CheckInRow=mysql_fetch_array($CheckInResult)){
                              $thisInNumber=$CheckInRow["Number"];
                              $thisInName=$CheckInRow["Name"];
                               echo "<option value='$thisInNumber'>$thisInName</option>";
                           }
					?>
				  </select>
                  <input name="table[]" type="hidden" id="table[]" value="A">                  
