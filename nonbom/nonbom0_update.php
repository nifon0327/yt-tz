...
ChangeWtitle("$SubCompany 更新非BOM供应商资料");//需处理
$fromWebPage=$funFrom."_read";
$nowWebPage =$funFrom."_update";
$toWebPage  =$funFrom."_updated";
$_SESSION["nowWebPage"]=$nowWebPage;
$upResult = mysql_query("SELECT A.CompanyId,A.FscNo,A.Forshort,A.PayMode,A.Currency,
B.Company,B.Tel,B.Fax,B.Area,B.ZIP,B.Address,B.Bank,B.Remark,A.Judge,A.AddTaxValue
...
if($upRow = mysql_fetch_array($upResult)){
	...
	$TempPay="PayModelSTR".strval($upRow["PayMode"]);$$TempPay="selected";
	$Company=$upRow["Company"];
	$AddTaxValueId=$upRow["AddTaxValue"];
	$Tel=$upRow["Tel"];
	$Judge=$upRow["Judge"];
	$Fax=$upRow["Fax"];
	...
	$Bank=$upRow["Bank"];
	$Remark=$upRow["Remark"];
	$lmanResult = mysql_query("SELECT * FROM $DataPublic.nonbom3_retailerlink WHERE CompanyId=$CompanyId AND Defaults=0 order by CompanyId DESC LIMIT 1",$link_id);
	$lmanRow = mysql_fetch_array($lmanResult);
	$LinkId=$lmanRow["Id"];
	$TempSex="SexSTR".strval($lmanRow["Sex"]);$$TempSex="selected";
    $sheetResult=mysql_query("SELECT * FROM $DataPublic.nonbom3_retailerother WHERE CompanyId=$CompanyId  LIMIT 1",$link_id);
...
$TypeIdStr="";$TypeNameStr="";
        ...
        while($CheckTypeRow=mysql_fetch_array($CheckTypeResult)){
              ...
              if($TypeIdStr=="")$TypeIdStr=$TypeId;
              else $TypeIdStr=$TypeIdStr."@".$TypeId;
              if($TypeNameStr=="")$TypeNameStr=$TypeName;
              else $TypeNameStr=$TypeNameStr."@".$TypeName;
              }
...
<td colspan="3" scope="col"><textarea   id="TypeName" name="TypeName" onclick="updateJq(this,1)" style="width:380px" ><?php echo $TypeNameStr?></textarea><input  type="hidden" id="TypeId" name="TypeId" value="<?php echo $TypeIdStr?>"></td>
</tr>

  <tr>
    <td align="right">结付方式</td>
    <td colspan="3" scope="col">
	<select name="PayMode" id="PayMode" style="width:380px">
	<option value="0" <?php  echo $PayModelSTR0?>>月结</option>
	<option value="1" <?php  echo $PayModelSTR1?>>现金</option>
	</select>
	</td>
  </tr>
  <tr>
            ...
            <td colspan="3"><select name="AddTaxValue" id="AddTaxValue" style="width:380px;" dataType="Require" msg="未选择">
			  ...
			  if($AddTaxRow = mysql_fetch_array($AddTaxResult)){
				...
				do{
					...
					if($AddTaxValueId== $AddTaxId){
						echo"<option value='$AddTaxId' selected >$AddTaxName</option>";
					}else{
		...
		switch(toObj){
			case 1:
				<?PHP
				  ...
				  do{
				        $rows++;
					    $echoInfo.="&nbsp;<input type='checkbox' name='TypeIdCheckId[]'  id='ProcessCheckId' value='$TypeRow[Id]|$TypeRow[TypeName]'>&nbsp;&nbsp;$TypeRow[TypeName] </br>";
					  } while($TypeRow = mysql_fetch_array($TypeResult));
			      }
				?>
				infoShow.style.width=250;
				 infoShow.style.height=<?php echo $rows; ?>*25+20;

				 theDiv.style.width=250;
		  ...
		  theDiv.style.top=event.clientY -7+'px';
