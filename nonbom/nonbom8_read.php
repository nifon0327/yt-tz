...
//步骤2：需处理
$ColsNumber=17;
ChangeWtitle("$SubCompany 非bom配件领用明细");
$funFrom="nonbom8";
$From=$From==""?"read":$From;
$Th_Col="选项|60|序号|30|申领人|60|申领日期|90|使用地点|50|分类|100|客户项目|150|编码|50|配件名称|250|申领数量|60|单位|40|在库|60|采购库存|60|申领备注|300|发放日期|90|发放人|60|状态|60|操作|60";
...
$mySql="SELECT A.Id,A.GoodsId,A.WorkAdd,A.Qty,A.Remark,A.ReturnReasons,A.OutDate,A.Estate,A.Locks,A.Date,A.Operator,
B.GoodsName,B.BarCode,B.Attached,B.Unit,C.TypeName,D.wStockQty,D.oStockQty,D.mStockQty,E.Name AS WorkAdd,F.Name AS OutOperator,G.Name AS GetName,T.Forshort AS Company
       ...
       $OutDate=$myRow["OutDate"]=="0000-00-00 00:00:00"?"&nbsp;":$myRow["OutDate"];
		...
		switch($myRow["Estate"]){
			...
			case 0:
				...
				$LockRemark="物品已发放,锁定操作.";
				$OutOperator=$myRow["OutOperator"];
			//	$OutDate=$myRow["OutDate"];
		         $OutDate=substr($myRow["OutDate"],0,10);
				break;
			}
