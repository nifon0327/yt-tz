...
$CheckDate=$targetDate==""?date("Y-m-d"):$targetDate;//默认日期
$targetDate = $CheckDate;
include 'kq_checkio_auto.php';
//步骤3：
...
if($From!="slist"){
	$SearchRows ="";
	echo"<input name='targetDate' type='text' id='targetDate' size='10' maxlength='10' value='$CheckDate' onchange='ResetPage(this.name)' >&nbsp;";
	$SearchRows.="and (
...
$mySql="SELECT C.Id,C.CheckTime,C.CheckType,C.dFrom,C.Estate,C.Locks,C.ZLSign,C.KrSign,C.Number,M.Name,M.BranchId,M.JobId,G.GroupName,A.Floor		
		FROM d7check.fakecheckinout C 
		...
		WHERE 1 $SearchRows and M.cSign = 7 and M.JobId != 38
		AND C.otReason = 'mc'
