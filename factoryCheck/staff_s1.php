...
switch($Action){
	case"0"://来自新增社保资料，需过滤已加入社保的记录OKM.cSign='$Login_cSign' and
		$AddTB=" LEFT JOIN $DataPublic.sbdata Z ON Z.Number=M.Number";
	...
	case "2"://来自员工等级设定OKM.cSign='$Login_cSign'  and 
	    ...
	    if($fSearchPage=="cw_jj_frist"){////ewen 加入年终奖金过滤条件：即使离职也需计算		条件1:入职日期需要在当年之内；条件2:离职日期在当年之后；条件3:还没有生成奖金的
			$Year=$theYear;
			$tempYear=$theYear+1;
			$NumberSTR=" AND M.ComeIn<'$tempYear-01-01' AND M.Number NOT IN (SELECT Number FROM $DataIn.cw11_jjsheet_frist WHERE ItemName='$jjItem') AND ( (M.Estate=1 $BranchIdSTR $JobIdSTR ) OR ( M.Number  IN (SELECT Number FROM $DataPublic.dimissiondata WHERE left(outDate,7)>'$Year-12' OR  outDate='$Year-12-31')  $BranchIdSTR $JobIdSTR  ))";
			}
		else {
			$NumberSTR=" AND M.Estate=1 ".$BranchIdSTR.$JobIdSTR;
			}		
	...
	case "6"://社保有效 且当月没有缴费的     来自社保缴费记录 OK" ".
		...
		$NumberSTR="  AND M.cSign='$Login_cSign'  and M.Number IN(SELECT Number FROM $DataPublic.sbdata WHERE Estate=1 AND (eMonth='' OR eMonth>'$Month' OR eMonth='$Month' ) ORDER BY Number) ".$MonthSTR.$BranchIdSTR;
	break;
    ...
    case "11"://住房公积金
		...
		$NumberSTR="  AND M.cSign='$Login_cSign' and M.Number IN(SELECT Number FROM $DataPublic.epfdata WHERE Estate=1 AND (eMonth='' OR eMonth>'$Month' OR eMonth='$Month' ) ORDER BY Number)".$MonthSTR.$BranchIdSTR;
      break;
    case "12"://来自新增公积金资料
		$AddTB=" LEFT JOIN $DataPublic.epfdata Z ON Z.Number=M.Number";
		$NumberSTR="  AND  M.Estate=1 AND Z.Number IS NULL ".$BranchIdSTR;
		break;
    case 13: //来自意外险
       // $MonthSTR=$Month==""?"":" and M.Number NOT IN(SELECT Number FROM $DataIn.sbpaysheet WHERE Month='$Month' AND TypeId=3 ORDER BY Number)";
	    $DateStr=$Month.'-01';
		$MonthSTR=$Month==""?"":" and M.Number AND NOT  EXISTS (SELECT X.Number FROM $DataIn.sbpaysheet X WHERE  date_add(DATE_FORMAT(CONCAT(X.Month,'-01'),'%Y-%m-%d'),interval 1 year)>'$DateStr' AND X.TypeId=3 AND X.Number=M.Number )
		";
         $NumberSTR="  AND M.cSign='$Login_cSign' AND M.Estate=1 ".$MonthSTR.$BranchIdSTR;
       break;
     default:
     break;	
	}   
...
//if ($Action==110) {
  ...
  case	"110":
   ...
   case "21": //离职人员
   		...
   		$mySql="SELECT 
		M.Id,M.Number,M.Name,M.BranchId,M.JobId,M.Grade,M.Introducer,M.ComeIn,
		S.Sex,S.Rpr,B.Name AS BranchName,J.Name AS JobName,K.Name AS KqSign,G.GroupName,M.Estate,D.outDate,T.Name AS TypeName,D.Reason AS LeaveReason
		FROM $DataPublic.staffmain M
		LEFT JOIN $DataPublic.staffsheet S ON M.Number=S.Number
		LEFT JOIN $DataPublic.jobdata J ON J.Id=M.JobId
		LEFT JOIN  $DataIn.staffgroup G ON G.GroupId=M.GroupId
...
if($myRow = mysql_fetch_array($myResult)){
	do{
		...
		switch($Action){
			case"2"://等级设定
				$Low=1;$Hight=30;
				$gResult = mysql_query("SELECT Low,Hight FROM $DataPublic.gradedata where 1 and JobId=$JobId LIMIT 1",$link_id);
	...
	/*	$sbResult = mysql_query("SELECT Id FROM $DataPublic.sbdata WHERE Number=$Number order by Id LIMIT 1",$link_id);
       ...
       if($Introducer!=""){
		...
		if($myRow["Estate"]==1){//在职 //ewen 标识
			...
			$outResult = mysql_fetch_array(mysql_query("SELECT outDate FROM $DataPublic.dimissiondata WHERE Number='$Number' order by Id LIMIT 1",$link_id));
