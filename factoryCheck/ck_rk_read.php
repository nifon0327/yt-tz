...
$MergeRows=5;
$Th_Col="操作|50|日期|70|送货单|80|供应商|80|采购|50|选项|60|序号|40|配件ID|50|配件名称|300|单位|40|品检<br>报告|40|采购总数|60|本次入库|60|领料数量|60|请款<br>方式|30|采购单号|80|需求单流水号|110|存储位置|50";
...
$mySql="SELECT M.BillNumber,M.Date,M.Remark,M.Operator,
        S.Id,S.Mid,S.StockId,S.StuffId,S.Qty,S.gys_Id,S.Locks,D.StuffCname,D.TypeId,D.Picture,P.Forshort,A.Name,G.OrderQty,G.FactualQty+G.AddQty AS cgQty,MP.Name AS Position,U.Name AS UnitName,
                G.POrderId,Y.ProductId,Y.OrderPO,Y.Qty as PQty,Y.PackRemark,Y.sgRemark,Y.ShipType,PI.Leadtime,C.Forshort AS Client,CP.cName,CP.TestStandard,GM.PurchaseID,GM.Id AS GMId, IFNULL(F.AutoSign,-1) as AutoSign
...
if($mainRows = mysql_fetch_array($mainResult)){
    ...
    do{
        ...
        if($BillNumber!=""){
              if ($gys_Id=="") {
                  ...
                  if(file_exists($FilePath1)){
                        ...
                        {
                            $GysSql=mysql_query("SELECT S.Mid FROM $DataIn.gys_shmain M
                            LEFT JOIN $DataIn.gys_shsheet S ON S.Mid=M.Id
                            WHERE M.BillNumber=$BillNumber",$link_id);
                            if(mysql_num_rows($GysSql) > 0)
                            {
                                $GysMid=mysql_result($GysSql,0,"Mid");
                            }
                            $MidSTR=anmaIn($GysMid,$SinkOrder,$motherSTR);
                            $BillNumber="<a href='../supplier/shorder_view.php?f=$MidSTR' target='_blank'>$BillNumber</a>";
                         }
                     }
          }
        $Forshort=$mainRows["Forshort"];
        $Name=$mainRows["Name"]==""?"&nbsp;":$mainRows["Name"];
                ...
                if ($funFrom=="gys_sh"){
                    $badResult = mysql_query("SELECT Id FROM $DataIn.qc_badrecord WHERE StockId='$StockId' AND shMid='$Mid' AND StuffId='$StuffId' LIMIT 1",$link_id);
                      if($badRow = mysql_fetch_array($badResult)){
                          $badId=$badRow["Id"];
                          $qualityReport="<a href='stuff_quality_report.php?Id=$badId&StockId=$StockId' target='_blank'>View</a>";
                         }
                }
                else{
        ...
        //成品配件，BOM里面只有一个配件，入库时直接备料，领料，-----待出
          $StuffCount=0;
          if($FinishProSign==1 && $POrderId>0){
                  $CheckCoutResult=mysql_fetch_array(mysql_query("SELECT  COUNT(*) AS StuffCount FROM $DataIn.cg1_stocksheet  WHERE POrderId=$POrderId",$link_id));
                  $StuffCount=$CheckCoutResult["StuffCount"];
                if($StuffCount==1){
                      $DateTime=date("Y-m-d H:i:s");
                       $Operator=$Login_P_Number;
                       $CheckResult=mysql_query("SELECT G.OrderQty ,G.StockQty,K.tStockQty ,G.StuffId,G.StockId
                       ...
                       if($CheckRow=mysql_fetch_array($CheckResult)){
                          do{
                                   ...
                                   $checkllQty=mysql_fetch_array(mysql_query("SELECT IFNULL(SUM(Qty),0) AS llQty FROM $DataIn.ck5_llsheet WHERE StockId='$thisStockId'",$link_id));
                                   $thisllQty=$checkllQty["llQty"];
                                   $thisQty=$thisOrderQty-$thisllQty;
                                    if($thisllQty<$thisOrderQty && $thistStockQty>=$thisQty){
                                               ...
                                               $Pid=mysql_insert_id();
                                               $llInSql="INSERT INTO $DataIn.ck5_llsheet (Id,Pid,Mid,StockId,StuffId,Qty,Locks,Estate) VALUES  (NULL,'$Pid','0','$thisStockId','$thisStuffId','$thisQty','0','0')";
                                               $llInAction=@mysql_query($llInSql);
                                               $signUpSql="UPDATE $DataIn.ck9_stocksheet SET tStockQty=tStockQty-$thisQty  WHERE StuffId='$thisStuffId' AND tStockQty>=$thisQty";
                                               $signUpResult = mysql_query($signUpSql);
                                           }
                                 }while($CheckRow=mysql_fetch_array($CheckResult));
                           }
                            $CheckblQty=mysql_fetch_array(mysql_query("SELECT SUM(G.OrderQty) AS blQty  FROM $DataIn.cg1_stocksheet G  WHERE G.POrderId='$POrderId'",$link_id));
                            $blQty=$CheckblQty["blQty"];
                            $CheckllQty=mysql_fetch_array(mysql_query("SELECT SUM(K.Qty) AS llQty  FROM   $DataIn.ck5_llsheet K  WHERE left(StockId,12)='$POrderId'",$link_id));
                            $llQty=$CheckllQty["llQty"];
                            if($blQty==$llQty){
                                 $UpdateSql="UPDATE $DataIn.yw1_ordersheet SET scFrom=0,Estate=2 WHERE POrderId=$POrderId AND scFrom!=0 AND Estate!=2";
                                 $UpdateResult=@mysql_query($UpdateSql);
                                 $UpdateSql1="UPDATE $DataIn.ck5_llsheet SET Estate=0  WHERE left(StockId,12)='$POrderId'  Estate=1";
