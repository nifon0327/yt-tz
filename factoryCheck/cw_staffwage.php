...
$funFrom="cw_staffwage";
if($From!="slist"){
    $sumCols="21,22,23,24,25,26,28";        //求和列
    $Th_Col="选项|40|序号|30|分类|30|工作</br>地点|40|部门|40|职位|40|小组|50|员工<br>姓名|50|工龄<br>Y(M)|40|底薪|40|加班费|40|工龄<br>津贴|40|岗位<br>津贴|40|奖金|40|其它<br>奖金|40|生活<br>补助|40|住宿<br>补助|40|交通<br>补助|40|夜宵<br>补助|40|个税<br>补助|40|考勤<br>扣款|40|津贴<br>扣款|40|小计|55|借支|40|社保|50|个税|40|公积金|50|餐费<br>扣款|40|其它<br>扣款|40|实付|55|状态|40|备注|40";
    $swidth=270;
...
else
{
    ...
    $sumCols="22,23,24,25,29";      //求和列
    $Th_Col="选项|40|序号|30|分类|30|月份|60|工作</br>地点|40|部门|40|职位|40|小组|50|员工<br>姓名|50|工龄<br>Y(M)|40|底薪|40|加班费|40|工龄<br>津贴|40|岗位<br>津贴|40|奖金|40|其它<br>奖金|40|生活<br>补助|40|住宿<br>补助|40|交通<br>补助|40|夜宵<br>补助|40|个税<br>补助|40|考勤<br>扣款|40|津贴<br>扣款|40|小计|55|借支|40|社保|50|个税|40|公积金|50|餐费<br>扣款|40|其它<br>扣款|40|实付|55|状态|40|备注|40";
    $swidth=330;
...
//步骤6：需处理数据记录处理
//初始化
$sumDx=$sumGljt=$sumGwjt=$sumJj=$sumShbz=$sumZsbz=$sumJtbz=$sumJbf=$sumYxbz=$sumtaxbz=$sumJz=$sumSb=$sumGjj=$sumCt=$sumKqkk=$sumdkfl=$sumRandP=$sumOtherkk=$sumAmount=$sumTotal=0;$sumJbjj=0;
$i=1;
$j=($Page-1)*$Page_Size+1;
List_Title($Th_Col,"1",1);

$mySql="SELECT 
S.Id,S.Month,S.KqSign,S.Number,S.Dx,S.Gljt,S.Gwjt,S.Jj,S.Jbjj,S.Shbz,S.Zsbz,S.Jtbz,S.Jbf,S.Yxbz,S.taxbz,S.Jz,S.Sb,S.Gjj,S.Ct,
S.Kqkk,S.dkfl,S.RandP,S.Otherkk,S.Amount,S.Estate,S.Remark,S.Locks,M.Name,M.ComeIn,M.Estate AS mEsate,M.WorkAdd,M.Id As PID,B.Name AS Branch,J.Name AS Job,M.GroupId,E.GroupName,M.BranchId
...
WHERE 1 $SearchRows 
AND M.WorkAdd != 6 AND M.JobId != 38 AND M.Number != 11757
...
if($myRow = mysql_fetch_array($myResult)){
    do{
        ...
        if($wageSignOverRow = mysql_fetch_assoc($wageSignResult)){
            ...
            $Jj += $wageKk;
            //echo $Number.' '.$dkfl.' '.$wageKk.'<br>';
        }
        $reCalculateBranch = array('6', '7', '8');
        ...
        $Total=$Dx+$Jbf+$Gljt+$Gwjt+$Jj+$Jbjj+$Shbz+$Zsbz+$Jtbz+$Yxbz+$taxbz-$Kqkk-$dkfl;
        
        $Jz=$myRow["Jz"];           //借支
        // if ($Jz==0 && $From!="slist"){
        //     if($Number == '10487'){
        //         echo "$Sum_Jz-$Sum_PayJz";
        //     }
        //     include "../public/kqcode/staffwage_jz.php";
        // }
        $Sb=$myRow["Sb"];           //社保
        $Gjj=$myRow["Gjj"];         //住房公积金
        $Ct=$myRow["Ct"];           //餐费
        ...
        if($KqSign<3){//加入考勤连接以便查询
            if($chooseMonth >= '2014-03'){
                $Jbf="<a href='kq_checkcount.php?CountType=1&Number=$Number&CheckMonth=$chooseMonth&needSign=no' target='_blank'>$Jbf</a>";//验厂
            }
            else{
                $Jbf="<a href='kq_checkcount.php?CountType=1&Number=$Number&CheckMonth=$chooseMonth&needSign=no' target='_blank'>$Jbf</a>";
            }
        }
...
$sumJbf=SpaceValue0($sumJbf);$sumGljt=SpaceValue0($sumGljt);$sumGwjt=SpaceValue0($sumGwjt);$sumJj=SpaceValue0($sumJj);$sumJbjj=SpaceValue0($sumJbjj);
