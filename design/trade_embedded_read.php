...
$ColsNumber=14;
//$tableMenuS=1490;
ChangeWtitle("$SubCompany 预埋件");
$funFrom="trade_embedded";
$nowWebPage=$funFrom."_read";
//$sumCols="4";		//求和列
$Th_Col="选项|60|序号|50|原始顺序号|60|项目编号|55|项目名称|100|构件类型|60|楼栋编号|60|楼层编号|60|构件编号|60|产品条码|60|审核状态|70|长<br/>(mm)|60|宽<br/>(mm)|60|厚<br/>(mm)|60";
...
//项目数据检索
$mySql="SELECT a.Id, a.Forshort, b.Titles,b.Specs,b.BuildingNo, c.Estate,b.TypeName FROM $DataIn.trade_object a
LEFT JOIN $DataIn.trade_embedded_data b on a.Id = b.TradeId
...
$buildList = array();
if ($proId) {
    $BmySql = "SELECT DISTINCT a.BuildingNo FROM $DataIn.trade_embedded a where a.TradeId = $proId order by a.BuildingNo";
...
$typeList = array();
if ($proId) {
    $TmySql="SELECT DISTINCT a.CmptType FROM $DataIn.trade_embedded a where a.TradeId = $proId and a.BuildingNo = '$build' order by a.CmptType";
...
$floorList = array();
if ($proId) {
    $mySql="SELECT DISTINCT a.FloorNo FROM $DataIn.trade_embedded a where a.TradeId = $proId order by a.CmptNo";
    $myResult = mysql_query($mySql, $link_id);
...
<table border="0" cellspacing="0" style='TABLE-LAYOUT: fixed; WORD-WRAP: break-word;width: 1000px;border: 1px solid #E2E8E8;border-radius:5px;-moz-box-shadow: 0px 0px 10px #c7c7c7; box-shadow: 0px 0px 10px #c7c7c7;margin:10px 10px 10px 0' bgcolor='#FFF' class="div-select">
	...
	<tr>
		<td class="tds1" style="padding-left: 10px;" colspan="2">
        ...
        <td  class="tds1" style="height: 45px;">
            ...
            <a href="../admin/openorload.php?d=<?php echo $d ?>&f=<?php echo $f ?>&Type=&Action=6"target="download" style="color: #169BD5;margin: 0 5px;">下载模板</a>
        </td>
    </tr>
</table>
<?php 
...
for ($i = 0; $i < $Count; $i = $i + 2) {
    ...
    if ($i > 26) {
        ...
        $TableStr .= "<td rowspan='2' width='$Field[$k]' Class='$Class_Temp'>$Field[$j]</td>";
    }
}

$TableStr .= "</tr><tr height='16' class='' align='center'>";
if ($specs) {
    foreach ($specs as $spec) {
        $TableStr .= "<td  Class='A0101'>$spec</td>";
    }
}
...
function toSearchResult(){
	...
	document.form1.action="trade_embedded_read.php";
...
function toImportData() {
	...
	var r=Math.random();
    window.open("trade_cmpt_add.php?r=" + r + "&fromWebPage=trade_embedded_read&type=3&proId=" + proId,"_self","dialogHeight=500px;dialogWidth=780px;center=yes;help=0;scroll=yes"); 
}
...
function toDeleteData() {
	...
	var proId = jQuery("#proId").val();
	document.form1.action="trade_embedded_del.php?Ids="+Ids+"&proId="+proId;
	document.form1.target = "_self";	
	document.form1.submit();		
}
...
function toExportData() {
	//document.form1.action="trade_embedded_toexcel.php";
	//document.form1.target = "_blank";
	document.form1.action="trade_cmpt_export.php?type=3";
	document.form1.target = "_self";
	document.form1.submit();
}
...
function proIdConChange() {
	...
	RefreshPage("trade_embedded_read");
}
