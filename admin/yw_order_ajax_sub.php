<?php   
//电信-EWEN    	 
	$sListResult = mysql_query("SELECT S.Id,S.Mid,S.StockId,S.POrderId,S.StuffId,S.Price,S.OrderQty,S.StockQty,S.AddQty,S.FactualQty,S.CompanyId,S.BuyerId,S.DeliveryDate,
M.Date,A.StuffCname,A.Picture,A.Gfile,A.Gstate,A.TypeId,A.DevelopState,B.Name,C.Forshort,C.Currency,MP.Name AS Position,ST.mainType,MT.TypeColor,MT.TitleName,MT.blSign,U.Name AS UnitName,U.Decimals,Y.ProductId ,Y.Estate,K.tStockQty,YM.Operator,YM.OrderDate,A.DevelopState
		...
		WHERE S.POrderId='$POrderId' AND S.Level>1  ORDER BY MT.SortId DESC,S.StockId",$link_id);	
		
	//$i=1;
	$d=anmaIn("download/stufffile/",$SinkOrder,$motherSTR);
	if ($StockRows = mysql_fetch_array($sListResult)) {
	       $TableId="ListSubTB".$RowId;
        echo"<table id='$TableId' width='$subTableWidth' cellspacing='1' border='1' align='left' style='margin-left:30px;margin-top:20px;'><tr bgcolor='#CCCCCC'>
			...
			if($CheckSignRow=mysql_fetch_array($CheckSignSql)){
             ...
             $showStr="&nbsp;"; $showTable="";
              ...
              if($ComboxMainSign==1){
                            ...
                            if ($Login_P_Number=='10868' && check_stuffbox($StockId,$StuffId,$DataIn,$link_id)==false){
                                  $Log="";
                               
	                              $addSign=addCg_StuffComBox_data($StockId,$StuffId,$DataIn,$link_id,$Login_P_Number,$Log);
	                              if ($addSign && $Log!="") echo $Log;
                             }
                         
                              $showStr="<img onClick='ShowDropTable(ShowTable$RowId$i,ShowDiv$RowId$i,\"stuffcombox_pand_ajax\",\"$StockId|$StuffId\",\"admin\");'  src='../images/showtable.gif' 
			title='显示或隐藏子配件资料.' width='13' height='13' style='CURSOR: pointer' bgcolor='$theDefaultColor'>";
			                  $showTable="<tr id='ShowTable$RowId$i' style='display:none'><td colspan='$Colsnum'><div id='ShowDiv$RowId$i' width='720'></div></td></tr>";
                     }
               ...
               if($CheckProcessRow=mysql_fetch_array($CheckProcessSql)){
                              $showStr="<img onClick='ShowDropTable(ShowTable$RowId$i,ShowDiv$RowId$i,\"processbom_ajax\",\"$ProductId|$StuffId|$OrderQty\");'  src='../images/showtable.gif' 
