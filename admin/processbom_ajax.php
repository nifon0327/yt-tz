
<?php   
//传入参数：$ProductId 、$StuffId、$OrderQty 电信---yang 20120801

include "../basic/chksession.php" ;
include "../basic/parameter.inc";
include "../model/modelfunction.php";
header("Content-Type: text/html; charset=utf-8");
header("expires:mon,26jul199705:00:00gmt");
header("cache-control:no-cache,must-revalidate");
header("pragma:no-cache");

$dataArray=explode("|",$args);
$gStuffId=$dataArray[0];
$StockId=$dataArray[1];
$OrderQty=$dataArray[2];
$MaxQty=0;
if($StockId>0){
      include "../model/subprogram/GetMaxGxQty.php";
}

$LossStr=$StockId>0?"<td width='80' align='center'>损耗比率</td>
<td width='80' align='center'>允许损耗数</td>
<td width='80' align='center'>最低登记数</td>":"";
$AddtdStr=$StockId>0?"<td width='80' align='center'>需求数量</td>
<td width='80' align='center'>完成数量</td>":"";
echo"<table  cellspacing='1' border='1' align='center' style='table-layout:fixed;word-break:break-all; word-wrap:break-word;margin:2px 0px 2px 30px;' ><tr bgcolor='#CCCCCC'>
<td width='30'  align='center'>操作</td>
		         <td width='40'  align='center'>序号</td>
                <td width='60'  align='center'>工序Id</td>
		        <td width='200' align='center'>工序名称</td>
                <td width='40'  align='center'>图档</td>
		        <td width='60' align='center'>对应关系</td>
		        <td width='100' align='center'>约束工序</td>
                $LossStr
		        $AddtdStr
		</tr>"; //<td width='60'  align='center'>单价</td>

if ($StockId>0){
	     $ProcessSql="SELECT D.ProcessId,D.ProcessName,A.Relation,D.Price,D.Picture,D.BassLoss,A.BeforeProcessId
	     FROM $DataIn.cg1_processsheet A  
         LEFT JOIN $DataIn.process_data D ON D.ProcessId=A.ProcessId   
         LEFT JOIN $DataIn.process_type PT ON PT.gxTypeId=D.gxTypeId
	     WHERE A.StockId='$StockId' AND A.StuffId='$gStuffId' ORDER BY PT.SortId,A.ProcessId ASC";
}
else{
	 $ProcessSql="SELECT D.ProcessId,D.ProcessName,A.Relation,D.Price,D.Picture,D.BassLoss,A.BeforeProcessId
	     FROM $DataIn.process_bom A  
         LEFT JOIN $DataIn.process_data D ON D.ProcessId=A.ProcessId   
         LEFT JOIN $DataIn.process_type PT ON PT.gxTypeId=D.gxTypeId
	     WHERE  A.StuffId='$gStuffId' ORDER BY PT.SortId,A.ProcessId ASC";
}
//echo $ProcessSql;
$ProcessResult=mysql_query($ProcessSql,$link_id);
$i=1;

if ($ProcessRow = mysql_fetch_array($ProcessResult)) {
$d=anmaIn("download/process/",$SinkOrder,$motherSTR);
$LowQty=0;
	do{
		$ProcessId=$ProcessRow["ProcessId"];
		$ProcessName=$ProcessRow["ProcessName"];
		$Relation=$ProcessRow["Relation"];
        $Price=$ProcessRow["Price"]==""?"&nbsp;":$ProcessRow["Price"];
        $Picture=$ProcessRow["Picture"];
        $ProcessQty=ceil($OrderQty*$Relation);
        include "subprogram/process_Gfile.php";	//图档显示
        $BeforeProcessId=$ProcessRow["BeforeProcessId"];
        $BeforeProcessNameStr ="";
      
        if($BeforeProcessId!=0){
            $BeforeResult = mysql_query("SELECT ProcessName FROM $DataIn.process_data WHERE ProcessId IN ($BeforeProcessId)",$link_id);
            while($BeforeRow = mysql_fetch_array($BeforeResult)){
                $BeforeProcessName = $BeforeRow["ProcessName"];
                $BeforeProcessNameStr = $BeforeProcessNameStr==""?$BeforeProcessName:$BeforeProcessNameStr.",".$BeforeProcessName;
            }
        }else{
            $BeforeProcessNameStr = "&nbsp;";
        }
  
        if($StockId>0 && in_array($Login_P_Number, array(10871,10025,10649,10019)) ){
	        $delR = "<a onclick='delProcessId(this,$StockId,$ProcessId)'><span style='color:#FF0000'>x</span></a>";
        }else{
	        $delR ="&nbsp;";
        }
  
		echo"<tr bgcolor=#EAEAEA>";
		echo"<td align='center'>$delR</td>";
		echo"<td align='center'>$i</td>";	 //序号
		echo"<td align='center'>$ProcessId</td>";	 //工序ID
                echo"<td>$ProcessName</td>";	 //工序名称
		echo"<td align='center'>$Gfile</td>";	 //工序图档
               // echo"<td align='center'>$Price</td>";	 //序号
		echo"<td align='center'>$Relation</td>";	 //对应关系
		echo"<td align='center'>$BeforeProcessNameStr</td>"; //约束关系
         if ($StockId>0){
                $POrderId=substr($StockId,0,12);
		        $CheckthisGxQty=mysql_fetch_array(mysql_query("SELECT IFNULL(SUM(C.Qty),0) AS gxQty FROM $DataIn.sc1_gxtj C WHERE C.POrderId='$POrderId' AND C.StockId='$StockId' AND C.ProcessId='$ProcessId'",$link_id));
		      $thisGxQty=$CheckthisGxQty["gxQty"]==""?0:$CheckthisGxQty["gxQty"];
                  $BassLoss=$ProcessRow["BassLoss"];
                  $BassQty=floor($BassLoss*$MaxQty);
                   $BassLoss=($BassLoss*100)."%";
                   if($LowQty==0){
                            $LowQty=$MaxQty-$BassQty;
                       }
                   else{
                         $LowQty=$LowQty-$BassQty;
                   }
                 if($thisGxQty>=$LowQty)$thisGxQty="<span class='greenB'>$thisGxQty</span>";
                 else{
                       $thisGxQty="<span class='yellowB'>$thisGxQty</span>";
                      }
                   echo"<td align='center'>$BassLoss</td>";	 //损耗比率
		           echo"<td align='center'>$BassQty</td>";	 //损耗量
		           echo"<td align='center'>$LowQty</td>";	 //最低登记量
                   echo"<td align='center'>$ProcessQty</td>";	 //需求数量
		           echo"<td align='center'>$thisGxQty</td>";	 //完成数量
                }
		echo"</tr>";
		$i++;
 	   }while ($ProcessRow = mysql_fetch_array($ProcessResult));
	}
else{
	echo"<tr><td height='30' colspan='8' >没有加工工序明细资料,请检查.</td></tr>";
	}
echo"</table>";
?>