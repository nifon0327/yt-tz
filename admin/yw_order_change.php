...
$checkBlRow = mysql_fetch_array(mysql_query("SELECT COUNT(*) AS blCount  FROM (
SELECT getCanStock(sPOrderId,3) AS canStock
FROM $DataIn.yw1_scsheet SC  WHERE POrderId ='$POrderId' AND ActionId =101) A  WHERE A.canStock>0",$link_id));
$blCount = $checkBlRow["blCount"];
$tableWidth=1130;$tableMenuS=700;

//if($blCount==0){ //有一张工单已备料，就不能做需求单异动
	$CustomFun="<span onClick='ViewStuffId(7)' $onClickCSS>添加配件</span>&nbsp;";//自定义功能
    $CheckFormURL="thisPage";
/*}else{
	$CheckFormURL ="";$CustomFun="";$SaveSTR ="NO";
}*/
...
?>
    ...
    <table width='<?php  echo $tableWidth?>' border="0" cellspacing="0" bgcolor="#FFFFFF">
		...
		<tr>
			<td width="10" class="A0010">&nbsp;</td>
			<td height="25" colspan="7" valign="bottom"><span class="redB">◆已有需求明细</span></td>
			<td width="10" class="A0001">&nbsp;</td>
		</tr>
	</table>
	<table width='<?php    echo $tableWidth?>' border="0" cellspacing="0" bgcolor="#FFFFFF">
		...
		<div style="width:100%;height:100%;overflow-x:hidden;overflow-y:auto">
                ...
                <table width='100%' cellpadding="0" cellspacing="0" bgcolor="#FFFFFF" >
                  <tr>  
                    ...
                    <td width="50" class="A0001" align="center">供应商</td>
		...
		$StockResult = mysql_query("SELECT 
			...
			WHERE A.POrderId='$POrderId' AND A.Level=1 
			...
			do{
               ...
               include"../model/subprogram/stuff_Property.php";//配件属性
				...
				echo"<td width='70' class='A0101' align='center'>$Date</td>
					...
					<td width='50' class='A0101'>$Forshort</td>
					<td width='' class='A0101' align='center'>
						<input name='Unites[]' type='text' id='Unites$i' size='14' value='$UniteStuffId'  onclick='updateJq(this,$i,1,\"StuffList\")'  readonly>
		...
		for(var i=0;i<Rowslength;i++){
			...
			if(Message==""){
				...
				if (returnValue){
					...
					oTD.innerHTML="<input name='Unite[]' type='text'  id='Unite"+tmpNum+"' value=''  size='14' onclick='updateJq(this,"+tmpNum+",1,\"ListTable\")'  readonly/>";
					oTD.className ="A0101";
					oTD.align="center";
					oTD.width="";//100
	...
	if(TableTemp==ListTable){	//新增需求单列表
		...
		if (message==true){
			...
			if(delRemark!=""){ 
				...
				myurl="yw_order_updated.php?StockId="+delStockId+"&ActionId=delStuff&StockRemark="+delRemark;
				var delinfo="";
...
function CheckForm(){
	//passvalue("StuffId|pandsQty|Unite");  // 必须与上面隐藏传递元素id0号一致,Pid0	
	var tableLength = parseInt(ListTable.rows.length);
	
    if(tableLength>0){
	    swal({   
	        title: "是否更改标准图？",   
	        text: "需求单异动后请确认是否更改标准图？",   
	        type: "warning",   
	        ...
	        closeOnCancel: false 
	          ...
	          if (isConfirm) {     
	               document.form1.action="yw_order_updated.php?ActionId=23&updateTestStandard=1"; 
	           } else {     
	               document.form1.action="yw_order_updated.php?ActionId=23&updateTestStandard=0";
	          }
