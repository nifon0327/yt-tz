...
$funFrom="yw_order";
$helpFile=1;//有帮助文件
$nowWebPage=$funFrom."_waiting";
$ColsNumber=28;
$sumCols="12,13,14";
$ActioToS="1";  
$unColorCol=0;//不着色列
$Th_Col="操作|55|序号|30|PO|80|中文名|210|&nbsp;|30|Product Code|150|已出<br>(下单次数)|70|退货|50|成品重<br>(g)|50|Unit|40|Price|55|利润|80|Qty|50|Amount|70|订单利润|60|订单备注|110|待出备注|110|Air/Sea|60|采购|45|备料|45|组装|45|待出|45|交期|70|操作员|55|期限|40|背卡<br>条码|30|PE袋<br>条码|30|白盒<br>坑盒|30|外箱<br>标签|30|报价规则|450";	
...
if($From!="slist"){
	...
	WHERE S.Estate>=2 $ClientStr  GROUP BY M.CompanyId order by Amount DESC,M.CompanyId",$link_id);
	...
	WHERE S.Estate>=2 $SearchRows GROUP BY P.TypeId ORDER BY Amount DESC,T.TypeId",$link_id);
...
WHERE 1 and S.Estate>=2 $SearchRows ORDER BY M.OrderDate ASC";
//echo $mySql;
...
if($myRow = mysql_fetch_array($myResult)){
	...
	do{
       ...
       /*//因为可备料订单和已备料订单是已中断本次循环得出，但是计算的总和还是已全部订单计算。所以不正确，在此再计算一次(目前没想到其他方面)         
			...
			if($checkExpressRow = mysql_fetch_array($checkExpress)){
				do{
					...
					switch($Type){
                        ...
                        //拆分订单
                $checkSplit=mysql_query("SELECT SPOrderId FROM $DataIn.yw1_ordersplit WHERE POrderId='$POrderId' LIMIT 1",$link_id);
			...
			if($Orders>0){
				...
				//	array(0=>$sgRemark, 		1=>"style='color:#F00;'",3=>"..."),
                   array(0=>$dcRemark, 		1=>"align='center'",3=>"..."),
...
ChangeWtitle($SubCompany.$DefaultClient."客户待出订单明细列表");
