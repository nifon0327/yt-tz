...
if($From!="slist"){
	...
	//年分
	echo "<select name='ReturnYear' id='ReturnYear' onchange='document.form1.submit()'>";
	/*
	...
	*/
	$result = mysql_query("SELECT substring(ReturnMonth,1,4) as ReturnYear from $DataIn.product_returned  group by ReturnYear   ORDER BY ReturnYear  DESC",$link_id);	
	
	if($myrow = mysql_fetch_array($result)){
		do{
			$theReturnYear=$myrow["ReturnYear"];
			$ReturnYear=$ReturnYear==""?$theReturnYear:$ReturnYear;
			if($ReturnYear==$theReturnYear){
				echo"<option value='$theReturnYear' selected>$theReturnYear 年</option>";
				$SearchRows.=" AND substring(R.ReturnMonth,1,4)='$theReturnYear' " ;  
				}
			 else{
			 	echo"<option value='$theReturnYear'>$theReturnYear 年</option>";
				}
	...
	*/
	...
	echo"</select>&nbsp;";
	/*
	echo "SELECT P.TypeId,T.TypeName,T.Letter,C.Color 
	...
	WHERE T.Estate=1 $SearchRows GROUP BY P.TypeId ORDER BY T.mainType DESC,T.Letter";
	*/
...
//步骤4：需处理-条件选项
//$Pagination=$Pagination==""?0:$Pagination;	//默认分页方式:1分页，0不分
