...
$mySql1="SELECT M.ModelId, M.CompanyId,M.InvoiceNO,M.Wise,M.Notes,M.Terms,M.PaymentTerm,M.Ship,M.Date,U.Symbol,I.Company,I.Fax,I.Address,D.InvoiceModel,D.SoldTo,D.Address AS ToAddress,D.FaxNo,D.SoldFrom,D.FromAddress,D.FromFaxNo,
B.Beneficary,B.Bank,B.BankAdd,B.SwiftID,B.ACNO,S.Nickname,S.Name as ZName,C.PriceTerm,
O.OutCompanyName,O.OutAddress,O.OutTel,O.OutFax,O.OutURL,O.OutRequistion,O.OutReqTel,O.OutBeneficiary,O.OutBeneficiaryCode,O.OutSWIFTAddress,O.OutAccountName,O.OutAccountNumber,O.OutBankAddress,O.OutRemark,UC.Symbol AS OutSymbol
...
LEFT JOIN $DataIn.currencydata UC ON O.OutCurrency=UC.Id 
LEFT JOIN $DataIn.my2_bankinfo B ON B.Id=M.BankId
...
if($mainRows = mysql_fetch_array($mainResult)){
	...
	$OutSymbol=$mainRows["OutSymbol"]=="USD"?"U.S.DOLLARS":$mainRows["OutSymbol"];
	...
	//**********************************
	if ($ModelId==482){
		$PaymentTerm="T/T AWB-B/L date at the end of each month";
		$Notes="GOODS MADE IN CHINA <br> $Notes";
	}

	$OutCompanyName=$mainRows["OutCompanyName"];
	$OutAddress=$mainRows["OutAddress"];
	$OutRequistion=$mainRows["OutRequistion"];
	$OutBeneficiary=$mainRows["OutBeneficiary"];
	$OutBeneficiaryCode=$mainRows["OutBeneficiaryCode"];
	$OutSWIFTAddress=$mainRows["OutSWIFTAddress"];
	$OutAccountName=$mainRows["OutAccountName"];
	$OutAccountNumber=$mainRows["OutAccountNumber"];
	$OutBankAddress=$mainRows["OutBankAddress"];
	$OutRemark=$mainRows["OutRemark"];

	$OutTel=$mainRows["OutTel"];
	$OutFax=$mainRows["OutFax"];
	$OutURL=$mainRows["OutURL"];
	$OutReqTel=$mainRows["OutReqTel"];

	$SoldFrom=$mainRows["SoldFrom"];
	$FromAddress=$mainRows["FromAddress"];
	$SoldTo=$mainRows["SoldTo"];
	$Address=$mainRows["ToAddress"];
...
$Total=0;
//$Id='184';
$mySql2="SELECT S.Id,S.POrderId,O.OrderPO,P.cName,P.eCode,P.Description,S.Qty,P.OutPrice as Price,S.Type,S.YandN,S.ProductId  as ProductId,PI.PaymentTerm,P.bjRemark,P.TypeId,R.OrderPO as OutOrderPO
...
S.Qty,S.Price,S.Type,S.YandN,'0' as ProductId,'' AS PaymentTerm ,'' AS bjRemark,'' AS TypeId,'' as OutOrderPO 
FROM $DataIn.ch1_shipsheet S 
...
if($sheetRows = mysql_fetch_array($sheetResult)){
	...
	do{
		...
		$OutOrderPO=$sheetRows["OutOrderPO"];  //add by zx 2015-11-11
		if($OutOrderPO!=""){
			$OrderPO=$OutOrderPO;
		}
...
include "invoicetopdf/invoicepublicTotalOUT.php";  //把每一项分离
...
if ($plRows = mysql_fetch_array($plResult)){
	...
	do{
		...
		<tr bgcolor=#CCCCCC>
		<td  width=16 valign=middle height=$RowsHight style=bold>Total</td>
		<td  width=138 valign=middle align=center style=bold>Cartons:$BoxSUM </td>		
		<td  width=15 valign=middle align=right style=bold>$packingSUMQty</td>
		<td  width=13 valign=middle align=right style=bold>$NgSUM</td>
		<td  width=13 valign=middle align=right style=bold>$WgSUM</td>
		</tr>		  
...
include "invoicetopdf/invoicemodel_1OUT.php";
