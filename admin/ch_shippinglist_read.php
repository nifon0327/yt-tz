<?php
echo "<script src='../cjgl/cj_function.js' type=text/javascript></script>";
...
if ($DeliverySign == 0 && $DeliverySign != "") {
    $Th_Col = "选项|60|序号|40|出货流水号|70|客户名称|90|车牌号|100|车次单号|120|送货单号|110|送货单|80|PI编号|80|出货方量|70|工字钢|60|木方|60|出货日期|100|货运信息|100|操作员|50|PI操作人|80|生成时间|100";
//	 $Th_Col="选项|60|序号|40|出货流水号|70|客户|90|销售模式|60|Invoice名称|110|Invoice|80|外箱标签|60|出货金额|70|出货数量|70|已出数量|70|出货日期|70|货运信息|120|How to Ship|80|报关公司|80|报关单|120|快递签收单|120|Forward签收单|120|发票单|120|出货分类|60|备注|140|操作员|50";
    ...
    $Th_Col = "选项|60|序号|40|出货流水号|70|客户名称|90|车牌号|100|车次单号|120|送货单号|110|出货照片|80|PI编号|80|出货方量|70|工字钢|60|木方|60|出货日期|100|货运信息|100|操作员|50|PI操作人|80|生成时间|100";
...
WHERE 1 $SearchRows GROUP BY M.Wise ORDER BY M.Wise", $link_id);
    ...
    $date_Result = mysql_query("SELECT  M.Date 
        ...
        LEFT JOIN $DataIn.productdata P ON P.ProductId=SS.ProductId
        INNER JOIN $DataIn.producttype T ON T.TypeId=P.TypeId 
        WHERE 1 $SearchRows GROUP BY M.Date ORDER BY M.Date DESC", $link_id);
    ...
    if ($DeliverySign != "") {
        switch ($DeliverySign) {
            case 0:
                $SearchRows .= " AND M.Id IN (SELECT ShipId FROM $DataIn.ch1_shipout)";
                break;
            default:
                $SearchRows .= "";
                break;
...
echo "<select name='Pagination' id='Pagination' onchange='CencelPage()'><option value='1' $Pagination1>分页</option><option value='0' $Pagination0>不分页</option></select>
  	$CencalSstr <span class='ButtonH_25' onclick=batchCreatePI()>生成PI</span> <span class='ButtonH_25' onclick=exportStatements(this)>导出对账单</span>";
//步骤5：
...
M.Id,M.CompanyId,M.BankId,M.Number,M.PINO,M.PIcreator,M.PIcreatetime,M.InvoiceNO,M.InvoiceFile,M.Wise,M.Date,M.Estate,M.Locks,M.Sign,M.Remark,M.ShipType,M.Ship,M.Operator,M.CarNo,M.CarNumber,M.GZG,M.MF,M.ImageUrl,
C.Forshort,C.PayType,S.InvoiceModel,S.LabelModel,S.OutSign ,T.Type as incomeType,F.Forshort AS bgForshort,C.SaleMode,T.BgBillNum,T.Attached AS BgAttached,IFNULL(M.VOL,SUM(IFNULL(L.volume,TG.DwgVol))) AS VOL 
FROM $DataIn.ch1_shipmain M
LEFT JOIN $DataIn.ch1_shiptypedata T ON T.Mid=M.Id 
LEFT JOIN $DataIn.freightdata  F ON F.CompanyId = T.CompanyId 
LEFT JOIN $DataIn.trade_object C ON C.CompanyId=M.CompanyId 
...
LEFT JOIN $DataIn.trade_drawing TG ON TG.ProdcutCname = P.cName 
WHERE 1 $SearchRows
GROUP BY M.InvoiceNO
ORDER BY M.InvoiceNO DESC";
//echo $mySql;
...
if ($myRow = mysql_fetch_array($myResult)) {
    ...
    do {
        ...
        $PINO = $myRow["PINO"];
        $PIcreatetime = $myRow["PIcreatetime"];
        $PIcreator = $myRow["PIcreator"];
        $Number = $myRow["Number"];
        ...
        $ImageUrls = explode(";", $ImageUrlRow);
        $ImageUrlCount = count($ImageUrls);
        if ($ImageUrlRow == "") {
            $ImageUrl = "";
        } else {
            $ImageUrl = "<span onclick='ImageUrl($Id)' style='cursor: pointer;'> 查看 ($ImageUrlCount) </span>";
        }
        $Forshort = "<img location.href=\"#\"' style='CURSOR: pointer' onclick='upMainData(\"ch_shippinglist_picture\",\"$Id\")' src='../images/edit.gif' title='更新附档' width='13' height='13'>
        ...
        $bgForshort = "<img location.href=\"#\"' style='CURSOR: pointer' onclick='upMainData(\"Ch_shippinglist_upshipType\",\"$Id\")' src='../images/edit.gif' title='更新报关公司' width='13' height='13'>
        ...
        $expressBack = "<img location.href=\"#\"' style='CURSOR: pointer' onclick='upMainData(\"Ch_shippinglist_expressBack\",\"$Id\")' src='../images/edit.gif' title='上传快递签收单' width='13' height='13'><span class='yellowB'>$expressBackNumber</span>";

        $ForwardBack = "<img location.href=\"#\"' style='CURSOR: pointer' onclick='upMainData(\"Ch_shippinglist_ForwardBack\",\"$Id\")' src='../images/edit.gif' title='上传Forward签收单' width='13' height='13'><span class='yellowB'>$ForwardBackNumber</span>";

        $BillNum = "<img location.href=\"#\"' style='CURSOR: pointer' onclick='upMainData(\"Ch_shippinglist_billnum\",\"$Id\")' src='../images/edit.gif' title='上传发票' width='13' height='13'><span class='yellowB'>$BillNum</span>";
        $BgBillNum = "<img location.href=\"#\"' style='CURSOR: pointer' onclick='upMainData(\"Ch_shippinglist_upshipType\",\"$Id\")' src='../images/edit.gif' title='上传报关单' width='13' height='13'><span class='yellowB'>$BgBillNum</span>";

        $BankId = "<img location.href=\"#\"' style='CURSOR: pointer' onclick='upMainData(\"Ch_shippinglist_upbankID\",\"$Id\")' src='../images/edit.gif' title='更新银行账号' width='13' height='13'>
        ...
        $Ship = $Ship = "" ? "&nbsp;" : $Ship;
        $Ship = "<img location.href=\"#\"' style='CURSOR: pointer' onclick='upMainData(\"Ch_shippinglist_upship\",\"$Id\")' src='../images/edit.gif' title='更新出货方式' width='13' height='13'>
        ...
        $Remark = "<img location.href=\"#\"' style='CURSOR: pointer' onclick='upMainData(\"Ch_shippinglist_upmain\",\"$Id\")' src='../images/edit.gif' title='更新备注' width='13' height='13'>
        ...
        // 出货数量+方量
        $sListSql = "SELECT sum(T.Weight) AS Weight,sum(T.CVol) AS CVol
	...
	WHERE S.Mid='$Id' AND S.Type='1'";
        ...
        if ($DeliverySign == 0) {
            ...
            $ValueArray = array(
                ...
                array(0 => '' . $OutPdfFile, 1 => "align='center'"),
                array(0 => floor($volume * 100) / 100, 1 => "align='center'"),
...
//                array(0 => $InvoiceFile . $OutPdfFile, 1 => "align='center'"),
                array(0 => $ImageUrl, 1 => "align='center'"),
                array(0 => $PINO . $OutPdfFile, 1 => "align='center'"),
                array(0 => floor($volume * 100) / 100, 1 => "align='center'"),
                array(0 => $gzg, 1 => "align='center'"),
                array(0 => $mf, 1 => "align='center'"),
                //			   array(0=>$Amount,	1=>"align='right'"),
        ...
        $checkidValue = $Id . '|' . $volume . '||' . $tWeight;
...
<script>
    function batchCreatePI() {
        ...
        if (jQuery('input:checked').length > 0) {
            ...
            alert('至少需选择一条记录，请选择生成PI的订单');
            return;
        }
        jQuery.ajax({
            url: 'create_PI_ajax.php',
            data: {
                Ids: Ids
            },
            type: 'post',
    ...
    function exportStatements(e) {
        openWinDialog(e, "export_statement.php", 500, 500, 'bottom');
    }

    function exportState() {
        var start_date = jQuery('input[name="start_date"]');
        var end_date = jQuery('input[name="end_date"]');
        var client = jQuery('select[name="Company"] option:selected');

        start_date.change(function () {
            start_date.attr("value", jQuery(this).val()); //赋值
        });
        end_date.change(function () {
            end_date.attr("value", jQuery(this).val()); //赋值
        });
        ...
        document.form1.action = "export_statement_ajax.php?clientValue=" + clientValue + '&startValue=' + startValue + '&endValue=' + endValue + '&clientName=' + clientName;
