...
$ClientResult = mysql_query("SELECT CompanyId,Sign FROM $DataIn.ch0_shipmain WHERE Id=$Id LIMIT 1",$link_id);
		...
		<table width='1110' border='0' cellspacing='0' id='ListTable$i' style='TABLE-LAYOUT: fixed; WORD-WRAP: break-word'>
			...
			<tr>
                ...
                <td  valign="top" bgcolor="#FFFFFF" class="A0110" align="left"> 
					<div style="width:1010;height:166;overflow-x:hidden;overflow-y:scroll"> 
						<table width='1009' cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
						<?php   	//记录输出
						$oIdArray=array();
						$rowArray=array();
						$POrderResult = mysql_query("
						SELECT C.POrderId,C.Type,C.ProductId,S.OrderPO,C.Qty,S.dcRemark,S.sgRemark,P.cName,P.eCode,P.Weight  
							FROM $DataIn.ch0_shipsheet C 
							LEFT JOIN $DataIn.yw1_ordersheet S ON S.POrderId=C.POrderId 
							LEFT JOIN $DataIn.productdata P ON P.ProductId=S.ProductId 
							WHERE C.Mid='$Id' and C.Type='1' ",$link_id);
						$i=1;
						...
						do{
								...
								$dcRemark=$POrderRows["sgRemark"]==""?"&nbsp;":$POrderRows["sgRemark"];
								$Qty=$POrderRows["Qty"];
								...
								//需检查是否已经装箱，计算未装箱的数量
								$Readonly="";$Disabled="";
								$checkPacking=mysql_query("SELECT SUM(BoxQty*BoxPcs) AS inboxQty FROM $DataIn.ch0_packinglist WHERE Mid='$Id' AND  POrderId='$POrderId'",$link_id);
								...
								//计算填入初始化装箱数据
								// add by zx 2013-03-18 增加了生产凳记的每个外箱数量
								$tmpShow="";
								$extraWeight=0;
							   ...
							   if ($Relation!=""){
								   ...
								   $tmpURelation= $Relation;
								   $BoxPcs=intval($unBoxQty/$Relation);
								   //echo "$i:Relation:" . $Relation ."<br>";
								   ...
								   if ($BoxPcs>=0){
									    ...
									    */
										 ...
										 if($BoxPcs>0){
											...
											$realWeight=number_format(($unBoxQty*$Weight+$extraWeight)/1000,2);
											$tmpURelation=$unBoxQty;  //no more than one box's Qty
										 }
										 //echo "$i:realWeight:" . $realWeight ."<br>";
										 //echo "realWeight:" . $realWeight;
										  //$tmp="$realWeight:(($Relation*$Weight+$extraWeight)/1000,2)";
										  $tmpXX=$realWeight;
									   }
									   else{
										 $realWeight=0;
								...
								//echo"<td width='200' class='A0101'><span class='redB'>$dcRemark ($tmpXX:($tmpURelation*$Weight+$extraWeight)/1000,2) </span></td>";
								echo"<td width='200' class='A0101'><span class='redB'>$dcRemark($eCode) </span></td>";
								...
								echo"<td width='130' class='A0100' align='Center'><input type='text' name='BoxSpec$i' size='20' class='I0000L'  value='$Spec' $Disabled> $tmpShow</td>";
								echo"<td width=''>&nbsp;</td>";
								echo"</tr>";
								$i++;
		...
		<table width='1110' height="90%" border='0' cellspacing='0' id='ListTable$i' style='TABLE-LAYOUT: fixed; WORD-WRAP: break-word'>
            ...
            <td  valign="top" bgcolor="#FFFFFF" class="A1010" align="left">
                ...
                <table width='1009' cellpadding="0" cellspacing="0" bgcolor="#FFFFFF" >
                   <tr>
                    ...
                    <td width='180' bgcolor='<?php    echo $Title_bgcolor?>' align='center' class="A0101">PO</td>
			...
			<tr>
				...
				<td  height="100%" valign="top" bgcolor="#FFFFFF" class="A0110" align="left">
					...
					<table width='1009' cellpadding="0" cellspacing="0" bgcolor="#FFFFFF" id='boxTB'>
					<?php   
					$OrderNum=count($rowArray);
					//装箱内容
					$plResult = mysql_query("SELECT L.BoxRow,L.BoxPcs,L.BoxQty,L.FullQty,L.WG,L.POrderId,L.BoxSpec,e.OrderPO,e.eCode,e.BoxCode,E.OtherEcode 
											FROM $DataIn.ch0_packinglist L
											LEFT JOIN $DataIn.ch0_packpoecodebar e ON e.Mid=L.id 
											WHERE L.Mid='$Id' ORDER BY L.Id ",$link_id);	
					//echo "SELECT L.BoxRow,L.BoxPcs,L.BoxQty,L.FullQty,L.WG,L.POrderId,L.BoxSpec FROM $DataIn.ch2_packinglist L WHERE L.Mid='$Id' ORDER BY L.Id ";
					if ($plRows = mysql_fetch_array($plResult)){
						...
						do{
							...
							if($BoxRow==0){//并箱非首行
							     //<td class='A0101' align='center' width='80'  height='20' onclick='setOrderPo(this)'>$j $OrderPO</td>
								echo"<tr>
									<td class='A0101' align='left' width='180'  height='20' onclick='setOrderPo(this)'> $OrderPO</td>			
									<td class='A0101' width='220' >$cName <br>(ecode:$eCode) </td>
								...
								echo"<tr>
									...
									<td class='A0101' width='180' align='left'  onclick='setOrderPo(this)'>$OrderPO</td>
									<td class='A0101' width='220'>$cName <br>(ecode:$eCode)</td>
...
function setOrderPo(object){
	var OrderPo=prompt("请输入PO|产品代码(英文)|打码|另一产品代码(英文)，使默认值*号代替，默认值：*|*|80123456789012，表示只更新条码","");
	if(OrderPo != null ){
		object.innerHTML=OrderPo;
	}
	/*
	alert(boxTB.rows[2].cells[1].innerHTML);
	alert(boxTB.rows[2].cells[2].innerHTML);
	alert(boxTB.rows[2].cells[3].innerHTML);
	*/
}
			...
			if(boxTB.rows[m].cells[2].data>1){//并行处理
				...
				for(var n=0;n<k;n++){
					if(n==0){					//首行
						...
						boxTB.rows[m].cells[5].innerText+"^^"+
						boxTB.rows[m].cells[1].data+"^^"+
						boxTB.rows[m].cells[6].innerText+"^^"+
						boxTB.rows[m].cells[7].innerText+"^^"+
						boxTB.rows[m].cells[8].innerText; */
						PackingList=PackingList+","+
						boxTB.rows[m].cells[2].data+"^^"+
						boxTB.rows[m].cells[3].data+"^^"+
						...
						boxTB.rows[m].cells[3].innerHTML;  //记录PO等记录
						}
					else{						//其它行
						m++;
						//PackingList=PackingList+","+"^^"+boxTB.rows[m].cells[0].data+"^^"+boxTB.rows[m].cells[2].innerText+"^^^^^^^^";
						PackingList=PackingList+","+"^^"+boxTB.rows[m].cells[0].data+"^^"+boxTB.rows[m].cells[2].innerHTML+"^^^^^^^^"+"^^"+boxTB.rows[m].cells[0].innerHTML;
						}
                       //alert(PackingList)
				...
				//非并行处理
				/*
				   			PackingList=PackingList+","+boxTB.rows[m].cells[2].data+"^^"+boxTB.rows[m].cells[3].data+"^^"+boxTB.rows[m].cells[5].innerText+"^^"+boxTB.rows[m].cells[1].data+"^^"+boxTB.rows[m].cells[6].innerText+"^^"+boxTB.rows[m].cells[7].innerText+"^^"+boxTB.rows[m].cells[8].innerText;
			*/
			PackingList=PackingList+","+boxTB.rows[m].cells[2].data+"^^"+boxTB.rows[m].cells[3].data+"^^"+boxTB.rows[m].cells[5].innerHTML+"^^"+boxTB.rows[m].cells[1].data+"^^"+boxTB.rows[m].cells[6].innerHTML+"^^"+boxTB.rows[m].cells[7].innerHTML+"^^"+boxTB.rows[m].cells[8].innerHTML+"^^"+boxTB.rows[m].cells[3].innerHTML;
				}
			}
		document.form1.PackingList.value=PackingList;
		document.form1.action="ch0_shippinglist_packingsave.php?toPackingList="+toPackingList;
...
function uniteBOX(){//并箱
	...
	var returnValue =window.showModalDialog("ch0_shippinglist_tountiebox.php",window,"dialogWidth=400px;dialogHeight=300px");
	if (returnValue){
		...
		if(checkNUM(wgValue)==1){//检查数字,合法则新建行			
			...
			//首行数据设定
			//定义列宽
			newTd1.width=80;newTd2.width=40;newTd3.width=60;newTd4up.width=180;newTd5up.width=220;newTd6up.width=60;newTd7.width=60;newTd8.width=60;newTd9.width="150";  //newTd9.width=150;			
			if(boxsValue>1){
			...
			newTd4up.height="20";
			//newTd4up.onclick=setOrderPo(this);
		...
		if ((e.type=="checkbox")&&(e.checked==true)){
			...
			if (Residualtemp>0){
				...
				if (newResidual>=0  && pcstemp>0 && wgtemp>0 && numtemp>0 && spectemp!=""){					
					...
					oTD.innerHTML= Split_ValueStr[2]+"(ecode:"+Split_ValueStr[3]+")";
					oTD.width="220";
