...
foreach ($Ids as $k => $v) {
    // 获取构件名称，生产时间
    $cName = $scDate = $liningNo = $RealLining = ' ';
    $mySql = "SELECT DISTINCT T.Forshort,P.cName,SC.scDate,S.liningNo,IFNULL(S.RealLining,S.liningNo) as RealLining,IFNULL(P.CStr,TD.CStr) as CStr ,S.POrderId,W.Name AS scName,P.ProductId 
        FROM yw1_scsheet SC
        ...
        LEFT JOIN trade_drawing TD ON  T.Id = TD.TradeId
        LEFT JOIN workshopdata W ON W.Id = SC.WorkShopId
        WHERE S.Estate > 0 AND S.Id=$v";
    ...
    $objPHPExcel->setActiveSheetIndex($k)->setTitle($POrderId);
    // 插入图片
    ...
    $objPHPExcel->getActiveSheet()->setCellValue("F2", $scName);
