	...
	if($pi==$Counts) //说明最后一条记录情况,有了统计，则要计算统计的高度。
	{
		...
		if(($$eurTableNo)!=""){   //输出表的内容。
			...
			//,Total
			{
				...
				$chSUMQty=number_format( $chSUMQty);


				$pdf->SetXY($CurMargin+140,$tmpTableY);
				$tmptable="<table border=0 >
							<tr >
							    <td width=14 align=right valign=middle style=bold  height=$RowsHight>$chSUMQty</td>
								<td width=14 align=right valign=middle style=bold  ></td>
								<td width=17 align=right valign=middle style=bold  >$T_Total</td>
