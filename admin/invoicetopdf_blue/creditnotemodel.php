...
<td width=128 align=left valign=middle >CHARGE DETAILS</td>				
<td width=14 align=right valign=middle >QTY</td>	
<td width=17 align=right valign=middle >U/P</td>	
<td width=17 align=right valign=middle >AMOUNT</td></tr></table>" ;//$eurTableList;
...
for ($pi=1;$pi<=$Counts;$pi=$pi+1){  //1.循环输出
	$eurTableNo="eurTableNo".strval($pi);
     if($addSign==0){  //2.新加一页
		...
		$tmpType="DEBIT NOTE";
		$tmpShipType="DEBIT TO:";
		if($ShipType=="credit"){   //分为DEBIT 和 CREDIT
			$tmpType="CREDIT NOTE";
			$tmpShipType="CREDIT TO:";
		}

		include "invoicetopdf_blue/creditpublicHeader.php";  //页头
