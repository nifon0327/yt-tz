...
<td width=128 align=left valign=middle >费用说明</td>				
<td width=14 align=right valign=middle >数量</td>	
<td width=17 align=right valign=middle >单价</td>	
<td width=17 align=right valign=middle >金额</td></tr></table>" ;//$eurTableList;
...
for ($pi=1;$pi<=$Counts;$pi=$pi+1){  //1.循环输出
	$eurTableNo="eurTableNo".strval($pi);
     if($addSign==0){  //2.新加一页
		...
		include "invoicetopdf_blue/creditpublicHeaderS.php";  //页头
	...
	if($pi==$Counts) //说明最后一条记录情况,有了统计，则要计算统计的高度。
	{
		...
		if(($$eurTableNo)!=""){   //输出表的内容。
			...
			//,Total
			{
				...
				$pdf->SetXY($CurMargin+139,$tmpTableY);
				$tmptable="<table border=0 >
							<tr >
								<td width=14 align=right valign=middle style=bold  height=$RowsHight >$chSUMQty</td>
								<td width=14 align=right valign=middle style=bold  height=$RowsHight ></td>
