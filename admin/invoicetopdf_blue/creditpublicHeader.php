<?php   
		...
		$pdf->Text($CurMargin,$ConTableBegin_Y+6,"$tmpType");  //输出I
		...
		*/
		...
		//$pdf->htmltable($eurSInvoice) //是从上往下画的,表在后写时不会盖掉到原来的，
		{
			...
			*/
			//加重色////////////////////////////////////////////
			$pdf->SetFont('Arial','',$FontSize_TiTle+1);
			$pdf->SetTextColor(0,0,0);
			...
			//$tmpDate=date("d-M-y");
			$tmpDate=date("d-M-y",strtotime($ShipDate));
			$tmptable="<table border=0 >
				<tr>
					<td width=31 align=right style=bold ></td>
					<td width=10 align=right style=bold ></td>
		...
		//Note : REQUISITION BY:
		{
			...
			$tmptable="<table border=0 >
				<tr>
					...
					<td width=106 align=right style=bold >TRANSACTOR:</td>
				<tr>	
			</table>" ;//$eurTableList;
			$pdf->htmltable($tmptable);			
			...
			$tmptable="<table border=0 >
				<tr>
					<td width=80 align=left  >$Commoditycode$StableNote$Notes <br> $PaymentTerm$Priceterm$Terms <br> Currency:$Symbol </td>
