...
<td width=1 align=left valign=middle height=$emptyHight></td>	
<td width=10 align=center valign=middle>序号</td>	
<td width=18  align=center valign=middle >构件类型</td>
<td width=33 align=center valign=middle style=bold>PC编号</td>	
<td width=20 align=center valign=middle style=bold>长</td>
<td width=20 align=center valign=middle style=bold>宽</td>			
<td width=20 align=center valign=middle style=bold>高</td>
<td width=15 align=center valign=middle style=bold>单位</td>	
<td width=15 align=center valign=middle style=bold>数量</td>
<td width=30 align=center valign=middle style=bold>备注</td>
</tr></table>" ;
...
<td width=15 align=left valign=middle style=bold></td>	
<td width=15 align=left valign=middle style=bold></td>
<td width=15 align=left valign=middle style=bold></td>			
<td width=15 align=right valign=middle style=bold></td>
<td width=10 align=center valign=middle style=bold></td>	
<td width=10 align=center valign=middle style=bold></td>
...
for ($pi=1;$pi<=$Counts;$pi=$pi+1){  //1.循环输出
	$eurTableNo="ChinaTableNo".strval($pi);
	$eurAppFileNo="eurAppFileNo".strval($pi);
     if($addSign==0){  //2.新加一页
		...
		include "invoicetopdf_blue/invoicepublicHeaderS.php";  //页头
...
//			$pdf->Image($Boss_Logo,$CurMargin-5,$MaxContainY-10.5,199,0,"JPG");
	...
	if(($$eurTableNo)!=""){   //输出表的内容。
	 ...
	 if($$eurAppFileNo!=""){
			$pdf->Image($$eurAppFileNo,$CurMargin+1,$appFileY,10,10,"jpg");
		}
	...
	if($pi<$Counts){
		...
		$tmpNo="ChinaTableNo".strval($pi+1);
	...
	if($pi==$Counts) //说明最后一条记录情况,有了统计，则要计算统计的高度。
	{
		...
		if(($$eurTableNo)!=""){   //输出表的内容。
			...
			//,Total
			{
				...
				//$Boss_Logo="invoicetopdf_blue/BossSignatureSP.jpg";  //***************************加印    卖方印章 by.lwh
				//$pdf->Image($Boss_Logo,$CurMargin-5,$tmpTableY-2,199,0,"JPG");

				$pdf->SetXY($CurMargin,$tmpTableY+3);  //倒退银行预留度
				...
				/*
				$pdf->SetXY($CurMargin+142,$tmpTableY);
				$tmptable="<table border=0 >
							<tr >
							    <td width=14 align=right valign=middle style=bold  height=$RowsHight >$orderSUMQty</td>
								<td width=14 align=right valign=middle style=bold  >$chSUMQty</td>
								<td width=14 align=right valign=middle style=bold   >$unSendSUMQty</td>
							<tr>
						</table>";
				*/
				$T_Total=number_format( $Total ,2, '.' , ',' );
				$pdf->SetXY($CurMargin+137,$tmpTableY+3);
				$tmptable="<table border=0 >
							<tr >
							    <td width=15 align=right valign=middle style=bold  height=$RowsHight >$chSUMQty</td>
								<td width=30 align=right valign=middle style=bold   >$T_Total</td>
							<tr>	
						</table>";
				...
				$tmptable="<table border=0 >
							<tr>
								<td width=35 align=left valign=middle style=bold  height=$RowsHight>$Total</td>
							<tr>
						</table>";
				$pdf->htmltable($tmptable);  //统计
				...
				$tmptable="<table border=0 >
							<tr >
								<td width=17 align=right valign=middle style=bold  height=$RowsHight >$Total</td>
							<tr>
						</table>";
				$pdf->htmltable($tmptable);  //统计
...
$eurEmptyField="<table  border=0 >
<tr >
<td width=1 align=center height=$emptyHight valign=middle style=bold> </td>	
