...
<td width=21 align=left valign=middle style=bold>PO#</td>			
<td width=35 align=left valign=middle style=bold>产品代码</td>
<td width=75 align=left valign=middle style=bold>产品描述</td>			
<td width=14 align=right valign=middle style=bold>数量</td>	
<td width=14 align=right valign=middle style=bold>单价</td>	
<td width=17 align=right valign=middle style=bold>金额</td></tr></table>" ;//$eurTableList;
...
for ($pi=1;$pi<=$Counts;$pi=$pi+1){  //1.循环输出
	...
	if($pi<$Counts){
		...
		$tmpY=$pdftmp->GetMaxY();  ////用GetMaxY必须跟transhtmltabler后面才有效，专门用来取中文行
	...
	if($pi==$Counts) //说明最后一条记录情况,有了统计，则要计算统计的高度。
	{
		...
		if(($$eurTableNo)!=""){   //输出表的内容。
			...
			//,Total
			{
				...
				<td width=14 align=right valign=middle >合计:</td>	
				<td width=17 align=right valign=middle ></td></tr></table>";
				$pdf->transhtmltable($TeurTableNoTotal,0,0);
