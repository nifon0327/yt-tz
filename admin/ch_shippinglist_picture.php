...
//echo "<br> Id='$Id'";
//步骤3：//需处理
$upResult = mysql_query("SELECT Number,Sign,ShipType FROM $DataIn.ch1_shipmain WHERE Id='$Id' LIMIT 1",$link_id);
...
if(($ShipType=='credit') || ($ShipType=='debit') ){  //说明是Credit note 或者 是debit note	
	...
	$Parameter="Id,$Id,funFrom,$funFrom,From,$From,CompanyId,$CompanyId,ActionId,36,Number,$Number,ShipSign,-1,ShipType,$ShipType";
	echo"<table border='0' width='$tableWidth' cellpadding='0' cellspacing='0' bgcolor='#FFFFFF'><tr><td class='A0011'>
			<table width='800' border='0' align='center' cellspacing='0' id='NoteTable'>
				<tr>
					<td height='30' align='center' class='A1111'>手动上传扣款单</td><td class='A1101'>
					<input name='Attached' type='file' id='Attached' size='60' DataType='Filter' Accept='pdf' Msg='格式不对,请重选' Row='0' Cel='1'>
					</td>
				</tr>
			</table>
		</td></tr></table>";
	...
	function deleteImg(ImgId,rowIndex){
		...
		if (message==true){
			myurl="ch_shippinglist_updated.php?ImgId="+ImgId+"&ActionId=936";
	...
	<table border="0" width="<?php    echo $tableWidth?>" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF"><tr><td class="A0011">
		...
		//检查是否有旧文件,如果有则列出
		//如果没有
		$checkImgSql=mysql_query("SELECT Id,Remark,Picture FROM $DataIn.ch7_shippicture WHERE Mid='$Id' ORDER BY Id",$link_id);
