...
*/
   ...
   SELECT M.CompanyId,C.Forshort,SUM(G.OrderQty) AS blQty,IFNULL(SUM(L.Qty),0) AS llQty
   FROM $DataIn.yw1_ordermain M 
   LEFT JOIN $DataIn.yw1_ordersheet S ON M.OrderNumber=S.OrderNumber 
...
) A WHERE  A.blQty=A.llQty Group by A.CompanyId",$link_id);
/*
$ClientResult= mysql_query("SELECT M.CompanyId,C.Forshort FROM $DataIn.ck_bldatetime B 
			                  ...
			                  LEFT JOIN $DataIn.trade_object C ON M.CompanyId=C.CompanyId
			                  WHERE B.Estate=0 AND S.Estate=1 AND S.scFrom>0 Group by M.CompanyId",$link_id);
	*/		                  
	...
	*/
	$TypeResult= mysql_query("SELECT T.TypeId,T.TypeName,C.Color FROM $DataIn.ck_bldatetime B 
			                   LEFT JOIN  $DataIn.yw1_ordersheet S ON S.POrderId=B.POrderId 
			                   LEFT JOIN $DataIn.productdata P ON P.ProductId=S.ProductId 
			                   LEFT JOIN $DataIn.producttype T ON T.TypeId=P.TypeId 
			                   LEFT JOIN $DataIn.productmaintype C ON C.Id=T.mainType 
			                  WHERE B.Estate=0 AND S.Estate=1 AND S.scFrom>0 Group by T.TypeId",$link_id);
...
,S.DeliveryDate,S.ShipType,S.Estate,S.Locks,P.cName,P.eCode,P.Weight,P.TestStandard,P.pRemark,P.bjRemark,U.Name AS Unit,PI.PI,PI.Leadtime,SUM(if(K.tStockQty>=(G.OrderQty-IFNULL(L.Qty,0)),(G.OrderQty-IFNULL(L.Qty,0)),0)) as K1,SUM(G.OrderQty-IFNULL(L.Qty,0)) AS K2,SUM(IFNULL(L.llEstate,0)) AS llEstate   
FROM $DataIn.yw1_ordermain M
LEFT JOIN $DataIn.yw1_ordersheet S ON M.OrderNumber=S.OrderNumber
LEFT JOIN $DataIn.productdata P ON P.ProductId=S.ProductId
...
WHERE A.K1>=A.K2  AND A.llEstate=0 ORDER BY A.OrderDate ASC,A.Id DESC";
//echo $mySql; 
*/
	...
	do{
		...
		$CheckblState=mysql_query("
		 ...
		 if($blStateRow = mysql_fetch_array($CheckblState)){
			  ...
			  if ($R_blQty!=$R_llQty || $R_llEstate>0){
				     continue;
			  }
		 } 
	   ...
	   if (strlen(trim($ShipType))>0){
		...
		if($checkColorRow = mysql_fetch_array($checkColor)){
			...
			$m=1;
		    $ClientPC=$thisTOrmbOUTsum==0?0:sprintf("%.0f",$GrossProfitSUM*100/$thisTOrmbOUTsum);
			$ValueArray=array(
				array(0=>"&nbsp;"	),
				array(0=>"&nbsp;"	),
				array(0=>"&nbsp;"	),
...
ChangeWtitle($SubCompany.$DefaultClient."已备料订单列表");
$ActioToS="1"; 
