...
for ($pi=1;$pi<=$Counts;$pi=$pi+1){  //循环输出
	...
	$RemarkTableNo="RemarkTableNo".strval($pi);
     if($addSign==0){  //新加一页
		...
		include "invoicetopdf/invoicepublicHeaderOUT.php";  //页头
		
		{
			...
			<td width=38 align=center valign=middle style=bold color=#06F>$OutRequistion</td>
	...
	*/
	...
	if($pi==$Counts) //说明最后一条记录情况,有了统计，则要计算统计的高度。
	{
		...
		if(($$eurTableNo)!=""){   //输出表的内容。	
			...
			*/
				...
				$pdf->SetFont('Arial','',$InvoiceHeadFontSize); //设回字体大小
				$pdf->Text($CurMargin+175,$NowSY+3.5,"$OutSymbol"); 			
			
				$NowSY=$pdf->GetY(); // 
				$pdf->SetFont('Arial','B',$InvoiceHeadFontSize); //设回字体大小
				$pdf->Text($CurMargin+1,$NowSY+4,"BANK:"); 
				$pdf->SetXY($CurMargin,$NowSY);
				$pdf->SetFont('Arial','',$InvoiceHeadFontSize); //设回字体大小
	...
	if($addSign==0)		
	{
		include "invoicetopdf/invoicepublicFooterOUT.php";  //页尾  
	...
	if($isSeal==1){
		...
		include "invoicetopdf/invoicepublicSealOUT.php";  //封印
	 }
}
...
for ($pi=1;$pi<=$plCounts;$pi=$pi+1){  //循环输出
	...
	$eurplNo="plTableNo".strval($pi); 
     if($addSign==0){  //新加一页
		...
		include "invoicetopdf/invoicepublicHeaderOUT.php";  //页头invoicepublicHeaderOUT

		{
			...
			<td width=30 align=center valign=middle style=bold>$Wise</td>	
			<td width=24  bgcolor=#CCCCCC align=center valign=middle style=bold>Requisition by:</td>	
			<td width=39 align=center valign=middle style=bold>$OutRequistion</td>
			<td width=16  bgcolor=#CCCCCC align=center valign=middle style=bold>Page NO.:</td>
