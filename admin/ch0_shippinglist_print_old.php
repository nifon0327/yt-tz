...
if($BoxRow=mysql_fetch_array($Box_Sql)){
	...
	*/
	...
	$LableID=1;
	$BoxNum=1;
	$PackingResult = mysql_query("SELECT L.POrderId,L.BoxRow,L.BoxPcs,L.BoxQty,L.WG,L.BoxSpec,e.OrderPO,e.eCode,e.BoxCode 
	FROM $DataIn.ch0_packinglist L 
		...
		do{
			...
			switch($Type){
				case 1:
					...
					//echo "SELECT Lotto, itf FROM $DataIn.productprintparameter WHERE productId='$ProductId' order by Id Limit 1";
			...
			$FromCounty="";	
			//输出标签
		   if($LabelModel==46 || $LabelModel==48){ //DCASia
			...
			if($BoxRow==1){//单箱
				for($k=0;$k<$BoxQty;$k++){
					...
					ToLable($PreSymbol,$LableID,$LableSUM,$BoxNum,$Udate,$BoxTotal,$InvoiceNO,$OrderPO,$Description,$WG,$BoxPcs,$eCode,$BoxSpec,$BoxCode,$PackingUnit,$PackingRemark,$StartPlace,$EndPlace,$LabelModel,$cName,$NgWeight,$FromCounty,$InBoxPcs);
					$BoxNum++;				
					$LableID++;
					}
				}
