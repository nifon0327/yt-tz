...
if ($TRow = mysql_fetch_array($TResult)) {
    $unColorCol = 25;//不着色列
    $Th_Col = "选项|55|序号|30|业务单号|120|产品名称|210|&nbsp;|30|产品条码|200|模具编号|60|台车号|40|分配时间|80|已出<br>(下单次数)|70|退货|50|成品重<br>(g)|50|装箱单位|60|单价|55|数量|50|金额|70|利润|80|订单利润|60|订单备注|110|英文备注|110|生管备注|110|采购备注|110|待出备注|110|装运方式|60|采购|60|备料|60|组装|45|待出|60|交期|80|操作人|55|期限|60|报价规则|450|转发对象名称|150|产品报关方式|80";
    $ColsNumber = 29;
    $myTask = 1;
    $sumCols = "11,12,13";
} else {
    $unColorCol = 24;//不着色列
    $Th_Col = "选项|55|序号|30|业务单号|120|产品名称|210|&nbsp;|30|产品条码|200|优先级<br>（↓1-9↑）|80|模具编号|60|台车号|40|分配时间|80|已出<br>(下单次数)|70|退货|50|成品重<br>(g)|50|装箱单位|60|单价|55|数量|50|金额|70|订单备注|110|英文备注|110|生管备注|110|采购备注|110|待出备注|110|装运方式|60|采购|60|备料|60|组装|45|待出|60|交期|80|操作人|55|期限|60|报价规则|450|转发对象名称|100|产品报关方式|80";
...
if ($taxtypeId > 1) {
    $Th_Col = "选项|55|序号|30|客户名称|80|业务单号|120|产品名称|210|&nbsp;|30|产品条码|200|模具编号|60|台车号|40|分配时间|80|已出<br>(下单次数)|70|装箱单位|60|单价|55|数量|50|金额|70|采购备注|110|产品报关方式|80|操作人|55";
    $ColsNumber = 13;
    $myTask = 2;
    $sumCols = "9,10";
}
...
if ($From != "slist") {
	...
	WHERE S.Estate>0 AND M.OrderPO <>'' $SearchRows $nullable GROUP BY M.CompanyId ORDER by Amount DESC,M.CompanyId ASC", $link_id);
	...
	WHERE S.Estate>0  $ClientStr  GROUP BY M.CompanyId ORDER by Amount DESC,M.CompanyId ASC", $link_id);
        if ($ClientRow = mysql_fetch_array($ClientResult)) {
        ...
        //楼栋-选择
        $BuildNoResult = mysql_query("SELECT IF(M.BuildNo =0,'999',M.BuildNo ) AS BuildNo 
		FROM $DataIn.yw1_ordermain M
		INNER JOIN $DataIn.yw1_ordersheet S ON M.OrderNumber = S.OrderNumber
		where S.Estate>0 and M.CompanyId='$CompanyId' GROUP BY BuildNo ORDER BY BuildNo", $link_id);
        ...
        //PO-选择
        $poResult = mysql_query("SELECT DISTINCT S.OrderPO
		FROM $DataIn.yw1_ordermain M
		INNER JOIN $DataIn.yw1_ordersheet S ON M.OrderNumber = S.OrderNumber
		INNER JOIN $DataIn.productdata P ON P.ProductId = S.ProductId
		INNER JOIN $DataIn.packingunit U ON U.Id = P.PackingUnit
		INNER JOIN $DataIn.trade_object C ON M.CompanyId = C.CompanyId
		where S.Estate>0 $SearchRows ORDER BY M.CompanyId,M.OrderDate ASC,M.Id DESC", $link_id);
...
//             echo"<option value='all' >全部楼层</option>";
            ...
            do {
                ...
                if ($OrderPO == $thisOrderPO) {
                    echo "<option value='$thisOrderPO' selected>$OrderPORes[$count] 层</option>";
                    $SearchRows .= " AND S.OrderPO='$thisOrderPO'";
                } else {
                    echo "<option value='$thisOrderPO'>$OrderPORes[$count] 层</option>";
                }
...
if ($myTask == 0 || $myTask == 1) {
    echo "&nbsp;&nbsp;<span class='ButtonH_25' onClick='ToSetDelivery(this,3)'>设置交期</span>";
}
...
echo "&nbsp;&nbsp;<span class='ButtonH_25' onClick='ToSetDelivery(this,1)'>绑定</span>";
echo "&nbsp;&nbsp;<span class='ButtonH_25' onClick='ToSetDelivery(this,2)'>解绑</span>";
...
S.liningNo, TG.MouldNo, S.liningDate, S.lining_mould_id,S.priority
FROM $DataIn.yw1_ordermain M
INNER JOIN $DataIn.yw1_ordersheet S ON M.OrderNumber=S.OrderNumber
INNER JOIN $DataIn.productdata P ON P.ProductId=S.ProductId
INNER JOIN $DataIn.packingunit U ON U.Id=P.PackingUnit 
...
WHERE 1 and S.Estate>0 $SearchRows ORDER BY M.CompanyId,M.OrderDate ASC,M.Id DESC";
    ...
    case 2:
        $mySql = "SELECT M.CompanyId,S.OrderPO,M.OrderDate,M.ClientOrder,M.Operator,S.Id,S.POrderId,S.ProductId,S.Qty,S.Price,S.PackRemark,S.sgRemark,S.DeliveryDate,S.ShipType,S.Estate,S.Locks,P.cName,P.eCode,P.Weight,P.MainWeight,P.TestStandard,P.pRemark,
P.bjRemark,U.Name AS Unit,PI.PI,PI.Leadtime,PI.Remark AS PIRemark,S.dcRemark,X.name as taxName, S.liningNo, TG.MouldNo, S.liningDate, S.lining_mould_id,S.priority
FROM $DataIn.yw1_ordermain M
INNER JOIN $DataIn.yw1_ordersheet S ON M.OrderNumber=S.OrderNumber
INNER JOIN $DataIn.productdata P ON P.ProductId=S.ProductId
...
WHERE 1 and S.Estate>0  AND (T.Type='2' OR K.Locks=0) $SearchRows ORDER BY M.CompanyId,M.OrderDate ASC,M.Id DESC";
        //LEFT JOIN $DataIn.lining_mould lm on lm.CompanyId = M.CompanyId and lm.liningNo = S.liningNo and lm.MouldNo = TG.MouldNo
        break;
...
SUM(G.OrderQty-IFNULL(L.Qty,0)) AS K2,X.name as taxName, S.liningNo, TG.MouldNo, S.liningDate, S.lining_mould_id,S.priority
FROM $DataIn.yw1_ordermain M
INNER JOIN $DataIn.yw1_ordersheet S ON M.OrderNumber=S.OrderNumber
INNER JOIN $DataIn.productdata P ON P.ProductId=S.ProductId
INNER JOIN $DataIn.packingunit U ON U.Id=P.PackingUnit 
...
WHERE A.K1>=A.K2 ORDER BY A.CompanyId,A.OrderDate ASC,A.Id DESC";
    ...
    default:
        ...
        $mySql = "SELECT M.CompanyId,S.OrderPO,M.OrderDate,M.ClientOrder,M.Operator,S.Id,S.POrderId,S.ProductId,
		S.Qty,S.Price,S.PackRemark,S.cgRemark,S.sgRemark,S.DeliveryDate,S.ShipType,S.Estate,S.Locks,P.cName,P.eCode,
		P.Weight,P.MainWeight,P.TestStandard,P.pRemark,P.bjRemark,U.Name AS Unit,PI.PI,PI.Leadtime,S.dcRemark,PI.Remark AS PIRemark,
		X.name as taxName,C.Forshort,C.Id AS CID, S.liningNo, TG.MouldNo, S.liningDate, S.lining_mould_id,S.priority
		FROM $DataIn.yw1_ordermain M
		INNER JOIN $DataIn.yw1_ordersheet S ON M.OrderNumber=S.OrderNumber
		INNER JOIN $DataIn.productdata P ON P.ProductId=S.ProductId
		INNER JOIN $DataPublic.packingunit U ON U.Id=P.PackingUnit 
...
if ($myRow = mysql_fetch_array($myResult)) {
    ...
    do {
       ...
       WHERE S.POrderId='$POrderId'", $link_id);
            ...
            //台车号 , S.liningDate, S.lining_mould_id
            $liningNo = $myRow["liningNo"];
            $liningDate = $myRow["liningDate"];
            $lining_mould_id = $myRow["lining_mould_id"];
            $MouldNo = $myRow["MouldNo"];

            if (!$liningNo && $MouldNo) {

                //没有分配台车号的地方分配台车号
                $liningNoList = $mliningArr[$CompanyId . $MouldNo]["liningNoList"];

                if (!$liningNoList) {
                    //从数据库中检索 可以分配的台车号
                    $liningResult = mysql_query("select id, liningNo, state from lining_mould where CompanyId=$CompanyId and MouldNo = '$MouldNo' and state=1 order by liningNo", $link_id);

                    if ($liningResult && $liningRow = mysql_fetch_array($liningResult)) {
                        do {
                            $liningNoList[] = array(0 => $liningRow["liningNo"], 1 => $liningRow["id"]);
                        } while ($liningRow = mysql_fetch_array($liningResult));

                        $mliningArr[$CompanyId . $MouldNo]["liningNoList"] = $liningNoList;
                    }
                }


                //分配台车号
                $indexSet = $mliningArr[$CompanyId . $MouldNo]["index"];
                $indexSet = $indexSet == 0 ? 0 : $indexSet;

                if (count($liningNoList) > 0 && $indexSet < count($liningNoList)) {

                    $liningNo = $liningNoList[$indexSet][0]; //台车号
                    $lining_mould_id = $liningNoList[$indexSet][1]; //关联记录id

                    $indexSet = $indexSet + 1;
                    $mliningArr[$CompanyId . $MouldNo]["index"] = $indexSet;
                }
            }
            $liningNo = $liningNo == "" ? "" : $liningNo;
            ...
            switch ($myTask) {
                case 0:
                    $ValueArray = array(
                        ...
                        array(0 => $priority, 1 => "align='center'", 2 => "onclick='updateJq($i,6,$POrderId,37)' style='CURSOR: pointer'"), //优先
                        array(0 => $MouldNo, 1 => "align='center' name='MouldNoTd' "),
//                        array(0 => $liningNo, 1 => "align='center'  name='liningNoTd' lining_mould_id='$lining_mould_id' ", 2 => "onmousedown='window.event.cancelBubble=true;' onclick='updateJq($i,8,\"$POrderId\",36)' style='CURSOR: pointer'"),
                        array(0 => $liningNo, 1 => "align='center'  name='liningNoTd' lining_mould_id='$lining_mould_id' "),
                        array(0 => $liningDate, 1 => "align='center' "),
                        array(0 => $ShipQtySum, 1 => "align='right'", 2 => $TempInfo),
                        array(0 => $ReturnedQty, 1 => "align='right'", 2 => $TempInfo2),
                        array(0 => $Weight, 1 => "align='right'"),
                        array(0 => $Unit, 1 => "align='right'"),
                        array(0 => $Price, 1 => "align='right'", 2 => "onmousedown='window.event.cancelBubble=true;' onclick='updateJq($i,14,\"$POrderId|$Price\",35)' style='CURSOR: pointer'"),
                        array(0 => $Qty, 1 => "align='right'"),
                        array(0 => $thisSaleAmount, 1 => "align='right'"),
                        array(0 => $PackRemark, 2 => "onmousedown='window.event.cancelBubble=true;' onclick='updateJq($i,17,$POrderId,2)' style='CURSOR: pointer'", 3 => "..."),
                        array(0 => $enRemark, 2 => "onmousedown='window.event.cancelBubble=true;' onclick='updateJq($i,18,$POrderId,9)' style='CURSOR: pointer'", 3 => "..."),
                        array(0 => $sgRemark, 1 => "style='color:#F00;'", 2 => "onmousedown='window.event.cancelBubble=true;' onclick='updateJq($i,19,$POrderId,6)' style='CURSOR: pointer'", 3 => "..."),
                        array(0 => $cgRemark, 1 => "align='right'", 2 => "onmousedown='window.event.cancelBubble=true;' onclick='updateJq($i,20,$POrderId,5)' style='CURSOR: pointer'", 3 => "..."),
                        array(0 => $dcRemark, 1 => "align='right'", 2 => "onmousedown='window.event.cancelBubble=true;' onclick='updateJq($i,21,$POrderId,8)' style='CURSOR: pointer'", 3 => "..."),
                        array(0 => $ShipType, 1 => "align='right'", 2 => "onmousedown='window.event.cancelBubble=true;' onclick='updateJq($i,22,$POrderId,3)' style='CURSOR: pointer'"),
                        ...
                        array(0 => $bjRemark, 3 => "..."),
                        array(0 => $ToOutName, 1 => "align='right'", 2 => "onmousedown='window.event.cancelBubble=true;' onclick='updateJq($i,28,$POrderId,33)' style='CURSOR: pointer'"),
                        array(0 => $taxName, 1 => "align='right'", 2 => "onmousedown='window.event.cancelBubble=true;' onclick='updateJq($i,29,$POrderId,34)' style='CURSOR: pointer'"),
...
if ($From != "slist") {
    if ($SignType != 12) {
        $ActioToS = "1,2,3,88,23,11,38,39,60,74,91,103,104,131";
    } else {
        $ActioToS = "1,2,3,88,23,11,38,39,60,74,91,103,104,146,147,131";
    }
} else {
    if ($operation != "splite") {
        $ActioToS = "1,2,3,88,23,11,38,39,60,74,103,104,131";  //22,
    } else {
        $ActioToS = "2,3,88,23,11,38,39,60,74,103,104,131";
    }
}
        ...
        if (theDiv.style.visibility == "hidden" || toObj != ObjId || TableId != tempTableId) {
            ...
            switch (toObj) {
                ...
                case 36://台车
                    ...
                    break;
                case 37://优先
                    InfoSTR = "更新订单流水号为<input name='runningNum' type='text' id='runningNum' value='" + runningNum + "' size='12' class='TM0000' readonly>的优先级<input name='priority' type='text' id='priority' size='10' maxlength='1' class='INPUT0100'>";
                    break;
    ...
    function aiaxUpdate() {
        ...
        switch (ObjId) {
            ...
            case "37"://更新优先
                ...
                var temppriority = document.form1.priority.value;
                if (!/^\d+(\.\d+)?$/.test(temppriority)) {
                    alert("请输入正确的优先级");
                    return;
                }

                myurl = "yw_order_updated.php?POrderId=" + temprunningNum + "&priority=" + temppriority + "&ActionId=priority";
                var ajax = InitAjax();
                ajax.open("GET", myurl, true);
                ajax.onreadystatechange = function () {
                    if (ajax.readyState == 4) {// && ajax.status ==200
                        eval("ListTable" + tempTableId).rows[0].cells[tempRowId].innerHTML = temppriority;
                        CloseDiv();
                    }
                }
                ajax.send(null);
    ...
    function ToSetDelivery(el, s) {
        ...
        jQuery('input[name^="checkid"]:checkbox').each(function () {
            if (jQuery(this).prop('checked') == true) {
                ...
                var liningObj = jQuery(this).parent().parent().find('td[name="liningNoTd"]');
                // console.log(liningObj.attr('lining_mould_id'));
                var liningNo = liningObj.text() + "^" + liningObj.attr('lining_mould_id');
        ...
        var saveFun = function () {
            if (weekdate.Value > 0) {
                ...
                // alert(liningNos);
                myurl = "yw_order_updated2.php?Ids=" + Ids + "&PIDate=" + tempPIDate + "&updateWeekRemark=" + updateWeekRemark + "&ReduceWeeks=" + ReduceWeeks + "&liningNos=" + liningNos + "&s=" + s;
                // alert(myurl); //el.innerHTML=tempWeeks;return;
                var ajax = InitAjax();
                ajax.open("GET", myurl, true);
                ajax.onreadystatechange = function () {
        ...
        var binding = function (x) {
            myurl = "yw_order_updated2.php?Ids=" + Ids + "&liningNos=" + liningNos + "&s=" + s + "&x=" + x;
            var ajax = InitAjax();
            ajax.open("GET", myurl, true);
            ajax.onreadystatechange = function () {
                if (ajax.readyState == 4) {// && ajax.status ==200
        ...
        if (s == 1) {
            ...
            var str = liningNos.replace(/\s+/g, "");
            if (str == "^" || str == "") {
                layer.msg('当前没有台车', function () {
                });
            } else {
                layer.msg('确定解绑当前台车？', {
                    time: 0 //不自动关闭
