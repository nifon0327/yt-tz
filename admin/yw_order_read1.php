...
$unColorCol=21;//不着色列
	$Th_Col="操作|55|序号|30|PO|80|中文名|210|&nbsp;|30|Product Code|150|已出数量<br>(下单次数)|70|退货<br>数量|50|单品重<br>(g)|40|成品重<br>(g)|50|Unit|40|Price|55|利润|80|Qty|50|Amount|70|订单利润|60|订单备注|110|生管备注|110|采购备注|110|待出备注|110|Air/Sea|60|采购|45|备料|45|组装|45|待出|45|交期|70|操作员|55|期限|40|背卡<br>条码|30|PE袋<br>条码|30|白盒<br>坑盒|30|外箱<br>标签|30|报价规则|450";
	$ColsNumber=31;
	$myTask=1;
	$sumCols="13,14,15";
//更新
...
if($From!="slist"){
	...
	if ($TypeRow = mysql_fetch_array($TypeResult)){
		...
		echo"<option value='' selected>产品分类</option>";
		do{
...
$mySql="SELECT M.CompanyId,S.OrderPO,M.OrderDate,M.ClientOrder,M.Operator,S.Id,S.POrderId,S.ProductId,S.Qty,S.Price,S.PackRemark,S.cgRemark,S.sgRemark,S.DeliveryDate,S.ShipType,S.Estate,S.Locks,P.cName,P.eCode,P.Weight,P.TestStandard,P.pRemark,P.bjRemark,U.Name AS Unit,PI.PI,PI.Leadtime,S.dcRemark
FROM $DataIn.yw1_ordermain M
LEFT JOIN $DataIn.yw1_ordersheet S ON M.OrderNumber=S.OrderNumber
LEFT JOIN $DataIn.productdata P ON P.ProductId=S.ProductId
...
if($myRow = mysql_fetch_array($myResult)){
	...
	do{
		...
		include "../model/subprogram/product_chjq.php";//交货期
		/*利润计算*////////////
		...
		$GrossProfitp=sprintf("%.3f",$thisTOrmbOUT-$cbAmountUSD-$cbAmountRMB);//订单利润
		$profitRMB=sprintf("%.3f",$GrossProfitp/$Qty);//单品利润
		$profitRMB2=sprintf("%.3f",($thisTOrmbOUT-$llcbAmountUSD-$llcbAmountRMB-$llcbAmountRMB*$HzRate)/$Qty);//单品利润		
			
		
		$GrossProfit=$GrossProfitp;
		$profitRMB2PC=$Price!=0?sprintf("%.0f",($profitRMB2*100)/($Price*$Rate)):0;
		if($profitRMB2<0){
			$sumSaleAmount=sprintf("%.3f",$sumSaleAmount+$thisSaleAmount);//订单金额累加
			$thisTOrmbOUTsum+=$thisTOrmbOUT;
			$sumQty=$sumQty+$Qty;			//订单数量累加
			...
			if($checkExpressRow = mysql_fetch_array($checkExpress)){
				do{
					...
					switch($Type){
						...
						case 2:$ColbgColor="bgcolor='#FF0000'";break;		//未确定产品
...
//步骤7：
echo '</div>';
pBottom($j-1,$j-1,$Page,$Pagination,$Page_Size,$timer,$Login_WebStyle,$tableWidth);
