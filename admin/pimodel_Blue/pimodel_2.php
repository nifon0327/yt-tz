...
if($remarkSign==1){
	...
	<td width=8  align=left height=$FiledHight valign=middle >序号</td>
	<td width=30 align=left valign=middle >订单号</td>			
	<td width=44 align=left valign=middle >产品代码</td>
	<td width=14 align=right valign=middle >单价</td>
	<td width=14 align=right valign=middle >数量</td>		
	<td width=17 align=right valign=middle >总金额</td>
	<td width=17 align=center valign=middle >运输方式</td>
	<td width=22 align=center valign=middle >交货期</td>	
	<td width=18 align=center valign=middle >备注</td>	
	...
	<td width=8  align=left height=$FiledHight valign=middle >序号</td>
	<td width=30 align=left valign=middle >订单号</td>			
	<td width=62 align=left valign=middle >产品代码</td>
	<td width=14 align=right valign=middle >单价</td>
	<td width=14 align=right valign=middle >数量</td>		
	<td width=17 align=right valign=middle >总金额</td>
	<td width=17 align=center valign=middle >运输方式</td>
...
for ($pi=1;$pi<=$Counts;$pi=$pi+1){  //1.循环输出
	...
	$RemarkTableNo="RemarkTableNo".strval($pi);
     ...
     if($addSign==0){  //2.新加一页
		...
		$Invoice_PI="$PI";
		$Com_Pack_PI="上海研砼治筑建筑科技有限公司 订单交期表";
		//include "invoicetopdf_blue/invoicepublicHeaderS.php";  //页头
		include "pimodel_Blue/PipublicHeaderS.php";  //页头
	...
	if(($$eurTableNo)!=""){   //输出表的内容。
		...
		if(($$RemarkTableNo)!=""){
			...
			$pdf->SetTextColor(140,140,140);
			 $pdf->transhtmltable($$RemarkTableNo,1,1);
			// $pdf->Text(0,$NowY,"$CompanyId");
		}

		if(($$RemarkTableNo)!=""){
			//$tmpTableY=$pdf->GetY()+1;
			$tmpTableY=$pdf->GetMaxY()+1;
		}
		else{
	...
	if($pi<$Counts){
		...
		if(($$tmpRemarkNo)!=""){
			$pdftmp->transhtmltable($$tmpRemarkNo);
		}
