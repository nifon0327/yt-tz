...
//步骤4：需处理
$upResult = mysql_query("SELECT G.POrderId,G.StockId,G.OrderQty,G.StockQty,G.AddQty,G.FactualQty,G.Price,G.StuffId,G.DeliveryWeek,
D.StuffCname,D.Picture,T.TypeName,M.PurchaseID
...
if ($upData = mysql_fetch_array($upResult)) {
	...
	$Price=$upData["Price"];
	$PurchaseID=$upData["PurchaseID"]==""?"&nbsp;":$upData["PurchaseID"];
	$DeliveryWeek=$upData["DeliveryWeek"];
...
?>
    ...
    <table width='<?php  echo $tableWidth?>' border="0" cellspacing="0" bgcolor="#FFFFFF">
		<tr>
			...
			<td height="25" colspan="4" valign="bottom"><span class="redB">◆半成品资料</div></td>
		...
		<tr class="">
			...
			<td width="80" height="22" class="A1111" align="center">采购单号</td>
			<td width="80" height="22" class="A1101" align="center">采购流水号</td>
			<td class="A1101" align="center">半成品名称</td>
			<td width="100" class="A1101" align="center">配件类型</td>
			<td width="75" class="A1101" align="center">售价</td>
			<td width="75" class="A1101" align="center">数量</td>
			<td width="60" height="22" class="A1101" align="center">交期</td>
			<td width="10" class="A0001" bgcolor="#FFFFFF">&nbsp;</td>
		</tr>
		<tr>
			<td width="10" class="A0010">&nbsp;</td>
			
			<td class="A0111" align="center"><?php    echo $PurchaseID?></td>
			<td class="A0101" align="center"><?php    echo $StockId?></td>
			<td class="A0101"><?php    echo $StuffCname?></td>
			<td class="A0101"><?php    echo $TypeName?></td>
		    ...
		    <td class="A0101" align="center"><?php    echo $DeliveryWeek?></td>
			<td width="10" class="A0001">&nbsp;</td>
		</tr>
		<tr>
			<td width="10" class="A0010">&nbsp;</td>
	...
	<table width='<?php    echo $tableWidth?>' border="0" cellspacing="0" bgcolor="#FFFFFF">
		...
		$StockResult = mysql_query("SELECT A.StuffId,A.StockId,A.Price,A.OrderQty,B.StuffCname,C.Forshort,D.Name		
			...
			LEFT JOIN $DataIn.cg1_stocksheet A ON A.StockId = S.StockId
			...
			WHERE S.mStockId = '$StockId' and T.mainType  = 3",$link_id);
...
function ViewStuffId(Action){
	var Message="";
	var num=Math.random();  
	BackData=window.showModalDialog("stuffdata_s1.php?r="+num+"&tSearchPage=stuffdata&fSearchPage=semistuffchange2&fromAction=1&SearchNum=1&Action="+Action,"BackData","dialogHeight =650px;dialogWidth=980px;center=yes;scroll=yes");
...
function CheckForm(){
    if(confirm("请确认置换的配件是否正确?")){
	    document.form1.action="semifinished_outorder_updated.php?ActionId=131";
