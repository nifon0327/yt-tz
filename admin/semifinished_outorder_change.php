...
//步骤2：
ChangeWtitle("$SubCompany 半成品外发需求单更新");//需处理
...
$Parameter="fromWebPage,$fromWebPage,funFrom,$funFrom,From,$From,CompanyId,$CompanyId,Id,$Id,ActionId,$ActionId,POrderId,$POrderId,mStockId,$mStockId,mStuffId,$StuffId,Level,$Level,OrderAction,$OrderAction";
//echo $Parameter;
?>
...
function ViewStuffId(Action){
	...
	if(BackData==null || BackData==''){  //专为safari设计的 
	...
	if(BackData){
		...
		var Rowslength=Rows.length;//数组长度即领料记录数
					...
					//加入如下的代码****************************************
		if(document.getElementById("TempMaxNumber")){  ////给firfox and  safari不能用javascript生成的元素
...
function deleteRow (RowTemp,TableTemp){
	if(RowTemp.parentElement==null || RowTemp.parentElement=="undefined" ){  // Firfox不支持 parentElement
	...
	if(TableTemp==ListTable){	//新增需求单列表
		...
		if (message==true){
			...
			if(delRemark!=""){ 
				...
				myurl="semifinished_outorder_updated.php?StockId="+delStockId+"&ActionId=delStuff&StockRemark="+delRemark;
				var delinfo="";
...
function CheckForm(){
	...
	document.form1.action="semifinished_outorder_updated.php?ActionId=23";
