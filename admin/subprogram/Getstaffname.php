...
//二合一已更新
function GetSName_Date($Id,$TempTable,$Type,$MCID,$DataIn,$DataPublic,$link_id){
	$result="";
	$pResult = mysql_query("SELECT Y.Name,X.SDate,Y.Number,X.Estate  FROM $DataPublic.staffmain  Y
								...
								WHERE X.Mid=$Id AND X.UserType=$Type AND X.MCID=$MCID ORDER BY X.SDate desc LIMIT 1",$link_id);
