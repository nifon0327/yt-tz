...
$Parameter="Id,$Id,funFrom,$funFrom,From,$From,CompanyId,$CompanyId,TempValue,,OrderIds,,ShipSign,$ShipSign,ShipType,$ShipType,BuildFloor,$Wise";
//步骤5：//需处理
 ?>
...
function CheckForm(){
	...
	document.form1.action="ch_shippinglist_updated.php";
...
function ViewOrderId(Action){
    ...
    if (!arguments[1]) {
        ...
        SafariReturnValue.callback = 'ViewOrderId("",true)';
        var ClientTemp = document.form1.CompanyId.value;
        var BuildFloor = document.form1.BuildFloor.value;
        var url = "/admin/ch_shippinglist_s1.php?num=" + num + "&Action=" + Action + "&Jid=" + ClientTemp + "&ShipSign=" + ShipSign + "&BuildFloor=" + BuildFloor
	...
	if(SafariReturnValue.value){
		...
		for(var i=0;i<Rowslength;i++){
			...
			//$Type."^^".$POrderId."^^".$OrderPO."^^".$cName."^^".$eCode."^^".$Price."^^".$Qty;
			//	0			1				2			3			4			5			6
			if(Message==""){
...
function changePrice(thisE,sId){
	...
	if(CheckSTR==0 || thisValue==""){
		...
		if (message==true){
			myurl="ch_shippinglist_updated.php?sId="+sId+"&ActionId=935&NewPrice="+thisValue+"&Id="+Id;
	...
	if(TableTemp==ListTable){
		...
		if (LengthTemp==1){
			...
			if (message==true){
				...
				myurl="ch_shippinglist_updated.php?POrderId="+ReBackId+"&sId="+sId+"&ActionId=934&Id="+Id;
				retCode=openUrl(myurl);
				if(retCode!=-2){
					TableTemp.deleteRow(rowIndex);
					ShowSequence(TableTemp);
