...
if($TRow = mysql_fetch_array($TResult)){
	$unColorCol=17;//不着色列
	//$Th_Col="操作|55|序号|30|客户|100|PO|80|中文名|210|&nbsp;|30|Product Code|150|已出数量<br>(下单次数)|70|退货<br>数量|50|成品重<br>(g)|40|单品重<br>(g)|50|Unit|35|Price|55|利润|100|Qty|50|Amount|65|订单利润|60|Remark|110|Product info|110|Air/Sea|70|交货日期|80|操作员|55|期限|40|报价规则|180";//出货时间|80|
	$Th_Col="操作|55|序号|30|客户|100|PO|80|中文名|210|&nbsp;|30|Product Code|150|已出<br>(下单次数)|70|退货|50|成品重<br>(g)|40|Unit|35|Price|55|利润|100|Qty|50|Amount|65|订单利润|60|Remark|110|Product info|110|Air/Sea|70|交期|80|操作员|55|期限|40|报价规则|180";//出货时间|80|	
	$ColsNumber=20;
	$myTask=1;
	$sumCols="13,14,15";
	}
	...
	$unColorCol=16;//不着色列
	//$Th_Col="操作|55|序号|30|客户|100|PO|80|中文名|210|&nbsp;|30|Product Code|150|已出<br>数量|50|退货<br>数量|50|成品重<br>(g)|40|单品重<br>(g)|50|Unit|35|Price|55|Qty|50|Amount|65|订单利润|60|Remark|110|Product info|110|Air/Sea|70|交货日期|80|操作员|55|期限|40|报价规则|180";//出货时间|80|
	$Th_Col="操作|55|序号|30|客户|100|PO|80|中文名|210|&nbsp;|30|Product Code|150|已出|50|退货|50|成品重<br>(g)|40|Unit|35|Price|55|Qty|50|Amount|65|订单利润|60|Remark|110|Product info|110|Air/Sea|70|交期|80|操作员|55|期限|40|报价规则|180";//出货时间|80|
...
$curDate=date("Y-m-d");
/*
$checkDay=$checkDay==""?$curDate:$checkDay;
echo"下单日期：<input name='checkDay' type='text' id='checkDay' size='15' maxlength='10' value='$checkDay' onchange='document.form1.submit()'>&nbsp;";
*/
$checkDay=$checkDay==""?date("Y-m-d"):$checkDay;	
	echo"下单日期：<input name='checkDay' type='text' id='checkDay' size='12' maxlength='10' value='$checkDay'   onFocus='WdatePicker({onpicked:function(dp){document.form1.submit();}})' />&nbsp;";
	//$SearchRows=" AND DATE_FORMAT(DateTime,'%Y-%m-%d') ='$checkDay' ";
	

/*$SearchRows ="";	
echo"下单日期：<select name='checkDay' id='checkDay' onchange='ResetPage(this.name)'>";
for ($d=0;$d<10;$d++){
	$eDate=date("Y-m-d",strtotime("$curDate  -$d  day"));
	if ($eDate==$checkDay){
		echo "<option value='$eDate' selected>$eDate</option>";
	    }
	else{
	    echo "<option value='$eDate'>$eDate</option>";	
	}
}
echo "</select>&nbsp;";*/
...
WHERE S.Estate>0  $ClientStr AND M.OrderDate='$checkDay'
GROUP BY M.CompanyId order by Amount DESC,M.CompanyId",$link_id);
...
WHERE S.Estate>0 $SearchRows AND M.OrderDate='$checkDay'
...
$mySql="SELECT M.CompanyId,S.OrderPO,M.OrderDate,M.ClientOrder,M.Operator,S.Id,S.POrderId,S.ProductId,S.Qty,S.Price,S.PackRemark,S.DeliveryDate,S.ShipType,S.Estate,S.Locks,P.cName,P.eCode,P.Weight,P.MainWeight,P.TestStandard,P.pRemark,P.bjRemark,U.Name AS Unit,PI.PI,PI.Leadtime,C.Forshort
FROM $DataIn.yw1_ordermain M
LEFT JOIN $DataIn.yw1_ordersheet S ON M.OrderNumber=S.OrderNumber
LEFT JOIN $DataIn.productdata P ON P.ProductId=S.ProductId
...
WHERE 1 and S.Estate>0 AND M.OrderDate='$checkDay'  $SearchRows ORDER BY M.CompanyId,M.OrderDate DESC,M.Id DESC";
...
if($myRow = mysql_fetch_array($myResult)){
	   ...
	   if (strlen(trim($ShipType))>0){
		...
		if($myTask==0){
			...
			$ValueArray=array(
				...
				array(0=>$PackRemark,		2=>"onmousedown='window.event.cancelBubble=true;' onclick='updateJq($i,9,$POrderId,2)' style='CURSOR: pointer'",3=>"..."),
				array(0=>$pRemark, 			3=>"..."),
				array(0=>$ShipType, 		1=>"align='center'"),
				array(0=>$Leadtime,		2=>"onmousedown='window.event.cancelBubble=true;'",3=>"..."),
			...
			$ValueArray=array(
				...
				array(0=>$thisSaleAmount,	1=>"align='right'"),
				array(0=>$GrossProfit,		1=>"align='right'"),
				array(0=>$PackRemark,		2=>"onmousedown='window.event.cancelBubble=true;' onclick='updateJq($i,16,$POrderId,2)' style='CURSOR: pointer'", 3=>"..."),
				array(0=>$pRemark,			3=>"..."),
				array(0=>$ShipType,			1=>"align='center'"),
				array(0=>$Leadtime,		2=>"onmousedown='window.event.cancelBubble=true;'",3=>"..."),
				array(0=>$Operator,			1=>"align='center'"),
	...
	if($myTask==1){
		...
		$pml.="<td class='A0101' width='100' align='right'>&nbsp;</td>";
	...
	echo"<td class='A0111' width='780' height='21'>合计</td>";
	echo $pml;
	...
	*/
		...
		else
		{
			...
			$sumSaleAmount=sprintf("%.2f",$sumSaleAmount);
		    $ClientPC=$thisTOrmbOUTsum!=0?sprintf("%.0f",$GrossProfitSUM*100/$thisTOrmbOUTsum):0;
			$ValueArray=array(
				array(0=>"&nbsp;"	),
				array(0=>"&nbsp;"	),
				array(0=>"&nbsp;"	),
...
ChangeWtitle($SubCompany.$DefaultClient."今天客户新单");
if($From!="slist"){
	$ActioToS="1,2,3,4,21,45,22,23,7,8,11,38,39,60,65,74";
	}
else{//查询页面可用功能
	$ActioToS="1,2,3,4,21,22,23,7,8,11";
	}
...
function updateLock(TableId,RowId,runningNum,Locks){//行即表格序号;列，流水号，更新源
	showMaskBack();  // add by zx 加入庶影   20110323  IE_FOX_MASK.js
	var InfoSTR="";
	var buttonSTR="";
	var theDiv=document.getElementById("Jp");
...
function aiaxUpdateLock(RowId){
	...
	ajax.onreadystatechange =function(){
		...
		if(tempmyLock=="1"){
			...
			eval(tempTableId).rows[RowId].cells[0].innerHTML="<div style='background-color:#FF0000' title='采购已锁定,"+tempLockRemark+"' > <img src='../images/lock.png' width='15' height='15'></div>";
			}
	CloseDiv();
			}
		}
