...
if($TRow = mysql_fetch_array($TResult)){
	$unColorCol=12;//不着色列
	$Th_Col="操作|55|序号|30|中文名|235|Product Code|150|Unit|35|Price|55|Profit|55|理论净利|80|Qty|50|Amount|70|订单毛利|60|报价规则|450";//出货时间|80||物料交期|80|产品备注|110
	$ColsNumber=27;
	$myTask=1;
	$sumCols="12,13,14";
	}
	...
	$unColorCol=19;//不着色列
	$Th_Col="操作|55|序号|30|PO|80|原单|40|中文名|235|Product Code|150|单重|40|检讨|40|Unit|35|Price|55|Qty|50|Amount|70|订单备注|110|生管备注|110|采购备注|110|How to Ship|80|PI交期|70|交货<br>均期|40|操作员|55|期限|40|PI|30|背卡<br>条码|30|PE袋<br>条码|30|白盒<br>坑盒|30|外箱<br>标签|30|报价规则|150";//出货时间|80|物料交期|80|产品备注|110|
	$ColsNumber=24;
	$myTask=0;
	$sumCols="10,11";
	}
...
$mySql="SELECT M.CompanyId,S.OrderPO,M.OrderDate,M.ClientOrder,M.Operator,S.Id,S.POrderId,S.ProductId,S.Qty,S.Price,S.PackRemark,S.cgRemark,S.sgRemark,S.DeliveryDate,S.ShipType,S.Estate,S.Locks,P.cName,P.eCode,P.Weight,P.TestStandard,P.pRemark,P.bjRemark,U.Name AS Unit,PI.PI,PI.Leadtime
FROM $DataIn.yw1_ordermain M
LEFT JOIN $DataIn.yw1_ordersheet S ON M.OrderNumber=S.OrderNumber
LEFT JOIN $DataIn.productdata P ON P.ProductId=S.ProductId
...
if($myRow = mysql_fetch_array($myResult)){
	...
	do{
		...
		if($ClientOrder!=""){
			...
			$ClientOrder="<a href=\"openorload.php?d=$d2&f=$f2&Type=&Action=6\" target=\"download\"  style='CURSOR: pointer;color:#FF6633'>查看</a>";
			}
		else{
			$ClientOrder="&nbsp;";
			}
...
if($From!="slist"){
	...
	$ActioToS="1,2,3,88,21,22,23,7,8,11,38,39,60";  //22,
	}
