...
$ALType="CompanyId=$CompanyId&OrderPO=$OrderPO&BuildNo=$BuildNo";
$Lens=count($checkid);
for($i=0;$i<$Lens;$i++){
	if($checkid[$i]!=""){
		$TEMP=explode("|",$checkid[$i]);
		$Id=$TEMP[0];$Type=$TEMP[2];
		if($Type==1){
...
if($Mid>0){
   ...
   if($Ids1!=""){
     ...
     $CheckSplitResult=mysql_query("SELECT SP.Id,SP.Qty,S.ProductId,S.POrderId,S.Price,IFNULL(SP.volume,IFNULL( P.dwgVol, TG.DwgVol )) AS volume
     ...
     while($CheckSplitRow=mysql_fetch_array($CheckSplitResult)){
               ...
               $volume=floor($CheckSplitRow["volume"] * 100) / 100;
               ...
               $ShipId=mysql_insert_id();
	            if($sheetInAction && $ShipId>0){
		               ...
		               //更新状态
	 	                $pUpSql="UPDATE $DataIn.ch1_shipsplit  SET Estate='1',ShipId='$ShipId',volume='$volume' WHERE Id='$SPId'";
