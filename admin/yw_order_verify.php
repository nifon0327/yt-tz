...
$From=$From==""?"m":$From;
ChangeWtitle("$SubCompany 配件需求单删除待审核列表");
$funFrom="yw_order";
$Th_Col="选项|60|序号|30|采购|45|供应商|80|采购流水号|110|配件ID|45|配件名称|250|历史<br>订单|30|单价|50|单位|45|订单<br>数量|40|使用<br>库存|40|需购<br>数量|40|增购<br>数量|40|删除原因|200|初审|100";
//必选，分页默认值
...
//供应商
	...
	WHERE 1 AND S.Estate=4 GROUP BY S.CompanyId ORDER BY P.Letter",$link_id);
...
if($myRow = mysql_fetch_array($myResult)){
	do{
		...
		$verifyResult=mysql_query("SELECT StockId  FROM stockdelverify WHERE StockId='$TempStockId'",$link_id);
		if(mysql_num_rows($verifyResult))
		{
			$Verify=mysql_result($verifyResult,0,"StockId");
		}
	    if($Verify==""){
	             $verifyEstate="<img src='../images/register.gif'  width='30px'/>";
                 $verifyClick="onclick='tsPass(this,\"$TempStockId\")'";
			     }
			else{
		...
		$ValueArray=array(
			...
			array(0=>$StockRemark,
					 3=>"..."),
			array(0=>$verifyEstate,	1=>"align='center'  $verifyClick")
			);
		$checkidValue=$Id;
...
function tsPass(e,StockId){
 
  if(confirm("初审通过确认?")) {
        var url="yw_order_verify_ajax.php?StockId="+StockId;
		//alert(url); 
        var ajax=InitAjax(); 
