...
$unColorCol = 0;//不着色列
$Th_Col = "操作|55|序号|30|PO|80|中文名|210|&nbsp;|30|Product Code|150|已出<br>(下单次数)|70|退货|50|成品重<br>(g)|50|Unit|40|Price|55|利润|80|Qty|50|Amount|70|订单利润|60|订单备注|110|生管备注|110|采购备注|110|待出备注|110|Air/Sea|60|采购|45|备料|45|组装|45|待出|45|交期|70|操作员|55|期限|40|背卡<br>条码|30|PE袋<br>条码|30|白盒<br>坑盒|30|外箱<br>标签|30|报价规则|450";
$ColsNumber = 30;
$myTask = 1;
$sumCols = "12,13,14";
...
$SearchRows = " AND left(PI.Leadtime,3)='201' AND  DATE_FORMAT(PI.Leadtime,'%Y-%m')<='$ThisMonth'";
if ($From != "slist") {
    $date_Result = mysql_query("SELECT  DATE_FORMAT(PI.Leadtime,'%Y-%m') AS Month
	FROM $DataIn.yw1_ordermain M 
	LEFT JOIN $DataIn.yw1_ordersheet S ON M.OrderNumber=S.OrderNumber 
    LEFT JOIN $DataIn.yw3_pisheet PI ON PI.oId=S.Id
  WHERE S.Estate>0 $ClientStr  AND DATE_FORMAT(PI.Leadtime,'%Y-%m')<='$ThisMonth'  GROUP BY  DATE_FORMAT(PI.Leadtime,'%Y-%m') ORDER BY DATE_FORMAT(PI.Leadtime,'%Y-%m') DESC ", $link_id);
    if ($dateRow = mysql_fetch_array($date_Result)) {
        ...
        do {
            ...
            if ($chooseDate == $dateValue) {
                ...
                $SearchRows .= "  and DATE_FORMAT(PI.Leadtime,'%Y-%m')='$dateValue'";
            }
            else {
    ...
    LEFT JOIN $DataIn.yw3_pisheet PI ON PI.oId=S.Id
	WHERE S.Estate>0 $SearchRows  $SearchRows GROUP BY P.TypeId ORDER BY Amount DESC,T.TypeId", $link_id);
...
if ($myRow = mysql_fetch_array($myResult)) {
    ...
    do {
        ...
        if ($Leadtime < $Temptoday) $Leadtime = "<span class='redB'>$Leadtime</span>";
        ...
        */
        ...
        if ($ViewSign == 1 || $ProfitType == "") {
            ...
            $ValueArray = array(
                ...
                array(0 => $sgRemark, 1 => "style='color:#F00;'", 2 => "onmousedown='window.event.cancelBubble=true;' onclick='updateJq($i,16,$POrderId,6)' style='CURSOR: pointer'", 3 => "..."),
                array(0 => $cgRemark, 1 => "align='center'", 2 => "onmousedown='window.event.cancelBubble=true;' onclick='updateJq($i,17,$POrderId,5)' style='CURSOR: pointer'", 3 => "..."),
                array(0 => $dcRemark, 1 => "align='center'", 2 => "onmousedown='window.event.cancelBubble=true;' onclick='updateJq($i,18,$POrderId,8)' style='CURSOR: pointer'", 3 => "..."),
                array(0 => $ShipType, 1 => "align='center'", 2 => "onmousedown='window.event.cancelBubble=true;' onclick='updateJq($i,19,$POrderId,3)' style='CURSOR: pointer'"),
...
ChangeWtitle($SubCompany . $DefaultClient . "当月预出列表");
