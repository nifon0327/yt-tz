...
if($TRow = mysql_fetch_array($TResult)){
	$unColorCol=5;//不着色列
	$Th_Col="操作|55|序号|30|PO|80|中文名|210|&nbsp;|30|Product Code|150|已出数量<br>(下单次数)|70|退货<br>数量|50|成品重<br>(g)|40|单品重<br>(g)|50|Unit|35|Price|55|利润|80|Qty|50|Amount|70|订单利润|60|订单备注|110|生管备注|110|Air/Sea|60|PI交期|70|交货<br>均期|40|操作员|55|期限|40|背卡<br>条码|30|PE袋<br>条码|30|白盒<br>坑盒|30|外箱<br>标签|30|报价规则|180";//出货时间|80||物料交期|80|产品备注|110
	$ColsNumber=25;
	$myTask=1;
	$sumCols="12,13,14";
	}
	...
	$unColorCol=5;//不着色列
	$Th_Col="操作|55|序号|30|PO|80|中文名|210|&nbsp;|30|Product Code|150|已出数量<br>(下单次数)|70|退货<br>数量|50|成品重<br>(g)|40|单品重<br>(g)|50|Unit|35|Price|55|Qty|50|Amount|70|订单备注|110|生管备注|110|Air/Sea|60|PI交期|70|交货<br>均期|40|操作员|55|期限|40|背卡<br>条码|30|PE袋<br>条码|30|白盒<br>坑盒|30|外箱<br>标签|30|报价规则|180";//出货时间|80|物料交期|80|产品备注|110|
	$ColsNumber=22;
	$myTask=0;
	$sumCols="11,12";
	}
...
if($From!="slist"){
	...
	WHERE S.Estate>0  AND T.Type>'6' $ClientStr 
	...
	LEFT JOIN $DataIn.yw2_orderexpress T ON T.POrderId=S.POrderId 
	WHERE S.Estate>0  AND T.Type>'6' $SearchRows GROUP BY P.TypeId ORDER BY E.mainType,E.TypeId",$link_id);
...
$mySql="SELECT M.CompanyId,S.OrderPO,M.OrderDate,M.ClientOrder,M.Operator,S.Id,S.POrderId,S.ProductId,S.Qty,S.Price,S.PackRemark,S.sgRemark,S.DeliveryDate,S.ShipType,S.Estate,S.Locks,P.cName,P.eCode,P.Weight,P.TestStandard,P.pRemark,P.bjRemark,U.Name AS Unit,PI.PI,PI.Leadtime,T.Type
FROM $DataIn.yw1_ordermain M
LEFT JOIN $DataIn.yw1_ordersheet S ON M.OrderNumber=S.OrderNumber
LEFT JOIN $DataIn.productdata P ON P.ProductId=S.ProductId
...
WHERE 1 and S.Estate>0  AND T.Type>'6' $SearchRows ORDER BY M.CompanyId,M.OrderDate DESC,M.Id DESC";
//echo "$mySql";
}
else {
$mySql="SELECT M.CompanyId,S.OrderPO,M.OrderDate,M.ClientOrder,M.Operator,S.Id,S.POrderId,S.ProductId,S.Qty,S.Price,S.PackRemark,S.sgRemark,S.DeliveryDate,S.ShipType,S.Estate,S.Locks,P.cName,P.eCode,P.Weight,P.MainWeight,P.TestStandard,P.pRemark,P.bjRemark,U.Name AS Unit,PI.PI,PI.Leadtime,T.Type
FROM $DataIn.yw1_ordermain M
LEFT JOIN $DataIn.yw1_ordersheet S ON M.OrderNumber=S.OrderNumber
LEFT JOIN $DataIn.productdata P ON P.ProductId=S.ProductId
LEFT JOIN $DataPublic.packingunit U ON U.Id=P.PackingUnit 
	...
	*/	
		...
		if($myTask==0){
			$ValueArray=array(
				...
				array(0=>$Weight,1=>"align='right'"),array(0=>$MainWeight,		1=>"align='center'"),
				array(0=>$Unit,				1=>"align='center'"),
				array(0=>$Price, 			1=>"align='right'"),
				array(0=>$Qty, 				1=>"align='right'"),
				array(0=>$thisSaleAmount,	1=>"align='right'"),
				array(0=>$PackRemark,		2=>"onmousedown='window.event.cancelBubble=true;' onclick='updateJq($i,10,$POrderId,2)' style='CURSOR: pointer'",3=>"..."),
				array(0=>$sgRemark, 		1=>"style='color:#F00;'",2=>"onmousedown='window.event.cancelBubble=true;' onclick='updateJq($i,11,$POrderId,6)' style='CURSOR: pointer'",	3=>"..."),
		//		array(0=>$pRemark, 			3=>"..."),
				array(0=>$ShipType, 		1=>"align='center'",2=>"onmousedown='window.event.cancelBubble=true;' onclick='updateJq($i,12,$POrderId,3)' style='CURSOR: pointer'", 3=>"..."),
		//		array(0=>$maxDeliveryDate, 	1=>"style='background:#FF9;'",	3=>"..."),
			...
			$ValueArray=array(
				...
				array(0=>$PackRemark,		2=>"onmousedown='window.event.cancelBubble=true;' onclick='updateJq($i,13,$POrderId,2)' style='CURSOR: pointer'", 3=>"..."),
				array(0=>$sgRemark, 		1=>"style='color:#F00;'",2=>"onmousedown='window.event.cancelBubble=true;' onclick='updateJq($i,14,$POrderId,6)' style='CURSOR: pointer'", 	3=>"..."),
		//		array(0=>$pRemark,			3=>"..."),
				array(0=>$ShipType,			1=>"align='center'",2=>"onmousedown='window.event.cancelBubble=true;' onclick='updateJq($i,15,$POrderId,3)' style='CURSOR: pointer'",3=>"..."),
...
ChangeWtitle($SubCompany.$DefaultClient."客户加急订单列表");
if($From!="slist"){
