...
$WorkAddResult = mysql_query("SELECT A.Id,A.Name,A.ColorValue FROM $DataPublic.staffworkadd A WHERE A.Estate=1  AND A.Id>1 ORDER BY A.Id",$link_id);
if($WorkAddRow = mysql_fetch_array($WorkAddResult))
{
   $AddWorkSelect1="<select name='WorkAddId' id='WorkAddId' onchange='document.form1.submit()' selected><option value=''>全部</option>";
   do{
			$theId=$WorkAddRow["Id"];
			$theName=$WorkAddRow["Name"];
			if ($theId==$WorkAddId){
				   $AddWorkSelect1.="<option value='$theId' selected>$theName</option>";
					$SearchRows=" AND M.WorkAdd='$theId'";
				}
			else{
				$AddWorkSelect1.="<option value='$theId'>$theName</option>";
				}
			}while ($WorkAddRow = mysql_fetch_array($WorkAddResult));
		$AddWorkSelect1.= "</select>&nbsp;";
 }
 
$CheckStaff= mysql_query("SELECT A.Number,A.Name,A.JobName,A.WorkAdd,A.GroupId,A.GroupName   FROM (
SELECT M.Number,M.Name,J.Name AS JobName,M.BranchId,M.JobId,M.WorkAdd,M.GroupId,G.GroupName     
...
WHERE C.CheckTime LIKE '$CheckMonth%' AND M.GroupId!='517' AND G.Estate=1 AND M.cSign='$Login_cSign' $SearchRows 
GROUP BY C.Number 
UNION ALL 
SELECT M.Number,M.Name,J.Name AS JobName,M.BranchId,M.JobId,M.WorkAdd,M.GroupId,G.GroupName  
FROM $DataPublic.staffmain M
...
LEFT JOIN $DataPublic.jobdata J ON J.Id=M.JobId
LEFT JOIN $DataIn.staffgroup G ON G.GroupId=M.GroupId 
WHERE (Q.StartDate  LIKE '$CheckMonth%'  OR Q.EndDate  LIKE '$CheckMonth%'  OR  (Q.StartDate<'$CheckMonth-01'  AND Q.EndDate>'$CheckMonth-01'))  AND M.GroupId!='517' AND G.Estate=1 AND M.cSign='$Login_cSign' AND M.KqSign=1  $SearchRows  
GROUP BY Q.Number 
) A GROUP BY A.Number 
ORDER BY A.WorkAdd,A.BranchId,A.GroupId,A.Number",$link_id);
if($CheckStaff && $StaffRow = mysql_fetch_array($CheckStaff)){
	$StaffList="<select name='Number' id='Number' onchange='document.form1.submit()'>";
	do{
		$NumberT=$StaffRow["Number"];
		$WorkAddFrom=$StaffRow["WorkAdd"];
		...
		if($Number==$NumberT){
			$StaffList.="<option value='$NumberT' selected>$WorkAdd  $JobName $NameT</option>";
			}
		else{
			$StaffList.="<option value='$NumberT' >$WorkAdd $JobName $NameT</option>";
			}
...
$SelectCode=$AddWorkSelect1 . $StaffList."
...
$sumQjTime6=0;$sumQjTime7=0;$sumQjTime8=0;$sumQjTime9=0;$sumQQTime=0;$sumKGTime=0;$sumYBs=0;$sumWXTime=0;
...
if($j==4 && date("Y-m")=='2012-10'){			
	...
	$sumWXTime=$sumWXTime+$WXTime;
       ...
       if($j==4 && substr($CheckDate,0,7)=='2012-10'  && $ZL_Hours!=0){
		...
		echo"<td class='A0101' align='center'>$QjTime9</td>";
