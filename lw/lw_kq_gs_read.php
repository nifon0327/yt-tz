...
$sumCols="";        //求和列
$Th_Col="选项|40|序号|30|月份|50|部门|50|职位|50|员工ID|50|姓名|50|实到<br>工时|38|迟到<br>次数|38|迟到<br>次数|38|事假|30|病假|30|有薪|30|无薪|30|缺勤工时|38|无效工时|38|旷工工时|38|有薪工时|38|夜班次数|38|";
...
?>
 <table border='0'    cellspacing='0' style='TABLE-LAYOUT: fixed; WORD-WRAP: break-word; border-top-width:0px' >
    <tr height='20' class='' align='center'>
      ...
      <td width='50' rowspan='2' class='A1101'>小组</td>
      ...
      <td width='120' colspan='4' class='A1101'>请、休假工时</td>
...
$mySql = "SELECT K.Id,K.Month,K.Number,K.Whours,K.Locks,M.Name,M.Estate,B.Name AS Branch,J.Name AS Job, C.GroupName
FROM $DataIn.lw_kqdata K 
LEFT JOIN $DataIn.lw_staffmain M ON M.Number=K.Number 
LEFT JOIN $DataIn.branchdata B ON B.Id=M.BranchId 
LEFt JOIN $DataIn.jobdata J ON J.Id=M.JobId
Left Join $DataIn.staffgroup C On C.GroupId = M.GroupId
...
<table border='0'    cellspacing='0' style='TABLE-LAYOUT: fixed; WORD-WRAP: break-word; border-top-width:0px' >
    <tr height='20' class='' align='center'>
      ...
      <td width='50' rowspan='2' class='A0101'>小组</td>
...
if ($Login_P_Number==11965 || $Login_P_Number==11093)echo 'cz-----'.number_format($all_sum_cz).'-----cz';
