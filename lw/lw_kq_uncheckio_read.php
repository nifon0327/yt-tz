...
if($From!="slist"){
	...
	$date_Result = mysql_query("SELECT DATE_FORMAT(CheckTime,'%Y-%m') AS Month FROM $DataIn.lw_checkinout WHERE dFrom=1 group by DATE_FORMAT(CheckTime,'%Y-%m') order by CheckTime DESC",$link_id);
...
$mySql="SELECT C.Id,C.CheckTime,C.CheckType,C.dFrom,C.Estate,C.Locks,C.Operator,M.Number,M.Name,
	...
	FROM $DataIn.lw_checkinout C,$DataPublic.lw_staffmain M
	WHERE 1 $SearchRows and C.Number=M.Number
