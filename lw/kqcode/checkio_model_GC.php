...
if($exChangeType != "" && strtotime($AI) < strtotime($dRestTime2))
{
	$InLates=0;
	$AIcolor = "";
}
...
if($AO<=$dRestTime2){//情况：中午签退；无效(通常人事在审核时已删除，一般情况下不出现这种情况)
	...
	if($AO<date("Y-m-d H:i:00",strtotime("$dDateTimeOut - $dOutEarly minute"))){//情况3：下午签退；按签退取整后的时间计算，需扣60分钟
			$WorkTime=abs(strtotime(rounding_out($AO))-strtotime($dRestTime2))/3600;//13:00上班至下班
			$test="C";
			}
		else{
			$AOcolor="";			//记录正常，不取色
			$OutEarlys=0;
			$WorkTime=abs(strtotime($dDateTimeOut)-strtotime($dRestTime2))/3600;			//情况4：正常签退17:00-18:00:实际上就是4个小时
			$test="D";
			if(($AO>=$dRestTime3)){	//情况5：18:00之后签退，有加班
				$GJTime=abs(strtotime(rounding_out($AO))-strtotime($dRestTime3))/3600;		//18:00至下
				$test="E";
