...
//步骤2：
$Log_Item="考勤记录";		//需处理
$upDataSheet="$DataIn.lw_checkinout";	//需处理
$Log_Funtion="更新";
...
switch($ActionId){
	...
	case "kq":
		  ...
		  $kqDel="DELETE FROM $DataIn.lw_kqdaytj WHERE Number='$Number' and Date='$CheckDate'";
		  $kqResult=mysql_query($kqDel);
		  
		  $GroupResult=mysql_query("SELECT GroupId From $DataPublic.lw_staffmain Where Number='$Number'",$link_id);
		  if($GroupRow=mysql_fetch_array($GroupResult)){
		   $GroupId=$GroupRow["GroupId"];
		   }
		  $kqdaySql="INSERT INTO  $DataIn.lw_kqdaytj(Id, GroupId, Number, SdTime, JbTime, JbTime2, JbTime3, Date, Locks)
