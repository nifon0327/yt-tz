...
$checkKQsql=mysql_query("SELECT * FROM $DataIn.checkinout WHERE Number='$Number' AND KrSign=0 AND DATE_FORMAT(CheckTime,'%Y-%m-%d')='$Today' ORDER BY CheckTime",$link_id);
...
switch($i){
	case 0://没有记录时，只能签到，否则不能保存
		if($CheckType=="I"){//状态为签到
			include "kq_sign_ajax_10.php";
			}
		else{//状态为签退,1为则无效，2为跨日签退
			//检查上一日是否只有一个签到记录，是则为跨日签退,需检查是否已经存在跨日签退，如果存在，记录无效或重复
			$Preday=date("Y-m-d",strtotime("$Today - 1 day"));
			$CheckPre=mysql_query("SELECT * FROM $DataIn.checkinout WHERE Number='$Number' AND KrSign=0 AND DATE_FORMAT(CheckTime,'%Y-%m-%d')='$Preday' ORDER BY CheckTime",$link_id);
			if($CheckPreRow=mysql_fetch_array($CheckPre)){
				//检查跨日签退记录是否已经存在，是则重复签卡//////////////有问题 ???????????????
				$CheckNow=mysql_query("SELECT * FROM $DataIn.checkinout WHERE Number='$Number' AND KrSign=1 AND DATE_FORMAT(CheckTime,'%Y-%m-%d')='$Today' ORDER BY CheckTime",$link_id);
