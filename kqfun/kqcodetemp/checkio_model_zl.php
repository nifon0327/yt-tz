...
//计算直落的工时  $DataPublic.kqzltime 二合一已更新*******电信---yang 20120801
...
if($ZL_Row = mysql_fetch_array($ZL_Result)){
	...
	switch($DateType){
		...
		case "F":
			if($jbTimes==3){
				$sumDTime+=$ZL_Hours;
				$FJTime="<span title='包括直落：$ZL_Hours 工时'style='CURSOR: pointer;color: #009900;font-weight: bold;'>".($DTime+$ZL_Hours)."</span>";
				}
			else{
				$sumCTime+=$ZL_Hours;
				$CTime="<span title='包括直落：$ZL_Hours 工时'style='CURSOR: pointer;color: #009900;font-weight: bold;'>".($CTime+$ZL_Hours)."</span>";
