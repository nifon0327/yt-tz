...
if($AO<$dRestTime1){//情况1：早上签退（早退）;按签退取整后的时间计算
	//检查实到时间，签退向下取整
	$jbTime=abs(strtotime(rounding_out($AO))-strtotime(rounding_in($AI)))/3600;
	$test="A";
	}
else{
	if($AO<=$dRestTime2){//情况：中午签退（早退）；按12点计算
		$jbTime=abs(strtotime($dRestTime1)-strtotime(rounding_in($AI)))/3600;
		$test="B";
		}
	else{
		if($AO<date("Y-m-d H:i:00",strtotime("$dDateTimeOut - $dOutEarly minute"))){//情况3：OK 下午签退；按签退取整后的时间计算，需扣60分钟
			$jbTime=abs(strtotime(rounding_out($AO))-strtotime(rounding_in($AI))-3600)/3600;
			$test="C";
			}
		else{
			$AOcolor="";			//记录正常，不取色
			$jbTime=abs(strtotime($dDateTimeOut)-strtotime(rounding_in($AI))-3600)/3600;	;			//情况4：正常签退17:00-18:00
			$test="D";
			if(($AO>=$dRestTime3)){	//情况5：18:00之后签退，有加班
				$jbTime=$jbTime+abs(strtotime(rounding_out($AO))-strtotime($dRestTime3))/3600;
				$test="E";
