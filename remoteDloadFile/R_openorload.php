<?php
//电信-joseph
include "../remoteDloadFile/model/loadtype.php";

include "../remoteDloadFile/subprogram/down_Large_File.php";	//下载大文件
...
if ($arrays!=""){    // add by zx 2011-01-27
	//echo "$arrays";
	//$Login_IP=GetIP();  //要直接从远程获取 2012-1224
	$temparrays=explode("|",$arrays);  //$arrays="Supplier|$StuffId|$myCompanyId|$Operator|$Login_IP";    // $Login_IP add by zx 2012-1214  from stuffimg_gfile.php
	$Login_IP=$temparrays[4];//
	...
	if($uFrom==3){  //如果外部IP则需要写入
		...
		switch($temparray0){   //来自客户，则需要插入到数据表。
			case "Supplier":  // add by zx 2011-04
				...
				include "../remoteDloadFile/basic/parameter.inc";
...
switch($Type){
	...
	default:
		...
		if($Action!=6 && $Action!=7){
			...
			include "../remoteDloadFile/basic/parameter.inc";//检查检讨报告
