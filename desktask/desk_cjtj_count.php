...
if ($_SESSION["Login_cSign"]==3) {Header("Location: desk_cjtj_count3.php");}
...
<table cellspacing="0">
	...
	<tr><td>
		<table valign="top" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
			<tr>
			<td><?php    include "desk_cjtj_count_1.php";?></td><!-- 说明表格 -->
			<?php   
			//显示年或月份明细统计
			switch($From){
			//显示年统计
			...
			default://显示月份统计
				...
				//初始化统计数组
				$m=0;
				$groupSql=mysql_query("SELECT G.GroupId FROM $DataIn.staffgroup G WHERE 1 AND G.TypeId>0 AND G.Estate=1 ORDER BY G.GroupId",$link_id);
				...
				for($count_i=0;$count_i<$MonthCount;$count_i++){
					...
					//当月天数
					$checkDays=date("t",strtotime("$NowMonth -$count_i month"));//echo $M."<br>";
...
<p>说明:</p>
1、需求支出按小组整月统计，预估支出和实际支出按小组每天的在岗人数计算(每天累计)<p>
2、未包括车间以下小组：601:成品组装检查 &nbsp;&nbsp; 602:皮套检查 &nbsp;&nbsp; 603:车缝检查 &nbsp;&nbsp; 604:来料检查 &nbsp;&nbsp; 701:仓库 &nbsp;&nbsp; 801:厂务<p>
3、需求支出=该组（对应生产加工类别）当月所登记的生产记录总金额<p>
4、预估支出：该组员工当月上班总工时*预估时薪(<?php echo $OneHourSalaryt;?>)<p>
5、当月无工资(请长假)的员工不统计预估和实际支出<p>
6、实际支出=该组员工中当月每一天上班工资总和<br>
	&nbsp;&nbsp;&nbsp;&nbsp;考勤员工每天上班工资=(当月该员工工资/上班总工时)*当天上班总工时	<br>
    &nbsp;&nbsp;&nbsp;&nbsp;非考勤员工每天上班工资=(当月该员工工资/上班天数)<p>
其中：<br>
&nbsp;&nbsp;&nbsp;&nbsp;上班总工时=工作日上班工时+工作日加班工时*1.5+周末加班工时*2+假日加班工时*3<br>
&nbsp;&nbsp;&nbsp;&nbsp;员工月工资=实发+借支扣款+社保扣款(个人)+个税扣款+公积金扣款(个人)+餐费扣款+其他扣款<p>
</body>
</html>
