...
if ($Year=="0"){
	 $sYear=date("Y")-1;
	 $Month=date("m");
	 $SearchRows="DATE_FORMAT(Date,'%Y-%m')>='$sYear-$Month'";
	 $ckFlag="DESC";
    }
else{
	 $SearchRows="DATE_FORMAT(Date,'%Y')=$Year";
	 $ckFlag="ASC";
    }
$checkMonth=mysql_query("SELECT Date FROM $DataIn.ch1_shipmain WHERE  $SearchRows GROUP BY DATE_FORMAT(Date,'%Y-%m') ORDER BY Date $ckFlag",$link_id);
...
if($checkRow=mysql_fetch_array($checkMonth)){
	...
	do{
		...
		/*O1:人工实际支出
		1.生产员工薪资(实发+借支+员工出社保费+个税+其它扣款)+节假日奖金
		2.生产员工假日加班费
		3.生产员工社保费(公司出的那部分)+622
		4.试用期薪资+635 AND X.Estate IN(0,3)SELECT SUM(Amount) AS Amount FROM()A
		*/
		$checkDataOSql=mysql_query("
		    SELECT SUM(Amount) AS Amount,'0' AS Sign,Sum(Rens) as Rens  FROM
			(
			SELECT SUM(X.Amount) AS Amount,Sum(1) as Rens 
			FROM $DataIn.cwxzsheet X  LEFT JOIN $DataPublic.branchdata B ON B.Id=X.BranchId  
			WHERE X.Month='$MonthTemp' AND B.TypeId=2 
			UNION ALL
			SELECT SUM(J.Amount) AS Amount,Sum(1)as Rens FROM $DataIn.cw11_jjsheet J 
			LEFT JOIN $DataPublic.branchdata B ON B.Id=J.BranchId
			WHERE J.Month='$MonthTemp'  AND B.TypeId='2'
			)A
			UNION ALL
			SELECT SUM(H.Amount) AS Amount,'1' AS Sign,Sum(1) as Rens FROM $DataIn.hdjbsheet H  
			LEFT JOIN $DataPublic.branchdata B ON B.Id=H.BranchId  
			WHERE H.Month='$MonthTemp'  AND B.TypeId=2 
			UNION ALL
			SELECT SUM(Amount) AS Amount,'2' AS Sign,Sum(Rens) as Rens  FROM
			(
			SELECT SUM(L.Amount) AS Amount,Sum(1) as Rens  FROM $DataIn.hzqksheet L 
			WHERE DATE_FORMAT(L.Date,'%Y-%m')='$MonthTemp' AND L.TypeId IN(622) 
			UNION ALL
			SELECT SUM(B.cAmount+B.mAmount) AS Amount,Sum(1) as Rens FROM $DataIn.sbpaysheet B  
			LEFT JOIN $DataPublic.branchdata D ON D.Id=B.BranchId 
			WHERE B.Month='$MonthTemp' AND D.TypeId=2 
			)B
			UNION ALL
			SELECT SUM(Amount) AS Amount,'3' AS Sign,Sum(Rens) as Rens  FROM
			(
			SELECT SUM(L.Amount) AS Amount,Sum(1) as Rens  FROM $DataIn.hzqksheet L 
			WHERE DATE_FORMAT(L.Date,'%Y-%m')='$MonthTemp' AND L.TypeId IN(635) 
			UNION ALL
			SELECT SUM(L.Amount) AS Amount,Sum(1) as Rens  FROM $DataIn.cwxztempsheet L 
			WHERE L.Month='$MonthTemp'
			) C",$link_id);
		...
		*/
		$checkDataO2Sql=mysql_query("
			SELECT SUM(Amount) AS Amount FROM (
				...
				WHERE  W.Estate IN (0,3) AND DATE_FORMAT(M.Date,'%Y-%m')='$MonthTemp'
			UNION ALL 
				SELECT SUM(F.mcWG*F.Price+F.depotCharge*C.Rate) AS Amount 
				FROM $DataIn.ch1_shipmain M
				LEFT JOIN $DataIn.ch4_freight F ON F.chId=M.Id
				LEFT JOIN $DataPublic.currencydata C ON C.Symbol='HKD'
				WHERE F.Estate IN (0,3) AND DATE_FORMAT(M.Date,'%Y-%m')='$MonthTemp'
			UNION ALL 
		...
		4.总务采购费(6,7+610+614)
		5.行政费用：厂房租金601,厂房水电费602,厂区管理费603,员工网络费606,电话费607,办公耗材608,
		           车辆支出费用609,其它总务费用610,差旅费613,报刊费614,交际费615,银行手续费617,
				   非月结快递费618,税款624,机模费626,模具费627,样品费630,运费632
		6.打样费用
		7.船务快递费(月结快递费)
		8.船务寄样费
		9.报关费用 add my zx 2010-12-01
		*/
		$JobIdTemp=" AND B.TypeId=1 ";//非生产的员工
		//审核状态
		$EstateSTR=" AND X.Estate IN (0,3)";
		//		5.行政费用：厂房租金601,厂房水电费602,厂区管理费603,员工网络费606,电话费607,办公耗材608,车辆支出费用609,其它总务费用610,差旅费613,报刊费614,交际费615,银行手续费617,非月结快递费618,税款624,机模费626,模具费627,样品费630,运费632,购买手机费用649,电工用品662,参展/广告费650
		//+X.Jz+X.Sb+X.RandP+X.Otherkk
		$checkO3Sql = mysql_query("
			SELECT SUM(Amount) AS Amount FROM(
	                SELECT SUM(X.Amount) AS Amount FROM $DataIn.cwxzsheet X 
		            LEFT JOIN $DataPublic.branchdata B ON B.Id=X.BranchId 
		            WHERE X.Month='$MonthTemp'  $JobIdTemp $EstateSTR			
             UNION ALL
			        SELECT SUM(X.Amount) AS Amount FROM $DataIn.hdjbsheet X 
			        LEFT JOIN $DataPublic.branchdata B ON B.Id=X.BranchId 
			        WHERE X.Month='$MonthTemp'  $JobIdTemp $EstateSTR			
            UNION ALL
			       SELECT SUM(X.mAmount+X.cAmount) AS Amount FROM $DataIn.sbpaysheet X 
		           LEFT JOIN $DataPublic.branchdata B ON B.Id=X.BranchId 
			       WHERE X.Month='$MonthTemp'  $JobIdTemp $EstateSTR			
            UNION ALL
			       SELECT SUM(X.Amount) AS Amount FROM $DataIn.cw11_jjsheet X  
			       LEFT JOIN $DataPublic.branchdata B ON B.Id=X.BranchId
			       WHERE X.Month='$MonthTemp'  $JobIdTemp $EstateSTR			
            UNION ALL
			       SELECT SUM(X.Qty*X.Price) AS Amount FROM $DataIn.zw3_purchases X 
		           LEFT JOIN $DataIn.zw3_purchaset S ON S.Id=X.TypeId 
		           WHERE 1 AND S.TypeId NOT IN (1,2,3,4,5) AND DATE_FORMAT(X.qkDate,'%Y-%m')='$MonthTemp' 
				   $EstateSTR 
            UNION ALL
			       SELECT SUM(X.Amount*C.Rate) AS Amount FROM $DataIn.hzqksheet X,$DataPublic.currencydata C 
                   WHERE C.Id=X.Currency AND DATE_FORMAT(X.Date,'%Y-%m')='$MonthTemp' $EstateSTR 
                   AND X.TypeId IN (601,602,603,606,607,608,609,610,613,614,615,617,618,624,626,627,630,632,639,643,649,650,662)
            UNION ALL
			      SELECT SUM(X.Amount*C.Rate) AS Amount FROM $DataIn.cwdyfsheet X
			      LEFT JOIN $DataPublic.currencydata C ON C.Id=X.Currency 
			      WHERE 1 AND DATE_FORMAT(X.Date,'%Y-%m')='$MonthTemp'  $EstateSTR			
            UNION ALL
			      SELECT SUM(X.Amount) AS Amount FROM $DataIn.ch9_expsheet X 
			      WHERE 1 AND DATE_FORMAT(X.Date,'%Y-%m')='$MonthTemp' $EstateSTR			
            UNION ALL
			      SELECT SUM(X.Amount) AS Amount FROM $DataIn.ch10_samplemail X 
		          WHERE 1 AND DATE_FORMAT(X.SendDate,'%Y-%m')='$MonthTemp' $EstateSTR		     
                   )A 
				",$link_id);
		...
		$DateArray[$MNumber]=$MonthTemp;
		$i++;
		//1111111111111111111111
		}while($checkRow=mysql_fetch_array($checkMonth));
	}
?>
<table border='0' cellspacing='0' id='ListTable1' style='TABLE-LAYOUT: fixed; WORD-WRAP: break-word' bgcolor='#FFFFFF'>
  <tr align="center" class="">

    <td class="A1111" colspan="2" width="198">项目</td>
    <?php
	$validM=0;
	while (list($Keys,$MNumber) = each($DateArray))
    {
	 echo "<td width='80' class='A1101'><b>".$MNumber."</b></td>";
	 $validM=$validM+1;
	}
 /*   for($i=1;$i<=$validM;$i++){
...
*/
   ...
   $rowspan="5";
   if(isFireFox()  || isSafari())  //如果是FireFox，Safari 则要
   {
	  $rowspan="4";
   }
  ...
  <tr align="center">
    <td width="70" class="A0111" rowspan='<?php    echo $rowspan?>' >人工</td>
...
<table width="1056" border='0' cellspacing='0' id='ListTable$i' style='TABLE-LAYOUT: fixed; WORD-WRAP: break-word' bgcolor='#FFFFFF'>
  ...
  <tr>
    ...
    <td class="A0101">1.生产员工薪资小计(=实发+借支+员工出社保费+个税+其他扣款)+节假日奖金,与薪资表小计一致<br>
      2.生产员工加班费（与假日加班费统计页面的生产员工加班费总和一致）<br>
      3.生产员工社保=公司付费部分（与社保缴费记录中生产员工的公司缴费总和一致）<br>
    4.试用期薪资+行政635+行政622</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td class="A0101">注意：以下项目均为请款通过项目<br>
      1.IT、行政、MTS、资材、开发、QC人工(实发+借支+个人出社保+个税+其他扣款)<br>
      2.IT、行政、MTS、资材、开发、QC假日加班费<br>
      3.IT、行政、MTS、资材、开发、QC社保(公司出的部分)<br>
      4.总务采购费<br>
...
function View(RowTemp,f){
	var e=eval(RowTemp);
	if(   document.all   ) {  //IE, IE会将隐藏行算进去，但FirFox不行
		e.style.display=(e.style.display=="none")?"block":"none";
	}
	else  //FirFox add by zx 20110326
	{
	   //alert ("Here!!");
	   var tab0 = document.getElementById('ListTable1'); //   getElementsByTagName('table');
	   if (e.style.display=="none") {
		    //alert ("Here1!!");
		    e.style.display="table-row";
	   		tab0.rows[1].cells[0].rowSpan=5;
	   }
	   else {  //FirFox add by zx 20110326
		   //alert ("Here2!!");
		   e.style.display="none";
	   	   tab0.rows[1].cells[0].rowSpan=4;
	   }
