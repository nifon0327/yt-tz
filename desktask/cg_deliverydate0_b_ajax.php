...
$mySql="SELECT S.CompanyId,G.Forshort,S.DeliveryDate FROM $DataIn.cg1_stocksheet S
                LEFT JOIN $DataIn.trade_object G ON G.CompanyId =S.CompanyId
		WHERE 1  AND S.rkSign>0 AND S.BuyerId='$BuyerId' AND S.Mid>0 AND S.DeliveryDate='$DeliveryDate'
		AND (S.AddQty+S.FactualQty)>(SELECT IFNULL(SUM(C.Qty),0) AS Qty  FROM $DataIn.ck1_rksheet C
		WHERE 1 AND C.StockId=S.StockId) group by S.CompanyId ORDER BY S.DeliveryDate";
//echo $mySql;
...
if($myRow = mysql_fetch_array($myResult)){
	...
	do{		
		...
		if($unQty>0){ */		
			...
			$showPurchaseorder="<img onClick='SandH(\"$DivNum\",$i,this,\"$TempId\",\"cg_deliverydate_c\",\"$FromDir\");' 
