...
$SearchRowsA=" AND F.Estate='3'  AND F.TypeId='1' AND F.PayType='0' AND DATE_FORMAT(M.Date,'%Y-%m')='$MonthTemp'";
$SearchRowsB=" AND F.Estate='3'  AND F.TypeId='2' AND F.PayType='0' AND DATE_FORMAT(M.DeliveryDate,'%Y-%m')='$MonthTemp'";
$mySql="SELECT  F.Id,F.Termini,F.ExpressNO,F.BoxQty,F.mcWG,F.Price,
       ...
       WHERE 1 $SearchRowsA  GROUP BY F.Id 
 UNION ALL 
     SELECT   F.Id,F.Termini,F.ExpressNO,F.BoxQty,F.mcWG,F.Price,
       F.depotCharge,F.declarationCharge,F.checkCharge,F.Remark,F.Estate,F.Locks,F.Date AS  fDate,F.Operator,D.Forshort
       FROM $DataIn.ch4_freight_declaration F
       LEFT JOIN $DataIn.ch4_freight_invoice I ON I.Mid=F.Id 
       LEFT JOIN $DataIn.ch1_deliverymain M ON M.Id=I.chId 	
       LEFT JOIN $DataPublic.freightdata D ON D.CompanyId=F.CompanyId
       WHERE 1  $SearchRowsB GROUP BY F.Id 
 ";
//echo $mySql;
