...
SELECT K.CompanyId,P.Forshort,P.Letter,I.Tel,I.Fax,C.Symbol 
	...
	WHERE 1 AND K.Estate=0 GROUP BY K.CompanyId ORDER BY P.Currency,P.Letter
	";
...
if($myRow = mysql_fetch_array($myResult)){
	do{
		...
		$checkAmountRow=mysql_fetch_array(mysql_query("SELECT SUM(K.Qty*G.Price) AS Amount
				...
				WHERE 1 AND K.Estate=0 AND K.CompanyId='$CompanyId'
				",$link_id));
		$AmountTemp =sprintf("%.2f",$checkAmountRow["Amount"]);
		$AmountTemp=number_format($AmountTemp,2);
		$showPurchaseorder="<img onClick='SandH(\"$DivNum\",$i,this,\"$TempId\",\"desk_cgfk1_b4\");' id='ThisImg_$DivNum$i' name='ThisImg_$DivNum$i' src='../images/showtable.gif' width='13' height='13' style='CURSOR: pointer'>";
