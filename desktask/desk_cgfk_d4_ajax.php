...
echo"<table id='$TableId' width='$tableWidth' cellspacing='1' border='1'><tr bgcolor='#99FF99'>
		...
		<td width='363' align='center'>配件名称</td>				
...
$sListResult = mysql_query("
	SELECT M.PurchaseID,KS.StockId,KS.StuffId,KS.Price,KS.OrderQty,KS.StockQty,(KS.AddQty+KS.FactualQty) AS Qty,KS.Month,D.StuffCname,U.Name AS UnitName,D.Gfile,D.Gstate,DATE_FORMAT(KM.PayDate,'%d') AS jfDay 
	FROM $DataIn.cw1_fkoutmain KM
	LEFT JOIN $DataIn.cw1_fkoutsheet KS ON KM.Id=KS.Mid
	LEFT JOIN $DataIn.cg1_stocksheet S ON KS.StockId=S.StockId
	LEFT JOIN $DataIn.cg1_stockmain M ON M.Id=S.Mid
	LEFT JOIN $DataIn.stuffdata D ON D.StuffId=S.StuffId
	LEFT JOIN $DataPublic.stuffunit U ON U.Id=D.Unit
	WHERE 1 AND KM.CompanyId='$CompanyId' AND DATE_FORMAT(KM.PayDate,'%Y-%m')='$Month' ORDER BY KM.Id",$link_id);
...
if ($StockRows = mysql_fetch_array($sListResult)){
	...
	echo"<tr  bgcolor=#99FF99><td colspan='12' align='center'>合计</td><td align='right'>$SumAmount</td></tr>";
	}
else{
	echo"<tr><td height='30' colspan='12'  bgcolor=#D0FFD0>没有资料,请检查.</td></tr>";
	}
echo"</table>";
?>
