...
<table width="<?php    echo $tableWidth?>" border="0" cellspacing="0"  bgcolor="#FFFFFF" style='TABLE-LAYOUT: fixed; WORD-WRAP: break-word'>
	<tr align="center">
		<td height="24" colspan="5">采购单未填交期明细</td>
    </tr>
	<tr>
		<td height="24" colspan="5">
	    <input type="radio" name="Action" value="0" id="Action0" checked><label for="Action0">以采购分类</label>
	    <input name="Action" type="radio" value="1" id="Action1" onClick="javascript:document.form1.action='desk_cgwtjq.php';document.form1.submit()"><label for="Action1">以供应商分类</label>
	    </td>
    </tr>
</table>
<table width="<?php    echo $tableWidth?>" border="0" cellspacing="0"  bgcolor="#FFFFFF" style='TABLE-LAYOUT: fixed; WORD-WRAP: break-word'>
...
$ShipResult = mysql_query("SELECT S.BuyerId,M.Name 
FROM $DataIn.cg1_stocksheet S 
LEFT JOIN $DataPublic.staffmain M ON M.Number=S.BuyerId 
WHERE 1 AND S.Mid>0 AND S.rkSign>0 AND S.DeliveryDate!='0000-00-00' GROUP BY S.BuyerId ORDER BY S.BuyerId",$link_id);
if ($ShipRow = mysql_fetch_array($ShipResult)) {
	...
	do{
		...
		$showPurchaseorder="<img onClick='SandH(\"$DivNum\",$i,this,\"$TempId\",\"desk_cgwtjq1_a\");' id='ThisImg_$DivNum$i' name='ThisImg_$DivNum$i' src='../images/showtable.gif' alt='显示或隐藏下级资料. ' width='13' height='13' style='CURSOR: pointer'>";
			$HideTableHTML="
