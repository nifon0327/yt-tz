...
<table width="<?php    echo $tableWidth?>" border="0" cellspacing="0"  bgcolor="#FFFFFF" style='TABLE-LAYOUT: fixed; WORD-WRAP: break-word'>
	...
	<tr>
	  <td height="24" colspan="5"><input type="radio" name="Action" value="0" id="Action0" checked><label for="Action0">以供应商为索引进行统计(采购使用)</label>
	    <input name="Action" type="radio" value="1" id="Action1" onClick="javascript:document.form1.action='desk_cgfk1.php';document.form1.submit()"><label for="Action1">以货款状态为索引进行统计(财务使用)</label>
		 </td>
    </tr>
</table>
<table width="<?php    echo $tableWidth?>" border="0" cellspacing="0"  bgcolor="#FFFFFF" style='TABLE-LAYOUT: fixed; WORD-WRAP: break-word'>
...
if ($ShipRow = mysql_fetch_array($ShipResult)) {
	...
	do{
		...
		/*送货总额
		$mySql=$checkAmountRow=mysql_fetch_array(mysql_query("SELECT SUM(S.Qty*G.Price) AS Amount
			FROM $DataIn.ck1_rkmain M
			LEFT JOIN $DataIn.ck1_rksheet S ON M.Id=S.Mid
			LEFT JOIN $DataIn.cg1_stocksheet G ON G.StockId=S.StockId 
			WHERE 1 AND M.CompanyId='$CompanyId'
			",$link_id));
		...
		*/
		...
		$showPurchaseorder="<img onClick='SandH(\"$DivNum\",$i,this,\"$TempId\",\"desk_cgfk_a\");' id='ThisImg_$DivNum$i' name='ThisImg_$DivNum$i' src='../images/showtable.gif' alt='显示或隐藏下级资料. ' width='13' height='13' style='CURSOR: pointer'>";
			$HideTableHTML="
