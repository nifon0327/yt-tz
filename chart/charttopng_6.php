...
$ToDate=date("Y-m-d");
switch($Id){
	case 8022:$Name="水贴";break;
	case 8036:$Name="slim 皮套";break;
	case 8041:$Name="触屏笔";break;
	}

//$TypeId=9039;
//统计的分类
$ProductType=" AND P.TypeId='$Id'";
include "chartgetcolor.php";   //取得相对应类别的颜
//出货或下单最高金额
...
$TFontSize=15;
imagettftext($image,$TFontSize,0,$titleX,20,$TextBlack,$UseFont,$Y."年 ");	//输出年份
...
if($MonthRow=mysql_fetch_array($MonthSql)){
	...
	$checkPreSql=mysql_fetch_array(mysql_query("
		SELECT SUM(S.Qty)/($LastY-DATE_FORMAT(Min(M.Date),'%c')+1) AS AvgQty 
		FROM $DataIn.ch1_shipsheet S 
		LEFT JOIN $DataIn.ch1_shipmain M ON M.Id=S.Mid
		LEFT JOIN $DataIn.productdata P ON P.ProductId=S.ProductId
		WHERE M.Estate=0  $ProductType AND DATE_FORMAT(M.Date,'%Y')='$Y'
	",$link_id));
...
imagefilledrectangle($image,$imW-100,$imT+20,$imW-80,$imT+40,$T2in); 	imagefilledrectangle($image,$imW-70,$imT+20,$imW-50,$imT+40,$T2out);	imagettftext($image,10,0,$imW-40,$imT+35,$T2out,$UseFont,"触屏笔");
imagefilledrectangle($image,$imW-100,$imT+60,$imW-80,$imT+80,$T1in); 	imagefilledrectangle($image,$imW-70,$imT+60,$imW-50,$imT+80,$T1out);	imagettftext($image,10,0,$imW-40,$imT+75,$T1out,$UseFont,"PDA笔");
