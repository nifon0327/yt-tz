...
for($j=0;$j<$m;$j++){
   ...
   for($i=0;$i<=$CheckMonths;$i++){
       ...
       $chAmountSql=mysql_fetch_array(mysql_query("SELECT IFNULL(SUM(S.Qty),0) AS Amount 
			...
			WHERE M.Estate=0 AND DATE_FORMAT(M.Date,'%Y-%m')='$theMonth' AND T.mainType='$TypeId'  AND D.CompanyId='$CompanyId' 
           GROUP BY DATE_FORMAT(M.Date,'%Y-%m')",$link_id)); 
  ...
  for($i=0;$i<=$CheckMonths;$i++){
       ...
       $odAmountSql=mysql_fetch_array(mysql_query("
			...
			LEFT JOIN $DataIn.trade_object D ON D.CompanyId=M.CompanyId 
			LEFT JOIN $DataPublic.currencydata C ON C.Id=D.Currency 
			WHERE 1 AND DATE_FORMAT(M.OrderDate,'%Y-%m')='$theMonth'  AND T.mainType='$TypeId'  AND D.CompanyId='$CompanyId'
            GROUP BY DATE_FORMAT(M.OrderDate,'%Y-%m')",$link_id));
