...
for($j=0;$j<10;$j++){
	...
	if($checkRow=mysql_fetch_array($checkSql)){
		...
		///////////////////////////////////////////////////
		$YB=$imB+10;//条形图底，不变
		if ($yJgStep!=0) $PointY=$YB-round($Nums*20/$yJgStep); else $PointY=$YB;//点的Y轴坐标
		$PointX=$imL+$jgSetp*($j+1)-25;			//点的X轴坐标
		...
		imagefilledarc($image,$PointX,$PointY,$Diameter,$Diameter,0,360,$TempColor,IMG_ARC_PIE);//画点
		//输出人数
		imagettftext($image,10,0,$PointX-5,$PointY-5,$TempColor,$UseFont,$Nums);
		//画线
		if($PrePointX!="" && $PrePointY!=""){
			imageline($image,$PrePointX,$PrePointY,$PointX,$PointY,$TempColor);
			}
		imagettftext($image,10,0,$PointX-15,$YB+15,$BlackColor,$UseFont,($Month*1)."月");
		$PrePointX=$PointX;
		$PrePointY=$PointY;
		$i++;
		}
...
for($j=$i;$j<13;$j++){
	...
	imagettftext($image,10,0,$PointX-15,$YB+15,$BlackColor,$UseFont,$Month."月");
	$PrePointX=$PointX;
	$PrePointY=$PointY;
	}
//职位
