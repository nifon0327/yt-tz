...
$ToDate=date("Y-m-d");
$Name="代购配件";
$StuffStr=" AND G.Currency=$Id".$KeyWords;
$Tile=$Name." 月下单、出货(USD)金额统计图";
//$TypeId=9039;
//统计的分类
...
$MaxResult = mysql_fetch_array(mysql_query("
	SELECT MAX(Qty) AS MaxValue1 FROM ( 
		SELECT SUM(Qty) AS Qty FROM(
			SELECT round(SUM(C.OrderQty*C.Price)) AS Qty,DATE_FORMAT(M.Date,'%Y-%m') AS Month 
			FROM $DataIn.ch1_shipsheet S 
			LEFT JOIN $DataIn.ch1_shipmain M ON M.Id=S.Mid
			LEFT JOIN $DataIn.cg1_stocksheet C ON C.POrderId=S.POrderId
			LEFT JOIN $DataIn.trade_object G ON G.CompanyId=C.CompanyId
			WHERE $TjOut $StuffStr GROUP BY DATE_FORMAT(M.Date,'%Y-%m')
		) B GROUP BY Month
	UNION ALL 
		SELECT SUM(Qty) AS Qty FROM(
			SELECT round(SUM(C.OrderQty*C.Price)) AS Qty,DATE_FORMAT(M.OrderDate,'%Y-%m') AS Month 
...
if($MonthRow=mysql_fetch_array($MonthSql)){
	...
	do{
		...
		$clientSql=mysql_query("
			SELECT SUM(Qty) AS Qty FROM (
				SELECT round(SUM(C.OrderQty*C.Price)) AS Qty 
				FROM $DataIn.ch1_shipsheet S 
				LEFT JOIN $DataIn.ch1_shipmain M ON M.Id=S.Mid
				LEFT JOIN $DataIn.cg1_stocksheet C ON C.POrderId=S.POrderId
				LEFT JOIN $DataIn.trade_object G ON G.CompanyId=C.CompanyId
		...
		if($clientRow=mysql_fetch_array($clientSql)){
			...
			if($Qty>0){
				...
				imagettftext($image,12,0,$XL-$MoveL,$YT-2,$T1001,$UseFont,number_format($Qty));
				}
			$YB=$YT;
			}
		//下单总数
		...
		if($clientRow=mysql_fetch_array($clientSql)){
			...
			if($Qty>0){
				...
				imagettftext($image,12,30,$XL-$MoveL+5,$YT-2,$T1002,$UseFont,number_format($Qty));
				}
			$YB=$YT;
			}
		//**********************
	...
	$checkPreSql=mysql_fetch_array(mysql_query("
		SELECT round(SUM(C.OrderQty*C.Price))/($Y+1) AS AvgQty 
		FROM $DataIn.ch1_shipsheet S 
		LEFT JOIN $DataIn.ch1_shipmain M ON M.Id=S.Mid
		LEFT JOIN $DataIn.cg1_stocksheet C ON C.POrderId=S.POrderId
		LEFT JOIN $DataIn.trade_object G ON G.CompanyId=C.CompanyId
		WHERE M.Estate=0  $StuffStr AND DATE_FORMAT(M.Date,'%Y-%m')>='$StartMonth'
	",$link_id));
	...
	if($AvgQty>0){
		...
		imagettftext($image,10,0,$imR+10,$imB-$tempY+18,$TextRed,$UseFont,"$PreAmount");
		}
	}
//说明
