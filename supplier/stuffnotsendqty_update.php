...
if ($SendWay==""){
		...
		for($i=0;$i<$Lens;$i++){
			...
			//待入库数量
	        $drkTemp=mysql_query("SELECT SUM(C.Qty) AS Qty 
	                                   FROM $DataIn.qc_cjtj C
                                       LEFT JOIN $DataIn.gys_shsheet S ON S.Id=C.Sid 
									   LEFT JOIN $DataIn.gys_shmain M ON M.Id=S.Mid
									   WHERE C.Estate>0 AND S.Estate=0  AND C.StuffId='$StuffId' AND M.CompanyId = '$myCompanyId' AND (S.StockId='-1' or S.SendSign='1')",$link_id);
			...
			if($tmpBSQty>0 ) {     //
				if($Mid==0){//如果没生成主送货单就先生成主送货单
					...
					$inRecode="INSERT INTO $DataIn.gys_shmain (Id,BillNumber,GysNumber,CompanyId,Locks,Date,Remark, Floor,creator,created)VALUES (NULL,'$MaxBillNumber','$GysNumber','$myCompanyId','1','$Date','', '$sendFloor','$Login_P_Number','$DateTime')";
        ...
        if($SumQty*1>0 ){
         ...
         if($ComSign==0){
               $checkSql =  "SELECT S.StockId, (S.AddQty + S.FactualQty) AS Qty,M.Date  
								   ...
								   ORDER BY S.DeliveryDate,M.Date,S.Id";
                     }
              else{
               $checkSql =  "SELECT M.StockId,  (M.AddQty + M.FactualQty) AS Qty,SM.Date 
								   ...
								   AND S.DeliveryDate<>'0000-00-00' 
		                           AND  YEARWEEK(S.DeliveryDate,1)<='$nextWeek' 
								   ORDER BY S.DeliveryDate,SM.Date,S.Id";
                     }
			...
			if($checkRow=mysql_fetch_array($checkResult)){
				do{
					...
					//待入库数量
	                $drkTemp=mysql_query("SELECT SUM(Qty) AS Qty FROM $DataIn.qc_cjtj WHERE Estate>0  AND StockId='$StockId' AND StuffId='$StuffId'",$link_id);
		           $drkQty=mysql_result($drkTemp,0,"Qty");
		           $drkQty=$drkQty==""?0:$drkQty;
					...
					if($NoQty>0 && $SumQty>0){//该单未送完货
						if($Mid==0){//如果没生成主送货单就先生成主送货单
					      ...
					      $MaxBillNumberResult  = mysql_fetch_array(mysql_query("SELECT MAX(BillNumber) AS MaxBillNumber FROM $DataIn.gys_shmain WHERE  BillNumber LIKE'$DateTemp%'",$link_id));
							...
							$inRecode="INSERT INTO $DataIn.gys_shmain (Id,BillNumber,GysNumber,CompanyId,Locks,Date,Remark, Floor,creator,created) VALUES (NULL,'$MaxBillNumber','$GysNumber','$myCompanyId','1','$DateTime','', '$sendFloor','$Login_P_Number','$DateTime')";
							$inAction=@mysql_query($inRecode);
							$Mid=mysql_insert_id();
							$MidArray[$sendFloor]=$Mid;
						}
					   ...
					   if ($Mid!=0){
							if($SumQty>=$NoQty){//可以全部送货
	                            ...
	                            //SendSign: 0送货，1补货, 2备品  
								$addRecodes="INSERT INTO $DataIn.gys_shsheet (Id,Mid,StockId,StuffId,Qty,SendSign,Estate,Locks) VALUES (NULL,'$Mid','$StockId','$StuffId','$NoQty','0','1','1')"; 
								$addAction=@mysql_query($addRecodes);
								}
							else{//部分送货
								$Log.="$j - 全部送货 $StockId - $SumQty";
	                            //SendSign: 0送货，1补货, 2备品  
								$addRecodes="INSERT INTO $DataIn.gys_shsheet (Id,Mid,StockId,StuffId,Qty,SendSign,Estate,Locks) VALUES (NULL,'$Mid','$StockId','$StuffId','$SumQty','0','1','1')";  
								$addAction=@mysql_query($addRecodes);
			...
			if($tmpBPQty>0) {     //
				if($Mid==0){//如果没生成主送货单就先生成主送货单
					...
					$inRecode="INSERT INTO $DataIn.gys_shmain (Id,BillNumber,GysNumber,CompanyId,Locks,Date,Remark, Floor,creator,created) 
								VALUES (NULL,'$MaxBillNumber','$GysNumber','$myCompanyId','1','$DateTime','', '$sendFloor','$Login_P_Number','$DateTime')";
					$inAction=@mysql_query($inRecode);
					$Mid=mysql_insert_id();
					$MidArray[$sendFloor]=$Mid;
				}
		...
		if ($Log==""){
			 echo "<SCRIPT LANGUAGE=JavaScript>alert('生成送货单失败');history.back();</script>"; 
		}
		else{
			 //自动插入送货信息 
