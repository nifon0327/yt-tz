...
if($mainRows = mysql_fetch_array($mainResult)){
	...
	do{
		...
		$upMian="<img location.href=\"#\"' style='CURSOR: pointer'  src='../images/edit.gif' title='主单资料' width='13' height='13'>";
		$Attached=$mainRows["Attached"];
		$Dir=anmaIn($ImgDir,$SinkOrder,$motherSTR);
		if($Attached==1){
			$Attached=$BillNumber.".jpg";
			$Attached=anmaIn($Attached,$SinkOrder,$motherSTR);
			$Attached="<span onClick='OpenOrLoad(\"$Dir\",\"$Attached\")' style='CURSOR: pointer;color:#FF6633'>查看</span>";
			}
		else{
			$Attached="&nbsp;";
			}
