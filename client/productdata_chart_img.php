...
$Info="Client：$Forshort     ProductName：$cName     Product Code：$eCode";
//提取12个月内最高数据
...
if($TypeRow=mysql_fetch_array($TypeResult)){
	...
	if($QtySum>0){
		...
		$Ship=$TextGreen;
		//输出说明
		$Tile="Product_Chart:$Pid";
		$titleX=$imW-148;												//标题输出的起始X位置
		...
		for($m=1;$m<$k;$m++){
			...
			imagettftext($image,10,0,$imL+$m*$MonthStep-$MonthStep/2-25,$imB+20,$TextBlack,$UseFont,$TempMonth);
			}//end for
		}
	else{
		$ErrorInfo="No OrderHistory.";
		$Tile="Product_Chart:$Pid";
		$titleX=$imW-145;												//标题输出的起始X位置
		imagettftext($image,11,0,$titleX,25,$TextBlack,$UseFont,$Tile);	//输出标题
		imagettftext($image,10,0,50,40,$TextBlack,$UseFont,$Info);	//输出标题
