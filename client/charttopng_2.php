...
//颜色列表
$T1out=imagecolorallocate($image,0,60,121); 	//出
$T1in=imagecolorallocate($image,111,141,185);	//

$T2out=imagecolorallocate($image,103,144,0);
$T2in=imagecolorallocate($image,153,204,0);

$T3out=imagecolorallocate($image,18,173,43); 	
$T3in=imagecolorallocate($image,159,217,139); 
...
///全部出货总数///////////////////////////////////////////////////////////
		...
		if($clientRow=mysql_fetch_array($clientSql)){
			...
			if($Qty>0){
				...
				imagettftext($image,12,0,$XL-$MoveL,$YT-2,$T1out,$UseFont,$Qty);
				}
			//PC壳：顶部显示,矩形反过来画
			$PDASql=mysql_query("
			SELECT SUM(Qty) AS Qty FROM (
				...
				LEFT JOIN $DataIn.ch1_shipmain M ON M.Id=S.Mid
				LEFT JOIN $DataIn.productdata P ON P.ProductId=S.ProductId
				WHERE M.Estate=0  $ProductType AND DATE_FORMAT(M.Date,'%Y-%m')='$theMonth' AND P.cName LIKE '%PC壳%'
			) A
			",$link_id);
			if($PDARow=mysql_fetch_array($PDASql)){
				$PDAQty=$PDARow["Qty"];
				if($PDAQty>0){
					$shipPDAQty=round($PDAQty/($Yvalue/25));		//PC出货总额高度
					$YB2 =$YT+$shipPDAQty;
					imagefilledrectangle($image,$XL,$YB2,$XR,$YT,$T2out); //画填充矩形
			...
			SELECT SUM(Qty) AS Qty FROM (
				...
				LEFT JOIN $DataIn.ch1_shipmain M ON M.Id=S.Mid
				LEFT JOIN $DataIn.productdata P ON P.ProductId=S.ProductId
				WHERE M.Estate=0  $ProductType AND DATE_FORMAT(M.Date,'%Y-%m')='$theMonth' AND P.cName LIKE '%单片%'
			) A
			",$link_id);
			if($PDARow=mysql_fetch_array($PDASql)){
				$PDAQty=$PDARow["Qty"];
				if($PDAQty>0){
					$shipPDAQty=round($PDAQty/($Yvalue/25));		//单片出货总额高度
					$YT3 =$YB-$shipPDAQty;
					imagefilledrectangle($image,$XL,$YB-1,$XR,$YT3,$T3out); //画填充矩形
...
////////////////////////////////////////////////////////////////////			
		...
		if($clientRow=mysql_fetch_array($clientSql)){
			...
			if($Qty>0){
				...
				imagettftext($image,12,30,$XL-$MoveL+5,$YT-2,$T1in,$UseFont,$Qty);
			...
			SELECT SUM(Qty) AS Qty FROM(
				...
				LEFT JOIN $DataIn.yw1_ordermain M ON M.OrderNumber=S.OrderNumber 
				LEFT JOIN $DataIn.productdata P ON P.ProductId=S.ProductId
				WHERE DATE_FORMAT(M.OrderDate,'%Y-%m')='$theMonth' $ProductType AND P.cName LIKE '%PC壳%'
			) A
			",$link_id);
			if($PDAOrderRow=mysql_fetch_array($PDAOrderSql)){
				...
				if($PDAOrderQty>0){
					...
					$YB2 =$YT+$PDAOrderQty;
					imagefilledrectangle($image,$XL,$YB2,$XR,$YT,$T2in); //画填充矩形
			...
			SELECT SUM(Qty) AS Qty FROM(
				...
				LEFT JOIN $DataIn.yw1_ordermain M ON M.OrderNumber=S.OrderNumber 
				LEFT JOIN $DataIn.productdata P ON P.ProductId=S.ProductId
				WHERE DATE_FORMAT(M.OrderDate,'%Y-%m')='$theMonth' $ProductType AND P.cName LIKE '%单片%'
			) A
			",$link_id);
			if($PDAOrderRow=mysql_fetch_array($PDAOrderSql)){
				$PDAOrderQty=$PDAOrderRow["Qty"];//imagettftext($image,10,0,$XL,$YB+50,$TextRed,$UseFont,"单片下".$PDAOrderQty);
				if($PDAOrderQty>0){
					$PDAOrderQty=round($PDAOrderQty/($Yvalue/25));		//下单总额高度
					$YT3 =$YB-$PDAOrderQty;
					imagefilledrectangle($image,$XL,$YB-1,$XR,$YT3,$T3in); //画填充矩形
...
imagefilledrectangle($image,$imW-100,$imT+20,$imW-80,$imT+40,$T2in); 	imagefilledrectangle($image,$imW-70,$imT+20,$imW-50,$imT+40,$T2out);	imagettftext($image,10,0,$imW-40,$imT+35,$T2out,$UseFont,"PC壳");
imagefilledrectangle($image,$imW-100,$imT+60,$imW-80,$imT+80,$T1in); 	imagefilledrectangle($image,$imW-70,$imT+60,$imW-50,$imT+80,$T1out);	imagettftext($image,10,0,$imW-40,$imT+75,$T1out,$UseFont,"镭雕");
imagefilledrectangle($image,$imW-100,$imT+100,$imW-80,$imT+120,$T3in);	imagefilledrectangle($image,$imW-70,$imT+100,$imW-50,$imT+120,$T3out);	imagettftext($image,10,0,$imW-40,$imT+115,$T1out,$UseFont,"单片漆");
//输出图像
