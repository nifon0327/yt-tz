...
if($From!="slist"){
	...
	<option value='3' $EstateSTR3>Total(产品ID)</option>
	<option value='0' $EstateSTR0>details</option>
	<option value='5' $EstateSTR5>Total(ecode)</option>
	</select>&nbsp;";	
	...
	*/
	...
	if($myrow = mysql_fetch_array($result)){
     ...
     $SearchRows.=" AND P.CompanyId IN (1004,1059,1072)";  
	echo"</select>&nbsp;";
	...
	echo"<select name='ReQtyType' id='ReQtyType' onchange='document.form1.submit()'>";
		...
		<option value='1' style= 'color:#FF00CC;'  $LastMStr1>超过2‰不良品</option>
	...
	*/
	...
	$flag=1;
	echo"<select name='Estate' id='Estate' onchange='ResetPage(this.name)'>
	<option value='5' $EstateSTR5>(ecode)</option>
	</select>&nbsp;";	
}
...
echo"<select name='Pagination' id='Pagination' onchange='CencelPage()'><option value='1' $Pagination1>分页</option><option value='0' $Pagination0>不分页</option></select>
  	$CencalSstr";
//步骤5：
$searchtable="$DataIn.productdata|P|eCode|1"; //快速搜索的表名，字段名. 表名|别名|字段|1  1表示带Estate字段,其它值无无
...
$mySql="SELECT R.Id,R.eCode,R.ReturnMonth,Sum(R.Qty) as ReturnedQty ,Sum(R.Qty*R.Price) AS ReturnedAmount,R.Locks,R.Operator
FROM (SELECT * from $DataIn.product_returned  ORDER BY ReturnMonth DESC )  R 
LEFT JOIN (select DISTINCT  companyID,eCode from $DataIn.productdata ) P ON P.eCode=R.eCode
WHERE 1 $SearchRows group by P.eCode, p.companyID  ORDER BY R.ReturnMonth DESC";
...
if($myRow = mysql_fetch_array($myResult)){
	do {
		...
		*/
		...
		//已出货数量
		$checkShipQty= mysql_query("SELECT SUM(S.Qty) AS ShipQty FROM $DataIn.ch1_shipsheet  S
									LEFT JOIN $DataIn.productdata P ON P.ProductId=S.ProductId		   
								    WHERE P.eCode='$eCode' ",$link_id);
		...
		*/
		...
		//高清图片检查		
		$checkImgSQL=mysql_query("SELECT Picture FROM $DataIn.productimg  S
								 LEFT JOIN $DataIn.productdata P ON P.ProductId=S.ProductId
				                 WHERE 1  AND  P.eCode='$eCode' Limit 1 ",$link_id);
...
<tr>
<td class='A0010' width=\"80\" align=\"right\">Orders:</td>
<td width=\"50\" align=\"center\"><span class=\"greenB\">(green)></span></td>
<td width=\"30\" align=\"center\">4</td>
<td width=\"80\" align=\"center\"><span class=\"yellowB\">>=(yellow)></span></td>
<td width=\"30\" align=\"center\">1</td>
<td width=\"70\" align=\"center\"><span class=\"redB\">=(red)</span></td>
<td width=\"20\">&nbsp;</td>
<td width=\"50\"><span class=\"redB\">&nbsp;</span></td>
<td class=\"A0001\">&nbsp;</td>
</tr>
<tr>
<td class='A0010'  align=\"right\">Rejected Rate:</td>
<td align=\"center\"><span class=\"greenB\">(green)<</span></td>
<td align=\"center\">2‰</td>
<td align=\"center\"><span class=\"yellowB\"><=(yellow)<</span></td>
<td align=\"center\">5‰</td>
<td align=\"center\"><span class=\"redB\"><=(red)</span></td>
<td>&nbsp;</td>
