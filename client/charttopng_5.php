...
$MaxResult = mysql_fetch_array(mysql_query("
	SELECT MAX(Qty) AS MaxValue1 FROM ( 
		SELECT SUM(Qty) AS Qty FROM(
			...
			WHERE M.Date>='$StartDay' $StuffType GROUP BY DATE_FORMAT(M.Date,'%Y-%m')
		) B GROUP BY Month
	UNION ALL 
		SELECT SUM(Qty) AS Qty FROM(
			...
			LEFT JOIN $DataIn.cg1_stocksheet G ON G.POrderId=S.POrderId
			LEFT JOIN $DataIn.stuffdata D ON D.StuffId=G.StuffId
			WHERE M.OrderDate>='$StartDay' $StuffType GROUP BY DATE_FORMAT(M.OrderDate,'%Y-%m')
		) B GROUP BY Month
	)A",$link_id));
...
//颜色列表
//触屏下
$T2in=imagecolorallocate($image,153,204,0);	$Fdefault=imagecolorallocate($image,153,204,0); 	$Rdefault=imagecolorallocate($image,153,204,0); 
$T2out=imagecolorallocate($image,103,144,0);	$Fdefault=imagecolorallocate($image,103,144,0); 	$Rdefault=imagecolorallocate($image,103,144,0); 

//PDA出
$T1in=imagecolorallocate($image,111,141,185); 	$Fpda=imagecolorallocate($image,111,141,185); 	$Rpda=imagecolorallocate($image,111,141,185);
$T1out=imagecolorallocate($image,0,60,121); 	$Fpda=imagecolorallocate($image,0,60,121); 	$Rpda=imagecolorallocate($image,0,60,121);
...
if($MonthRow=mysql_fetch_array($MonthSql)){
	...
	do{
		...
		$clientSql=mysql_query("
			SELECT SUM(Qty) AS Qty FROM (
				...
				LEFT JOIN $DataIn.cg1_stocksheet G ON G.POrderId=S.POrderId
				LEFT JOIN $DataIn.stuffdata D ON D.StuffId=G.StuffId
				WHERE M.Estate=0  $StuffType AND DATE_FORMAT(M.Date,'%Y-%m')='$theMonth'
			) A
			",$link_id);
		...
		if($clientRow=mysql_fetch_array($clientSql)){
			...
			if($Qty>0){
				$mS=$mS==0?$i:$mS;
				imagefilledrectangle($image,$XL,$YB-1,$XR,$YT,$T2out); //画填充矩形
				imagettftext($image,12,0,$XL-$MoveL,$YT-2,$TextBlack,$UseFont,$Qty);
				}
			//其中PDA的数量
			/*
			$PDASql=mysql_query("
			SELECT SUM(Qty) AS Qty FROM (
				...
				WHERE M.Estate=0  $ProductType AND DATE_FORMAT(M.Date,'%Y-%m')='$theMonth' AND P.cName LIKE '%PDA%'
			) A
			",$link_id);
			*/
			//其中清水套出货的数量
			...
			SELECT SUM(Qty) AS Qty FROM (
				...
				LEFT JOIN $DataIn.cg1_stocksheet G ON G.POrderId=S.POrderId
				LEFT JOIN $DataIn.stuffdata D ON D.StuffId=G.StuffId
				WHERE M.Estate=0  $StuffType AND DATE_FORMAT(M.Date,'%Y-%m')='$theMonth'  AND D.StuffCname LIKE '%清水套%'
			) A
			",$link_id);
			
			if($PDARow=mysql_fetch_array($PDASql)){
				$PDAQty=$PDARow["Qty"];
				if($PDAQty>0){
					$shipPDAQty=round($PDAQty/($Yvalue/25));		//PDA出货总额高度
					$YT =$YB-$shipPDAQty;
		...
		*/
			...
			SELECT SUM(Qty) AS Qty FROM(
				...
				LEFT JOIN $DataIn.cg1_stocksheet G ON G.POrderId=S.POrderId
				LEFT JOIN $DataIn.stuffdata D ON D.StuffId=G.StuffId
				WHERE DATE_FORMAT(M.OrderDate,'%Y-%m')='$theMonth' $StuffType 
			) A
			",$link_id);
		if($clientRow=mysql_fetch_array($clientSql)){
			//坐标计算：X轴坐标每月只变化一次
			...
			//画柱形图正面
			if($Qty>0){
				imagefilledrectangle($image,$XL,$YB-1,$XR,$YT,$T2in); //画填充矩形
				imagettftext($image,12,0,$XL-$MoveL,$YT-2,$T1in,$UseFont,$Qty);
				}
			//其中PDA的下单数量
			/*
			$PDAOrderSql=mysql_query("
			SELECT SUM(Qty) AS Qty FROM(
				SELECT SUM(S.Qty) AS Qty FROM $DataIn.yw1_ordersheet S 
				LEFT JOIN $DataIn.yw1_ordermain M ON M.OrderNumber=S.OrderNumber 
				LEFT JOIN $DataIn.productdata P ON P.ProductId=S.ProductId
				WHERE DATE_FORMAT(M.OrderDate,'%Y-%m')='$theMonth' $ProductType AND P.cName LIKE '%PDA%'
			) A
			",$link_id);
			*/
			//其中清水套下单数量
			...
			SELECT SUM(Qty) AS Qty FROM(
				...
				LEFT JOIN $DataIn.cg1_stocksheet G ON G.POrderId=S.POrderId
				LEFT JOIN $DataIn.stuffdata D ON D.StuffId=G.StuffId
				WHERE DATE_FORMAT(M.OrderDate,'%Y-%m')='$theMonth' $StuffType AND D.StuffCname LIKE '%清水套%'
			) A
			",$link_id);
			
			if($PDAOrderRow=mysql_fetch_array($PDAOrderSql)){
				$PDAOrderQty=$PDAOrderRow["Qty"];
				if($PDAOrderQty>0){
					$PDAOrderQty=round($PDAOrderQty/($Yvalue/25));		//下单总额高度
					$YT =$YB-$PDAOrderQty;
	...
	imagettftext($image,10,0,$imR+10,$imB-$tempY+20,$TextRed,$UseFont,"(出货均线)");
	}
//说明
imagettftext($image,10,0,$imW-105,$imT+5,$TextBlack,$UseFont,"下单 出货");
imagefilledrectangle($image,$imW-100,$imT+20,$imW-80,$imT+40,$T2in); 	imagefilledrectangle($image,$imW-70,$imT+20,$imW-50,$imT+40,$T2out);	imagettftext($image,10,0,$imW-40,$imT+35,$T2out,$UseFont,"镜面壳");
imagefilledrectangle($image,$imW-100,$imT+60,$imW-80,$imT+80,$T1in); 	imagefilledrectangle($image,$imW-70,$imT+60,$imW-50,$imT+80,$T1out);	imagettftext($image,10,0,$imW-40,$imT+75,$T1out,$UseFont,"清水套");
//输出图像
