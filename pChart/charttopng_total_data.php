...
$TypeSql=mysql_query("SELECT A.ColorCode,B.TypeId,B.TypeName 
	 FROM $DataIn.chart3_color A
    LEFT JOIN $DataIn.producttype B ON A.TypeId=B.TypeId 
    WHERE B.Estate='1' AND A.Estate='1' ORDER BY B.SortId",$link_id);
   	...
   	for($i=0;$i<=$CheckMonths;$i++){
		$theMonth=$MonthArray[$i]; 
       	$chAmountSql=mysql_fetch_array(mysql_query("SELECT IFNULL(SUM(A.Qty*A.Price*A.YandN*E.Rate*B.Sign),0) AS Amount 
			...
			WHERE B.Estate=0 AND DATE_FORMAT(B.Date,'%Y-%m')='$theMonth' AND C.TypeId='$TypeId' GROUP BY DATE_FORMAT(B.Date,'%Y-%m')",$link_id)); 
	...
	for($i=0;$i<=$CheckMonths;$i++){
    	$theMonth=$MonthArray[$i];
       	$odAmountSql=mysql_fetch_array(mysql_query("
			...
			LEFT JOIN $DataIn.trade_object D ON D.CompanyId=B.CompanyId 
			LEFT JOIN $DataPublic.currencydata E ON E.Id=D.Currency 
			WHERE 1 AND DATE_FORMAT(B.OrderDate,'%Y-%m')='$theMonth'  AND C.TypeId='$TypeId' GROUP BY DATE_FORMAT(B.OrderDate,'%Y-%m')",$link_id));
...
$Test->setFontProperties("Fonts/simhei.ttf",9);
$Title = "   说明：第一柱:当月下单金额;  第二柱:当月出货总额;   第三柱:当月车间人工薪资";
