...
$ReadPower=1;
 ...
 $SearchWeek=$checkWeek>0?" AND S.Estate=1 AND IFNULL(PI.LeadWeek,PL.LeadWeek)='$checkWeek'":"";
 $SearchWeek=$checkWeek=="TBC" ?" AND S.Estate=1  AND IFNULL(PI.LeadWeek,0)=0 AND  IFNULL(PL.LeadWeek,0)=0 ":$SearchWeek;
 $SearchWeek=$checkWeek=="WAIT"?" AND S.Estate>1":$SearchWeek ;
...
$mySql="SELECT M.CompanyId,S.OrderPO,M.OrderDate,S.Id,S.POrderId,S.ProductId,S.Qty,S.Price,S.ShipType,S.Estate,S.scFrom,P.cName,P.TestStandard,C.Forshort,PI.Leadtime,IFNULL(PI.LeadWeek,PL.LeadWeek)  AS Weeks,S.PackRemark  
		    ...
		    WHERE 1  $SearchCompany $SearchWeek $OrderBySTR";
		    //echo $mySql;
    $curDate=date("Y-m-d");
...
WHERE  G.POrderId='$POrderId' AND GL.Locks=0 ",$link_id);
					...
					*/
					...
					$Remark="";$RemarkDate="";$RemarkOperator="";
					...
					$CompanyId=$myRow["CompanyId"];	
		               ...
		               $ScQtyResult=mysql_query("SELECT C.boxId,SUM(C.Qty) AS Qty 
		                FROM $DataIn.sc1_cjtj C
		                INNER JOIN $DataIn.yw1_scsheet S ON S.sPOrderId=C.sPOrderId AND S.ActionId=101 
		                WHERE C.POrderId='$POrderId' ",$link_id);
                      ...
                      "Remark"=>array("Text"=>"$Remark","Date"=>"$RemarkOperator","Operator"=>"$RemarkDate"),
                      "rTopTitle"=>array("Text"=>"$odDays"."d","Margin"=>"-22,0,0,0","Color"=>"#358FC1"),
                      "rIcon"=>"ship$ShipType"
                   );
                    $jsonArray[]=array("Tag"=>"data","onEdit"=>"3","onTap"=>array("Target"=>"Order","Args"=>"$POrderId"),"data"=>$tempArray);
                     $sumQty+=$Qty;
                     $sumAmount+=$Amount;
