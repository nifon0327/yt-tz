...
$orderResult=mysql_query("SELECT M.CompanyId,S.OrderPO,M.OrderDate,S.Id,S.POrderId,S.ProductId,S.Qty,S.Price,S.ShipType,S.Estate,S.scFrom,P.cName,P.TestStandard,
		    ...
		    WHERE S.Id>0 AND M.OrderDate='$checkDate'  AND M.CompanyId='$checkCompanyId' ORDER BY OrderDate",$link_id);
while($orderRow = mysql_fetch_array($orderResult)) {
        ...
        include "order/order_TestStandard.php";
         ...
         if ($ReadPower==1){
          ...
          //  include "order_Profit.php";
           // $profitRMB2PC.="%";
           /*
             $checkCost= mysql_fetch_array(mysql_query("SELECT SUM(IFNULL((A.AddQty+A.FactualQty)*A.Price*IFNULL(C.Rate,1),0)) AS oTheCost 
		        ...
		        WHERE  S.POrderId='$POrderId' AND A.Level=1  ", $link_id)); 
		     $oTheCost=$checkCost["oTheCost"]==""?0:round($checkCost["oTheCost"],2);
		    $profitRMB2PC=$saleRmbAmount==0?-100:round((($saleRmbAmount-$oTheCost)/$saleRmbAmount)*100);
		    ...
		    $profitRMB2PC.="%";
              ...
              $profitRMB2PC="";$profitColor="";$profitbgColor="";
         }       
        //检查BOM表配件是否锁定
       ...
       $ScQtyResult=mysql_query("SELECT boxId,SUM(Qty) AS Qty FROM $DataIn.sc1_cjtj WHERE POrderId='$POrderId'",$link_id);
	...
	if ($Estate==0){
     ...
     if ($OrderSignColor==4 || $OrderSignColor==2 || $OrderSignColor==6)
      ...
      "Id"=>"$POrderId",
       ...
       "Col4"=>array("Text"=>"$profitRMB2PC","Color"=>"$profitColor","bgColor"=>"$profitbgColor"),
