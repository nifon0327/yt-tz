...
function getDetailArray($Month, $DataT, $ItemId, $parameter, $HKD_Rate) {
	...
	for ($detailIndex = 0; $detailIndex < count($detailList); $detailIndex++) {
		...
		$forwardWG = $responseData["forwardWG"];
		$tmpData["Col5"] = array(
			"lIcon" => ($forwardWG == "") ? "" : "ic_ship_flower.png",
			"text" => ($forwardWG == "") ? "" : sprintf("%.2fkg", $forwardWG)
		);
		
		$formatAmount = sprintf("¥%.2f",$responseData["AmountRMB"]);
