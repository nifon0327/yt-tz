...
WHERE  B.Estate=3 AND  DATE_FORMAT(B.Date,'%Y-%m')='$checkMonth' ORDER BY B.Date",$link_id);
		    while($orderRow = mysql_fetch_array($orderResult)) {
              ...
              // $onEdit=$Estate==0?3:0;
               $dataArray[]=array("Tag"=>"data","onTap"=>array("Target"=>"Picture","Args"=>"$ImagePath"),"onEdit"=>"$Estate","data"=>$tempArray);
	   }   
 }
?>