...
if ($mStockId==""){
        $mStockResult=mysql_fetch_array(mysql_query("SELECT mStockId,mStuffId FROM $DataIn.cg1_stuffcombox WHERE StockId='$StockId'  LIMIT 1",$link_id));
        $mStockId=$mStockResult["mStockId"];
        $mStuffId=$mStockResult["mStuffId"];
 }

if ($mStockId>0 && $mStuffId>0){
          $cgmainResult=mysql_query("SELECT Id FROM $DataIn.cg1_stocksheet WHERE StuffId='$mStuffId'  AND Mid>0 LIMIT 1",$link_id);
          if(mysql_num_rows($cgmainResult)<=0){
				$developResult=mysql_query("SELECT D.GroupId,D.Estate,D.finishdate,TIMESTAMPDIFF(DAY,D.Date,IF(D.Estate=0,D.finishdate,CURDATE())) AS Days,IF(YEARWEEK(Targetdate,1)<YEARWEEK(IF(D.Estate=0,D.finishdate,CURDATE()),1),1,0) AS Over FROM $DataIn.stuffdevelop  D WHERE D.StuffId='$mStuffId'",$link_id);
			...
			WHERE L.StockId='$mStockId'",$link_id);
