...
$SearchRows=$CheckMonth>0?" AND DATE_FORMAT(M.Date,'%Y-%m')='$CheckMonth' ":"";  
...
$curWeeks=$dateResult["CurWeek"];   
	$mySql="SELECT A.StockId,S.StuffId,M.BuyerId,P.Forshort,M.PurchaseID,M.CompanyId,N.Name,YEARWEEK(S.DeliveryDate,1) AS Weeks,
       A.Qty,A.rkQty,S.Price,D.StuffCname,D.Picture,C.Rate,C.PreChar,R.Mid,YEARWEEK(A.rkDate,1) AS rkWeeks   
	     FROM (
					    SELECT S.StockId,(S.FactualQty+S.AddQty) AS Qty,SUM(IFNULL(R.Qty,0)) AS rkQty,MAX(RM.rkDate) AS rkDate  
					          ...
					          LEFT JOIN $DataIn.ck1_rkmain RM  ON RM.Id=S.Mid 
		...
		WHERE 1 $SearchRows  ORDER BY M.CompanyId,S.DeliveryDate";    
		  ...
		  //检查是否订单中最后一个需备料的配件 传入参数:$StuffId/$POrderId
           ...
           //备注信息
            ...
            $tempArray=array(
                       ...
                       "Index"=>array("Text"=>"$Weeks","Color"=>"$colorSign","bgColor"=>"$wbgColor","onTap"=>"$TapSign","Tag"=>"PILog","Args"=>"$Args"),
