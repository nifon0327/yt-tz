...
AND NOT EXISTS(SELECT R.StockId FROM $DataIn.ck1_rksheet R WHERE R.StockId=S.StockId) ";
 $myResult = mysql_query("SELECT S.Id,S.StockId,S.StuffId,S.CompanyId,(S.FactualQty+S.AddQty) AS Qty,S.Price,S.ywOrderDTime AS Date,S.BuyerId,
           A.StuffCname,A.Picture,A.Price AS dPrice,P.Forshort,M.Name,E.Rate,E.PreChar,S.ywOrderDTime,TIMESTAMPDIFF(HOUR,S.ywOrderDTime,NOW()) as Hours,
           S.Estate,S.StockRemark,S.AddRemark,S.POrderId  
			...
			WHERE S.Mid=0 and  T.mainType<2 and (S.FactualQty>0 OR S.AddQty>0) and M.BranchId=4 and S.CompanyId<>'2166'  $SearchRows  
...
if ($SegmentIndex==1){
			       ...
			       if ($CompanyId!=$oldCompanyId){
				           ...
				           $tempArray=array(
							                      ...
							                      "Title"=>array("Text"=>"$Forshort","Color"=>"#000000","FontSize"=>"14"),
							                       ...
							                       "Col2"=>array("Text"=>"$PreChar$CompanyAmount","FontSize"=>"13","Margin"=>"20,0,0,0"),
				        ...
				        $tempArray1=array();
				         ...
				         $jsondata[]=array("head"=>$tempArray,"ModuleId"=>"1184","data"=>$dataArray); 
				         $dataArray=array();
			       }
			     ...
			     $CompanyAmount+=$Amount;
				  ...
				  $WeekColor=$jhWeeks>0 && $jhWeeks<=$curWeeks?"#FF0000":"";
		           ...
		           $tempArray=array(
		                       ...
		                       "Col1"=>array("Text"=>"$Qty","bgColor"=>"$FactualQty_Color"),
		                       "Col2"=>array("Text"=>"$PreChar$Price","Color"=>"$PriceColor"),
		                       "Col4"=>array("Text"=>"$PreChar$Amount"),
		                       "Col5"=> array("Text"=>"$xdTime"),
       ...
       $COUNT_1=mysql_num_rows($myResult);
}
?>