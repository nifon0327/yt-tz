...
$mySql="SELECT DATE_FORMAT(M.rkDate,'%Y-%m') AS Month,C.Forshort,SUM(S.Qty) AS Qty,SUM(IF(YEARWEEK(M.rkDate,1)>YEARWEEK(G.Deliverydate,1),S.Qty,0)) AS OverQty,SUM(S.Qty*G.Price*D.Rate) AS Amount    
	FROM $DataIn.ck1_rksheet S
	LEFT JOIN $DataIn.ck1_rkmain M  ON M.Id=S.Mid 
	LEFT JOIN $DataIn.cg1_stocksheet G  ON G.StockId=S.StockId  
	LEFT JOIN $DataIn.trade_object C ON C.CompanyId=M.CompanyId
	LEFT JOIN $DataPublic.currencydata D ON D.Id=C.Currency
	WHERE  DATE_FORMAT(M.rkDate,'%Y-%m')>='$CheckMonth' AND M.CompanyId='$CompanyId' GROUP BY  DATE_FORMAT(M.rkDate,'%Y-%m') ORDER BY Month DESC";
 ...
 while($myRow = mysql_fetch_array($myResult)) 
{
	  ...
	  $Amount=number_format($Amount);
       ...
       $totalArray=array(
				                      ...
				                      "Title"=>array("Text"=>"$Month","Color"=>"$titleColor","FontSize"=>"13"),
		...
		$onTapArray=array("Title"=>"$Forshort/$Month","ModuleId"=>"List","Target"=>"stuff","Args"=>"$CompanyId|$Month");		                   
