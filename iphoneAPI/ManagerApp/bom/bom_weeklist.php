...
if ($BuyerId>0){
    $SearchRows.=" AND M.BuyerId='$BuyerId'  AND S.Mid>0 ";
       ...
       A.Qty,A.rkQty,S.Price,D.StuffCname,D.Picture,C.Rate,C.PreChar,R.Mid  
	     FROM (
					    SELECT S.StockId,(S.FactualQty+S.AddQty) AS Qty,SUM(IFNULL(R.Qty,0)) AS rkQty 
					         ...
					         WHERE   S.rkSign>0 AND M.CompanyId NOT IN (getSysConfig(106)) $SearchRows   GROUP BY S.StockId
					)A 
             ...
             include "submodel/stuffname_color.php";
              ...
              if ($CompanyId!=$oldCompanyId){
	                ...
	                $headArray=array(
							                      ...
							                      "Title"=>array("Text"=>"  $Forshort","FontSize"=>"12","light"=>"12"),
							                      "Col2"=>array("Text"=>"$sumQty","RLText"=>"($m)","RLColor"=>"#bbbbbb","FontSize"=>"12","Margin"=>"-88,0,40,0","light"=>"12"),
							                      "Col3"=>array("Text"=>"$PreChar$sumAmount","FontSize"=>"12","Margin"=>"0,0,0,0","light"=>"12")
							                   );  
							                   
				   $jsondata[]=array("data"=>$headArray,"Tag"=>"Total","hidden"=>"1","s_tap"=>"1",'list'=>$dataArray);                 
...
*/
		//}
	           ...
	           //           "Col3"=>array("Text"=>"¥$totalAmount","FontSize"=>"12","Margin"=>"0,0,10,0")
//                    );
