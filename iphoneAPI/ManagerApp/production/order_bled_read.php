...
//SUM(if(K.tStockQty>=(G.OrderQty-IFNULL(L.Qty,0)),(G.OrderQty-IFNULL(L.Qty,0)),0)) as K1,SUM(G.OrderQty-IFNULL(L.Qty,0)) AS K2 
  ...
  //待组装
      ...
      $mySql="SELECT B.*,C.Forshort,P.cName,P.TestStandard,IFNULL(PI.Leadtime,PL.Leadtime) AS Leadtime,
               YEARWEEK(IFNULL(PI.Leadtime,PL.Leadtime),1)  AS Weeks FROM (
               SELECT A.*,IFNULL(L.Qty,0) AS scedQty,sum(IF(D.TypeId<>7100,G.OrderQty,0)) AS scQty,SUM(IF(D.TypeId<>7100,1,0)) AS gSign FROM (
						SELECT 
						M.CompanyId,M.OrderPO,M.OrderDate,S.Id,S.POrderId,S.ProductId,S.Qty,S.ShipType,S.Price,S.sgRemark,SUM(G.OrderQty) AS blQty,IFNULL(SUM(L.Qty),0) AS llQty
						FROM $DataIn.yw1_ordermain M
						LEFT JOIN $DataIn.yw1_ordersheet S ON M.OrderNumber=S.OrderNumber
						LEFT JOIN $DataIn.cg1_stocksheet G ON G.POrderId=S.POrderId
						LEFT JOIN $DataIn.stuffdata D ON D.StuffId=G.StuffId 
						...
						WHERE S.scFrom>0 AND S.Estate=1 AND ST.mainType<2 $SearchCompany  
						AND NOT EXISTS(SELECT T.StuffId FROM $DataIn.stuffproperty T WHERE T.StuffId=G.StuffId AND T.Property='8')
						 GROUP BY S.POrderId 
						) A 
			           ...
			           WHERE  A.blQty=A.llQty  AND ST.mainType=3  and ST.TypeId<>7100 GROUP BY A.POrderId 
			         )B 
                        LEFT JOIN $DataIn.trade_object C ON C.CompanyId=B.CompanyId  
			            LEFT JOIN $DataIn.yw3_pisheet PI ON PI.oId=B.Id 
			            LEFT JOIN  $DataIn.yw3_pileadtime PL ON PL.POrderId=B.POrderId  
                        LEFT JOIN $DataIn.productdata P ON P.ProductId=B.ProductId
				WHERE B.scQty>B.scedQty AND B.gSign>0  $SearchWeek $OrderBySTR"; //and L.Estate=0
}
                   ...
                   if($checkExpressRow = mysql_fetch_array($checkExpress)){
					...
					$SortId=$checkWeekSign==1?$myRow["Weeks"]:$myRow["CompanyId"];
                     ...
                     if ($oId!=$SortId){  
		               ...
		               $rowColor=$myRow["llEstate"]==0?"#F3EBC4":"";
		               //生产数量
		               $ScQty="";$ScLine="";
...
//"picker"=>array("Style"=>"2","Planar"=>"1","data"=>$PickArray),
  $jsonArray=array("Segment"=>array("Segmented"=>$SegmentArray,"SegmentedId"=>$SegmentIdArray),"data"=>$jsondata); 
?>