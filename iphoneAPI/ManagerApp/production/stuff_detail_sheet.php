...
$hasError = 0;
	...
	if($tStockQty!=$c2TEMP || $oStockQty!=$c3TEMP){
		...
		$sortedArr[]=array("合  计:","$sum1","$sum2","$sum3","$sum4","$sum5","$sum6",
							 "$sum7","$sum8","$sum9","$sum10","$c2TEMP".($tempE2==1?"|#FF0000":""),"$c3Row".($tempE2==1?"|#FF0000":""));
	 ...
	 $jsonArray = array("ColData"=>$sortedArr,"ColTitle"=>$colTitle,"Sheet"=>1,
	 					 "Segment"=>array("Segmented"=>array("配件图","数据分析表","配件详情"),
						 "SegmentedId"=>array("0","1","2")),
						 "hasError"=>"$hasError","Args"=>"$StuffId|$c2TEMP|$c3Row");
						 
} else if ($ModuleType == "2") {
	
	$mysql = mysql_query("select g.StockId,P.Forshort,M.PurchaseID,g.Price,PI.pi,a.Name ,c.PreChar
				from $DataIn.cg1_stocksheet  g 
				...
				WHERE 1 And g.StuffId='$StuffId' limit 0,1");
	$StockId = $Forshort = $PurchaseID =  $pi="";		
	$Dir= "http://".$_SERVER ['HTTP_HOST']."/download/PurchasePDF/";
	$detailArr = array();
	if ($myRow = mysql_fetch_assoc($mysql)) {
		$pi = $myRow["pi"];
		
		$StockId = $myRow["StockId"];
		$Forshort = $myRow["Forshort"];
		$PurchaseID = $myRow["PurchaseID"];
		$filea = $Dir.$$PurchaseID.".pdf";
		$Price = $myRow["PreChar"].$myRow["Price"];
		$Name = $myRow["Name"];
		
		
		$detailArr[]= array("Title"=>"采  购  ID:","Text"=>"$StockId");
		$detailArr[]= array("Title"=>"供  应  商:","Text"=>"$Forshort");
		$detailArr[]= array("Title"=>"采购单号:","Text"=>"$PurchaseID");
		
		if ($PurchaseID!=''){
			 $detailArr[]= array("Img"=>"1","Type"=>"PDF","ImageFile"=>"$filea");
		}
		$detailArr[]= array("Title"=>"单        价:","Text"=>"$Price");
		$detailArr[]= array("Title"=>"采        购:","Text"=>"$Name");
	}
	
	$jsonArray = array("detail"=>$detailArr,"Sheet"=>"2");
}
