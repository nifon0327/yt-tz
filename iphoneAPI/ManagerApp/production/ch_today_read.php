...
$mySql="SELECT M.Date,SUM(IF(S.Type=2,0,S.Qty)) AS Qty,SUM(S.Price*S.Qty*D.Rate*M.Sign) AS Amount 
        FROM $DataIn.ch1_shipmain M
        LEFT JOIN $DataIn.ch1_shipsheet S ON S.Mid = M.Id
        LEFT JOIN $DataIn.trade_object C ON C.CompanyId=M.CompanyId 
        LEFT JOIN $DataPublic.currencydata D ON D.Id = C.Currency 
        WHERE  M.Estate='0'  AND M.Date>='$endDate' GROUP BY  M.Date ORDER BY Date DESC";
$Result=mysql_query($mySql, $link_id); 
$jsondata=array();$hiddenSign=0;$sortAmount=array();
while($myRow = mysql_fetch_array($Result)) {
	  $Date=$myRow["Date"];
	  $Qty=number_format($myRow["Qty"]);
	  $Amount=number_format(round($myRow["Amount"]));
	  /*
	 ...
	 $checkAmount=mysql_fetch_array(mysql_query("SELECT SUM(IFNULL(S.Amount*D.Rate*M.Sign,0)) AS Amount 
			 ...
			 LEFT JOIN $DataIn.trade_object C ON C.CompanyId=M.CompanyId 
             LEFT JOIN $DataPublic.currencydata D ON D.Id = C.Currency 
			WHERE  M.Estate='0' AND M.cwSign IN (1,2) AND M.Date='$Date' ",$link_id));	
