...
$newData = array();
$mySql="SELECT M.CompanyId,M.OrderDate,M.OrderPO,S.POrderId,S.ProductId,S.Qty,S.ShipType,
C.Forshort,P.cName,P.ProductId, P.Weight, P.eCode,P.TestStandard,S.Price,U.Name AS Unit,IFNULL(PI.Leadtime,PL.Leadtime) AS Leadtime,E.Type,E.Remark, YEARWEEK(substring(IFNULL(PI.Leadtime,PL.Leadtime),1,10),1) AS Weeks ,
CL.Estate as CLEstate,CL.OPdatetime as clTime
...
LEFT JOIN $DataIn.sc_canceled CL ON CL.POrderId=S.POrderId
    ...
    if($myRow = mysql_fetch_assoc($myResult))
    {
            ...
            {	
                  ...
                  if ($KBLSign==1){
					...
					$SortId=$checkWeekSign==1?$myRow["Weeks"]:$myRow["CompanyId"];
                     ...
                     if ($oId!=$SortId){  
                      ...
                      $bgColor=$myRow["Weeks"]<$curWeek?"#FF0000":"";//#00BA61
					   ...
					   $CLEstate = $myRow["CLEstate"];
					   if ($CLEstate && $CLEstate == 1) {
						   $clTime = $myRow["clTime"];
						   if (strtotime($BlDate) < strtotime($clTime)) {
						   $rowColor = "#FFE2E9";
						   }
					   }
                      $tempArray=array(
                      "Id"=>"$POrderId","line"=>"$ScLine",
                      "RowSet"=>array("bgColor"=>"$rowColor"),
                       "weeks"=>array("Text"=>"$Weeks","bg"=>"$bgColor","iIcon"=>"$Locks"),//,"Badge"=>"$ScLine"
                      "Title"=>array("Text"=>"$cName","Color"=>"#3b3e41"),
                      "Col1"=> array("Text"=>"$odDays"."d","Color"=>"#358FC1"),
					  "Col2"=> array("Text"=>$myRow["Forshort"],"Color"=>"#358FC1"),
					  "Col3"=> array("Text"=>"$OrderPO"),
                      "Col4"=>array("Text"=>"$QtySTR"), //,"bgColor"=>"$FactualQty_Color"
                      "Col5"=>array("Text"=>"$Date","Color"=>"#858888"),
				   ...
				   $POrderId = $POrderId;
				   $TasksQty=0;
					$checkTasksQty=mysql_query("SELECT Qty AS TasksQty FROM $DataIn.sc3_printtasks WHERE POrderId='$POrderId|$QtySTR' AND (CodeType=1 OR CodeType=2 OR CodeType=4)",$link_id);
				...
				$arQty = $Qty;
                   ...
                   $newData[]=array("Tag"=>"data","data"=>$tempArray,"CellID"=>"data1","Args"=>"$POrderId|$arQty",
				   "onTap"=>array("hidden"=>"1","shrink"=>"UpAccessory_blue","value"=>"1","Args"=>"$CompanyId|$ProductId|$POrderId"),"List"=>$products,
				   
				   "Swap"=>array("Sub"=>"1","Right"=>"358FC1-占用"),
				    "Tap"=>"0",
