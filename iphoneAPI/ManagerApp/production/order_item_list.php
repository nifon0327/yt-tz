	   ...
	   //订单产品对应的配件信息
			$products = array();
            	$checkStockSql=mysql_query("SELECT G.OrderQty,G.StockId,K.tStockQty,D.StuffId,D.StuffCname,D.Picture,D.TypeId,F.Remark,M.Name,P.Forshort,U.Name AS UnitName ,T.mainType,S.ProductId
										...
										AND ((T.mainType in (0,1)) or (T.mainType = 5 and D.TypeId = 9124) $zuzhuang )
										ORDER BY field(T.mainType,1,5,2,4,3),field(D.TypeId,8000,9118,9117,9101,7090,7100)",$link_id);
...
$StarHid = "0";
   ...
   if ($StuffId==114133 || $StuffId==127622 || $StuffId==129301 || $StuffId==126088 ){
    ...
    $ImagePath=$Picture>0?"$donwloadFileIP/download/stufffile/".$StuffId. "_s.jpg":"";
					 ...
					 $tempdata = array("Title"=>array("Text"=>"$StuffId-$StuffCname","Color"=>"$StuffColor"),"Id"=>"$StuffId",
					 //"Col1"=>array("Text"=>""),
					 "Col2"=>array("Text"=>"".$checkStockRow["OrderQty"].""),
					 "Col3"=>array("Text"=>"$tStockQty"),
					 "Col4"=>array("Text"=>"$llQty","Color"=>"$llQtyColor"),
...
/*
							$aPdf = "../../download/stufffile/$StuffId".".pdf";
			$aJpg = "../../download/stufffile/$StuffId.jpg";
			$aJpg0 = "../../download/stufffile/$StuffId".'_s'.".jpg";
				if (file_exists($aPdf) && $Login_P_Number==-11965 && in_array($aTypeId, $aTypeIds) && !file_exists($aJpg) && !file_exists($aJpg0)) {
exec("$execImageMagick -colorspace sRGB -transparent white -trim $aPdf $aJpg0");
			}
...
if ($llQty>$OrderQty) {
				 ...
				 $tempdataa = array("RowSet"=>array("bgColor"=>"$SetColor"),"Title"=>array("Text"=>"$StuffId-$StuffCname","Color"=>"$StuffColor"),"Col1"=>array("Text"=>""),"Col2"=>array("Text"=>"$OrderQty"),"Col3"=>array("Text"=>"$tStockQty"),"Col4"=>array("Text"=>"$llQty","Color"=>"$llQtyColor"),"Col5"=>array("Text"=>"$Forshort"),"Picture"=>"$ImagePath","star"=>"$StarHid","Prop"=>"gys$Property","Id"=>"$StuffId");
       		//$products[] = array("Tag"=>"aessery","$Date", "$StockId", "$StuffCname", "$OrderQty", "$rkQty", "$tStockQty", "$llQty", "$scQty", "$Buyer", "$Position", "$StuffId", "$Picture");

		$products[] = array("Tag"=>"acessery","data"=>$tempdataa,"Args"=>"$StockId|$OrderQty|$llQty|$StuffId" ,"CellID"=>"$cellId",

			"onTap"=>array("Value"=>"$Picture","File"=>"$ImagePath"),"onEdit"=>"$onEdit","Swap"=>$swapDic,"type_id"=>"$TypeId");
			...
			"CellID"=>"$cellId","onTap"=>array("Value"=>"$Picture","File"=>"$ImagePath"),"onEdit"=>"1","Swap"=>$swapDic,"type_id"=>"$TypeId");
