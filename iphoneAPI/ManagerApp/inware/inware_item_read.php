...
$SumTotalValue = $overQty>0?"/$SumTotalValue":$SumTotalValue;
			      ...
			      SELECT S1.* FROM (
							          SELECT S0.Id,S0.POrderId,S0.ProductId,S0.Qty,S0.CompanyId,SUM(S0.OrderQty) AS blQty,
							               SUM(S0.llQty) AS llQty,SUM(S0.llEstate) AS llEstate,Count(S0.POrderId) as count  FROM (      
							             SELECT 
													S.Id,S.POrderId,S.ProductId,S.Qty,S.CompanyId,G.StockId,G.OrderQty,
													IFNULL(SUM(L.Qty),0) AS llQty,IFNULL(SUM(L.Estate),0) AS llEstate  
							                        FROM (
													    SELECT S.Id,S.POrderId,S.ProductId,S.Qty,M.CompanyId  
							                        ...
							                        WHERE 1   AND ST.mainType<2   AND T.StuffId is NULL 
							                        GROUP BY G.StockId 
	     ...
	     case 3:   	
	         if ( in_array("215",$itemArray)){	        			          
			      $Floor=3;//全检
	               ...
	               $QtyArray=array(); $OverQtyArray=array(); $NumsArray=array();
		           $qcResult=mysql_query("SELECT A.LineId,COUNT(*) AS Nums,SUM(A.Qty) AS Qty,SUM(IF (A.UnLocks=1  AND TIMESTAMPDIFF(minute,A.shDate,Now())>1200,A.Qty,0)) AS OverQty
                       FROM (  
...
*/
		           ...
		           $qcResult=mysql_query("SELECT C.LineId,COUNT(*) AS Nums,SUM(S.Qty) AS Qty,
		                   SUM(IF (GL.StockId IS NULL  AND TIMESTAMPDIFF(minute,H.shDate,Now())>1200,S.Qty,0)) AS OverQty    
			                ...
			                LEFT JOIN $DataIn.qc_mission C ON C.Sid=S.Id
			                LEFT JOIN $DataIn.cg1_lockstock GL ON S.StockId=GL.StockId  AND GL.Locks=0  
			                WHERE  S.Estate=2   AND S.SendSign IN (0,1)  AND M.Floor='$Floor'  GROUP BY  C.LineId",$link_id);
			      while ($qcRow = mysql_fetch_array($qcResult)){
			              $LineId=$qcRow["LineId"];
			              $Nums=$qcRow["Nums"];
			              $LineQty=$qcRow["Qty"];
			              $LineOverQty=$qcRow["OverQty"];
			      }
     ...
     case 5:         
			...
			$QtyResult= mysql_fetch_array(mysql_query("SELECT SUM(IF (TIMESTAMPDIFF(MONTH,A.DTime,Now())>3,A.tStockQty,0)) AS YearQty,SUM(IF (TIMESTAMPDIFF(MONTH,A.DTime,Now())<=3 ,A.tStockQty,0)) AS YearQty2
			FROM (
			...
			WHERE  TIMESTAMPDIFF(MONTH,A.DTime,Now())>=1 $NoCompanySTR",$link_id));//AND D.Estate>0  
