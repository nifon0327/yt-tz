		...
		case "BL":{
			...
			$qty = $info[2];
			 //判断实物库存和备料数
			     ...
			     //获取数量
            $orderQtySql = mysql_fetch_assoc(mysql_query("Select OrderQty From $DataIn.cg1_stocksheet Where StockId=$stockId"));
            $orderQtyCheck = $orderQtySql["OrderQty"];
            ...
            $llqtyOne = 0;
			$checkLL = mysql_query("select IFNULL(sum(L.Qty),0) as Qty from $DataIn.ck5_llsheet L where L.StockId=$stockId");
			if ($checkLLRow = mysql_fetch_array($checkLL)) {
				$llqtyOne = $checkLLRow["Qty"];
			}
           if($orderQtyCheck < ($qty + $llqtyOne)){
	            $Log.= "备料数量超过订单数量";
            	$OperationResult = "N";
            	break;
            }
