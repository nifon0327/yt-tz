	...
	if ($SegmentIndex==1 && $FromMainPage=="production"){
		include "ck_replenish_sub1.php";
		   ...
		   if ($FromMainPage=="production"){
		       $SearchRows=" R.Estate>0 ";
		   }
		   else{
		      $SearchRows=$SegmentIndex==1 ?" R.Estate=1  AND R.Lid>0":" R.Estate=1 AND R.Lid=0 ";
		   }
			
		   $mySql="SELECT R.Id,R.POrderId,R.StuffId,R.Qty,R.Remark,R.Estate,R.OPdatetime,R.Operator,R.Lid,
						   S.OrderPO,S.Qty AS OrderQty,S.ProductId,D.cName,D.TestStandard,P.Forshort,
							...
							WHERE $SearchRows  ORDER BY R.POrderId,R.Id";
			...
			while($myRow = mysql_fetch_array($Result)) 
			{
			     ...
			     if ($POrderId!=$oldPOrdrId){
					   ...
					   $addArray[]=array("Copy"=>"Col_3","Text"=>"$OrderQty","LIcon"=>"scdj_1","Margin"=>"-20,0,0,0");
				        ...
				        $tempArray=array(
						   ...
						   $dataArray[]=array("Tag"=>"Total","onTap"=>array("Target"=>"Order","Args"=>"$POrderId"),"data"=>$tempArray,'Estate'=>array('val'=>$Estate,'Frame'=>'200,20,13,13'));
						   $oldPOrdrId=$POrderId;
			    }
			    ...
			    $tempArray=array(
						   ...
						   $dataArray[]=array("Tag"=>"stuff", "onTap"=>array("Target"=>"NewStuffDetail","Args"=>"$StuffId","File"=>"$ImagePath"),"data"=>$tempArray,"onEdit"=>"$onEdit");
						   $m++;
			 }
