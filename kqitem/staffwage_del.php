...
for($i=0;$i<count($checkid);$i++){
	...
	if ($Id!=""){
		...
		if($result && mysql_affected_rows()>0){
			...
			//相关项目处理:奖惩、其它、借支
			//借支数据连接主ID
			$upSql1= mysql_query("UPDATE $DataIn.cwygjz SET Mid='0',Locks='1' WHERE Mid='$Mid'",$link_id);
			$upResult1 = mysql_query($upSql1);
			//奖惩数据连接主ID
			$upSql2 = mysql_query("UPDATE $DataIn.staffrandp SET Mid='0',Locks='1' WHERE Mid='$Mid'",$link_id);
