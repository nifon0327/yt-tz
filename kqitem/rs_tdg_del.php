...
for($i=0;$i<count($checkid);$i++){
	...
	if ($Id!=""){
		...
		$CheckSql=mysql_query("SELECT B.Month,B.Number,B.ActionOut,M.Name FROM $DataPublic.redeployg B,$DataPublic.staffmain M WHERE B.Id='$Id' AND M.Number=B.Number  LIMIT 1",$link_id);
		if($CheckRow=mysql_fetch_array($CheckSql)){
			...
			///////////
			/*
			$DelSql= "DELETE $DataPublic.redeployg FROM $DataPublic.redeployg,(SELECT MAX(Id) AS MAXId FROM $DataPublic.redeployg WHERE Number='$Number') A,
			...
			*/	
			...
			if($DelResult && mysql_affected_rows()>0){
				...
				//恢复部门ID
				$UpSql="UPDATE $DataPublic.staffmain SET Grade='$ActionOut' WHERE Number='$Number' LIMIT 1";
...
//$OPTIMIZE=mysql_query("OPTIMIZE TABLE $DataPublic.redeployg");
