...
switch($ActionId){
	...
	case 16:
		...
		$AmountSTR="(S.mAmount+S.cAmount) AS Amount";
	...
	case 15://两种情况：审核或财务，如果是财务，要处理现金流水帐
		...
		if($fromWebPage==$funFrom."_m"){	//审核退回
			...
			if($Estate==3){					//未结付退回
				...
				$Log_Funtion="已结付退回";		$SetStr="Mid=0,Estate=1,Locks=1";		$AmountSTR="(S.mAmount+S.cAmount) AS Amount";
				include "../model/subprogram/updated_model_3c.php";
				}			
			}
		break;
	case 18://结付
		$AmountValue="mAmount+cAmount";
	...
	case 20://财务更新
		...
		break;
	default:
		$SetStr="mAmount='$mAmount',cAmount='$cAmount',Date='$Date',Locks='0',Operator='$Operator'";
