...
if($DataIn !== 'ac'){
	$inRecode = "INSERT INTO $DataPublic.redeployk SELECT NULL,Number,'$KqSign','$ActionIn','$Month','$Remark','$Date','0','$Operator' FROM $DataPublic.staffmain WHERE 1 $NumberSTR ";
}else{
	$inRecode = "INSERT INTO $DataPublic.redeployk SELECT NULL,Number,'$KqSign','$ActionIn','$Month','$Remark','$Date','0','$Operator', 1, 0, '$Operator', '$Date', '$Operator', '$Date' FROM $DataPublic.staffmain WHERE 1 $NumberSTR ";
}
$inResult=@mysql_query($inRecode);
if($inResult){	
	$Log="&nbsp;&nbsp; $TitleSTR 成功. ($Ids)</br>";//更新
	$upSql = "UPDATE $DataPublic.staffmain SET KqSign='$ActionIn' WHERE 1 $NumberSTR";
