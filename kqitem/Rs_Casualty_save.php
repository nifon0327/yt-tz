...
if($DataIn !== 'ac'){
	...
	SELECT NULL,'3',M.BranchId,M.JobId,M.Number,'$newMonth','0','$cAmount','$Date','1','0','$Operator','0' 
	...
	$inRecode="INSERT INTO $DataIn.sbpaysheet 
	SELECT NULL,'3',M.BranchId,M.JobId,M.Number,'$newMonth','0','$cAmount','$Date','1','0','$Operator','0', 0, '$Operator', '$Date', '$Operator', '$Date' 
	FROM $DataPublic.staffmain M
	WHERE 1 $BranchIdSTR  AND M.cSign='$Login_cSign' AND M.Estate=1
	AND M.Number NOT IN (SELECT Number FROM $DataIn.sbpaysheet WHERE Month='$newMonth' and TypeId=3)";
}
