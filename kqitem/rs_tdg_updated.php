...
//步骤2：
$Log_Item="等级调动资料";		//需处理
$upDataSheet="$DataPublic.redeployg";	//需处理
$Log_Funtion="更新";
...
switch($ActionId){
	...
	default:
		//限制条件:月份大于已结付薪资的月份，则可以更新
		$Date=date("Y-m-d");
		$Remark=FormatSTR($Remark);
		//更新
		$UpSql="UPDATE $upDataSheet B,$DataPublic.staffmain M,
			(SELECT MAX(Month) MaxMonth FROM $DataPublic.redeployg WHERE Number='$Number' AND Id!='$Id') C,
		...
		if($UpResult && mysql_affected_rows()>0){
			$Log="员工 $Number 的 $Log_Item$Log_Funtion更新成功. $UpSql";
			}
		else{
