...
*/
   ...
   <?php 
	...
	for ($i=0;$i<$listC;$i++) {
		...
		if ($wsId==105){
			$price_bgimg ="boxPrice_kl.png";
		}
		else{
			$price_bgimg ="boxPrice_$wsId.png";
		}
...
white-space:nowrap; 
overflow:hidden;"><?php echo($row['cName']) ?>
		</div>
		
		<img style="width: 40px; height: 40px; position:absolute; top: 99px; left: 230px"			src="images/qty.png">
		...
		<img style="width: 178px; height: 40px; position:absolute; top: 99px; left: 420px"			src="images/<?php  echo($price_bgimg) ?>">
        ...
        <div style="position: absolute; width: 200px; height: 28px; right: 20px; top: 46px; font-size: 30px;text-align: right;color:#666666;"><?php echo($row['creator']) ?></div>	
...
<script type="text/javascript">
   ...
   function ajax() {
        var wsId=<?php echo($wsId)?>;
    	var ajax=InitAjax(); 
	　	ajax.open("GET",'../../web.php/kltv/totals_ajax?wsId='+wsId,true);
...
// alert(models['id']);
				...
				//console.log("值为:",runCounts);
				if  (models['id'] > lasid || runCounts>6) {
