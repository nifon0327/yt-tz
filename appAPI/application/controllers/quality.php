...
class Quality extends MC_Controller {
/*
	功能:质检功能页面
*/
	...
	public function main()
	{
        ...
        if ($numsOfTypes>0){
	        ...
	        //待检
	        $this->load->model('GysshsheetModel');
	        $rows=$this->GysshsheetModel->get_sh_counts($Floor,2,$checkSign);
	       ...
	       $addedLineid = '';
	        ...
	        $dataArray[]=array(
	                            ...
	                            'col1'        =>array('Text'=>'',
	                                                  'Color'=>$lightgreen),
	                            'col2'        =>array('Text'=>$rows['OverQty']>0?number_format($rows['OverQty']):'',
	                                                  'Color'=>$red),
	                            'col3'        =>array(
	                                             'isAttribute'=>'1',
								                 'attrDicts'  =>array(
			...
			//品检报告					    
	        $rows=$this->GysshsheetModel->get_checked_counts($Floor);
			$dataArray[]=array(
	                            'tag'         =>'qcllj',
	                            'method'      =>'segment',
	                            'type'        =>$Floor,
	...
	function get_segment_list($Floor,$segmentIndex)
	{
       ...
       switch($segmentIndex){
	       ...
	       case 2: //品检中
	           $actions=$this->pageaction->get_actions('register,returnstock');
	           $this->load->model('BaseMPositionModel');
	           $records=$this->BaseMPositionModel->get_records($Floor);
	           $checkSign=$records['CheckSign'];
	   ...
	   foreach ($sheets as $rows){
		   ...
		   if($segmentIndex==2){
		        ...
		        $actions[0]['MaxQty']=$rows['Qty']-$djQty;
		        $actions[0]['DjQty'] =$djQty;
		   }
	...
	public function arrive_main(){
		...
		if ($CompanyId>0){
		    ...
		    foreach ($mrows as $rows){
			    ...
			    $rowArray[]=array(
			                'tag'     =>'distance',
							'col1'    =>$rows['OverQty']==0?'':number_format($rows['OverQty']),
							'col2'    =>array(
                                                'isAttribute'=>'1',
							                    'attrDicts'  =>array(
                                                 array('Text'=>number_format($rows['Qty']),'Color'=>"$black",'FontSize'=>'13'),
				...
				foreach ($sheets as $rows){
				   ...
				   $rowArray[]= array(
						...
						'rtIcon'  =>$rtIcon,
						'title'   =>$rows['StuffId'] . '-' . $rows['StuffCname'],
					    'col2'    =>number_format($rows['OrderQty']),
						'col3'    =>number_format($rows['Qty']),
						'Picture' =>$rows['Picture'],
						'selected'=>'',
				        'stuffImg'=>$stuffImg,
				        'process' =>array()
	...
	public function get_location()
	{
	    ...
	    if ($CheckSign==0 && $FrameCapacity>0){
		    ...
		    $unrkCounts=$unrkCounts<$frames?$frames:$unrkCounts;
	    }
	...
	{
	   return $Amount>=200000?'qc_car_1':($Amount>=50000?'qc_car_3':'qc_car_2');
	}
	...
	public function register()
	{
	    ...
	    if ($action=='register' && $djtype=='qc' && $frameCapacity>0){
		   ...
		   $dataArray=array(
		    ...
		    $data['jsondata']=array('status'=>'0','message'=>$message,'totals'=>'0','rows'=>array());
	    }
