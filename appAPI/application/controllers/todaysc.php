...
class Todaysc extends MC_Controller {
/*
	功能:今日生产工单
*/
...
// 		    $monthAmountNext=$this->ScCjtjModel->get_month_valuation($gpId);
		    ...
		    if ($month == date('Y-m')) {
			    $daygroupnums=$this->StaffMainModel->get_checkInNums_ingroup($gpId);
			    $newDay = $hoursNow *$daygroupnums*$laborCost;
				$monthAmountNext += $newDay;
		    }
		...
		for ($i = 0; $i < $rownums; $i++) {
		  ...
		  if ($boolfindToday == true || $date != $this->Date) {
			...
			$dataArray[]=array(
				...
				'showArrow'   =>'1',
				'open'        =>'0',
				'title'       =>array('Text'=>substr($date, 5),'Color'=>($weekday==0 ||$weekday==6)?'#ff665f':'#9a9a9a','dateCom'=>$dateCom,'fmColor'=>($weekday==0 ||$weekday==6)?'#ffa5a0':'#cfcfcf','light'=>'12.5','dateBg'=> $istoday?'#ffff2e':''),
