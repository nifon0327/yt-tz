...
class Yzz extends MC_Controller {
/*
	功能:今日生产工单
*/
	...
	public function segment()
	{
		...
		$rowArray=$this->ScSheetModel->get_month_ysced($wsid);
...
continue;
		    ...
		    if ($ActionId=='102') {
			    $qtyarr = $this->ScGxtjModel->mon_workshop_abnormals($wsid,$month);
		    } else {
			   $qtyarr = $this->ScCjtjModel->mon_workshop_abnormals($wsid,$month); 
		    }		    
...
if ($monthQty > 0) 
		    $percent = intval($qty3/$monthQty*100+0.5);
			 else    $percent='0';
			    
$allAbnormalPer[]=$percent;
		    ...
		    if ($i==0) {
			    $open = '1';
			    $subdataArray=$this->get_yscmon_subList($wsid,$month);
		    }
			...
			$dataArray[]=array(
				...
				'open'        =>''.$open,
				'title'       =>$titleAttr,
				'col2'        =>number_format($monthQty,0),
				'col1'        =>number_format($qty3),
				'percent'     =>array(
							   		'isAttribute'=>'1',
							   		'attrDicts'=>array(
								   		array('Text'    =>"$percent",
				...
				'pieValue'=>array(
					array("value"=>''.($monthQty-$qty3-$qty2),"color"=>"#e0e0e0"),
					array("value"=>"$qty2","color"=>"#f5ab47"),
					array("value"=>"$qty3","color"=>"#ff0000")
				),
			);
			$lastMon = $month;
		}
		...
		for ($i = 0; $i < $cts; $i++) {
			...
			$keduVal = $this->_get_up3_avg($allAbnormalPer,$i,$cts);
			$kedu = 1 - $keduVal/100;
			$dataArray[$i]['kedu'] = ''.($kedu);
		}
	...
	function _get_up3_avg($vals,$ind,$cts) {
		$val = 0;
		$findCts = 0;
		$allval = 0;
		for ($i = $ind+1;$i<$cts;$i++) {
	...
	public function get_yscmon_subList($wsid,$iddate) {
		...
		$rowArray=$this->ScSheetModel->mon_yscsheet($wsid,$actionid,$iddate);
		$rownums =count($rowArray);
...
$index ++;
				...
				'col2Img'    =>'',
				'bigCol1'    =>'1',
				'col3'       =>array('Text'=>number_format($Qty),'Align'=>'R'),
				'col3Img'    =>'',
				'sctime'     =>$minus,
				'col4'       =>array('Text'=>$scedInterval,
									'Color'=>$timeColor),
				'col1'       =>''.date('m/d H:i',$beginTime).'~'.date('m/d H:i',$lastTime),
		...
		usort($dataArray, function($a, $b) {
		 ...
		 if ($index>0) {
					   $dataArray[$rownums-1]['lineLeft'] = 0;
				   }
		return $dataArray;
	}
	...
	public function subList(){
	    ...
	    switch ($upTag) {
			...
			case   'yscMon':
					$listTag = 'order';
			        $dataArray=$this->get_yscmon_subList($wsid,$id);
					break;
		}
