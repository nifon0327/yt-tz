...
for ($i= 0;$i< count($sPOrderIdArr) && $i< count($StockIdArr); $i++){
    ...
    if($upResult){
        ...
        $login_detail = $login_user.'于今日'.$time.'完成'.$Log_Item.'流程。现需要您完成下一步"生产"工作，请及时登录研砼治筑运营平台进行操作。';//批量操作详情
        ...
        //echo "N";
        $Log.="<div class=redB>" . $sPOrderId . "领料单确认失败!</div><br>";
        $OperationResult="N";
    }
}
