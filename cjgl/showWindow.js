...
function showWindowDialog(url,vArguments,vPosition){
	var dialogWidth="auto";
	var dialogHeight="auto";
	var dialogCenter="No";
	var dialogScroll="Yes";
	var sPlace=0;
	var dPlace=0;
	var Args=['dialogWidth','dialogHeight','center','scroll'];
	var sLen=vArguments.length;
	for (var i=0;i<4;i++){
		sPlace=vArguments.indexOf(Args[i]); 
		if(sPlace>=0){
          dPlace=vArguments.indexOf(";",sPlace);
		  if (dPlace<0) dPlace=sLen;
		  sPlace=Args[i].length+sPlace+1;
		  dPlace=dPlace-sPlace;
		  //alert (Args[i]+":"+vArguments.substr(sPlace,dPlace));
		  eval(Args[i])+ '=\'vArguments.substr(sPlace,dPlace)\'';
		  alert (Args[i]);
		}
	}
}
...
function showWindowDialog(url,dialogHeight,dialogWidth,vPosition){
    if (url=="") return false;
	if (dialogHeight>0) {dialogHeight=dialogHeight+"px";} else{dialogHeight="auto";}
	if (dialogWidth>0) {dialogWidth=dialogHeight+"px";} else{dialogWidth="auto";}
	var newDiv=document.createElement("div");
    newDiv.id="Dialog_div";
	newDiv.width=dialogWidth;
	newDiv.height=dialogHeight;
	 var RemoveDiv=function(){var obj=document.getElementById('Dialog_div');obj.parentNode.removeChild('Dialog_div');};
	var closeBtn="<span onClick='removeDiv' style='float: right; display: block; CURSOR: pointer'> × </span>";
	newDiv.innerHTML=closeBtn;
