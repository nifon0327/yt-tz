...
$checkProduct=mysql_fetch_array(mysql_query("SELECT Y.ProductId,Y.OrderPO,Y.Qty as PQty,Y.PackRemark,Y.sgRemark,Y.ShipType,PI.Leadtime,C.Forshort AS Client,P.cName,P.TestStandard    
...
$POrderSTR="<span class='redB'>PO:</span>$OrderPO&nbsp;<span class='redB'>业务单流水号: </span>$POrderId($Client : $TestStandard)&nbsp;<span class='redB'>数量:</span>$PQty&nbsp; <span class='redB'>";
...
echo"<table id='$TableId'  cellspacing='0' border='0' align='center' bgcolor='#FFFFFF'>
	...
	<td width='40' align='center' class='A1101'>REACH</td>
...
$sListResult = mysql_query("SELECT 
	S.Id,S.Mid,S.StockId,S.POrderId,S.StuffId,S.Price,S.OrderQty,S.StockQty,S.AddQty,S.FactualQty,S.CompanyId,S.BuyerId,S.DeliveryDate,M.Date,A.StuffCname,A.Picture,
A.Gfile,A.Gstate,A.TypeId,B.Name,C.Forshort,C.Currency,MP.Remark AS Position,ST.mainType,MT.TypeColor ,U.Name AS UnitName,K.tStockQty 
	...
	WHERE S.POrderId='$POrderId' AND S.Level = 1 ORDER BY S.StockId",$link_id);
$i=1; //,K.tStockQty 
...
/////////////////////////////
		  ...
		  if($mainType<=1) {	
		   ...
		   //母配件显示子配件
            /* if($ComboxMainSign==1){
                   $ListId=getRandIndex();      
                   $showStr="<img onClick='ShowNewDropTable(ShowTable$ListId,ShowGif$ListId,ShowDiv$ListId,\"stuffcombox_pand_ajax\",\"$StockId|$StuffId\",\"admin\");' name='ShowGif$ListId' src='../images/showtable.gif' 
			title='显示或隐藏子配件资料.' width='13' height='13' style='CURSOR: pointer' bgcolor='$theDefaultColor' >";
                 ...
                 //半成品配件
                  ...
                  if($CheckSemiRow=mysql_fetch_array($CheckSemiSql)){
		              ...
		              $showStr="<img onClick='ShowNewDropTable(ShowTable_$ListId,ShowGif_$ListId,ShowDiv$ListId,\"$ajaxFile\",\"$StockId\",\"$ajaxDir\");' name='ShowGif_$ListId' src='../images/showtable.gif' 
			...
			<td bgcolor='$Sbgcolor' align='right' height='20' class='A0111'>$showStr$i</td>";//配件状态 
...
echo"<br><table  width='1000' cellspacing='1' border='1' align='center'><tr bgcolor='#FFFFFF'>";
		echo "<td><img  src='../public/bomflow/createorderflow.php?POrderId=$POrderId&ProductId=$ProductId' width='1000'/><td>";
		echo"</tr></table>";
?>