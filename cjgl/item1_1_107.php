...
WHERE 1 $SearchRows AND getCanStock(SC.sPOrderId,$checkScSign)=$checkScSign GROUP BY PD.ProcessId ORDER BY PT.SortId ,B.Id ",$link_id);
...
function RegisterGxQty(sPOrderId,StockId,gxQty,LastPos,thisProcessId,MaxQty){
	...
	divShadow.innerHTML="";
	var url="item1_1_107_dj.php?StockId="+StockId+"&sPOrderId="+sPOrderId+"&gxQty="+gxQty+"&LastPos="+LastPos+"&thisProcessId="+thisProcessId+"&MaxQty="+MaxQty;
　	alert(url);
	var ajax=InitAjax();
