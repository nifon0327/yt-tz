...
function InputRemark(RowId,Id){
		divShadow.innerHTML="";
	var url="item1_1_sgRemark.php?Id="+Id;
　	var ajax=InitAjax();
　	ajax.open("GET",url,true);
	ajax.onreadystatechange =function(){
	　　if(ajax.readyState==4 && ajax.status ==200){
...
function ToUpdatedsgRemark(){
	...
	var url="item1_1_updated.php?sPOrderId="+sPOrderId+"&sgRemark="+RemarkValue;
	var ajax=InitAjax();
	ajax.open("GET",url,true);
	ajax.onreadystatechange =function(){
		if(ajax.readyState==4){// && ajax.status ==200
...
function ShowOrHide(e,f,RowId,POrderId,tempId,showPage,fromPage){//行，列，行号
    if(showPage == "" || showPage ==undefined){
	    showPage = "showOrder";
    }
	...
	if (f.innerText=="[ - ]"){//展开
		//动态加入采购明细
		    if(showPage =="showOrder"){
			    var url="item1_ajax.php?POrderId="+POrderId+"&RowId="+RowId;  //产品订单显示所有明细
			}else if(showPage == "showSemi"){
				var url="item1_semi_ajax.php?mStockId="+tempId+"&RowId="+RowId;  //半成品显示所有明细
			}
			else{
				var url="item1_sc_ajax.php?POrderId="+POrderId+"&sPOrderId="+tempId+"&RowId="+RowId+"&fromPage="+fromPage; //生产工单显示所有明细
...
function RegisterQty(POrderId,sPOrderId,StockId,fromAction){
	var divShadow=document.getElementById('divShadow');
	divShadow.innerHTML="";
	var url="item1_1_scdj.php?POrderId="+POrderId+"&sPOrderId="+sPOrderId+"&StockId="+StockId+"&fromAction="+fromAction;
　	var ajax=InitAjax();
　	ajax.open("GET",url,true);
	ajax.onreadystatechange =function(){
	　　if(ajax.readyState==4 && ajax.status ==200){
...
function PrintTasks(POrderId,sPOrderId){
	var divShadow=document.getElementById('divShadow');
	divShadow.innerHTML="";
	var url="item1_1_scdy.php?POrderId="+POrderId+"&sPOrderId="+sPOrderId;
　	var ajax=InitAjax();
　	ajax.open("GET",url,true);
	ajax.onreadystatechange =function(){
	　　if(ajax.readyState==4 && ajax.status ==200){
...
function ToSavePT(){
	...
	if(Msg==1){
		...
		var sPOrderId=document.getElementById("sPOrderId").value;
		var url="item1_1_scdy_ajax.php?POrderId="+POrderId+"&sPOrderId="+sPOrderId+"&Qty1="+Qty1+"&Qty2="+Qty2+"&Qty3="+Qty3+"&Qty4="+Qty4;
	　	var ajax=InitAjax();
	　	ajax.open("GET",url,true);
		ajax.onreadystatechange =function(){
		　　if(ajax.readyState==4 && ajax.status ==200){
...
function ToSaveDj(){
    ...
    var CheckSTR=fucCheckNUM(Qty,"Price");
    var POrderId=document.getElementById("POrderId").value;
    var sPOrderId = document.getElementById("sPOrderId").value;
    var StockId = document.getElementById("StockId").value;
    var url="item1_1_scdj_ajax.php?StockId="+StockId+"&POrderId="+POrderId+"&sPOrderId="+sPOrderId+"&Qty="+Qty+"&Relation="+Relation;
    if(CheckSTR==0){
		...
		if(document.getElementById("tmpRelation") !=null  ){
			var tmpRelation=document.getElementById("tmpRelation").value;
			var Relation=document.getElementById("Relation").value;
			if (Relation!=""){
					var CheckSTR=fucCheckNUM(Relation,"");
					if(CheckSTR==0){
						document.getElementById("InfoBack").innerHTML="装箱数量不是规范的数字！";
						document.form1.Relation.value="";
						return false;
						}
			         if (tmpRelation==Relation){
				         Relation="";
			         }
	         }
         }
...
//打开DIV弹出窗口
function openWinDialogWithParas(e,url,w,h,Pos,para){
    var showDialog=document.getElementById("winDialog");
    showDialog.innerHTML="";
    var ajax=InitAjax();
    ajax.open("get",url+'?ids='+para,true);
    ajax.onreadystatechange =function(){
        if(ajax.readyState==4 && ajax.status ==200){
            showDialog.innerHTML=ajax.responseText;
        }
...
function ShowDropTable(e,d,toWebPage,args){

    if (e.style.display=="none"){
         var url="../admin/"+toWebPage+".php?args="+args;
	 var ajax=InitAjax();
	 ajax.open("GET",url,true);
	 ajax.onreadystatechange =function(){
...
function ShowNewDropTable(e,ff,d,toWebPage,args,fromPage){
...
function NewShowOrHide(e,f,RowId,POrderId,StockId){//行，列，行号
	...
	if (f.innerText=="[ - ]"){//展开
		...
		if(POrderId!=""){
		    ...
		    var url="item1_ajax.php?POrderId="+POrderId+"&StockId="+StockId+"&RowId="+RowId;
		　	var show=eval("ShowDiv"+RowId);
		　	var ajax=InitAjax();
		　	ajax.open("GET",url,true);
			   ajax.onreadystatechange =function(){
