<?php
$Th_Col="选项|30|配件|40|序号|30|客户名称|80|业务单号|100|产品名称|120|交期|100|工单流水号|100|半成品ID|60|采购流水号|100|半成品名称|220|生管备注|150|数量|60|已生产|60|登记数|60|操作|60";
...
if($clientRow = mysql_fetch_array($clientResult)) {
    echo"<select name='scDate' id='scDate' onchange='ResetPage(1,1)'>";
    do{
        $thisscDate=$clientRow["scDate"];
        $scDate=$scDate==""?$thisscDate:$scDate;
        if($scDate==$thisscDate){
            echo"<option value='$thisscDate' selected>$thisscDate</option>";
            $SearchRows.=" and SC.scDate='$thisscDate' ";
        }
        else{
            echo"<option value='$thisscDate'>$thisscDate</option>";
        }
...
if ($Login_JobId == 42) {
 ...
 echo"</td><td colspan='4' align='right' class=''><span class='ButtonH_25'  id='batchUpdate'  onclick='batchOperate();' $disabled>保存</span>&nbsp;$searchList1&nbsp;</td><td colspan='2'  class=''><input name='NowInfo' type='text' id='NowInfo' value='$nowInfo' class='text' disabled></td></tr>";
	//输出表格标题
...
$j=2;$i=1;
$mySql="SELECT O.Forshort,SC.Id,SC.POrderId,SC.sPOrderId,SC.Qty,SC.Estate,SC.ActionId,SC.StockId AS scStockId,SC.Remark,SC.mStockId,
		...
		switch($scQty){
		 ...
		 if($czSign==1){//可以操作
			//if($SubAction==31  ){//有权限:需要是该类别下的小组成员，方有权登记
                  ...
                  $CheckData="<input type='checkbox' id='checkId$i' name='checkId$i' value='$POrderId|$sPOrderId|$scStockId|$mStockId' />";
             // }
           }
        /*
                   if($llSign!=2){
...
function batchOperate() {
	...
	jQuery('input[name^="checkId"]:checkbox').each(function() {
        if (jQuery(this).prop('checked') ==true) {
    		...
    		if (choosedRow == 1) {
    			...
    			mStockIds=mStockIds+ '|' + splitArr[3];
    			qtys=qtys+ '|' + updateQty;
			}
        }
	});
	...
	if (qtyerror)
  {
   ...
   ajax.send("POrderIds="+POrderIds+"&sPOrderIds="+sPOrderIds+"&scStockIds="+scStockIds+"&qtys="+qtys+'&mStockIds='+mStockIds);
}
