...
//电信-EWEN
		...
		//加班费
$kq_Result = mysql_query("SELECT * FROM $DataIn.cwxzsheet  WHERE Month='$defaultMonth' and Number=$DefaultNumber",$link_id);
if($kq_Row=mysql_fetch_array($kq_Result)){
	...
	echo"<tr height='25' bgcolor='#FFCC99'><td colspan='4' class='A0111'> 加班费(当前时薪:$oneHours)：$JBF</td>";
	...
	$kqdata_Result = mysql_query("SELECT * FROM $DataIn.kqdatd  WHERE Month='$defaultMonth' and Number=$DefaultNumber and Locks=0",$link_id);
	if($kqdata_row=mysql_fetch_array($kqdata_Result)){
		$ActionSTR="锁定";
		}
	echo"<tr height='25' bgcolor='#FFCC99'><td colspan='19' class='A0111'>(注:G为工作日加点工时,X为休息日加班工时,F为法定假日加班工时)</td>
		<td colspan='5' class='A0101' align='center'>&nbsp;</td></tr>";//$ActionSTR
?>