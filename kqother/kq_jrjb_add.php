	...
	do{
		...
		if($jrAmount>0){
			...
			if($DataIn !== 'ac'){
				...
				$inRecode2="INSERT INTO $DataIn.hdjbsheet SELECT NULL,'0',staffmain.BranchId,staffmain.JobId,staffmain.Number,'$CheckMonth','$sumXJTime','$HourlyWage2','$sumFJTime','$HourlyWage3','$jrAmount','$Date','1','1','$Operator', 0, '$Operator', NOW(), '$Operator', NOW()
				FROM $DataPublic.staffmain WHERE Number='$Number'  and  cSign='$Login_cSign'  and Number NOT IN (SELECT Number FROM $DataIn.hdjbsheet WHERE Month='$CheckMonth' and Number='$Number')";	
			}
	...
	do{
		...
		if($jrAmount>0){
			...
			if($DataIn !== 'ac'){
				...
				$inRecode2="INSERT INTO $DataIn.hdjbsheet SELECT NULL,'0',staffmain.BranchId,staffmain.JobId,staffmain.Number,'$CheckMonth','$sumXJTime','$HourlyWage2','$sumFJTime','$HourlyWage3','$jrAmount','$Date','1','1','$Operator', 0, '$Operator', '$Date', '$Operator', '$Date'
				FROM $DataPublic.staffmain WHERE Number='$Number' and  cSign='$Login_cSign'   and Number NOT IN (SELECT Number FROM $DataIn.hdjbsheet WHERE Month='$CheckMonth' and Number='$Number')";
			}
			$inAction2=@mysql_query($inRecode2);
			if ($inAction2){ 
