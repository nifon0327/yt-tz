...
S.Dx,S.Gljt,S.Gwjt,S.Jj,S.Shbz,S.Zsbz,S.Jtbz,S.Jbf,S.Yxbz,S.taxbz,S.Jz,S.Sb,S.Kqkk,S.dkfl,S.RandP,S.Otherkk,S.Amount,S.Estate,S.Locks,S.Gjj,
	P.Name,P.ComeIn,P.Estate AS mEsate,B.Name AS Branch,J.Name AS Job,BK.Title
FROM $mainData M
...
WHERE 1 $SearchRows
order by M.Date DESC,S.BranchId,S.JobId,P.ComeIn";
...
if($mainRows = mysql_fetch_array($mainResult)){
	...
	do{
		...
		$sumShbz+=$Shbz;	$sumZsbz+=$Zsbz;		$sumJtbz+=$Jtbz; $sumYxbz+=$Yxbz;	 $sumtaxbz+=$taxbz;	$sumKqkk+=$Kqkk;$sumdkfl+=$dkfl;
		$sumTotal+=$Total;	$sumJz+=$Jz;			$sumSb+=$Sb;	$sumGjj+=$Gjj;		$sumAmount+=$Amount;  $sumCt+=$Ct;
		$sumJbf+=$Jbf;
		$Dx=SpaceValue0($Dx);$Gljt=SpaceValue0($Gljt);$Gwjt=SpaceValue0($Gwjt);$Jj=SpaceValue0($Jj);
		$Shbz=SpaceValue0($Shbz);$Zsbz=SpaceValue0($Zsbz);$Jtbz=SpaceValue0($Jtbz);$Yxbz=SpaceValue0($Yxbz);$taxbz=SpaceValue0($taxbz);$Kqkk=SpaceValue0($Kqkk);$dkfl=SpaceValue0($dkfl);
		$Total=SpaceValue0($Total);
		$Jz=SpaceValue0($Jz);$Sb=SpaceValue0($Sb);$Gjj=SpaceValue0($Gjj);$RandP=SpaceValue0($RandP);$Otherkk=SpaceValue0($Otherkk);$Amount=SpaceValue0($Amount);
		$Jbf=SpaceValue0($Jbf);$sumCt=SpaceValue0($sumCt);
		...
		if($midDefault!="" && $midDefault==$Mid){//同属于一个主ID，则依然输出明细表格
			...
			$m=$m+2;
			echo"<td  class='A0001' width='$Field[$m]' align='center'>$Zsbz</td>";
			$m=$m+2;
			echo"<td  class='A0001' width='$Field[$m]' align='center'>$Jtbz</td>";
			$m=$m+2;
			echo"<td  class='A0001' width='$Field[$m]' align='center'>$Yxbz</td>";
			$m=$m+2;
			echo"<td  class='A0001' width='$Field[$m]' align='center'>$taxbz</td>";
			$m=$m+2;
			...
			$m=$m+2;
			echo"<td  class='A0001' width='$Field[$m]' align='center'>$Ct</td>";
			$m=$m+2;
			...
			echo"<td  class='A0001' width='' align='center'>$EstateSign</td>";//请款状态
			echo"</tr></table>";
			$i++;
			}
		else{
