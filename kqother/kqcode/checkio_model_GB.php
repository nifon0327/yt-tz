...
if($AO<$dRestTime1){//情况1：早上签退（早退）;按签退取整后的时间计算
	...
	if($AO<=$dRestTime2){//情况：中午签退（早退）；按12点计算
		$WorkTime=abs(strtotime($dRestTime1)-strtotime(rounding_in($AI)))/3600;
		$test="B";
		}
	else{
		if($AO<date("Y-m-d H:i:00",strtotime("$dDateTimeOut - $dOutEarly minute"))){//情况3：下午签退；按签退取整后的时间计算，需扣60分钟
			$WorkTime=abs(strtotime(rounding_out($AO))-strtotime(rounding_in($AI))-3600)/3600;
			$test="C";
			}
		else{
			$AOcolor="";			//记录正常，不取色
			$OutEarlys=0;
			$WorkTime=abs(strtotime($dDateTimeOut)-strtotime(rounding_in($AI))-3600)/3600;			//情况4：正常签退17:00-18:00
			$test="D";
