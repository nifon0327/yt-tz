...
for($k=0;$k<=$Days;$k++){
	...
	if($DataIn !== 'ac'){
		...
		SELECT NULL,Number,'$StartDateTime','$EndDateTime','$InLate','$OutEarly','$TimeType','$RestTime','0','$Operator' 
		...
		$inRecode="INSERT INTO $DataIn.kqlspbb
		SELECT NULL,Number,'$StartDateTime','$EndDateTime','$InLate','$OutEarly','$TimeType','$RestTime','0','$Operator', 1, 0, '$Operator', NOW(), '$Operator', NOW(), NOW()
		FROM $DataPublic.staffmain WHERE 1 $NumberSTR AND KqSign!='3' AND Estate='1' AND cSign='$Login_cSign'
		AND Number NOT IN(SELECT Number FROM $DataIn.kqdata WHERE Month='$thisMonth' ORDER BY Number)";
	}
