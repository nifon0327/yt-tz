...
if($DataIn !== 'ac'){
	$inRecode = "INSERT INTO $DataPublic.redeployj SELECT NULL,Number,'$JobId','$ActionIn','$Month','$Remark','$Date','0','$Operator' FROM $DataPublic.staffmain WHERE 1 $NumberSTR ";
}else{
	$inRecode = "INSERT INTO $DataPublic.redeployj SELECT NULL,Number,'$JobId','$ActionIn','$Month','$Remark','$Date','0','$Operator', 0, '$Operator', '$Date', '$Operator', '$Date' FROM $DataPublic.staffmain WHERE 1 $NumberSTR ";
}
$inResult=@mysql_query($inRecode);
if($inResult){	
	$Log="&nbsp;&nbsp; $TitleSTR 成功. ($Ids)</br>";//更新
	$upSql = "UPDATE $DataPublic.staffmain SET JobId='$ActionIn' WHERE 1 $NumberSTR";
