...
if($DataIn !== 'ac'){
	...
	SELECT NULL,BranchId,JobId,Number,'$CheckTime','$CheckType','1','1','1','0','$KrSign','$Operator' FROM $DataPublic.staffmain WHERE 1 $NumberSTR 
	AND KqSign!='3' AND Estate='1'  AND Number NOT IN(SELECT Number FROM $DataIn.kqdata WHERE Month='$theMonth')";
}else{
	$inRecode = "INSERT INTO $DataIn.checkinout 
	SELECT NULL,BranchId,JobId,Number,'$CheckTime','$CheckType','1','1','1','0','$KrSign','$Operator', 0, '$Operator', NOW(), '$Operator', NOW(), NOW() FROM $DataPublic.staffmain WHERE 1 $NumberSTR 
	AND KqSign!='3' AND Estate='1'  AND Number NOT IN(SELECT Number FROM $DataIn.kqdata WHERE Month='$theMonth')";
}
