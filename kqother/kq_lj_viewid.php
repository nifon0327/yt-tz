...
$Login_help="kq_lj_viewid";
$FunctionFrom="viewid";
//session_register("Login_help"); 
$_SESSION["Login_help"] = $Login_help;
$DefaultY=date("Y");
$ColsNumber=9;
$Th_Col="序号|40|薪酬类型|80|部门|60|职位|60|员工姓名|80|入职日期|100|总天数/年|80|有效天数|100|$DefaultY 年假|80|可用年假|80";
   ...
   <td width="150" id="menuT2" align="center" class="A1100" <?php  echo $Fun_bgcolor?>>
		<table border="0" align="center" cellspacing="0">
   			<tr>
				...
				<?php  
					//权限设定
					echo"<nobr><span onclick='window.close()' $onClickCSS>关闭</span></nobr>";
			   ?>
				</td>
			</tr>
	 </table>
	...
	WHERE 1 AND M.cSign='$Login_cSign' AND M.Dimission='0000-00-00' and M.Number='$Tid' order by 
		...
		do{
			...
			if($Action==1){
				...
				*/
				...
				//计算已用年假
				$ljResult = mysql_query("SELECT * FROM $DataPublic.kqqjsheet WHERE Number=$Tid and Type='L' and left(StartDate,4)='$DefaultY' order by StartDate DESC",$link_id);
				$HoursSUM=0;
				if($ljRow = mysql_fetch_array($ljResult)) {					
					do{
						$StartDate=$myrow["StartDate"];
						$EndDate=$myrow["EndDate"];
						$HoursTemp=abs(strtotime($StartDate)-strtotime($EndDate))/3600;//向上取整
						$HoursSUM=$HoursSUM+$HoursTemp;
						}while ($ljRow = mysql_fetch_array($ljResult));
					}
				...
				//
				$useAbaleLJ=$AnnualLeave-$HoursSUM;//年假总工时-请假总工时
				$ljDays=intval($useAbaleLJ/8);//取整求相隔天数
				//计算请假工时
				$ljHours=($useAbaleLJ*10)%(8*10)/10;//求余取相隔小时数
				$ljDays=$ljDays==0?"&nbsp;":$ljDays."天";
				//$ljHours=$ljHours==0?$ljDays:$ljDays.$ljHours."小时";
				...
				echo"<td class='A0101' width='$Field[19]' align='center'><div class='greenB'>$ljDays</div></td>";
				echo"</tr></table>";
				$i++;
			}
