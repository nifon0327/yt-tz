...
if($From!="slist"){
	...
	$date_Result = mysql_query("SELECT DATE_FORMAT(GDate,'%Y-%m') AS Month FROM $DataIn.kqrqdd group by DATE_FORMAT(GDate,'%Y-%m') order by GDate DESC",$link_id);
...
$mySql="SELECT D.Id,D.Number,D.GDate,D.XDate,D.Operator,D.Locks,M.Name 
FROM $DataIn.kqrqdd D
LEFT JOIN $DataPublic.staffmain M ON M.Number=D.Number
WHERE 1 $SearchRows 
order by D.GDate DESC,D.Number";
//echo "$mySql";
