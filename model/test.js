//2	改变选定行状态	表格	  行号（表格数目） 鼠标动作		非选定色	  鼠标经过色		选定色		   着色列数 求和列数

//求和列改变
function ChangeSumCol(){
	if(document.all("sumCol")!=null){
		if(document.form1.sumCol.value!=""){
			theCol=document.form1.sumCol.value;
			var Rows=document.form1.IdCount.value;
			document.form1.sumAmount.value=0;
			for(var i=1;i<=Rows;i++){
				...
				//alert(i+" - "+currentColor);
					/*
					if(eval("document.form1.checkid"+i).checked){
						//计算
						var sumAmount=Number(document.form1.sumAmount.value)+Number(theRow.cells[theCol].innerText);
						sumAmount=sumAmount.toFixed(2);
						document.form1.sumAmount.value=sumAmount;
						}
						*/
						
						
				}
			//var outAmount=document.form1.sumAmount.value;
			//window.status=sumAmount!=0?"选定行合计："+outAmount:"";
			}
		}
	}
...
function OpenPhotos(d,f,s){
	win=window.open("openphotos.php?d="+d+"&f="+f+"&s="+s,"new","toolbar=no, menubar=no, scrollbars=yes,resizable=yes,location=no, status=no");
	}
...
//16	显示或隐藏配件采购单列表
function ShowOrHide(e,f,Order_Rows,POrderId,RowId){
	...
	if (yy.indexOf("showtable")==-1){
		...
		//动态加入采购明细
		if(POrderId!=""){			
			var url="../admin/clientorder_ajax.php?POrderId="+POrderId+"&RowId="+RowId; 
...
//4		全选记录行												着色列数
function All_elects(theDefaultColor,thePointerColor,theMarkColor,theMerge,theCol){
	...
	for(var i=1;i<=$Rows;i++){
		...
		if(theTable.rows[0].cells[4].getAttribute('bgcolor').toUpperCase()!=theMarkColor){
			...
			chooseRow(theTable,i,"click",theDefaultColor,thePointerColor,theMarkColor,"",theMerge,theCol);
			}
		}
	}
	
//3		反选记录行
function Instead_elects(theDefaultColor,thePointerColor,theMarkColor,theMerge,theCol){
	var Rows=document.form1.IdCount.value;
	for(var i=1;i<=Rows;i++){
		var theTable=eval("ListTable"+i);
		if(document.all("checkid"+i)!=null){
...
//2	改变选定行状态	表格	  行号（表格数目） 鼠标动作		非选定色	  鼠标经过色		选定色		   着色列数 求和列数
function chooseRow(theTable, theRowNum, theAction, theDefaultColor, thePointerColor, theMarkColor, theFrom,theMerge,theCol){
	//求和两种形式：1、列表框选择求和的列	2、直接指定求和的列
	var sumColArray=theCol.split(",");
	var sumColLength=sumColArray.length;//长度
...
//分解求和列数据,以，分开
	...
	if (currentColor == '' || currentColor.toLowerCase() == theDefaultColor.toLowerCase()) {//全选
		...
		//求和处理************************
		if (theCol!=""){
			if(sumColLength==1){//单列求和
				var sumAmount=Number(document.form1.sumAmount.value)+Number(theRow.cells[theCol].innerText);
				sumAmount=sumAmount.toFixed(2);
				document.form1.sumAmount.value=sumAmount;
				window.status=sumAmount!=0?"选定行合计："+sumAmount:"";
				}
			else{
				var OutText="选定行合计：";
				for(var k=0;k<sumColLength;k++){//第一求和列为金额
					var tempTheCol=sumColArray[k];
					k++;
					var tempDecimal=sumColArray[k];		//保留小数位数
					//读取表格标题表列相应数据
					var tt=TableHead.rows[0].cells[tempTheCol].innerText;
					var oldSum=TableHead.rows[0].cells[tempTheCol].data;
					if(oldSum==undefined){
						oldSum=0;
						}
					var tempsumAmount=Number(oldSum)+Number(theRow.cells[tempTheCol].innerText);
					tempsumAmount=tempsumAmount.toFixed(tempDecimal);
					TableHead.rows[0].cells[tempTheCol].data=tempsumAmount;
					//输出
					OutText=OutText+TableHead.rows[0].cells[tempTheCol].innerText+":"+tempsumAmount+" ";
					}
