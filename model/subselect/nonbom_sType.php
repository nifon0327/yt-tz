...
$RowFromSTR.=$mainType==""?"":" AND A.mainType='$mainType'";
$TypeResult = mysql_query("SELECT A.Id,A.TypeName FROM $DataPublic.nonbom2_subtype A WHERE A.Estate=1 AND (A.cSign='0' OR A.cSign='$Login_cSign') $RowFromSTR ORDER BY A.Id",$link_id);
if($TypeRow = mysql_fetch_array($TypeResult)){
	if($TypeFrom!=""){//来自于记录行，输出类型名称
		...
		switch($SelectFrom){
			...
			default://来自于新增或更新页面
				$TypeIdWidth= $TypeIdWidth==""?"380px": $TypeIdWidth;
				$TypSelect="<select name='TypeId' id='TypeId' style='width:$TypeIdWidth' dataType='Require' msg='未选择' onchange=\"getRule(this)\"><option value='' selected>--请选择--</option>";
			break;
			}	
		do{
			$TypeName = $TypeRow["TypeName"];
			$TypeColor ="#000000";// $TypeRow["Color"];
			$theId=$TypeRow["Id"];
			if($TypeId==$theId){
