...
$TypeResult = mysql_query("SELECT A.Id,A.Name, A.Color FROM $DataPublic.usertype A WHERE A.Estate=1 $RowFromSTR ORDER BY A.Id",$link_id);
if($TypeRow = mysql_fetch_array($TypeResult)){
	if($TypeFrom!=""){//来自于记录行，输出类型名称
		...
		switch($SelectFrom){
			case 1://来自于浏览页面
				$TypSelect="<select name='uType' id='uType' onchange='ResetPage(this.name)'>";
			break;
			case 4://来自于查询页面
				$uTypeWidth= $uTypeWidth==""?"380px": $uTypeWidth;
				$TypSelect="<select name=value[] id='value[]' style='width:$uTypeWidth'><option value='' selected>全部</option>";
			break;
			default://来自于新增或更新页面
				$uTypeWidth= $uTypeWidth==""?"380px": $uTypeWidth;
				$TypSelect="<select name='uType' id='uType' style='width:$uTypeWidth' dataType='Require' msg='未选择'><option value='' selected>--请选择--</option>";
			break;
			}	
		do{
			$TypeName = $TypeRow["Name"];
			...
			if($uType==$theId){
				...
				if($cSignTB!=""){
					$SearchRows.=" AND $cSignTB.uType='$theId'";
					}
				}
			else{
