...
$TypeResult = mysql_query("SELECT A.Id,A.Name FROM $DataPublic.aqsc07_type A WHERE A.Estate=1 $RowFromSTR ORDER BY A.Id",$link_id);
if($TypeRow = mysql_fetch_array($TypeResult)){
	if($TypeFrom!=""){//来自于记录行，输出类型名称
		$TypeName=$TypeRow["Name"];echo $TypeName;
		}
	else{//输出选择框
		switch($SelectFrom){
			case 1://来自于浏览页面
