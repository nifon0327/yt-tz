...
$TypeResult = mysql_query("SELECT A.Id,A.Name FROM $DataPublic.branchtype A WHERE A.Estate=1 $RowFromSTR ORDER BY A.Id",$link_id);
if($TypeRow = mysql_fetch_array($TypeResult)){
	if($TypeFrom!=""){//来自于记录行，输出类型名称
		...
		switch($SelectFrom){
			case 1://来自于浏览页面
				$TypSelect="<select name='TypeId' id='TypeId' onchange='ResetPage(this.name)'>";
			break;
			case 4://来自于查询页面
