...
//for($i=0;$i<count($actionArray);$i++){
	//$Id=$actionArray[$i];
if ($ActioToS!=""){
 $checkPageMenuS=mysql_query("SELECT Id,Name,ToRecord,ToPower,ToPage,ToWin,ToWarn FROM $DataPublic.sys3_pagemenu WHERE Id IN($ActioToS) AND Estate=1 ORDER BY FIELD(Id,$ActioToS)",$link_id);
  while($checkPageMenuR=mysql_fetch_array($checkPageMenuS)){
		$Id=$checkPageMenuR["Id"];
