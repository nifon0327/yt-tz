...
class BarCode128 { //生成条码类
	...
	public function createBarCode($image_type ='png',$file_name=null){
		...
		$this->image = ImageCreate($this->width,$this->height+2*$this->quiet_zone); 
		...
		$ey = 0;
		$ex = $this->poiX+$this->codeWidth;
                $t_num = strlen($this->text);
                if ($t_num<3) $sy=$this->poiY+(3-$t_num)*15;
		for($i=0;$i<strlen($this->bin_code);$i++){
			...
			$sy =$ey + 1;
                ...
                $font_1="Fonts/ARIALBD.TTF"; 
                $font_omb="Fonts/OMB___WT.TTF";
                
                imagettftext($this->image,10,90,$this->poiX+$this->font_height,$this->poiY-$this->poiY/3,$this->color,$font_1,'Qty:');
               // imagestringup($this->image,$this->font_type,15,38,'Qty:',$this->color);  
                $t_num = strlen($this->text);
                if ($t_num<=3) $this->font_height=18;
                $t_y=$t_num*6;
                $sx=$this->poiX+2*$this->font_height+5;
                $sy=$this->poiY-$this->poiY/2+$t_y;
                //ImageFilledRectangle($this->image,28,$sy-$t_y*2,56,$sy,$this->color);
                imagettftext($this->image,$this->font_height,90,$sx,$sy,$this->color,$font_omb,$this->text);
                
                //imagestringup($this->image,$this->font_type,40,32,$this->text,$this->bgcolor);  
	/*	$t_num = strlen($this->text);
