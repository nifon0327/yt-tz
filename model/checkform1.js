function ActionTo(actionId,chooseNum){
	//$ActionStr="查询,		新增,	更新,		删除,		可用,		禁用			,锁定		,解锁,		全选		反选
				//列印,		统计,	其它,		请款,		退回,		取消结付,		审核通过,	结付,		财务查询，	处理
				//加急,		拆分,	需求单异动,	联系人,		FAXBOOK,	重置,			还原,		装箱,		出货,		异动出货(未使用),
				//厂牌列印，离职,	工资条，	出货退回	取消出货	附档			领用;		生成Excel;	生成PI
	var Message="";
	var upId="";
	var LinkChar=["select",	"add",	"update",	"del",		"updated",	"updated",		"updated",	"updated",		"",		"",
				  "print",	"count","other",	"updated",	"updated",	"updated",		"updated",	"updated",		"countp","updated",
				  "updated","analyzes","change","linkman",	"faxbook",	"updated",		"updated",	"Packing",		"updated",		"",
				  "cardprint","updated","list","updated",	"updated",	"picture",		"Recipients","toexcel","topi"];
	...
	if(Message!=""){
		...
		if(LinkChar[actionId]=="linkman"){
			var funFrom=document.form1.funFrom.value;
			document.form1.action=LinkChar[actionId]+"_read.php?ComeFrom=otherPage";
			}
		else{
			document.form1.action=funFrom+"_"+LinkChar[actionId]+".php?ActionId="+actionId+upId;
			}
		if(actionId==10 || actionId==18 || actionId==32 || actionId==37){
...
Validator = {
	...
	Validate :
		function(thisFromId, mode,toWebPage,thisAction,thisExtra){
	...
	DoFilter : 			//上传文件格式过滤	(不指定)默认可以：jpg,gif,png,rar,zip,pdf,ai,psd,eps,titf,doc,xsl
		function(input, theFilter,theName,theId,theSize,theMsg,theRow,theCel,theTable){
			...
			if(input){
				...
				if(upInfo==false){
					NoteTable.rows[theRow].cells[theCel].innerHTML=" <input name="+theName+" type='file' id="+theId+" size="+theSize+" DataType='Filter' Accept="+theFilter+" Msg="+theMsg+" Row="+theRow+" Cel="+theCel+">";
					}
				return upInfo;
				}
			else{
