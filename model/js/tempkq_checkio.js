...
function updateTHIS(theID,Fields,ID,CHECKTYPE){
    ...
    eval(theID).innerHTML="<select id='TableSel' style='width: 70px;' OnBlur='backTHIS("+theID+",\""+thisFields+"\","+ID+",\""+CHECKTYPE+"\")' onchange='selectChange(this.options[this.selectedIndex].value,this.options[this.selectedIndex].innerText,"+theID+",\""+thisFields+"\","+ID+",\""+CHECKTYPE+"\")'>"+optionText+"</select>";
...
function selectChange(selectVALUE,selectTEXT,theID,Fields,ID,CHECKTYPE)//(选框ID值,选框显示名,,操作字段)
{
	...
	myurl="stafftempkq_updated.php?ActionId=902&Id="+ID+"&CheckType="+selectVALUE+"&Field="+Fields;
