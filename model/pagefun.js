function ToTextbox(TempFrom, DoorId, WeekDay, TimeType) {//单元格点击编辑,门禁系统用 EWEN 2012-09-19
    if (window.event.srcElement != TempFrom) {
        return;
    }
    ;
    var oldText = TempFrom.innerHTML;
    var obj = document.createElement("input");
    var str = TempFrom.innerHTML;
    TempFrom.innerHTML = "";
    obj.name = "v";
    obj.type = "text";
    if (TempFrom.width == "") {
        ...
        obj.style.width = TempFrom.width;
    }
    obj.style.fontSize = "12px";
    obj.value = str;
    obj.onblur = function () {
        TempFrom.innerHTML = this.value;
        if (oldText != TempFrom.innerHTML) {
            var TempPowerType = document.form1.PowerType.value;
            var TempParameters = document.form1.Parameters.value;
            var url = "mj_power_ajax.php?DoorId=" + DoorId + "&WeekDay=" + WeekDay + "&TimeType=" + TimeType + "&PowerType=" + TempPowerType + "&Parameters=" + TempParameters + "&Time=" + TempFrom.innerHTML;
            var ajax = InitAjax();
            ajax.open("GET", url, true);
            ajax.onreadystatechange = function () {
                if (ajax.readyState == 4 && ajax.status == 200) {
                    //alert(ajax.responseText);// 0保存不成功
                }
            }
            //发送空
            ajax.send(null);
...
function DateDiff(checkDate) {
    var aDate, oDate1, oDate2, iDays
    aDate = checkDate.split("-")
    oDate1 = new Date(aDate[1] + '-' + aDate[2] + '-' + aDate[0]) //转换为12-18-2002格式
    var today = new Date();
    var year = today.getYear();//取当前时间的年
    var month = today.getMonth() + 1;//取当前时间的月
    var day = today.getDate();//取当前时间的日
    oDate2 = new Date(month + '-' + day + '-' + year)
...
function showMaskDiv(WebPage, CompanyId, FileDir) {	//显示遮罩对话框
    ...
    if (UpdataIdX == 0 || CompanyId == "") {
        ...
        sOrhDiv("" + WebPage + "", CompanyId, FileDir);
    }
}
...
function sOrhDiv(WebPage, DeliveryValue, FileDir) {
    if (DeliveryValue != "") {
        if (FileDir != "admin") {
...
function s1ReBack() {
    ...
    for (var i = 0; i < form1.elements.length; i++) {
        var e = form1.elements[i];
        var NameTemp = e.name;
        var Name = NameTemp.search("CencalS");//过滤查询的checkbox
    ...
    if (window.opener) {
        if (window.opener.document.getElementById("SafariReturnValue")) {  //专为safari设计 zx 2011-05-04
            window.opener.document.getElementById("SafariReturnValue").value = returnq;
        }
    }
    if (window.parent.location.href.indexOf('desk/mc.php') != -1) {//模态框值回调 by ckt 2017-12-23
        var mainFrame = window.parent.frames['mainFrame'];
        var SafariReturnValue = mainFrame.document.getElementById("SafariReturnValue");
        SafariReturnValue.value = returnq;
        eval('window.parent.frames[\'mainFrame\'].' + SafariReturnValue.callback);
        mainFrame.clearFrame();
        return false;
    }
    window.opener.window.returnVaule = returnq;
    console.log(returnq);
    parent.window.close();
...
function SearchRecord(tSearchPage, fSearchPage, SearchNum, Action) {
    ...
    switch (theType) {
        ...
        case "button"://按钮
            ...
            if (document.all("theYear") != null) {//奖金年度
                var theYearT = document.getElementById('theYear').value;
                theYearT = "&theYear=" + theYearT;
            }
            if (document.all("ItemName") != null) {//奖金项目
                var ItemNameT = document.getElementById('ItemName').value;
                ItemNameT = "&ItemName=" + ItemNameT;
            }

            var BackData = window.open(tSearchPage + "_s1.php?r=" + r + "&tSearchPage=" + tSearchPage + "&fSearchPage=" + fSearchPage + "&SearchNum=" + SearchNum + "&Action=" + Action + uTypeT + JobIdT + BranchIdT + MonthT + KqSignT + theYearT + ItemNameT, "BackData", "dialogHeight =500px;dialogWidth=930px;center=yes;scroll=yes");
            if (!BackData) {  //专为safari设计的 ,add by zx 2011-05-04
                if (document.getElementById('SafariReturnValue')) {
                    //alert("return");
                    var SafariReturnValue = document.getElementById('SafariReturnValue');
...
//简繁转换
function gbStr() {
    return '啊阿埃挨哎唉哀皑癌蔼矮艾碍爱隘鞍氨安俺按暗岸胺案肮昂盎凹敖熬翱袄傲奥懊澳芭捌扒叭吧笆八疤巴拔跋靶把耙坝霸罢爸白柏百摆佰败拜稗斑班搬扳般颁板版扮拌伴瓣半办绊邦帮梆榜膀绑棒磅蚌镑傍谤苞胞包褒剥薄雹保堡饱宝抱报暴豹鲍爆杯碑悲卑北辈背贝钡倍狈备惫焙被奔苯本笨崩绷甭泵蹦迸逼鼻比鄙笔彼碧蓖蔽毕毙毖币庇痹闭敝弊必辟壁臂避陛鞭边编贬扁便变卞辨辩辫遍标彪膘表鳖憋别瘪彬斌濒滨宾摈兵冰柄丙秉饼炳病并玻菠播拨钵波博勃搏铂箔伯帛舶脖膊渤泊驳捕卜哺补埠不布步簿部怖擦猜裁材才财睬踩采彩菜蔡餐参蚕残惭惨灿苍舱仓沧藏操糙槽曹草厕策侧册测层蹭插叉茬茶查碴搽察岔差诧拆柴豺搀掺蝉馋谗缠铲产阐颤昌猖场尝常长偿肠厂敞畅唱倡超抄钞朝嘲潮巢吵炒车扯撤掣彻澈郴臣辰尘晨忱沈陈趁衬撑称城橙成呈乘程惩澄诚承逞骋秤吃痴持匙池迟弛驰耻齿侈尺赤翅斥炽充冲冲虫崇宠抽酬畴踌稠愁筹仇绸瞅丑臭初出橱厨躇锄雏滁除楚础储矗搐触处揣川穿椽传船喘串疮窗幢床闯创吹炊捶锤垂春椿醇唇淳纯蠢戳绰疵茨磁雌辞慈瓷词此刺赐次聪葱囱匆从丛凑粗醋簇促蹿篡窜摧崔催脆瘁粹淬翠村存寸磋撮搓措挫错搭达答瘩打大呆歹傣戴带殆代贷袋待逮怠耽担丹单郸掸胆旦氮但惮淡诞弹蛋当挡党荡档刀捣蹈倒岛祷导到稻悼道盗德得的蹬灯登等瞪凳邓堤低滴迪敌笛狄涤翟嫡抵底地蒂第帝弟递缔颠掂滇碘点典靛垫电佃甸店惦奠淀殿碉叼雕凋刁掉吊钓调跌爹碟蝶叠谍叠丁盯叮钉顶鼎锭定订丢东冬董懂动栋侗恫冻洞兜抖斗陡豆逗痘都督毒犊独读堵睹赌杜镀肚度渡妒端短锻段断缎堆兑队对墩吨蹲敦顿囤钝盾遁掇哆多夺垛躲朵跺舵剁惰堕蛾峨鹅俄额讹娥恶厄扼遏鄂饿恩而儿耳尔饵洱二贰发罚筏伐乏阀法珐藩帆番翻樊矾钒繁凡烦反返范贩犯饭泛坊芳方肪房防妨仿访纺放菲非啡飞肥匪诽吠肺废沸费芬酚吩氛分纷坟焚汾粉奋份忿愤粪丰封枫蜂峰锋风疯烽
于盂榆虞愚舆余俞逾鱼愉渝渔隅予娱雨与屿禹宇语羽玉域芋郁吁遇喻峪御愈欲狱育誉浴寓裕预豫驭鸳渊冤元垣袁原援辕园员圆猿源缘远苑愿怨院曰约越跃钥岳粤月悦阅耘云郧匀陨允运蕴酝晕韵孕匝砸杂栽哉灾宰载再在咱攒暂赞赃脏葬遭糟凿藻枣早澡蚤躁噪造皂灶燥责择则泽贼怎增憎曾赠扎喳渣札轧铡闸眨栅榨咋乍炸诈摘斋宅窄债寨瞻毡詹粘沾盏斩辗崭展蘸栈占战站湛绽樟章彰漳张掌涨杖丈帐账仗胀瘴障招昭找沼赵照罩兆肇召遮折哲蛰辙者锗蔗这浙珍斟真甄砧臻贞针侦枕疹诊震振镇阵蒸挣睁征狰争怔整拯正政帧症郑证芝枝支吱蜘知肢脂汁之织职直植殖执值侄址指止趾只旨纸志挚掷至致置帜峙制智秩稚质炙痔滞治窒中盅忠钟衷终种肿重仲众舟周州洲诌粥轴肘帚咒皱宙昼骤珠株蛛朱猪诸诛逐竹烛煮拄瞩嘱主著柱助蛀贮铸筑住注祝驻抓爪拽专砖转撰赚篆桩庄装妆撞壮状椎锥追赘坠缀谆准捉拙卓桌琢茁酌啄着灼浊兹咨资姿滋淄孜紫仔籽滓子自渍字鬃棕踪宗综总纵邹走奏揍租足卒族祖诅阻组钻纂嘴醉最罪尊遵昨左佐柞做作坐座';
}
...
function ResetPage(obj) {
    ...
    if (document.all("BillNumber") != null && obj != "BillNumber") {
        document.forms["form1"].elements["BillNumber"].value = "";
    }
    document.form1.submit();
}
...
//取消分页
function CencelPage() {
    openLoading();//增加loading，by ckt 2017-12-26
...
*/
function OpenOrLoad(d, f, Action, Type, arrays) {//Action下载6  modify zx 2011-04-21  function OpenOrLoad(d,f,Action,Type)
    var newnow = new Date().getTime();
    //alert (Action);
    //alert (arrays);
    win = window.open("../admin/openorload.php?d=" + d + "&f=" + f + "&Action=" + Action + "&Type=" + Type + "&arrays=" + arrays, newnow, "toolbar=no, menubar=no, scrollbars=yes,resizable=yes,location=no, status=no");
    //win.close();
}

function OpenPhotos(d, f, s, path) {
    var newnow = new Date().getTime();
    if (path.length > 0) {
        win = window.open("../" + path + "/openphotos.php?d=" + d + "&f=" + f + "&s=" + s, newnow, "toolbar=no, menubar=no, scrollbars=yes,resizable=yes,location=no, status=no");
    }
    else {
...
function CH_SC_SandH(e, f, Order_Rows, POrderId, RowId) {
    ...
    if (yy.indexOf("showtable") == -1) {
        ...
        //动态加入采购明细
        if (POrderId != "") {
            var url = "../admin/ch_shippinglist_ajax.php?POrderId=" + POrderId + "&RowId=" + RowId;
...
function SCQL(e, f, Order_Rows, StuffId, RowId, BuyerId) {
    e.style.display = (e.style.display == "none") ? "" : "none";
    var yy = f.src;
    if (yy.indexOf("showtable") == -1) {
        f.src = "../images/showtable.gif";
        ...
        Order_Rows.myProperty = false;
        //动态加入采购明细
        var url = "../public/sc_qlsheet_read_ajax.php?StuffId=" + StuffId + "&RowId=" + RowId + "&BuyerId=" + BuyerId;
        var show = eval("showStuffTB" + RowId);
        var ajax = InitAjax();
        ajax.open("GET", url, true);
        ajax.onreadystatechange = function () {
...
function Showotherfee(e, f, Order_Rows, ShipId, RowId) {
    e.style.display = (e.style.display == "none") ? "" : "none";
    var yy = f.src;
    if (yy.indexOf("showtable") == -1) {
        f.src = "../images/showtable.gif";
        ...
        Order_Rows.myProperty = false;
        if (ShipId != "") {
            var url = "../public/cw_mdtax_otherfee.php?ShipId=" + ShipId + "&RowId=" + RowId;
            var show = eval("showStuffTB" + RowId);
            var ajax = InitAjax();
            ajax.open("GET", url, true);
            ajax.onreadystatechange = function () {
...
//16	显示或隐藏配件采购单列表
function ShowOrHide(e, f, Order_Rows, POrderId, RowId, FromT) {
    e.style.display = (e.style.display == "none") ? "" : "none";
    var yy = f.src;
    if (yy.indexOf("showtable") == -1) {
        f.src = "../images/showtable.gif";
        ...
        //动态加入采购明细
        if (POrderId != "") {
            var url = "../admin/yw_order_ajax.php?POrderId=" + POrderId + "&RowId=" + RowId + "&FromT=" + FromT;
            var show = eval("showStuffTB" + RowId);
            var ajax = InitAjax();
            ajax.open("GET", url, true);
            ajax.onreadystatechange = function () {
...
function ShowOrHideSaleAjax(e, f, Order_Rows, POrderId, RowId, FromT) {
    e.style.display = (e.style.display == "none") ? "" : "none";
    var yy = f.src;
    if (yy.indexOf("showtable") == -1) {
        f.src = "../images/showtable.gif";
        ...
        //动态加入采购明细
        if (POrderId != "") {
            var url = "../Ashhk/sale_order_ajax.php?POrderId=" + POrderId + "&RowId=" + RowId + "&FromT=" + FromT;
            var show = eval("showStuffTB" + RowId);
            var ajax = InitAjax();
            ajax.open("GET", url, true);
            ajax.onreadystatechange = function () {
...
function ShowOrHideThisLayer(e, f, Order_Rows, StockId, RowId, FromT) {
    e.style.display = (e.style.display == "none") ? "" : "none";
    var yy = f.src;
    if (yy.indexOf("showtable") == -1) {
        f.src = "../images/showtable.gif";
        ...
        Order_Rows.myProperty = false;
        if (StockId != "") {
            var url = "../public/cg_order_ajax.php?StockId=" + StockId + "&RowId=" + RowId + "&FromT=" + FromT;
            var show = eval("showStuffTB" + RowId);
            var ajax = InitAjax();
            ajax.open("GET", url, true);
            ajax.onreadystatechange = function () {
...
function showOrHideSaleOrder(e, f, Order_Rows, ShipId, RowId, FromDir) {
    e.style.display = (e.style.display == "none") ? "" : "none";
    var yy = f.src;
    if (yy.indexOf("showtable") == -1) {
        f.src = "../images/showtable.gif";
        ...
        //动态加入采购明细
        if (ShipId != "") {
            var url = "../" + FromDir + "/sale_shiporder_ajax.php?ShipId=" + ShipId + "&RowId=" + RowId;
            var show = eval("showStuffTB" + RowId);
            var ajax = InitAjax();
            ajax.open("GET", url, true);
            ajax.onreadystatechange = function () {
...
function ShowOrHideSc(e, f, Order_Rows, sPOrderId, RowId, FromT, fromPage) {
    e.style.display = (e.style.display == "none") ? "" : "none";
    var yy = f.src;
    if (yy.indexOf("showtable") == -1) {
        f.src = "../images/showtable.gif";
        ...
        //动态加入采购明细
        if (sPOrderId != "") {
            var url = "../pt/pt_scorder_ajax.php?sPOrderId=" + sPOrderId + "&RowId=" + RowId + "&FromT=" + FromT + "&fromPage=" + fromPage;

            var show = eval("StuffListDiv" + RowId);
            var ajax = InitAjax();
            ajax.open("GET", url, true);
            ajax.onreadystatechange = function () {
                if (ajax.readyState == 4) {// && ajax.status ==200
...
//16	显示或隐藏配件采购单列表
function ShowOrHideSemi(e, f, Order_Rows, StockId, RowId, FromT) {
...
function ShowOrScHideSemi(e, f, Order_Rows, sPOrderId, RowId, FromT, fromPage) {
    e.style.display = (e.style.display == "none") ? "" : "none";
    var yy = f.src;
    if (yy.indexOf("showtable") == -1) {
        f.src = "../images/showtable.gif";
...
function All_elects(theDefaultColor, thePointerColor, theMarkColor, theMerge) {
    ...
    jQuery(".ListTableUd").each(function (index, element) {//加入ListTableUd的全选记录 add by ckt 2018-1-4
        jQuery(this).find('input[type="checkbox"]:eq(0)')[0].checked = true;
        jQuery(this).find('td:lt(' + theMerge + ')').attr('bgcolor', theMarkColor);
    })
    if (jQuery(".ListTableUd").length) {
        return false;
    }
    var bgcolor = null;
    for (var i = 1; i <= $Rows; i++) {
        if (document.getElementById("ListTable" + i) != null) {
            ...
            if (bgcolor == null || bgcolor == '' || bgcolor.toUpperCase() != theMarkColor) {	//modify by zx 20110321
                ...
                chooseRow(theTable, i, "click", theDefaultColor, thePointerColor, theMarkColor, "", theMerge);
            }
        }
    }
}
...
function Instead_elects(theDefaultColor, thePointerColor, theMarkColor, theMerge) {
    ...
    jQuery(".ListTableUd").each(function (index, element) {//加入ListTableUd的反选记录 add by ckt 2018-1-4
        var host = jQuery(this).find('input[type="checkbox"]:eq(0)')[0];
        var tds = jQuery(this).find('td:lt(' + theMerge + ')');
        if (host.checked) {
            host.checked = false;
            tds.removeAttr('bgcolor');
        } else {
            host.checked = true;
            tds.attr('bgcolor', theMarkColor);
        }
    })
    if (jQuery(".ListTableUd").length) {
        return false;
...
function ClickKeyCheck(theTable, theRowNum, theAction, theDefaultColor, thePointerColor, theMarkColor, theFrom, theMerge) {

    IE_FireFox();  //加入IE,fireFox兼容的函数
...
function showWindowStatus(InfoSum) {
    ...
    for (var i = 0; i < ListCheck.length; i++) {
        if (ListCheck[i].checked) {
            num++;
            var Ids = ListCheck[i].value.split('|');
            if (Ids[1]) {
                if (num == 1) {
                    tWeights = parseFloat(Ids[1]);
                    tvolumes = parseFloat(Ids[3]);
                } else {
                    tWeights += parseFloat(Ids[1]);
                    tvolumes += parseFloat(Ids[3]);
                }
    ...
    if (isSafari = navigator.userAgent.indexOf("MSIE") > 0 || totalStatusBar == 'undefined' || totalStatusBar == null) {
        window.status = InfoSum != "" ? "选定行求和：" + num : ""+"&nbsp;&nbsp;&nbsp;&nbsp;"+ " 方量：" + tWeights.toFixed(2)+"&nbsp;&nbsp;&nbsp;&nbsp;"+ " 重量：" + tvolumes.toFixed(2);
    // window.status = InfoSum != "" ? "选定行求和：" + InfoSum : "";
    }
    else {
        if (InfoSum != "") {
            totalStatusBar.innerHTML = "选定行求和：" + num  +"&nbsp;&nbsp;&nbsp;&nbsp;"+" 方量：" + tWeights.toFixed(2)+"&nbsp;&nbsp;&nbsp;&nbsp;"+ " 重量：" + tvolumes.toFixed(2);
        ...
        var x = 2 * (60 / totalStatusBar.innerHTML.length);

        for (i = 0; i < x; i++) {
            sta += totalStatusBar.innerHTML;
        }
        //setInterval("statusScroll('"+sta+"','"+position+"','"+totalStatusBar+"')",100);
    }
}

function statusScroll(sta, position, totalStatusBar) {
    totalStatusBar.innerHTML = sta.substring(position, position + 60);
    if (position++ == 60) position = 0;
}

//显示开发添加配件
function Showaddstuff(e, f, Order_Rows, Id, RowId) {
    e.style.display = (e.style.display == "none") ? "" : "none";
    var yy = f.src;
    if (yy.indexOf("showtable") == -1) {
        f.src = "../images/showtable.gif";
        ...
        Order_Rows.myProperty = false;
        if (Id != "") {
            var url = "../public/development_ajax.php?Id=" + Id + "&RowId=" + RowId;
            var show = eval("showStuffTB" + RowId);
            var ajax = InitAjax();
            ajax.open("GET", url, true);
            ajax.onreadystatechange = function () {
...
//******************在刀模图片上打水印
function addpicture(POrderId, StuffId, cutId, OrderQty) {
    var url = "cutbom_addpicture.php?cutId=" + cutId + "&POrderId=" + POrderId + "&StuffId=" + StuffId + "&OrderQty=" + OrderQty;
    //alert(url);
    var ajax = InitAjax();
    ajax.open("GET", url, true);
    ajax.onreadystatechange = function () {
...
//11月已确认的函数结束
//********************
function ShowDiv(e, TempId, RowId, ToPage, Dir) {//e:隐藏的表格名称  TempId:传递的参数  RowId:第几行表格 ToPage:前往的页面
    Dir = Dir == "" ? "admin" : Dir;
    if (e.style.display == "none") {
        e.style.display = "";
        //动态加入采购明细
        if (TempId != "") {
            var url = "../" + Dir + "/" + ToPage + ".php?ProductId=" + TempId;
...
function viewImage(PId, RT, ST) {
    win = window.open("../admin/Productimage/openimage.php?ID=" + PId + "&RT=" + RT + "&ST=" + ST, "new", "toolbar=no, menubar=no, scrollbars=yes,resizable=yes,location=no, status=no,width=" + (screen.availWidth - 20) + ",height=" + (screen.availHeight - 30));
    win.focus();
}

function viewMistakeImage(PId, RT, ST) {
    win = window.open("../admin/Productimage/openmistakeimage.php?ID=" + PId + "&RT=" + RT + "&ST=" + ST, "new", "toolbar=no, menubar=no, scrollbars=yes,resizable=yes,location=no, status=no,width=" + (screen.availWidth - 20) + ",height=" + (screen.availHeight - 30));
    win.focus();
}

function ShowDropTable(e, ff, d, toWebPage, args, fromPage) {
    fromPage = fromPage == "" ? "admin" : fromPage;
    if (e.style.display == "none") {
        ff.src = "../images/hidetable.gif";
        var url = "../" + fromPage + "/" + toWebPage + ".php?args=" + args;
        var ajax = InitAjax();
        ajax.open("GET", url, true);
        ajax.onreadystatechange = function () {
        ...
        e.style.display = "none";
        ff.src = "../images/showtable.gif";
    }
}

function addStuffComBox(e, StockId) {
    if (StockId != "") {
        e.disabled = 'disabled';
        var url = "../admin/yw_order_updated.php?ActionId=AddStuffCombox&StockId=" + StockId;
        var ajax = InitAjax();
        ajax.open("GET", url, true);
        ajax.onreadystatechange = function () {
            if (ajax.readyState == 4) {// && ajax.status ==200
...
function ShowOrHideDel(e, f, Order_Rows, POrderId, RowId, FromT) {
    ...
    if (yy.indexOf("showtable") == -1) {
        ...
        //动态加入采购明细
        if (POrderId != "") {
            var url = "../admin/yw_order_del_ajax.php?POrderId=" + POrderId + "&RowId=" + RowId + "&FromT=" + FromT;
            var show = eval("showStuffTB" + RowId);
            var ajax = InitAjax();
            ajax.open("GET", url, true);
            ajax.onreadystatechange = function () {
...
//打开一个模态框 by ckt 2017-12-19
function openFrame(url, width, height) {
    var onloadFlag = '';
    if (arguments[3])//子页面无closeLoading的情况
        onloadFlag = 'onload="document.getElementById(\'innerDivUd\').style.display=\'none\'"';
    var iTop = (window.screen.availHeight - 30 - height) / 2;
    var iLeft = (window.screen.availWidth - 10 - width) / 2;
    var div = $('<div id="coverUd" onclick="window.frames[\'mainFrame\'].clearFrame()" style="position: fixed;top: 0;right: 0;bottom: 0;left: 0;z-index:1005;background:#000;filter:alpha(Opacity=60);-moz-opacity:0.6;opacity: 0.6;"></div>');
    var frame = $('<iframe name="iframeUd" ' + onloadFlag + ' frameborder="0" width=' + width + ' height=' + height + ' src="' + url + '" id="iframUd" style="border: 1px solid rgba(0,0,0,.2);border-radius: 6px;outline: 0;-webkit-box-shadow: 0 3px 9px rgba(0,0,0,.5);box-shadow: 0 3px 9px rgba(0,0,0,.5);overflow:auto;position: fixed;background:white;opacity: 1;z-index:1006;top:' + iTop + ';left:' + iLeft + '"></iframe>');
    var innerDiv = $('<div id="innerDivUd" style="border: 1px solid rgba(0,0,0,.2);border-radius: 6px;outline: 0;-webkit-box-shadow: 0 3px 9px rgba(0,0,0,.5);overflow:auto;box-shadow: 0 3px 9px rgba(0,0,0,.5);width:' + width + 'px;height:' + height + 'px;position: fixed;background: #f5f5f5 url(/images/loadingUd.gif) no-repeat fixed center;display:none;z-index:1007;top:' + iTop + ';left:' + iLeft + '"></div>');
    var frameset = $(window.parent.parent.document.getElementsByTagName("frameset")[0]);
    div.insertAfter(frameset);
    innerDiv.insertAfter(frameset).fadeIn('nomal', function () {
        frame.insertAfter(frameset)
    });
}
//清除模态框 by ckt 2017-12-19
function clearFrame() {
    var topParent = $(window.parent.parent.document);
    var innerDiv = topParent.find('#innerDivUd');
    var frame = topParent.find('#iframUd');
    var div = topParent.find('#coverUd');
    if (innerDiv.is(':hidden')) {
        ...
        frame.remove();
        innerDiv.fadeOut('normal', function () {
            $(this).remove();
            div.remove()
        });
    }
}
//开启loading页面 by ckt 2017-12-22
function openLoading() {
    var innerDiv = window.parent.document.getElementById("innerDivUd");
    if (innerDiv)
        innerDiv.style.display = "block";
}
//关闭loading页面 by ckt 2017-12-22
...
function selectAllb1Stuff(e) {
    var j = jQuery(e).parent().parent().parent().find(':checkbox');
    if (jQuery(e).attr('class') === 'A0101') jQuery(e).addClass('odd');
    if (jQuery(e).attr('class') === 'A0101 odd') {
        jQuery(e).removeClass('odd');
        jQuery(e).addClass('even');
        j.each(
            function () {
                var k = jQuery(this).attr('disabled');
                if (k !== 'disabled') {
                    jQuery(this).click();
                }
            }
    ...
    else if (jQuery(e).attr('class') === 'A0101 even') {
        j.removeAttr('checked');
        jQuery(e).removeClass('even');
        jQuery(e).addClass('odd');
    }

}

function selectAllSuivre(e) {
    var j = jQuery(e).find(':checkbox');
    if (jQuery(e).attr('class') === 'A0101') jQuery(e).addClass('odd');
    if (jQuery(e).attr('class') === 'A0101 odd') {
        jQuery(e).removeClass('odd');
        jQuery(e).addClass('even');
    ...
    var k = jQuery(e).next().next().next().next().find(':checkbox');
    k.each(
        function () {
            var q = jQuery(this).attr('disabled');
            if (q !== 'disabled') {
                jQuery(this).click();
            }
        });
