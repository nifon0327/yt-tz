...
$From=$From==""?"read":$From;
$Th_Col="选项|30|序号|30|配件ID|50|片材配件名称|320|单价|60|成本价|60|单位|30|NO.|30|配件ID|50|原材料名称|320|单位|30|对应关系|60|采购|60|刀模编号|120|图档|30";
//必选，分页默认值
...
INNER JOIN  $DataIn.stuffproperty  P ON P.StuffId=A.mStuffId AND Property = " . $APP_CONFIG['PT_CUT_PROPERTY'] . "
WHERE 1 $SearchRows GROUP BY A.mStuffId order by A.mStuffId DESC";
//echo $mySql;
       ...
       $m=$m+2;
		...
		if($numrows>0){
			...
			$StuffResult = mysql_query("SELECT A.Id,A.Relation,A.StuffId,D.StuffCname,D.Price,D.CostPrice,D.Picture,D.Gfile,D.Gstate,  
			    D.Gremark,D.TypeId,D.SendFloor,U.Name AS Unit,ST.mainType 
				FROM  $DataIn.semifinished_bom A  
			...
			if($StuffMyrow=mysql_fetch_array($StuffResult)) {//如果设定了产品配件关系
				do{	
					...
					if ($StuffMyrow["mainType"]==$APP_CONFIG['WORKORDER_ACTION_MAINTYPE']){
						continue;
					}
