...
echo"<table  width='685' cellspacing='1' border='1' align='left' style='table-layout:fixed;word-break:break-all; word-wrap:break-word;margin:2px 0px 2px 15px;' ><tr bgcolor='#CCCCCC'>
			...
			if($StuffMyrow=mysql_fetch_array($StuffResult)) {//如果设定了产品配件关系
				...
				do{	
			     ...
			     if($drawingRow=mysql_fetch_array($drawingSql)){
			       ...
			       //检查是否为属性2、3配件
			        //二级BOM表
				    $showProcess="&nbsp;";$ProcessTable="";$colspan=13;
					...
					if ($StockId>0){
                        ...
                        $llDate="&nbsp;";
                       //检查是否已备料
		               $llSql=mysql_query("SELECT SUM(S.Qty) AS llQty,Max(K.Date) AS Date FROM $DataIn.ck5_klsheet  K LEFT JOIN  $DataIn.ck5_llsheet S ON S.Id=K.Sid  WHERE  K.StockId='$StockId' AND S.StuffId='$mStuffId'",$link_id);
		               if($llRows=mysql_fetch_array($llSql)){
		                        if ($llRows["Date"]!=""){
		                                 $llDate=$llRows["llQty"]==$mQty?$llRows["Date"]:"<div style='color:#FFAA00' title='已备数量:" .$llRows["llQty"] . "'>".$llRows["Date"]. "</div>"; 
		                        }
		                        else{
					                    $POrderId=substr($StockId, 0,12);
						                $llSql=mysql_query("SELECT G.OrderQty,SUM(S.Qty) AS llQty FROM $DataIn.cg1_stocksheet  G LEFT JOIN  $DataIn.ck5_llsheet S ON G.StockId=S.StockId  WHERE  G.POrderId='$POrderId' AND G.StuffId='$mStuffId'",$link_id); 
						                if($llRows=mysql_fetch_array($llSql)){
						                     if ($llRows["OrderQty"]==$llRows["llQty"]){
							                          $checkIdResult=mysql_query("SELECT S.Id FROM $DataIn.cg1_stocksheet  G LEFT JOIN  $DataIn.ck5_llsheet S ON G.StockId=S.StockId  WHERE  G.POrderId='$POrderId' AND G.StuffId='$mStuffId' AND S.Qty>'$mQty' LIMIT 1",$link_id); 
							                          if($checkIdRows=mysql_fetch_array($checkIdResult)){
							                                 $LLid=$checkIdRows["Id"];
							                                 $inResult=mysql_query("INSERT INTO $DataIn.ck5_klsheet (Id,Sid,POrderId,StockId,Estate,Date) VALUES(NULL,'$LLid', '$POrderId','$StockId','1','$today') ",$link_id);
							                                if (mysql_affected_rows()>0){
								                                    echo "新增 $StockId 开料单成功！<br>";
								                            }
							                          }
						                       }
						                  }
		               ...
		               $klDate="&nbsp;";$klQty="&nbsp;";
		               $klSql=mysql_query("SELECT Max(K.Date) AS Date,SUM(Qty) AS klQty FROM $DataIn.sc5_kltj  K  WHERE  K.StockId='$StockId' ",$link_id);
		                if($klRows=mysql_fetch_array($klSql)){
		                        $klQty=$klRows["klQty"];
		                        $klDate=$klQty>=$pcsQty?$klRows["Date"]:"<div style='color:#FFAA00'>".$klRows["Date"]. "</div>";
		               }
		               
                        //$checkScQty= mysql_query("SELECT SUM(Qty) FROM $DataIn.sc_cuttj WHERE StockId='$StockId' AND Diecut='$CutId' ",$link_id);
