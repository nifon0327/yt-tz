...
$MonthSTR1=$Month==""?"":" AND A.Month='$Month'";
switch($DataT){
...
";
//读取记录
$checkSql=mysql_query("SELECT * FROM (
                                         SELECT '1' AS Type,A.Id,A.Content,A.Operator,A.Bill,A.Date,C.Symbol,A.Amount,(A.Amount*C.Rate) AS AmountRMB,B.Name AS ItemName 
										  ...
										  $MonthSTR $Parameters $EstateSTR   
                                         UNION ALL 
                                           SELECT '2' AS Type,A.Id,CONCAT(A.Remark,'-',B.Name) AS Content,A.Operator,A.Attached AS Bill,A.Date,'1' AS Symbol,A.Amount,A.Amount AS  AmountRMB,'体检费用' AS  ItemName
                                            FROM $DataIn.cw17_tjsheet  A
											LEFT JOIN $DataPublic.staffmain B ON B.Number=A.Number
                                            WHERE 1 $MonthSTR1 $EstateSTR
                                          ) D ORDER BY D.Date DESC ",$link_id);
		...
		switch($Type){
		 ...
		 default:
			if($Bill!="" && $Bill!=0){
				...
				//add by cabbage 20141210 app用，附件路徑
				$appFileLink = "/download/tjfile/".$Bill;
