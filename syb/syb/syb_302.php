...
<td class='A1001'>2倍时薪</td>
<td class='A1001'>2倍工时</td>
<td class='A1001'>3倍时薪</td>
<td class='A1001'>3倍工时</td>
<td class='A1001'>加班费</td>
</tr>
<tr>
...
$checkSql=mysql_query("
					  SELECT A.Month,A.xHours,A.xWage,A.fHours,A.fWage,A.Amount,A.Estate,B.Name,C.Name AS Branch,D.Name AS Job
FROM $DataIn.hdjbsheet A
...
WHERE  A.Month>='2008-07' AND A.Month<'2013-05' $MonthSTR $EstateSTR $Parameters ORDER BY C.SortId,A.JobId,B.ComeIn",$link_id);
$i=1;
$SumAmount=0;
if($checkRow=mysql_fetch_array($checkSql)){
	do{
		...
		$Job=$checkRow["Job"];
		$xHours=$checkRow["xHours"];
		$xWage=$checkRow["xWage"];
		$fHours=$checkRow["fHours"];
		$fWage=$checkRow["fWage"];
		$Estate=$checkRow["Estate"]==3?"<span class=\"redB\">未付</span>":"<span class=\"greenB\">已付</span>";
		$Amount=$checkRow["Amount"];
		$SumAmount+=$Amount;
		$xHours=SpaceValue0($xHours);
		$fHours=SpaceValue0($fHours);
		echo"
			...
			<td class='A0101'>$xWage</td>
			<td class='A0101' >$xHours</td>
			<td class='A0101'>$fWage</td>
			<td class='A0101'>$fHours</td>
