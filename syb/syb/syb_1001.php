<?
//抵付的订金 ewen 2013-03-29 OK
$MonthSTR=$Month==""?"":" AND DATE_FORMAT(A.Date,'%Y-%m')='$Month'";
switch($DataT){
	case "Y":$DataTSTR="已抵付记录"; $EstateSTR=" AND (A.Estate=0 OR A.Estate=3) AND A.Did>0"; break;
	case "W":$DataTSTR="未抵付记录"; $EstateSTR=" AND (A.Estate=0 OR A.Estate=3) AND A.Did=0";break;
...
<td width='150' class='A1001'>结付银行</td>
<td width='80' class='A1001'>抵付日期</td>
<td width='100' class='A1001'>供应商</td>
<td class='A1001'>预付说明</td>
...
SELECT A.Date,A.Remark,A.PurchaseID,A.Amount,(A.Amount*C.Rate) AS AmountRMB,A.Did,B.Forshort,C.Symbol,D.PayDate,E.Title,F.PayDate AS PayDateOut
...
LEFT JOIN $DataIn.nonbom11_djmain D ON D.Id=A.Mid
LEFT JOIN $DataPublic.my2_bankinfo E ON E.Id=D.BankId
LEFt JOIN $DataIn.nonbom12_cwmain F ON F.Id=A.Did
WHERE 1 $MonthSTR $EstateSTR
ORDER BY A.Date DESC,A.Id",$link_id);
...
if($checkRow=mysql_fetch_array($checkSql)){
	...
	do{
		...
		$PayDateOut=$checkRow["PayDateOut"];
		$Forshort=$checkRow["Forshort"];
		$Remark=$checkRow["Remark"];
		$Estate=$checkRow["Did"]==0?"<span class=\"redB\">未抵</span>":"<span class=\"greenB\">已抵</span>";
		...
		echo"
			...
			<td width='150' class='A0101'>$BankName</td>
			<td width='80' class='A0101'>$PayDateOut</td>
