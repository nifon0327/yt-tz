...
SELECT A.Id,A.Content,A.Operator,A.Bill,A.Date,C.Symbol,A.Amount,(A.Amount*C.Rate) AS AmountRMB,B.Name AS ItemName ,'1' AS Type
										  ...
										  $EstateSTR   
				UNION ALL
  SELECT  A.Id,A.Content,A.Operator,A.Bill,A.Date,C.Symbol,A.Amount,(A.Amount*C.Rate) AS AmountRMB,B.Name AS ItemName,'2' AS Type 
 	FROM $DataIn.carfee A 
	LEFT JOIN $DataPublic.carfee_type B ON A.TypeId=B.Id
   LEFT JOIN $DataPublic.currencydata C ON C.Id=A.Currency 
   	WHERE 1 AND A.Date>='2016-01-01' $MonthSTR  $EstateSTR
) A   ORDER BY A.Date DESC,A.Id DESC",$link_id);
$i=1;
$SumAmount=0;
