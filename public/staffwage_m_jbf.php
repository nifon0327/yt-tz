...
$ColsNumber=27;
$tableMenuS=450;
$From=$From==""?"m_jbf":$From;
ChangeWtitle("$SubCompany 薪资列表（合并加班费）");
$funFrom="staffwage";
if($From!="slist"){
	$sumCols="21,22,23,24,25";		//求和列
	$Th_Col="选项|40|序号|30|分类|30|工作</br>地点|40|部门|40|职位|40|小组|50|员工<br>姓名|50|工龄<br>Y(M)|40|支付<br>货币|40|底薪|40|加班费|40|工龄<br>津贴|40|岗位<br>津贴|40|生活/住宿<br>补助|60|交通<br>补助|40|就学<br>补助|40|购房<br>补助|40|绩效<br>奖金|40|其他<br>奖金|40|考勤<br>扣款|40|取消<br>津贴|40|小计|55|社保|50|公积金|50|个税|40|实付|55|状态|40|备注|40";
	$swidth=270;
...
else
{
	...
	$Th_Col="选项|40|序号|30|分类|30|月份|60|工作</br>地点|40|部门|40|职位|40|小组|50|员工<br>姓名|50|工龄<br>Y(M)|40|支付<br>货币|40|底薪|40|加班费|40|工龄<br>津贴|40|岗位<br>津贴|40|生活/住宿<br>补助|60|交通<br>补助|40|就学<br>补助|40|购房<br>补助|40|绩效<br>奖金|40|其他<br>奖金|40|考勤<br>扣款|40|取消<br>津贴|40|小计|55|社保|50|个税|40|公积金|50|实付|55|状态|40|备注|40";
	$swidth=330;
	$ActioToS="1,17";
...
//步骤6：需处理数据记录处理
//初始化
$sumDx=$sumGljt=$sumGwjt=$sumJj=$sumShbz=$sumZsbz=$sumJtbz=$sumStudybz=$sumJbf=$sumYxbz=$sumtaxbz=$sumJz=$sumSb=$sumGjj=$sumCt=$sumKqkk=$sumdkfl=$sumRandP=$sumOtherkk=$sumAmount=$sumTotal=$sumJbjj=$sumYwjj=$sumHousebz=0;
$i=1;
...
S.Kqkk,S.dkfl,S.RandP,S.Otherkk,S.Amount,S.Estate,S.Remark,S.Locks,M.Name,M.ComeIn,M.Estate AS mEsate,M.WorkAdd,M.Id As PID,B.Name AS Branch,J.Name AS Job,E.GroupName,S.Currency,C.Symbol,C.Rate 
...
if($myRow = mysql_fetch_array($myResult)){
	do{
		...
		$hdjbf=0;$oRandP=0;
		$checkResult = mysql_query("SELECT Amount,oRandP FROM $DataIn.hdjbsheet WHERE Number='$Number' and Month='$chooseMonth'",$link_id);
		if($checkRow = mysql_fetch_array($checkResult)){
			$hdjbf=sprintf("%.0f",$checkRow["Amount"]);
			$oRandP=sprintf("%.0f",$checkRow["oRandP"]);
	    }
        $Jbf+=$hdjbf+$oRandP;
		...
		$Total=$Dx+$Jbf+$Gljt+$Gwjt+$Jj+$Ywjj+$Shbz+$Jtbz+$Studybz+$Yxbz+$taxbz+$Housebz-$Kqkk-$dkfl;
		...
		$sumRandP+=$RandP*$Rate;		$sumYwjj+=$Ywjj*$Rate;	$sumStudybz+=$Studybz*$Rate; 					
		$sumOtherkk+=$Otherkk*$Rate;  $sumHousebz+=$Housebz;
...
$sumShbz=SpaceValue0($sumShbz);$sumZsbz=SpaceValue0($sumZsbz);$sumJtbz=SpaceValue0($sumJtbz);$sumYxbz=SpaceValue0($sumYxbz);$sumtaxbz=SpaceValue0($sumtaxbz);$sumKqkk=SpaceValue0($sumKqkk);$sumdkfl=SpaceValue0($sumdkfl);  $sumHousebz=SpaceValue0($sumHousebz);
$sumRandP=SpaceValue0($sumRandP);$sumStudybz=SpaceValue0($sumStudybz);
