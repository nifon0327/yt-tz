...
$funFrom="staffwage";
if($From!="slist"){
	$sumCols="22,23,24,25,26,27,28,29";		//求和列
	$Th_Col="选项|40|序号|30|分类|30|工作</br>地点|40|部门|40|职位|40|小组|50|员工<br>姓名|50|工龄<br>Y(M)|40|支付<br>货币|40|底薪|40|加班费(1)|40|工龄<br>津贴|40|岗位<br>津贴|40|加班费(奖金)|40|其它<br>奖金|40|额外<br>(奖金)|40|生活<br>补助|40|住宿<br>补助|40|交通<br>补助|40|夜宵<br>补助|40|个税<br>补助|40|考勤<br>扣款|40|津贴<br>扣款|40|小计|55|借支|40|社保|50|个税|40|公积金|50|餐费<br>扣款|40|其它<br>扣款|40|实付|55|状态|40|备注|40";//加班费(2)
	$swidth=270;
...
else
{
	...
	$sumCols="23,24,25,26,27,30,31,32";		//求和列
	$Th_Col="选项|40|序号|30|分类|30|月份|60|工作</br>地点|40|部门|40|职位|40|小组|50|员工<br>姓名|50|工龄<br>Y(M)|40|支付<br>货币|40|底薪|40|加班费(1)|40|工龄<br>津贴|40|岗位<br>津贴|40|加班费(奖金)|40|其它<br>奖金|40|额外<br>(奖金)|40|生活<br>补助|40|住宿<br>补助|40|交通<br>补助|40|夜宵<br>补助|40|个税<br>补助|40|考勤<br>扣款|40|津贴<br>扣款|40|小计|55|借支|40|社保|50|个税|40|公积金|50|餐费<br>扣款|40|其它<br>扣款|40|实付|55|状态|40|备注|40";
	$swidth=330;
...
S.Kqkk,S.dkfl,S.RandP,S.Otherkk,S.Amount,S.Estate,S.Remark,S.Locks,M.Name,M.ComeIn,M.Estate AS mEsate,M.WorkAdd,M.Id As PID,B.Name AS Branch,J.Name AS Job,M.GroupId,E.GroupName,S.Currency,C.Symbol,C.Rate  
...
if($myRow = mysql_fetch_array($myResult)){
	do{
		$m=1;
		//初始化数据
		$Dx=$Gljt=$Gwjt=$Jj=$Ywjj=$Shbz=$Zsbz=$Jtbz=$Jbf=$Yxbz=$taxbz=$Jz=$Sb=$Gjj=$Ct=$Kqkk=$dkfl=$RandP=$Otherkk=$Amount=$Total=0;
		$Id=$myRow["Id"];
		$SMonth=$myRow["Month"];
		$KqSign=$myRow["KqSign"];
		$Branch=$myRow["Branch"];
		...
		$sumShbz+=$Shbz*$Rate;		$sumZsbz+=$Zsbz*$Rate;		$sumJtbz+=$Jtbz*$Rate;     	$sumJbf+=$Jbf*$Rate;
		...
		$sumRandP+=$RandP*$Rate;		$sumYwjj+=$Ywjj*$Rate;						
