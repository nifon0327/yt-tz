...
for($i=0;$i<$Lens;$i++){
	...
	if($Id!=""){
		//检查是否未结付可删除
		$my_Result = mysql_query("SELECT Bill,Number FROM $DataIn.staff_outsubsidysheet  WHERE Id='$Id' and Estate='1' and Mid='0' ORDER BY Id DESC LIMIT 1",$link_id);
		if($my_Rows=mysql_fetch_array($my_Result)){//可删除
			$Bill=$my_Rows["Bill"];
			$Number=$my_Rows["Number"];
			$Del = "DELETE FROM $DataIn.staff_outsubsidysheet WHERE Id='$Id' and Estate=1 and Mid=0"; 
