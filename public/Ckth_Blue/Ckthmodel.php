...
<td width=18 align=center valign=middle height=8></td>
<td width=98 align=left  valign=middle >配件名称</td>
<td width=50 align=left valign=middle >退料原因</td>
<td width=18 align=center valign=middle >数量</td>
</tr></table>";
...
for ($pi=1;$pi<=$Counts;$pi=$pi+1){  //1.循环输出
     ...
     if($addSign==0){  //2.新加一页
		...
		include "Ckth_Blue/HeaderS.php";  //页头
	...
	if($addSign==0)	{
	    if($pi==$Counts){ //最后一页
			include "Ckth_Blue/FooterS.php";  //最后固定页脚
		}
		else {
