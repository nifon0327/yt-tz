...
//步骤1：
$mainData="$DataIn.hzqkmain";
$sheetData="$DataIn.hzqksheet";
...
$mySql="SELECT 
	M.PayDate,M.PayAmount,M.Payee,M.Receipt,M.CheckSheet,M.Remark AS PayRemark,M.Locks AS MLocks,S.BillNumber,
	S.Id,S.Mid,S.Content,S.Amount,S.Bill,S.Date,S.Operator,T.Name AS Type,C.Symbol AS Currency,B.Title,M.cSign
 	FROM $DataIn.hzqkmain M
	LEFT JOIN $DataIn.hzqksheet S ON S.Mid=M.Id
...
if($mainRows = mysql_fetch_array($mainResult)){
	...
	do{
		...
		$Content=$mainRows["Content"]==""?"&nbsp":$mainRows["Content"];
		...
		if($Keys & mUPDATE || $Keys & mLOCK){
			$upMian="<img location.href=\"#\"' style='CURSOR: pointer' onclick='upMainData(\"adminicost_upmain\",$Mid)' src='../images/edit.gif' title='行号:$mRow 编号:$Mid 更新结付单资料!' width='13' height='13'>";
			}
		else{
			$upMian="&nbsp;";
			}
		...
		if($midDefault!="" && $midDefault==$Mid){//同属于一个主ID，则依然输出明细表格
			...
			$m=$m+2;
			echo"<td  class='A0001' width='$Field[$m]' align='left'>$Content</td>";
			$m=$m+2;
			...
			$m=$m+2;
			echo"<td width='$Field[$m]' class='A0000' align='center'>$BillNumber</td>";		//内部单号
			echo"</tr></table>";
			$i++;
			}
		else{
