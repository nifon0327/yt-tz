...
$From = $From == "" ? "read" : $From;
if ($StuffType != 9040) {
    $Th_Col = "选项|55|序号|40|配件Id|50|配件名称|280|规格|120|配件编码|100|库位编号|80|App图|35|图档|30|图档日期|70|历史<br>订单|40|QC图|40|认证|40|开发|80|品检</br>方式|40|状态|30|含税价|60|单位|40|装箱数量|60|AQL|40|配件类型|80|默认供应商|100|采购|50|送货</br>楼层|40|装框数量<br>(仓库)|60|主产品<br>重(g)|50|在库|55|可用库存|55|最低库存|55|购买选择|50|备注|30|更新日期|80|下单需求|80|开发责任人|80|图档审核|80|操作|50";
} else {
    $Th_Col = "选项|55|序号|40|配件Id|50|配件名称|280|App图|35|图档|30|图档日期|70|历史<br>订单|40|QC图|40|认证|40|开发|80|品检</br>方式|40|状态|30|含税价|60|单位|40|装箱数量|60|AQL|40|配件类型|80|默认供应商|100|采购|50|送货</br>楼层|40|规格|120|装框数量<br>(仓库)|60|主产品<br>重(g)|50|承重<br>(kg)|40|在库|55|可用库存|55|最低库存|55|购买选择|50|备注|30|更新日期|80|下单需求|80|开发责任人|80|图档审核|80|操作|50";
}

$Pagination = $Pagination == "" ? 1 : $Pagination;
$Page_Size = 200;
$ActioToS = "1,2,3,4,5,7,8,107,171,13,40,98,38";
...
if ($From != "slist") {
    ...
    echo "<option value='' >--开发负责人--</option>";
...
$BuyChooseSign = 0;
$NumberArray = explode(',', $APP_CONFIG['BUY_STUFFDATA_NUMBER']);
...
$mySql = "
	SELECT 
	A.Id,A.StuffId,A.StuffCname,A.StuffEname,A.TypeId,A.Gfile,A.Gstate,A.Picture,A.FrameCapacity,A.SeatId,
	...
	A.Gremark,A.Estate,A.NoTaxPrice,A.Price,A.CostPrice,A.PriceDetermined,A.SendFloor,A.jhDays,E.Forshort,B.BuyerId,C.Name,A.Spec,
	A.Remark,A.Weight,A.Date,IF(A.GcheckNumber>0,L.Name,IF(A.GcheckNumber=-1,'系统默认','不需图档审核')) as GcheckNumber,
	A.DevelopState,A.BoxPcs,A.GfileDate,A.ForcePicSpe,A.Operator,A.Locks,A.CheckSign,H.mStockQty,
	H.tStockQty,H.oStockQty,G.TypeName,G.ForcePicSign,G.jhDays AS TypeJhDays,D.Name AS UnitName,B.CompanyId,
    DP.dFile as developFile,G.mainType,G.AQL
	...
	WHERE 1 $SearchRows  $ShipMonthStr  Group BY A.StuffId
...
if ($myRow = mysql_fetch_array($myResult)) {
    ...
    do {
        ...
        include "../model/subprogram/stuff_CheckSign.php";    //品检方式
        ...
        switch ($Estate) {
            ...
            case 3: {
                ...
                $Estate = "<div class='redB' title='退回-$returnReason'>×.</div>";
            }
                break;
        }
        ...
        $theParam = "$StuffId";
        $ListId = getRandIndex();
        $showPurchaseorder = "<img onClick='ShowDropTable(ShowTable$ListId,ShowGif$ListId,ShowDiv$ListId,\"Stuffdata_Gfile_ajax\",\"$theParam\",\"public\");' name='ShowGif$ListId' src='../images/showtable.gif' 
        ...
        if ($mainType != 7) {
            /*$UpdateSql = "UPDATE  $DataIn.stuffdata SET CostPrice = $CostPrice  WHERE StuffId = $StuffId";
        ...
        if ($StuffType != 9040) {
            $ValueArray = array(
                ...
                array(0 => $StuffCname),
                array(0 => $Spec, 1 => "align='left'", 2 => "onmousedown='window.event.cancelBubble=true;' onclick='updateJq($i,21,$StuffId,3)' style='CURSOR: pointer'"),
                array(0 => $StuffEname),
                array(0 => $SeatId, 1 => "align='center'"),//库位编号
                ...
                array(0 => $DevelopState . ' ' . $developFile, 1 => "align='center'"),
                ...
                array(0 => $FrameCapacity, 1 => "align='center'"),
            ...
            $ValueArray = array(
                ...
                array(0 => $Buyer, 1 => "align='center'"),
                array(0 => $SendFloor, 1 => "align='center'"),
                array(0 => $Spec, 1 => "align='left'", 2 => "onmousedown='window.event.cancelBubble=true;' onclick='updateJq($i,19,$StuffId,3)' style='CURSOR: pointer'"),
                array(0 => $FrameCapacity, 1 => "align='center'"),
                array(0 => $Weight, 1 => "align='center'"),
    ...
    function ChooseBuy(index, StuffId) {
        ...
        ajax.onreadystatechange = function () {
            if (ajax.readyState == 4 && ajax.status == 200) {
                ...
                if (BackData == "Y") document.getElementById(tabIndex).rows[0].cells[26].innerHTML = "<img src='../images/ok.gif' width='30' height='30'>";
                else document.getElementById(tabIndex).rows[0].cells[26].innerHTML = "";
            }
        }
        ajax.send(null);
    }
            ...
            ajax.onreadystatechange = function () {
                if (ajax.readyState == 4) {// && ajax.status ==200
                    ...
                    if (ajax.responseText == "Y") {//更新成功
                        var tabIndex = "ListTable" + index;
                        var TDid = document.getElementById(tabIndex).rows[0].cells[21];
                        if (inputStr == 0) {
