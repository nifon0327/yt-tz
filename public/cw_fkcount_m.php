...
<table width="<?php  echo $tableWidth?>" border="0" cellspacing="0"  bgcolor="#FFFFFF" style='TABLE-LAYOUT: fixed; WORD-WRAP: break-word'>
	...
	<tr>
      ...
      <input name="Type" type="radio" id="Type1" value="m" checked><LABEL for="Type1">请款月份</LABEL>
...
<?php 
//读取全部货款
$ShipResult = mysql_query("SELECT SUM(S.Amount*C.Rate) AS Amount,S.Month
...
if ($ShipRow = mysql_fetch_array($ShipResult)) {
	...
	do{
		...
		LEFT JOIN $DataIn.trade_object P ON P.CompanyId=S.CompanyId
		LEFT JOIN $DataPublic.currencydata C ON C.Id=P.Currency
		WHERE S.Month='$Month' AND S.Mid>0 AND S.Estate=0",$link_id));
		...
		$Amount3=$Amount3==0?"&nbsp;":number_format($Amount3);
			...
			$DivNum="a";
			$TempId="$Month|$DivNum";			
			$showPurchaseorder="<img onClick='SandH(\"$DivNum\",$i,this,\"$TempId\",\"cw_fkcount_m_a\",\"public\");' id='ThisImg_$DivNum$i' name='ThisImg_$DivNum$i' src='../images/showtable.gif' width='13' height='13' style='CURSOR: pointer'>";
			$HideTableHTML="
...
LEFT JOIN $DataIn.trade_object P ON P.CompanyId=S.CompanyId
LEFT JOIN $DataPublic.currencydata C ON C.Id=P.Currency
where  S.Estate IN(0,3) AND S.CompanyId>0 
GROUP BY S.Month ORDER BY S.Month
",$link_id);
//$Total1=0;$Total2=0;$Total3=0;
if ($ShipRow = mysql_fetch_array($ShipResult)) {
	...
	do{
		...
		$Amount3=$Amount3==0?"&nbsp;":number_format($Amount3);
			...
			$DivNum="b";
			$TempId="$Month|$DivNum";			
			$showPurchaseorder="<img onClick='SandH(\"$DivNum\",$i,this,\"$TempId\",\"cw_fkcount_m_at\");' id='ThisImg_$DivNum$i' name='ThisImg_$DivNum$i' src='../images/showtable.gif' width='13' height='13' style='CURSOR: pointer'>";
			$HideTableHTML="
			<table width='$tableWidth' border='0' cellspacing='0' name='HideTable_$DivNum$i'  id='HideTable_$DivNum$i'   style='display:none'>
				<tr bgcolor='#B7B7B7'>
					<td class='A0111' height='30'>
...
?>
	<table width="<?php  echo $tableWidth?>" border="0" cellspacing="0" id="ListTable<?php  echo $i?>" bgcolor="#FFFFFF" style='TABLE-LAYOUT: fixed; WORD-WRAP: break-word'>
		<tr id='A'>
			<td class="A0111" width="700" height="25">&nbsp;<?php  echo $showPurchaseorder?>&nbsp;<span class="redB"><?php  echo $Month?></span></td>
			<td class="A0101" width="80" align="right"><?php  echo $Amount1?></td>
			<td class="A0101" width="80" align="right"><div class="greenB"><?php  echo $Amount2?></div></td>
			<td class="A0101" width="80" align="right"><div class="redB"><?php  echo $Amount3?></div></td>
		</tr>
