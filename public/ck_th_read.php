...
if($From!="slist"){	//非查询：过滤采购、结付方式、供应商、月份
	...
	$date_Result = mysql_query("SELECT A.Date FROM (
	           SELECT Date FROM $DataIn.ck2_thmain WHERE 1 GROUP BY DATE_FORMAT(Date,'%Y-%m')
	   UNION ALL 
	          SELECT Date FROM $DataIn.ck12_thmain WHERE 1 GROUP BY DATE_FORMAT(Date,'%Y-%m')
	   )A ORDER BY Date DESC",$link_id);
	...
	$providerSql = mysql_query("
	         SELECT M.CompanyId,P.Forshort,P.Letter 
			 FROM $DataIn.ck2_thmain M,$DataIn.trade_object P 
			 WHERE M.CompanyId=P.CompanyId $SearchRows 
			 GROUP BY M.CompanyId  ORDER BY Letter",$link_id);
...
$mySql="SELECT '1' AS Sign,M.BillNumber,M.Date,M.Attached,M.CompanyId,S.Id,S.Mid,S.StuffId,S.Qty,S.Remark,S.Estate,S.Locks,D.StuffCname,D.Picture,
U.Name AS UnitName,'0' AS thPicture,S.Id AS thisId
...
WHERE 1 $SearchRows $SearchRowsA 
UNION ALL 
SELECT  '2' AS Sign,M.BillNumber,M.Date,M.Attached,M.CompanyId,S.Id,S.Mid,S.StuffId,S.Qty,S.Remark,S.Estate,S.Locks,D.StuffCname,D.Picture,U.Name AS UnitName,S.Picture AS thPicture,S.Id AS thisId
...
if($mainRows = mysql_fetch_array($mainResult)){
	...
	do{
		...
		$BillNumberStr="<a href='ck_th_view.php?f=$MidSTR&Sign=$Sign' target='_blank'>$BillNumber</a>";



		$upMian=$Sign==1?"&nbsp;":"<img location.href=\"#\"' style='CURSOR: pointer' onclick='upMainData(\"ck_th_upmain\",$Mid)' src='../images/edit.gif' title='更新退换主单资料' width='13' height='13'>";
		...
		if($StuffId>0){
			...
			switch($mainRows["Estate"]){
				...
				case 1:$EstateSTR="<div class='redB'>未审核</div>";break;
