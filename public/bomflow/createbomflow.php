...
if ($FromPage=="pands"){
	$StuffIdList="";$UniteIdList="";$RelationList=""; $k=1;
	$StuffResult=mysql_query("SELECT StuffId FROM $DataIn.pands  WHERE ProductId='$ProductId' order by Id",$link_id);
	while($StuffRow=mysql_fetch_array($StuffResult)){
		$StuffId=$StuffRow["StuffId"];
		$StuffIdList.=$k==1?$StuffId:"|$StuffId";
		
		$UniteId="";$Relation="";
		$UniteResult=mysql_query("SELECT U.uStuffId,U.Relation FROM $DataIn.pands_unite U   WHERE  U.ProductId='$ProductId' AND U.StuffId='$StuffId' AND U.uStuffId!='$StuffId'",$link_id);
		while($UniteRow = mysql_fetch_array($UniteResult)){
		 ...
		 $RelationList.=$k==1?$Relation:"|$Relation";
		 $k++;
	}
}
if ($ProductId>0){
	$cNameResult=mysql_fetch_array(mysql_query("SELECT cName FROM $DataIn.Productdata  WHERE ProductId='$ProductId' Limit 1",$link_id));
	$ProductName=$cNameResult["cName"];
}

$StuffIdArray=explode("|", $StuffIdList);
$UniteIdArray=explode("|", $UniteIdList);
$RelationArray=explode("|", $RelationList);
...
$mainResult=mysql_query("SELECT S.StuffId,S.StuffCname,P.Forshort
            ...
            WHERE S.StuffId in ($idStr)  ORDER BY Field(T.mainType,". $APP_CONFIG['PANDS_FINAL_MAINTYPE'] .") DESC ",$link_id);
...
if ($outImageSign){
	...
	$outFileName="../../download/bomflow/" . $ProductId . ".png";
...
function getSemifinishedStuff(&$dataArray,$StuffIdArray,$UniteIdArray,$DataIn,$link_id,$FontName,$FontSize,$mianType,$noType)
{
    ...
    for($i=0,$counts=count($dataArray);$i<$counts;$i++){ 
	  ...
	  $UniteResult=mysql_query("SELECT S.StuffId,S.StuffCname,S.Picture,P.Forshort,T.mainType     
		                FROM $DataIn.semifinished_bom  M 
		                LEFT JOIN $DataIn.stuffdata S  ON S.StuffId=M.StuffId 
		                LEFT JOIN $DataIn.stufftype T ON T.TypeId=S.TypeId
		                LEFT JOIN $DataIn.bps B ON B.StuffId=S.StuffId
                        LEFT JOIN $DataIn.trade_object P ON P.CompanyId=B.CompanyId
		                WHERE M.mStuffId='$mStuffId'  AND T.mainType NOT IN ($noType) ORDER BY T.mainType",$link_id);
