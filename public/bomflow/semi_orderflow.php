...
$StuffResult=mysql_query("SELECT StuffId FROM $DataIn.cg1_semifinished  WHERE mStockId='$mStockId' ",$link_id);
...
$mainResult=mysql_query("SELECT G.StockId,S.StuffId,S.StuffCname,P.Forshort
...
do{
   $uniteArray=getSemifinishedStuff($dataArray[$k],$DataIn,$link_id,$FontName,$FontSize,$APP_CONFIG['WORKORDER_ACTION_MAINTYPE'],$APP_CONFIG['STATISTICS_MAINTYPE']);
   if (count($uniteArray)>0){
      $k++;
	  $dataArray[$k]=$uniteArray;
   }
	   ...
	   if ($tmp_Mid!=$Mid && $Mid!="" && $StuffCname!=""){
		   ...
		   if ($rows>0){
			  ...
			  if ($rows>0){
			    ...
			    $position=RoundedRectangle($im,$x2,$y2,$Forshort,$FontName,$FontSize,"#004F8A","$RecbgColor","$RecLineColor");
...
if ($outImageSign){
	...
	$outFileName="../../download/ptflow/" . $mStockId . ".png";
...
//获取半成品配件列表
function getSemifinishedStuff(&$dataArray,$DataIn,$link_id,$FontName,$FontSize,$mianType,$noType)
{
