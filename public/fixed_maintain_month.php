...
<P align="center" style="font-size:25px;font-weight:bold;"><a href="fixed_maintain_day.php" target="_self" ><img src='../images/TheBack.gif' width='35' height='23' border='0' title='设备(日/周/半月)保养记录表'></a>设备(月/季/年)保养记录表</P>
<center>
        ...
        <table cellspacing="0" class="Table1" Width="1050">
          <tr align="center">
   	    <td width="72" height="35" class="A0101">设备名称</td>
            <td width="248" class="A0101" align="left">
				...
				*/
				...
				$DaysIDStr=" (1,2 ) "; //日 或 周,及维护周期少于或等于15天的
				...
				*/
				...
				LEFT JOIN $DataPublic.fixed_assetsdata D ON D.Id=F.Mid
				LEFT JOIN (select  distinct S.TypeId, S.DaysID,S.Days from $DataPublic.oa3_maitaintype S where S.DaysID not in $DaysIDStr $DaysStr ) 
				S ON S.TypeId=D.TypeId
				LEFT JOIN $DataPublic.oa3_maitaindays K ON K.ID=S.DaysID
				LEFT JOIN $DataPublic.staffmain M ON M.Number=F.User
				LEFT JOIN $DataPublic.branchdata B ON B.Id=D.BranchId
				WHERE 1 AND D.Estate=1 $cSignSTR AND S.DaysID not in $DaysIDStr $DaysStr $FstrFrom order by S.DaysID  ",$link_id);  //S.DaysID=1 表示只是每天维护的
			...
			*/
            ...
            <td width="72"  class="A0101">保养年</td>
            <td width=""  class="A0100">
			<?php
			//这里可以看到前一年到现在的记录。
			$tempCurY=$Date=date("Y");  //当前年，倒推至前五年
			$DateTime=date("Y-m-d");
			//echo "$tempBuyDate";
			//echo "SYYYYMM:$SYYYYMM <br>";
			$SYYYYMM=SYYYYMM==""?"0":$SYYYYMM;
			//echo "SYYYYMM:$SYYYYMM <br>";
			echo"<select name='SYYYYMM' id='SYYYYMM' onchange='zhtj(this.name)'>";
			echo"<option value='0' >今年(前10次)</option>";
			for ($i=0;$i<5;$i++) {
				$StartYYY=date("Y",strtotime("$DateTime-$i years"));
				//$SubOneMonth=$i;
				//$StartDate=date("Y-m",strtotime("$DateTime-$SubOneMonth months"))."-01"; //一个月的第一天
				$tempBuyYYYY=substr($tempBuyDate,0,4);
				if ($StartYYY<$tempBuyYYYY) //如果比购买日期还前，则直接跳出循环
				{
					break;
				}

				//$SYYYYMM=SYYYYMM==""?$tempCurY:$SYYYYMM;

					if ($StartYYY==$SYYYYMM){
					   echo"<option value='$StartYYY' selected>$StartYYY</option>";

					}else{
						echo"<option value='$StartYYY'>$StartYYY</option>";
					}
			}
			echo"</select>";
			//=$Date=date("Y-m");
        ...
        <table cellspacing="0" class="Table1">
         <tr bgcolor="#CCCCCC" align="center">
   	    ...
   	    <td width="165" class="A0101">保养检查项目</td>
			...
			$MonthArray=array(); // 保存年月份
			array_push($MonthArray,""); //

			$This_Counts=0;
			$This_FirstMonth=0; //本年度的统计
			$Last_Count=0;  //以前的年度
			$weekarray=array("日","一","二","三","四","五","六",);
			$This_LastMM=1;

			if($SYYYYMM=="0" || $SYYYYMM==""){  //取得最非本年度的前面记录，如果没有，则用最近月份补足,，最后一格下一次维护的时间
				$This_YYYY=($SYYYYMM=="0" || $SYYYYMM=="")?date("Y"):$SYYYYMM;  //取得
				/*
				$count_Temp=mysql_query("select count(*) as Counts
				FROM $DataPublic.fixed_m_main M
				LEFT JOIN $DataPublic.oa3_maitaintype S ON S.DaysID=M.DaysID
				WHERE 1  and M.DaysID='$DaysID' $DaysStr AND M.FixedID='$FixedID' AND substring(M.CycleDate,1,4)='$This_YYYY'  group by substring(M.CycleDate,1,7) Order by M.CycleDate  ",$link_id);
				$This_Counts=mysql_num_rows($count_Temp); //本年度的统计
				*/
				/*
				$count_Temp=mysql_query("select M.CycleDate
				FROM $DataPublic.fixed_m_main M
				LEFT JOIN $DataPublic.oa3_maitaintype S ON S.DaysID=M.DaysID
				WHERE 1  and M.DaysID='$DaysID' $DaysStr AND M.FixedID='$FixedID' AND substring(M.CycleDate,1,4)='$This_YYYY'   Order by M.CycleDate Limit 1  ",$link_id);  //取得今年第一个月的维护记录

				$This_FirstDate=mysql_result($count_Temp,0,"CycleDate"); //当月的最后检查日期
				if($This_FirstDate!=null && $This_FirstDate!="") {
					$This_FirstMonth=substr($This_FirstDate,5,2)*1;
				}

				*/
				/*
				echo "This_FirstMonth:$This_FirstMonth <br>";
				$This_NowMonth=date("m")*1;
				//$LastPassCount=12-($This_NowMonth-$This_FirstMonth+1)-2;  //控制原月份,非本年的最多9条显示
				$LastPassCount=$This_FirstMonth-1;
				echo "LastPassCount:$LastPassCount <br>";
				if($LastPassCount>0) {  //如果还有不够月份，则补旧的记录
				*/
					$This_LastMonth=date("Y-m",strtotime("$DateTime-1 months"));  //当前月的上一个月
					$This_LastMM=substr($This_LastMonth,5,2)*1;

					$QuestionSql= mysql_query("select M.CycleDate 
					FROM $DataPublic.fixed_m_main M
					LEFT JOIN $DataPublic.oa3_maitaintype S ON S.DaysID=M.DaysID 
					WHERE 1  and M.DaysID='$DaysID' and M.Days='$Days' $DaysStr AND M.FixedID='$FixedID' AND substring(M.CycleDate,1,7)<'$This_LastMonth'  group by substring(M.CycleDate,1,7) Order by M.CycleDate  Limit 8 ",$link_id);  //把前一个月以前的9条维护记录或取出来
					/*
					echo "select M.CycleDate
					FROM $DataPublic.fixed_m_main M
					LEFT JOIN $DataPublic.oa3_maitaintype S ON S.DaysID=M.DaysID
					WHERE 1  and M.DaysID='$DaysID' $DaysStr AND M.FixedID='$FixedID' AND substring(M.CycleDate,1,7)<'$This_LastMonth'  group by substring(M.CycleDate,1,7) Order by M.CycleDate  Limit 9   ";
					*/
					if($QuestionRow = mysql_fetch_array($QuestionSql)){
						do{
							$M_ID=$QuestionRow["ID"];
							$M_CycleDate=$QuestionRow["CycleDate"];
							$M_YYYYMM=substr($M_CycleDate,0,7);

							array_push($MonthArray,$M_YYYYMM); //
							/*
							$count_Temp=mysql_query("SELECT Question,Operator FROM $DataPublic.fixed_m_sheet
							...
							*/
							...
							$This_title="";
							$This_Class="";
							if ($M_YYYYMM==date("Y-m")) {  //如果当月,则下划线
								$underline=" text-decoration:underline; background:#F0F";
								$TodayStr="今天：$DateTime &nbsp; ";
								$This_WDay=$weekarray[date('w',strtotime($DateTime))];  //0是星期天，6是星期六
								$This_Class='redB';
								$This_title='$TodayStr 星期$This_WDay';
							}


							//if($This_WDay=="日" || $This_WDay=="六") {

								echo "<td width='65' class='A0101'><div style='$underline' class='$Class' $title>$M_YYYYMM</div></td> ";

							//}
							//else {
								//echo "<td width='' style='$underline' class='A0101'><div  class='$Class' title='$TodayStr 星期$This_WDay'>$M_YYYYMM</div></td> ";
							//}
						...
						$CycleDate_Temp=mysql_query("select M.CycleDate 
							FROM $DataPublic.fixed_m_main M
							LEFT JOIN $DataPublic.oa3_maitaintype S ON S.DaysID=M.DaysID 
							WHERE 1  and M.DaysID='$DaysID' and M.Days='$Days' $DaysStr AND M.FixedID='$FixedID' AND substring(M.CycleDate,1,7)>='$This_LastMonth'  group by substring(M.CycleDate,1,7) Order by M.CycleDate desc Limit 1 ",$link_id);  //
				     if (mysql_num_rows($CycleDate_Temp)>0)
				     {
						$Last_CycleDate=mysql_result($CycleDate_Temp,0,"CycleDate");
					}
						$M_CycleDate=$Last_CycleDate==""?$M_CycleDate:$Last_CycleDate; //最近两个月是否有保养

						$M_CycleDate=$M_CycleDate==""?$tempBuyDate:$M_CycleDate; //如果无维护记录，则把购买记录当第一次

						$NextDate=date("Y-m-d",strtotime("$M_CycleDate+$Days days"));

						echo "<td width='88' class='A0101'><div  title='下一次保养时间' >△</div></td> ";
						array_push($MonthArray,'NextDate'); // 表示下一次维护时间

						echo "<td width='65' class='A0101'><div style='$underline' class='$Class' $title>$This_LastMonth</div></td> ";
						array_push($MonthArray,$This_LastMonth); //

						$This_NowMonth=date("Y-m");
						array_push($MonthArray,$This_NowMonth); //

						//echo "<td width='' class='A0101'><div style='$underline' class='$Class' $title>$This_NowMonth</div></td> ";
						$underline="";
						$TodayStr="";
						if ($This_NowMonth==date("Y-m")) {  //如果当月,则下划线
						...
						if($This_WDay=="日" || $This_WDay=="六") {
							...
							echo "<td width='65' class='A0101'><div style='$underline' class='redB' title='$TodayStr 星期$This_WDay'>$This_NowMonth</div></td> ";

						}
						else {
							echo "<td width='65' style='$underline' class='A0101'><div  title='$TodayStr 星期$This_WDay'>$This_NowMonth</div></td> ";
						}



						echo "<td width='' class='A0101'><div   >备注</div></td> ";
				//} //if($LastPassCount>0) {

			}
			else {
				$This_YYYY=($SYYYYMM=="0" || $SYYYYMM=="")?date("Y"):$SYYYYMM;  //取得今年
				//echo "$YYYYMM=$SYYYYMM";
				//for ($i=1;$i<13;$i++){
				for ($i=1;$i<13;$i++){
					$DD=$i;
					if($i<10){$DD='0'."$i";}  //取得月
					$This_MM=$This_YYYY.'-'."$DD"; //取得月
					array_push($MonthArray,$This_MM); //
					$underline="";
					if ($This_MM==date("Y-m")) {  //如果当月,则下划线
						$underline=" text-decoration:underline; background:#F0F";
						$TodayStr="今天：$DateTime &nbsp; ";
					}
					...
					if($This_WDay=="日" || $This_WDay=="六") {
						...
						echo "<td width='65' class='A0101'><div style='$underline' class='redB' title='$TodayStr 星期$This_WDay'>$This_MM</div></td> ";

					}
					else {
						echo "<td width='65' style='$underline' class='A0101'><div  title='$TodayStr 星期$This_WDay'>$This_MM</div></td> ";
					}
				 }
				echo  "<td width='' class='A0100'>&nbsp;</td>";
			}
            ?>
      ...
      <td>
        <table cellspacing="0" class="Table1" id="<?php  echo $Curtable?>" >
         <tr align="center">
			...
			$lenArray=count($MonthArray);
			for ($i=1;$i<$lenArray;$i++){

				$Array_YYYYMM=$MonthArray[$i];
				if($Array_YYYYMM=='NextDate') {  //表示下一次维护时间
					$P_NextDate=$i; //记住这个位置
					$DivStr='';
					$DivTitle='';
					...
					echo "<td width='88' class='A0101' >
							<div  class='$DivStr' style='display:inline' title='$NextDate $DivTitle '>$NextDate</div>
					</td> ";
					...
					$Question_Operator="";
						//查找是否正常,如不正常则显示问题
					$ConditioSTR=" AND substring(M.CycleDate,1,7)='$Array_YYYYMM'";	//把月份相同的找出来
					$QuestionSql= mysql_query("select M.ID,M.CycleDate 
					FROM $DataPublic.fixed_m_main M
					WHERE 1 $ConditioSTR and M.DaysID='$DaysID' and M.Days='$Days' AND M.FixedID='$FixedID' Order by M.CycleDate ",$link_id);
					/*
					echo "select M.ID,M.CycleDate
					FROM $DataPublic.fixed_m_main M
					WHERE 1 $ConditioSTR and M.DaysID='$DaysID' and M.Days='$Days' AND M.FixedID='$FixedID' Order by M.CycleDate";
					*/
					if($QuestionRow = mysql_fetch_array($QuestionSql)){
						do{
							$M_ID=$QuestionRow["ID"];
							$M_CycleDate=$QuestionRow["CycleDate"];
							$M_CycleDay=substr($M_CycleDate,8,2).'日';
							$count_Temp=mysql_query("SELECT Question,Solution,Operator FROM $DataPublic.fixed_m_sheet 
													 WHERE  Mid='$M_ID' AND maitainID='$maitainID' ",$link_id);  //
							/*
							echo "SELECT Question,Operator FROM $DataPublic.fixed_m_sheet
													 WHERE  Mid='$M_ID' AND maitainID='$maitainID' ";*/
							$Question_Question=mysql_result($count_Temp,0,"Question");
							$Question_Solution=mysql_result($count_Temp,0,"Solution");
							$Question_Operator=mysql_result($count_Temp,0,"Operator");

							$passvalue="$FixedID|$maitainID|$DaysID|$SubName|$M_CycleDate|$Days";

							if ($Question_Question!=null && $Question_Question!="") { //表示当天有问题，

								echo "<td width='65' class='A0101' >
									<div  >
										<div  style='display:inline' title='$M_CycleDate'>$M_CycleDay</div>
										<div  style='display:inline' class='redB' title='$Question_Operator:$Question_Question  \n $Question_Solution' 
										onmousedown='window.event.cancelBubble=true;' onclick='showCCDiv(\"fixed_maintain_monthmask.php\",\"$Curtable\",$Dcount,$i,500,\"$passvalue\")'>×</div>
									</div>	
								</td> ";
								...
								echo "<td width='65' class='A0101' >
									<div  >
										<div  style='display:inline' title='$M_CycleDate'>$M_CycleDay</div>
										<div style='display:inline' class='greenB' title='正常' 
										onmousedown='window.event.cancelBubble=true;' onclick='showCCDiv(\"fixed_maintain_monthmask.php\",\"$Curtable\",$Dcount,$i,500,\"$passvalue\")'>√</div>
									</div>
								</td> ";
							}
						...
						}while ($QuestionRow = mysql_fetch_array($QuestionSql));  //do


					 }		//if($QuestionRow = mysql_fetch_array($QuestionSql)){
					 else {  //不存在有操作记录，则可以点击维护

					 	if ($Array_YYYYMM!=date('Y-m')) { //如果同一月，
							$M_CycleDate=$Array_YYYYMM.'-01'; //置成当月一号
						}
						else{
							$M_CycleDate=date('Y-m-d');
						}
					 	$passvalue="$FixedID|$maitainID|$DaysID|$SubName|$M_CycleDate|$Days";
						echo "<td width='65' class='A0101' onmousedown='window.event.cancelBubble=true;' onclick='showCCDiv(\"fixed_maintain_monthmask.php\",\"$Curtable\",$Dcount,$i,500,\"$passvalue\")'>
																				<div  title='点击保养'>◎</div>
																				</td> ";
					 }

				 } // if($Array_YYYYMM=='NextDate') {  //表示下一次维护时
			}  //for ($i=1;$i<$lenArray;$i++){
            ...
            for ($i=1;$i<32;$i++){
				...
				$CycleDate=$YYYYMM.'-'."$DD";
				$passvalue="$FixedID|$maitainID|$DaysID|$SubName|$CycleDate";
				...
				$QuestionSql= mysql_query("select M.ID,M.CycleDate
				FROM $DataPublic.fixed_m_main M
				WHERE 1 $ConditioSTR and M.DaysID='$DaysID' AND M.FixedID='$FixedID' Order by M.CycleDate DESC Limit 1 ",$link_id);
				if($QuestionRow = mysql_fetch_array($QuestionSql)){
					do{
						$M_ID=$QuestionRow["ID"];
						$M_CycleDate=$QuestionRow["CycleDate"];
					...
					if ($Question_Question!=null && $Question_Question!="") { //表示当天有问题，
						...
						<div class='redB' title='$Question_Operator:$Question_Question'>×</div>
						</td> ";
      ...
      <td>
        <table cellspacing="0" class="Table1">
         <tr align="center">
            ...
            for ($i=1;$i<13;$i++){
				if($P_NextDate==$i) {
                	echo "<td width='88' class='A0101'>&nbsp;</td> ";
				}
				else {
					echo "<td width='65' class='A0101'>&nbsp;</td> ";
				}
              }
            ?>
        <td width="" class="A0100">&nbsp;</td>
...
function zhtj(obj){
	...
	document.form1.action="fixed_maintain_month.php";
...
function CCCurentSave(passvalue)  //需要自已改变的函数
{
	...
	var MaintainDate=document.getElementById('MaintainDate').value;
	MaintainDate=trimStr(MaintainDate);
	...
	var tempMaintainDate=MaintainDate;
	var tempDay=MaintainDate.substr(8,2)+"日";
	MaintainDate=encodeURIComponent(MaintainDate);
	var myurl="../public/fixed_maintain_ajax_updated.php?passvalue="+passvalue+"&Question="+Question+"&Solution="+Solution+"&MaintainDate="+MaintainDate+"&Action=EveryDay";
	//alert(tempDay+myurl);
	//return false;
	var ajax=InitAjax();
	ajax.open("GET",myurl,true);
	ajax.onreadystatechange =function(){
		if(ajax.readyState==4){// && ajax.status ==200
			if (Question=="") {
				eval(CTable).rows[0].cells[CCol].innerHTML=tempDay+" <div style='display:inline' class='greenB' title='"+tempQuestion+" \n "+tempSolution+"'>√</div>";
			}
			else{
				eval(CTable).rows[0].cells[CCol].innerHTML=tempDay+"<div  style='display:inline' class='redB' title='"+tempQuestion+" \n "+tempSolution+"'>×</div>";
			}
