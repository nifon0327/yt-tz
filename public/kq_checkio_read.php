...
if($From!="slist"){
	...
	echo"<select name='showTypeFlag' id='showTypeFlag' onchange='ResetPage(this.name)'>
	         <option value='0' $selectType0>全部</option>
		     <option value='1' $selectType1>非固定薪</option>
			 <option value='2' $selectType2>固定薪</option>";
	echo "</select>&nbsp;";
	switch($showTypeFlag){
		case 1:  $SearchRows.=" AND (M.BranchId IN ( 6, 7, 8 ) OR M.JobId IN ( 10, 38 )) "; break;
		case 2:  $SearchRows.=" AND (M.BranchId NOT IN ( 6, 7, 8 ) AND M.JobId NOT IN ( 10, 38 )) ";break;
		default:break;
	}


	$FormalSign=$FormalSign==""?1:$FormalSign;
		...
		$hideStateSql = "SELECT * FROm $DataIn.disablecheckid WHERE checkId = '$CheckDate'";
		$hideStateResult = mysql_query($hideStateSql);
		if(mysql_num_rows($hideStateResult) == 0){
			$showSelect = 'selected';
			$hideSelect = '';
		}else{
			$showSelect = '';
		...
		echo "<select name='isHideInList' id='isHideInList' onchange='setHideList()''>
			  <option value='1' $showSelect>显示在列表</option>
			  <option value='0' $hideSelect>不显示在列表</option>
			  </select>";
...
if($myRow = mysql_fetch_array($myResult)){
	...
	do{
		...
		include "kqcode/checkio_model_".$CheckType.".php";
...
function setHideList(){
	...
	var isHideInList = document.getElementById('isHideInList').value;
	myurl="../model/subprogram/kq_recordDisable.php?CheckDate="+checkDate+"&isHideInList="+isHideInList;
	var ajax=InitAjax();
	ajax.open("GET",myurl,true);
	ajax.onreadystatechange =function(){
