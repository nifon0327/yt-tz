...
if($IsType==0){		
		...
		echo "<SCRIPT LANGUAGE=JavaScript>alert('不能操作！只能对为【类】图的QC标准图进行相关配件的剔除');";
...
<table width="<?php  echo $tableWidth?>" height="306" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF" id="NoteTable">
  <tr>
    ...
    <tr>
     ...
     <?php 
	  $result = mysql_query("SELECT S.StuffId,S.StuffCname FROM $DataIn.stuffqcless D LEFT JOIN $DataIn.Stuffdata S ON S.StuffId=D.StuffId WHERE D.QcId=$Id order by S.StuffId",$link_id);
