<?php 
//步骤1 $DataIn.cw16_modelfee  二合一已更新电信---yang 20120801
...
if($From!="slist"){
	$monthResult = mysql_query("SELECT M.Date FROM $DataIn.cw16_modelfee  M  
	        WHERE 1 $SearchRows AND M.Estate='2' group by DATE_FORMAT(M.Date,'%Y-%m') order by M.Date DESC",$link_id);
...
WHERE 1 $SearchRows  AND M.Estate='2'";
	...
	do{
		...
		if($Bill==1){
			...
			$cg_stuffResult=mysql_query("SELECT MAX(Qty) AS sumQty FROM (
				...
				//动态读取
		$showPurchaseorder="<img onClick='Showstuff(stuffList$i,showtable$i,stuffList$i,\"$Id\",$i);' name='showtable$i' src='../images/showtable.gif' title='显示相关配件.' width='13' height='13' style='CURSOR: pointer'>";
		$stufflistTB="
