...
WHERE C.CheckTime LIKE '$CheckMonth%'  AND M.Number='$Number'
...
if($CheckMonth<"2013-05"){
	$Amount001=floor(($sumGJTime+$Sum_OverTimeG+$sumZLG)*$HourlyWage102);
	$Amount002=floor(($sumXJTime+$Sum_OverTimeX+$sumZLX)*$HourlyWage103+($sumFJTime+$Sum_OverTimeF+$sumZLF)*$HourlyWage104);
	...
	$Amount002=floor(($Sum_OverTimeG+$sumZLG)*$HourlyWage102+($Sum_OverTimeX+$sumZLX)*$HourlyWage103+($Sum_OverTimeF+$sumZLF)*$HourlyWage104);
	$Amount003=$Amount001+$Amount002;
...
echo "&nbsp;<br>";
//<p><a href='kq_checkio_count_auao.php?CheckMonth=$CheckMonth' target='_blank'>自动录入统计</a></p>";

/*用于更新5-6月份的数据：原加班和日加班费
$Amount004=floor(($sumGJTime+$Sum_OverTimeG+$sumZLG)*$HourlyWage102);
$checkJrjb=mysql_fetch_array(mysql_query("SELECT IFNULL(Amount,0) AS Jrjb FROM $DataIn.hdjbsheet WHERE Number='$Number' AND Month='$CheckMonth'",$link_id));
$Amount005=$checkJrjb["Jrjb"];
$Amount006=$Amount004+$Amount005;
if($Amount003!=$Amount006){//前后不一致
	$Amount002=$Amount002-($Amount003-$Amount006);
	$Amount003=$Amount001+$Amount002;
	}

echo "更新前(原数据)<br>加班费用:<input name='oldJbf' type='text' id='oldJbf' value='$Amount004'><br>";
echo "假日费用:<input name='oldJrjb' type='text' id='oldJrjb' value='$Amount005'><br>";
echo "费用合计:".$Amount006."<br>";*/
//步骤5：
