<?php 
$mainData="$DataIn.cw18_workhurtmain";
$sheetData="$DataIn.cw18_workhurtsheet";
$Pagination=$Pagination==""?0:$Pagination;	//默认分页方式:1分页，0不分页
$Page_Size = 100;							//每页默认记录数量	
//步骤1：
include "../model/subprogram/cw0_model1.php";
$mySql="SELECT 
	M.PayDate,M.PayAmount,M.Payee,M.Receipt,M.Checksheet,M.Remark AS PayRemark,M.Locks AS MLocks,P.ComeIn,S.CheckT,S.HurtDate,
	S.Id,S.Mid,S.Number,S.Month,S.Amount,S.Date,S.Estate,S.Locks,P.Name,B.Title,S.Remark,BD.Name AS BranchName,J.Name AS JobName,S.Attached,S.HostpitalInvoice,M.cSign  
	FROM $mainData M
...
if($mainRows = mysql_fetch_array($mainResult)){
	...
	do{
        ...
        $HurtDate=$mainRows["HurtDate"];
        $CheckT=$mainRows["CheckT"];
		...
		if($Keys & mUPDATE || $Keys & mLOCK){
			$upMian="<img location.href=\"#\"' style='CURSOR: hand' onclick='upMainData(\"rs_staffHurt_upmain\",$Mid)' src='../images/edit.gif' title='更新结付单资料!' width='13' height='13'>";	
			}
		else{
			$upMian="&nbsp;";
			}
		if($MLocks==0){
			$Choose="<img src='../images/lock.gif' title='主记录已锁定!' width='9' height='15'>";
			}
		else{
			if($Keys & mUPDATE){
				...
				$Choose="<img src='../images/lock.gif' title='没有操作权限!' width='9' height='15'>";
				}
			}
		...
		if($midDefault!="" && $midDefault==$Mid){//同属于一个主ID，则依然输出明细表格
			...
			$m=$m+2;
			echo"<td class='A0001' width='$Field[$m]' align='center'>$BranchName</td>";
			$m=$m+2;
			echo"<td class='A0001' width='$Field[$m]' align='center'>$JobName</td>";			
			$m=$m+2;
			echo"<td class='A0001' width='$Field[$m]' align='center'>$ComeIn</td>";			
			$m=$m+2;
			echo"<td class='A0001' width='$Field[$m]' align='center'>$Amount</td>";
			$m=$m+2;
			echo"<td class='A0001' width='$Field[$m]' align='center'>$HurtDate</td>";
			$m=$m+2;			
			echo"<td class='A0001' width='$Field[$m]' align='center'>$Attached</td>";
			$m=$m+2;
			echo"<td class='A0001' width='$Field[$m]' align='center'>$HostpitalInvoice</td>";
			$m=$m+2;
