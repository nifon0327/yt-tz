...
if ($ActionId==17 ){
	...
	switch ($ActionId){
		...
		case 17:
			...
			for($i=0;$i<$Lens;$i++){
			    ...
			    $myResult=$myPDO->query("CALL proc_ck8_bfsheet_updatedestate('$Id','1',$Operator);");
	...
	case 15://记录退回
		...
		break;
	        ...
	        case 167:
	            $UpdateSql = "UPDATE  $DataIn.ck8_bfsheet  SET DealResult='$DealResult' WHERE Id=$Id";
	            ...
	            if($UpdateResult && mysql_affected_rows()>0){
			     	...
			     	$Log.="配件ID $StuffId 报废的最终处理结果为： $DealResult <br>";
	             }
			else{
				$Log.="配件ID $StuffId 报废的最终处理结果 失败! $UpdateSql <br>";
				$OperationResult="N";
				}
			break;
		default:
		...
		if($Attached!=""){//有上传文件
	       ...
	       $Type=$Type==""?0:$Type;
			$upSql = "UPDATE $DataIn.ck8_bfsheet F 
				      SET F.ProposerId='$ProposerId',F.Qty=F.Qty+$changeQty*$Operators,
			...
			if($upResult && mysql_affected_rows()>0){
				...
				$Log.="<div class='redB'>报废记录更新失败!</div>$upSql<br>";
				$OperationResult="N";
				}
