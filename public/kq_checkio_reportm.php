<?php
	...
	if ($KqSign=="" or $KqSign == '1') {
		$KqSignStr= "AND (M.KqSign='1' or (M.KqSign='2' AND (M.BranchId IN ( 6, 7, 8 ) OR M.JobId IN ( 10, 28 )))) ";
	}else{
		$KqSignStr= "AND M.KqSign='2' AND M.Number not in (select Number From staffmain where M.KqSign='2' AND (M.BranchId IN ( 6, 7, 8 ) OR M.JobId IN ( 10, 28 ))) ";
	}
	
	//$SearchRows=$KqSign==1?"":"  AND (M.BranchId IN ( 6, 7, 8 ) OR M.JobId IN ( 10, 38 )) ";
	if ($APP_FACTORY_CHECK==true) {
		$SearchRows=$KqSign==1?"":"  AND (M.BranchId IN ( 6, 7, 8 ) OR M.JobId IN ( 10, 28 ) OR M.Number IN ( 10776, 12018, 12042,10773) ) AND M.WorkAdd!=6 AND M.Number != 10744";
	}
	...
	for($i=0;$i<$Days;$i++){
        ...
        if($CheckDate>='2016-05-16' && $CheckDate<='2016-07-31'){
	        $personalotherleave = $dayAttendanceInfomation["personalLeave"];  
        }else{
	        $personalotherleave = 0 ;
        }
        $sumpersonalotherleave +=$personalotherleave;
