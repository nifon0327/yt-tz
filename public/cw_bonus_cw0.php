...
//步骤1：
$mainData="$DataIn.cw20_bonusmain";
$sheetData="$DataIn.cw20_bonussheet";
...
$mySql="SELECT 
	M.PayDate,M.PayAmount,M.Payee,M.Receipt,M.CheckSheet,M.Remark AS PayRemark,M.Locks AS MLocks,S.Id,S.Mid,S.Content,S.Amount,S.Bill,S.Date,
   S.Operator,C.Symbol AS Currency,Bank.Title,D.Name,B.Name AS BranchName,J.Name AS JobName,M.cSign
 	FROM $DataIn.cw20_bonusmain M
	LEFT JOIN $DataIn.cw20_bonussheet  S ON S.Mid=M.Id
	...
	LEFT JOIN $DataPublic.my2_bankinfo Bank ON Bank.Id=M.BankId
...
if($mainRows = mysql_fetch_array($mainResult)){
	...
	do{
		...
		if($Keys & mUPDATE || $Keys & mLOCK){
			$upMian="<img location.href=\"#\"' style='CURSOR: pointer' onclick='upMainData(\"cw_bonus_upmain\",$Mid)' src='../images/edit.gif' title='行号:$mRow 编号:$Mid 更新结付单资料!' width='13' height='13'>";
			}
		else{
			$upMian="&nbsp;";
			}
