	...
	do{
		...
		switch($ValueCode){
			...
			case "104"://3倍时薪
				...
				break;
			case "108":// 有薪工时扣福利费,指不上用上班，有工资，就扣福利费这一块
...
$myResult = mysql_query("SELECT Number,Currency,Name,BranchId,JobId,GroupId,Grade,KqSign,ComeIn,WorkAdd,cSign   
FROM $DataPublic.staffmain 
WHERE left(ComeIn,7)<='$chooseMonth'
AND Number NOT IN (SELECT Number FROM $DataIn.dimissiondata WHERE left(outDate,7)<'$chooseMonth')  
...
if($myRow = mysql_fetch_array($myResult)){
	do{
		...
		//津贴计算
		if($Grade>0){
			$jtResult = mysql_fetch_array(mysql_query("SELECT Subsidy FROM $DataIn.gradesubsidy WHERE 1 and Grade=$Grade LIMIT 1",$link_id));
		...
		//预设奖金
		$baseResult = mysql_fetch_array(mysql_query("SELECT Dx,Jj,Jtbz,Sbkk,Taxkk FROM $DataIn.paybase where  Number='$Number' LIMIT 1",$link_id));
		...
		if ($Currency==1){  //人民币工资计算
				...
				//餐费扣款 
		        $CtRow = mysql_fetch_array(mysql_query("SELECT IFNULL(Amount,0) AS Ct FROM $DataIn.ct_monthamount  WHERE Number=$Number and Month='$chooseMonth' LIMIT 1",$link_id));
		        ...
		        $YwjjRow = mysql_fetch_array(mysql_query("SELECT IFNULL(Amount,0) AS Ywjj,Content FROM $DataIn.cwxz_bonus  WHERE Number='$Number' and Month='$chooseMonth' LIMIT 1",$link_id));
				$Ywjj=$YwjjRow["Ywjj"]==""?0:$YwjjRow["Ywjj"];
				$YwRemark=$YwjjRow["Ywjj"]==""?'':$YwjjRow["Content"];
			    ...
			    $Amount=$Dx+$Gljt+$Gwjt+$Jj+$Ywjj+$Shbz+$Zsbz+$Jtbz+$taxbz-$Sb-$Kqkk-$dkfl-$RandP-$Otherkk;
		}
		...
		if ($YwRemark!='')  $remark.= $YwRemark;
		//数据写入薪资表			
		$inRecode="INSERT INTO $DataIn.cwxzsheet 		(Id,Mid,cSign,Currency,Month,BranchId,JobId,GroupId,Grade,KqSign,Number,Dx,Gljt,Gwjt,Jj,Shbz,Zsbz,Jtbz,Studybz,Housebz,Jbf,Jbjj,Yxbz,taxbz,Jz,Sb,Gjj,Ct,Ywjj,Kqkk,dkfl,RandP,Otherkk,Amount,Remark,Estate,Locks,Operator) VALUES 		
		(NULL,'0','$cSign','$Currency','$chooseMonth','$BranchId','$JobId','$GroupId','$Grade','$KqSign','$Number','$Dx','$Gljt','$Gwjt','$Jj','$Shbz','$Zsbz','$Jtbz','$Studybz','$Housebz','$Jbf','$Jbjj','$Yxbz','$taxbz','$Jz','$Sb','$Gjj','$Ct','$Ywjj','$Kqkk','$dkfl','$RandP','$Otherkk','$Amount','$remark','1','0','$Operator')";
	...
	if($myRow = mysql_fetch_array($myResult)){
		...
		$inRecode="INSERT INTO $DataIn.cwxzsheet 
			(Id,Mid,cSign,Currency,Month,BranchId,JobId,GroupId,Grade,KqSign,Number,Dx,Gljt,Gwjt,Jj,Shbz,Zsbz,Jtbz,Jbf,Jbjj,Yxbz,taxbz,Jz,Sb,Gjj,Ct,Ywjj,Kqkk,dkfl,RandP,Otherkk,Amount,Remark,Estate,Locks,Operator) VALUES 		
			(NULL,'0','$cSign','$Currency','$chooseMonth','$BranchId','$JobId','$GroupId','$Grade','$KqSign','$Number','$Dx','$Gljt','$Gwjt','$Jj','$Shbz','$Zsbz','$Jtbz','$Jbf','$Jbjj','$Yxbz','$taxbz','$Jz','$Sb','$Gjj','$Ct','$Ywjj','$Kqkk','$dkfl','$RandP','$Otherkk','$Amount','$remark','1','0','$Operator')";
			$inAction=@mysql_query($inRecode);
			$Mid=mysql_insert_id();
			if($inAction){ 
				$Log.="$Name $chooseMonth 的 $TitleSTR 成功!<br>";			
