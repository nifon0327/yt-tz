...
LEFT JOIN $DataIn.kq_office C ON  M.Number=C.Number
LEFT JOIN $DataPublic.jobdata J ON J.Id=M.JobId
LEFT JOIN $DataIn.staffgroup G ON G.GroupId=M.GroupId 
WHERE C.CheckTime LIKE '$CheckMonth%' AND (M.BranchId NOT IN ( 6, 7, 8 ) AND M.JobId NOT IN ( 10, 38 ))  AND G.Estate=1 AND M.cSign='$Login_cSign' $KqSignStr
GROUP BY C.Number 
UNION ALL 
SELECT M.Number,M.Name,J.Name AS JobName,M.BranchId,M.JobId  
FROM $DataPublic.staffmain M
...
LEFT JOIN $DataPublic.jobdata J ON J.Id=M.JobId
LEFT JOIN $DataIn.staffgroup G ON G.GroupId=M.GroupId 
WHERE (Q.StartDate  LIKE '$CheckMonth%'  OR Q.EndDate  LIKE '$CheckMonth%'  OR  (Q.StartDate<'$CheckMonth-01'  AND Q.EndDate>'$CheckMonth-01'))  AND G.Estate=1 AND M.cSign='$Login_cSign' AND M.KqSign=1 AND (M.BranchId NOT IN ( 6, 7, 8 ) AND M.JobId NOT IN ( 10, 38 )) $KqSignStr
GROUP BY Q.Number 
)  A GROUP BY A.Number 
...
if($StaffRow = mysql_fetch_array($CheckStaff)){
	...
	do{
		...
		if($Number==$NumberT){
			$StaffList.="<option value='$NumberT' selected>$JobName $NameT</option>";
			}
		else{
			$StaffList.="<option value='$NumberT' >$JobName $NameT</option>";
			}
