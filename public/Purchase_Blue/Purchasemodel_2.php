	...
	if($pi==$Counts) //说明最后一条记录情况,有了统计，则要计算统计的高度。
	{
		...
		if(($$eurTableNo)!=""){   //输出表的内容。
			...
			$tmpTableY=$pdf->GetY();
			{
				...
				$tmpTableY=$MaxContainY-$TotalH-2;
				...
				$pdf->Rect($CurMargin,$tmpTableY+6.5,5,14,"F");

				$pdf->SetFont('uGB','B',8,'','true');
				$pdf->SetTextColor(255,255,255);
				$pdf->Text($CurMargin+1,$tmpTableY+10,"声");
				$pdf->Text($CurMargin+1,$tmpTableY+18,"明");
				...
				$tmptable="<table border=0 >
					...
					<tr>
					<tr>
						<td width=90 align=left style=bold >3.此物料需满足Ash Cloud-002A/ASH cloud CPHB-001A文件要求。</td>
					<tr>				
				</table>" ;
				$pdf->transhtmltable($tmptable,0,0);  //统计
