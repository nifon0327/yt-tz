...
$mySql="SELECT S.Id,S.Month,S.StockId,S.POrderId,S.StuffId,S.Price,S.OrderQty,S.StockQty,S.AddQty,S.FactualQty,
S.BuyerId,S.InvoiceId,P.Forshort,M.Name,D.StuffCname,U.Name AS UnitName,D.TypeId,S.CompanyId,
S.AutoSign,I.InvoiceNo,I.InvoiceFile,I.Remark,I.Estate AS InvoiceEstate,ROUND((S.AddQty+S.FactualQty)*S.Price,2) AS Amount  
FROM $DataIn.cw1_fkoutsheet S 
LEFT JOIN $DataIn.trade_object P ON P.CompanyId=S.CompanyId
...
if($myRow = mysql_fetch_array($myResult)){
	...
	do{
		...
		$InvoiceEstate=$myRow["InvoiceEstate"];
		switch($InvoiceEstate){
			case 1:  $InvoiceEstate="<div class='greenB'>√</div>";break;
			case 2:  $InvoiceEstate="<div class='yellowB' onclick='setInvoiceEstate(this,\"$InvoiceNo\",\"$InvoiceId\")' style='CURSOR: pointer;'>√.</div>";break;
			default: $InvoiceEstate = "&nbsp;"; break;
		}
		...
		$ValueArray=array(
			...
			array(0=>$InvoiceEstate,1=>"align='center'"),
...
function setInvoiceEstate(e,invoiceNo,invoiceId){
	
	  var msgStr="发票编号: " + invoiceNo+" 的信息确认正确？";
	  if(confirm(msgStr)) {
	        var url="cw_fkout_updated.php?InvoiceId="+invoiceId+"&ActionId=912"; 
	        var ajax=InitAjax(); 
		    ajax.open("GET",url,true);
		    ajax.onreadystatechange =function(){
			 if(ajax.readyState==4){// && ajax.status ==200
				  if (ajax.responseText.substr(ajax.responseText.length-1, 1)=='Y'){//更新成功
