...
for($i=0;$i<count($checkid);$i++){
	...
	if ($Id!=""){
		//读取附件
		$Result = mysql_query("SELECT Caption,Attached FROM $DataPublic.aqsc15 WHERE Id='$Id' AND Locks='1' ORDER BY Id DESC LIMIT 1",$link_id);
		if ($myrow = mysql_fetch_array($Result)) {
			$Caption=$myrow["Caption"];
			$Attached=$myrow["Attached"];
			//删除数据库记录
			$Del = "DELETE FROM $DataPublic.aqsc15 WHERE Id='$Id' AND Locks='1'"; 
