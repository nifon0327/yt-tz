...
*/
$AOcolor="class='yellowB'";$test="GA-A";
if($AO<$dRestTime1){//情况1：早上签退（早退）;按签退取整后的时间计算
	//检查实到时间，签退向下取整
	$ATime=abs(strtotime(rounding_out($AO))-strtotime($dDateTimeIn))/3600;
	$test="GA-A";
	}
else{
	if($AO<=$dRestTime2){//情况：中午签退（早退）；按12点计算
		$ATime=abs(strtotime($dRestTime1)-strtotime($dDateTimeIn))/3600;
		$test="GA-B";
		}
	else{
		if($AO<$dDateTimeOut){//情况3：下午签退；按签退取整后的时间计算，需扣60分钟
			$ATime=abs(strtotime(rounding_out($AO))-strtotime($dDateTimeIn)-3600)/3600;
			$test="GA-C";
			}
		else{
			$OutEarlys=0;
			$AOcolor="";			//记录正常，不取色
			$ATime=8;			//情况4：正常签退17:00-18:00
			$test="GA-D";
