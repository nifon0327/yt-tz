...
//步骤2：
ChangeWtitle("$SubCompany 更新部门调动资料");//需处理
...
FROM $DataPublic.redeployb B,$DataPublic.staffmain M 
WHERE M.Number=B.Number AND B.Id=$Id AND B.Id=(SELECT MAX(Id) FROM $DataPublic.redeployb WHERE Number=B.Number) LIMIT 1",$link_id);
$UpInfo="";
...
<table border="0" width="<?php  echo $tableWidth?>" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF"><tr><td class='A0011'>
	<table width="750" height="143" border="0" cellspacing="5">
		...
		<tr>
	  		...
	  		<td valign="middle" scope="col"><input name='Month' type='text' id='Month' value='<?php  echo $Month?>' size='79' maxlength='7' dataType="Month" msg="月份格式不对"></td>
	 	</tr>
	  	<tr>
	 		<td height="18" align="right" scope="col">3、调出部门</td>
		...
		<tr>
	 	 	<td height="18" align="right" scope="col">4、调入部门</td>
	 	 	<td valign="middle" scope="col">
			<select name="ActionIn" id="ActionIn" style="width:430px">
			<?php 
			$inResult=mysql_query("SELECT Id,Name FROM $DataPublic.branchdata WHERE Estate=1 
								   AND (cSign=$Login_cSign OR cSign=0 ) and Id!=$ActionOut order by Id DESC",$link_id);
