...
if($mainRows = mysql_fetch_array($mainResult)){
	...
	do{
		...
		$upMian="<img location.href=\"#\"' style='CURSOR: hand' onclick='upMainData(\"zwwp_input_upmain\",$Mid)' src='../images/edit.gif' alt='更新入库主单资料' width='13' height='13'>";	
		//明细资料
			...
			if($Locks==0){//锁定状态:A一种是可以操作记录（分权限）；B一种是不可操作记录（不分权限）
				if($Keys & mLOCK){
					if($LockRemark!=""){//财务强制锁定
						$Choose="&nbsp;&nbsp;&nbsp;<img src='../images/lock.gif' alt='$LockRemark' width='9' height='15'>";
						}
					else{
						$Choose="<input name='checkid[]' type='checkbox' id='checkid$i' value='$checkidValue' disabled><img src='../images/lock.gif' width='9' height='15'>";
						}
					}
				else{		//A2：无权限对锁定记录操作
					$Choose="&nbsp;&nbsp;&nbsp;<img src='../images/lock.gif' alt='锁定操作!' width='9' height='15'>";
					}
				}
			else{	
				if(($BuyerId==$Login_P_Number && ($Keys & mUPDATE || $Keys & mDELETE)) || $Keys & mLOCK){//有权限
					if($LockRemark!=""){
						...
						$Choose="<input name='checkid[]' type='checkbox' id='checkid$i' value='$checkidValue' disabled><img src='../images/unlock.gif' width='9' height='15'/>";
						}
					}
				else{//无权限
					$Choose="&nbsp;&nbsp;&nbsp;<img src='../images/lock.gif' alt='锁定操作!' width='9' height='15'/>";
