...
$Date=date("Y-m-d");
	...
	if($_POST['ListId']){//如果指定了操作对象
		...
		for($i=0;$i<$Counts;$i++){
			...
			SELECT NULL,M.cSign,'4',M.BranchId,M.JobId,M.Number,'$newMonth','$ValidMonth','0','$cAmount',
			'$Date','1','0','$Operator','0', '0', '$Operator', NOW(), '$Operator', NOW()
			FROM $DataIn.staffmain M
			WHERE 1 and M.Number IN ($thisId)   AND M.Estate=1
			AND M.Number NOT IN (SELECT Number FROM $DataIn.sbpaysheet WHERE Month='$newMonth' and TypeId=4)";
		...
		$inRecode="INSERT INTO $DataIn.sbpaysheet 
		SELECT NULL,M.cSign,'4',M.BranchId,M.JobId,M.Number,'$newMonth','0','$cAmount','$Date','1','0','$Operator','0', '0', '$Operator', NOW(), '$Operator', NOW()
		FROM $DataIn.staffmain M
