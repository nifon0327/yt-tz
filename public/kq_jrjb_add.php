...
/*有效员工的条件：
	...
	且薪资没有结付
*/
$Result = mysql_query("SELECT M.Number,M.Name,M.cSign
FROM $DataPublic.staffmain M 
WHERE  ((M.Number NOT IN (SELECT K.Number FROM $DataPublic.redeployk K GROUP BY K.Number ORDER BY K.Id) and M.KqSign=1)
...
if($Row = mysql_fetch_array($Result)){
	do{
		...
		if($jrAmount>0){
			$Date=date("Y-m-d");
			$inRecode2="INSERT INTO $DataIn.hdjbsheet SELECT NULL,'$cSign','0',staffmain.BranchId,staffmain.JobId,staffmain.Number,'$CheckMonth','$sumXJTime','$HourlyWage2','$sumFJTime','$HourlyWage3','$jrAmount','$Date','1','1','$Operator',0,'$Operator',NOW(),'$Operator',NOW()
