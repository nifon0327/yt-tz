...
";
//现有在职员工
$PD_Sql = mysql_query("SELECT * FROM $DataPublic.staffmain WHERE 1 AND Estate=1 AND JobId>0 order by BranchId,JobId,ComeIn",$link_id);
if($PD_Row = mysql_fetch_array($PD_Sql)) {
	...
	do{
		...
		//取该员工年度工资
		$kq_Result = mysql_query("SELECT Amount,Month FROM(
		SELECT Amount,Month FROM $DataIn.hdjbsheet WHERE Number=$Number and left(Month,4)='$thisYear'
		) M order by Month
		",$link_id);
		if($kq_Row = mysql_fetch_array($kq_Result)){
			...
			$Averages=SpaceValue0($Total/12);//$hereMonth
