...
//步骤2：
ChangeWtitle("$SubCompany 开发项目查询");			//需处理
...
<TR>
				...
				<TR>
                  <TD align="right">开 发 人 员
                      <input name="Field[]" type="hidden" id="Field[]" value="Developer">
                  </TD>
                  <TD align="center">
                    <select name="fun[]" id="fun[]" style="width: 60px;">
                      <option value="=" selected>=</option>
                 ...
                 $CheckSql = mysql_query("SELECT T.Developer,M.Name FROM $CheckTb T LEFT JOIN 
                             $DataPublic.staffmain M ON M.Number=T.Developer where T.Developer!='' GROUP BY T.Developer ORDER BY T.Developer",$link_id);
                 if($CheckRow=mysql_fetch_array($CheckSql)){
	                    do{
		                     $Developer=$CheckRow["Developer"];
		                     $Name=$CheckRow["Name"];
		                     echo "<option value='$Developer'>$Name</option>";
