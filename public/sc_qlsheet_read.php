...
$sumCols="5,6,7";			//求和列,需处理
$Th_Col="选项|60|序号|30|配件ID|45|配件名称|300|单位|45|备料需求数|80|在库|80|缺料总数|80|供应商|80|采购员|80|配件分析|60";
//必选，分页默认值
...
$buyerSql = mysql_query("SELECT S.BuyerId,M.Name FROM $DataIn.yw9_blsheet B 
						...
						WHERE B.Estate=1 AND M.Name!='' GROUP BY S.BuyerId ORDER BY S.BuyerId
						",$link_id);
...
//供应商
	...
	WHERE 1 $SearchRows and B.Estate=1 AND P.Forshort!='' GROUP BY S.CompanyId ORDER BY P.Letter",$link_id);
...
$mySql="SELECT S.StuffId,A.StuffCname,SUM(S.OrderQty) AS OrderQty,K.tStockQty,A.Picture,A.TypeId,U.Name AS UnitName,P.Forshort,M.Name    
FROM $DataIn.yw9_blsheet B 
LEFT JOIN $DataIn.cg1_stocksheet S ON S.POrderId=B.POrderId 
...
if($myRow = mysql_fetch_array($myResult)){
	do{
		...
		//检查已领料数
		 $checkllQty=mysql_query("SELECT SUM(C.Qty) AS llQty FROM $DataIn.yw9_blsheet B 
                                  LEFT JOIN $DataIn.cg1_stocksheet S ON S.POrderId=B.POrderId 
								  LEFT JOIN $DataIn.ck5_llsheet C ON C.StockId=S.StockId 
								  WHERE  B.Estate=1  and C.StuffId='$StuffId' $SearchRows 
								  ",$link_id);
	    $llQty=mysql_result($checkllQty,0,"llQty");
		$noQty=$OrderQty-$tStockQty-$llQty;
	  if ($noQty>0){
		$TypeId=$myRow["TypeId"];
		...
		$toReport="<a href='stuffreport_result.php?Idtemp=$StuffId' target='_blank'>查看</a>";
			//动态读取 $thisTOrmbINo
		$showPurchaseorder="<img onClick='SCQL(StuffList$i,showtable$i,StuffList$i,\"$StuffId\",$i,\"$BuyerId\");' name='showtable$i' src='../images/showtable.gif' 
