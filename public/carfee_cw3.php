...
if($From!="slist"){
	...
	$monthResult = mysql_query("SELECT S.Date FROM $DataIn.carfee S 
...
$mySql="SELECT S.Id,S.Mid,S.Content,S.Amount,S.Bill,S.ReturnReasons,S.Date,S.Estate,S.Locks,S.Operator,T.Name AS Type,C.Symbol AS Currency,D.CarNo,E.Name AS AuditStaff,F.ListName AS FirstName,S.cSign
 	FROM $DataIn.carfee S 
	LEFT JOIN $DataIn.carfee_type T ON S.TypeId=T.Id
    LEFT JOIN $DataIn.cardata  D ON D.Id=S.CarId
	LEFT JOIN $DataIn.currencydata C ON C.Id=S.Currency
	LEFT JOIN $DataIn.staffmain E ON E.Number=S.Auditor 
	LEFT JOIN $DataIn.acfirsttype F ON F.FirstId=S.FirstId 
