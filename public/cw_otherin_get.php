...
$ColsNumber=8;			
$sumCols="5";			//求和列,需处理
$Th_Col="选项|60|序号|40|类别|110|收款日期|80|收款单号|100|收款金额|80|币种|50|收款备注|380|行政费用<br>对应款项|120|状态|50|操作人|70";
$ActioToS="1";
...
$mySql="SELECT S.Id,S.getmoneyNO, S.Amount, C.Symbol AS Currency, S.payDate, S.Remark, S.Estate, S.Locks, S.Operator,T.Name AS TypeName,H.Content,H.Bill,H.Operator AS HzOperator,H.Id AS Hid
   ...
   LEFT JOIN $DataPublic.cw4_otherintype T ON T.Id=S.TypeId
    ...
    LEFT JOIN $DataIn.hzqksheet  H ON H.OtherId=S.Id AND H.Property=1
...
if($myResult  && $myRow = mysql_fetch_array($myResult)){
	...
	do{
        ...
        $Hid=$myRow["Hid"];	
		$Bill=$myRow["Bill"];	
		$HzOperator=$myRow["HzOperator"];	$HStr="&nbsp;";
         if($Hid!=""){//该笔收入对应行政费用款项
		      $Dir=anmaIn("download/cwadminicost/",$SinkOrder,$motherSTR);
                 		   if($Bill==1){
									$Bill="H".$Hid.".jpg";
									$Bill=anmaIn($Bill,$SinkOrder,$motherSTR);
									$HStr="<span onClick='OpenOrLoad(\"$Dir\",\"$Bill\",\"\",\"Limit\")' style='CURSOR: pointer;color:#FF6633'>$Content</span>";
									}
                      else    $HStr=$Content;
              }
		$Locks=$myRow["Locks"];	
