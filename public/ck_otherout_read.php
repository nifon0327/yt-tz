...
$ColsNumber = 16;
$tableMenuS = 600;
ChangeWtitle("$SubCompany 物料出库记录");
$funFrom = "ck_otherout";
$nowWebPage = $funFrom . "_read";
$sumCols = "5,6,7,10";      //求和列,需处理
$Th_Col = "选项|40|序号|40|出库日期|70|配件|45|配件名称|250|历史<br>订单|40|在库|60|可用库存|60|出库数量|60|单价|50|单位|40|小计|60|库位|80|单据|50|出库原因|200|处理结果|200|分类|80|状态|40|操作|50";
...
//步骤4：需处理-条件选项
$SearchRows .= " AND F.OutSign=2";
if ($From != "slist") {
    $date_Result = mysql_query("SELECT F.Date FROM $DataIn.ck8_bfsheet  F WHERE 1  $SearchRows
	GROUP BY DATE_FORMAT(F.Date,'%Y-%m') ORDER BY F.Date DESC", $link_id);
    ...
    if ($TypeRow = mysql_fetch_array($Type_Result)) {
        echo "<select name='chooseType'  onchange='ResetPage(this.name)'>";
        echo "<option value='' selected>出库分类</option>";
        do {
...
echo "<select name='Pagination' id='Pagination' onchange='CencelPage()'><option value='1' $Pagination1>分页</option><option value='0' $Pagination0>不分页</option></select> <span class='ButtonH_25'  onClick='showScrapFrame(this)'>批量操作</span>
  	$CencalSstr";
...
if ($myRow = mysql_fetch_array($myResult)) {
    do {
        ...
        switch ($Estate) {
            case "1":
                $Estate = "<div class='redB'>未核</div>";
                break;
            case "0":
                $Estate = "<div class='greenB'>已核</div>";
                break;
            case "2":
...
function aiaxUpdate(sId) {
    ...
    var myurl = "ck_otherout_updated.php?StuffId=" + temprunningNum + "&DealResult=" + tempDealResult1 + "&ActionId=167" + "&Id=" + sId;
    //alert (myurl);
    var ajax = InitAjax();
...
// 显示设置出库对话框
function showScrapFrame(e) {
    openWinDialog(e, "batch_scrap.php", 405, 300, 'bottom');
}
// 批量设置出库
function doBatchScrap() {
    jQuery('.response').show();
    var scrap = jQuery('#scrap').val();
    jQuery.ajax({
        url: 'do_batch_scrap.php',
        data: {
            scrap: scrap
        },
        type: 'post',
