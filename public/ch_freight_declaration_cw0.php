...
$mainData="$DataIn.cw4_freight_declaration";
$sheetData="$DataIn.ch4_freight_declaration";
//步骤1：
...
if($TypeId==1){
    ...
    $mySql="SELECT M.PayDate,M.PayAmount,M.depotCharge AS PaydepotCharge,M.Payee,M.Receipt,M.CheckSheet,M.Remark AS PayRemark,M.Locks AS MLocks,M.declarationCharge AS Paydeclaration, M.checkCharge AS PaycheckCharge,	S.Id,S.Mid,S.Termini,S.ExpressNO,S.BoxQty,S.mcWG,S.Price,S.Amount,S.depotCharge,S.Remark,S.Estate,S.Locks,C.Date,B.Title,C.InvoiceNO,C.InvoiceFile,D.Forshort,W.forwardWG,S.checkCharge,S.declarationCharge,S.PayType,S.carryCharge,S.xyCharge,S.wfqgCharge,S.ccCharge,S.djCharge,S.stopcarCharge,S.expressCharge,S.otherCharge
	...
	$SearchRows.=" AND S.TypeId='$TypeId'";
    $mySql="SELECT M.PayDate,M.PayAmount,M.depotCharge AS PaydepotCharge,M.Payee,M.Receipt,M.CheckSheet,M.Remark AS PayRemark,M.Locks AS MLocks,M.declarationCharge AS Paydeclaration, M.checkCharge AS PaycheckCharge,	S.Id,S.Mid,S.Termini,S.ExpressNO,S.BoxQty,S.mcWG,S.Price,S.Amount,S.depotCharge,S.Remark,S.Estate,S.Locks,C.DeliveryDate AS Date,B.Title,C.DeliveryNumber AS InvoiceNO,D.Forshort,W.forwardWG,S.checkCharge,S.declarationCharge,S.PayType,S.carryCharge,S.xyCharge,S.wfqgCharge,S.ccCharge,S.djCharge,S.stopcarCharge,S.expressCharge,S.otherCharge
 	FROM $mainData M
	LEFT JOIN $sheetData S ON S.Mid=M.Id
	LEFT JOIN $DataIn.$TempTable C ON S.chId=C.Id
	LEFT JOIN $DataIn.ch3_forward W ON W.chId=M.Id
...
if($mainRows = mysql_fetch_array($mainResult)){
	...
	do{
		...
		$Id=$mainRows["Id"];
		$Termini=$mainRows["Termini"]==""?"&nbsp;":$mainRows["Termini"];
		...
		$mcWG=$mainRows["mcWG"];$forwardWG=$myRow["forwardWG"]==""?"&nbsp;":$myRow["forwardWG"];
	    ...
	    $PaydepotCharge=$mainRows["PaydepotCharge"];
		$Paydeclaration=$mainRows["Paydeclaration"];//总报关费用
		$PaycheckCharge=$mainRows["PaycheckCharge"];//总商检费用

		$PayAmount = $PayAmount + $Paydeclaration + $PaycheckCharge + $PaydepotCharge;

		$totalCharge = 0.00;
		$declarationCharge=$mainRows["declarationCharge"];
		$carryCharge=$mainRows["carryCharge"];
		$xyCharge=$mainRows["xyCharge"];
		$wfqgCharge=$mainRows["wfqgCharge"];
		$ccCharge=$mainRows["ccCharge"];
		$djCharge=$mainRows["djCharge"];
		$stopcarCharge=$mainRows["stopcarCharge"];
		$expressCharge=$mainRows["expressCharge"];
		$otherCharge=$mainRows["otherCharge"];
		...
		if($Keys & mUPDATE || $Keys & mLOCK){
			$upMian="<img location.href=\"#\"' style='CURSOR: pointer' onclick='upMainData(\"ch_freight_declaration_upmain\",$Mid)' src='../images/edit.gif' title='行号:$mRow 编号:$Mid 更新结付单资料!' width='13' height='13'>";
			}
		else{
			$upMian="&nbsp;";
			}
		...
		if($midDefault!="" && $midDefault==$Mid){//同属于一个主ID，则依然输出明细表格
			...
			$m=$m+2;
			echo"<td class='A0001' width='$Field[$m]' align='center'>$Forshort</td>";//货运公司
			$m=$m+2;

			echo"<td  class='A0001' width='$Field[$m]' align='center'>$ExpressNO</td>";//提单号码
			$m=$m+2;
			echo"<td  class='A0001' width='$Field[$m]' align='center'>&nbsp;$BoxQty</td>";//件数
			$m=$m+2;
			echo"<td  class='A0001' width='$Field[$m]' align='center'>$mcWG</td>";//称重
			$m=$m+2;
			echo"<td  class='A0001' width='$Field[$m]' align='center'>$forwardWG</td>";//称重
			$m=$m+2;
			echo"<td  class='A0001' width='$Field[$m]' align='center'>&nbsp;$Price</td>";//单价
			$m=$m+2;
			echo"<td class='A0001' width='$Field[$m]' align='center'>$Amount</td>"; //运费
			$m=$m+2;
            echo"<td class='A0001' width='$Field[$m]' align='center'>$carryCharge</td>"; //搬运费
			$m=$m+2;
			echo"<td class='A0001' width='$Field[$m]' align='center'>$declarationCharge</td>";//报关费
			$m=$m+2;
			echo"<td class='A0001' width='$Field[$m]' align='center'>$xyCharge</td>"; //续页费
			$m=$m+2;
			echo"<td class='A0001' width='$Field[$m]' align='center'>$wfqgCharge</td>";//无缝清关
			$m=$m+2;
			echo"<td class='A0001' width='$Field[$m]' align='center'>$ccCharge</td>";//仓储费
			$m=$m+2;
			echo"<td class='A0001' width='$Field[$m]' align='center'>$djCharge</td>";//登记费
			$m=$m+2;
			echo"<td class='A0001' width='$Field[$m]' align='center'>$stopcarCharge</td>";//停车费
			$m=$m+2;
			echo"<td class='A0001' width='$Field[$m]' align='center'>$expressCharge</td>";//快递费
			$m=$m+2;
			echo"<td class='A0001' width='$Field[$m]' align='center'>$otherCharge</td>";//其它费用
			$m=$m+2;
			echo"<td class='A0001' width='' align='center'>$totalCharge</td>";//总计费用
			echo"</tr></table>";
			$i++;
			}
		else{
