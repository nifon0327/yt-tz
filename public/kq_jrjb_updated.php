...
//$Str0="#总计信息\r\n#注意：本文件中的金额均以分为单位！\r\n#币种|日期|总计标志|总金额|总笔数|\r\n";
        $Str1=''; $Str2='';
		$checkSql=mysql_query("SELECT M.Name,MS.Bank,MS.Bank2,S.Amount 
		                FROM $upDataSheet S 
						LEFT JOIN $DataPublic.staffmain M ON M.Number=S.Number
						LEFT JOIN $DataPublic.staffsheet MS ON MS.Number=S.Number WHERE S.Id IN ($Ids)",$link_id);
		if($checkRow=mysql_fetch_array($checkSql)){
			...
			$file="../download/bankbill/jbf".$datetime."_1.txt";
			if(!file_exists($file)){
				$handle=fopen($file,"a");
				fwrite($handle,$Str);
				fclose($handle);
			...
			$file_2="../download/bankbill/jbf".$datetime."_2.txt";
			...
			$filename1=anmaIn("jbf".$datetime."_1.txt",$SinkOrder,$motherSTR);
			$filename2=anmaIn("jbf".$datetime."_2.txt",$SinkOrder,$motherSTR);
