...
switch($Action){
	case "1"://配件替换
		...
		$SMResult=mysql_fetch_array(mysql_query("SELECT Number FROM $DataIn.sbpaysheet  WHERE  Number=$oldNumber AND Month='$Month' AND TypeId=3 limit 1",$link_id));
	    ...
	    if ($SMNumber>0){
		   ...
		   $up_Sql = "UPDATE  $DataIn.sbpaysheet  S 
			...
			if($up_Result && mysql_affected_rows()>0){
				   ...
				   $Log.="员工ID $oldNumber 替换ID为 $newNumber 的意外险操作成功!$up_Sql";
				}
			else{
				...
				$Log.="<div class='redB'>员工ID $oldNumber 没有意外险操作记录，不能替换! </div>";
				$OperationResult="N";
			}
		break;
       case 2:
