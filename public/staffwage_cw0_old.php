...
S.Dx,S.Gljt,S.Gwjt,S.Jj,S.Jbjj,S.Ywjj,S.Shbz,S.Zsbz,S.Jtbz,S.Jbf,S.Yxbz,S.taxbz,S.Jz,S.Sb,S.Kqkk,S.dkfl,S.RandP,S.Otherkk,S.Amount,S.Estate,S.Locks,S.Gjj,S.Ct,
	P.Name,P.ComeIn,P.Estate AS mEsate,B.Name AS Branch,J.Name AS Job,BK.Title,F.Symbol AS Currency
FROM $mainData M
...
LEFT JOIN $DataPublic.currencydata F ON F.Id=S.Currency
WHERE 1 $SearchRows
...
if($mainRows = mysql_fetch_array($mainResult)){
	...
	do{
		...
		$JRJBRow=mysql_fetch_array(mysql_query("SELECT IFNULL(SUM(Amount),0) AS JRJB FROM ptsub.hdjbsheet WHERE Number='$Number' AND Month='$Month'",$link_id));
		$JRJB=$JRJBRow["JRJB"];
		$chooseMonth=$Month;
		...
		if($midDefault!="" && $midDefault==$Mid){//同属于一个主ID，则依然输出明细表格
			...
			$m=$m+2;
			echo"<td  class='A0001' width='$Field[$m]' align='center'>$Jbjj</td>";
			$m=$m+2;
