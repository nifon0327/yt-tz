...
$sumCols="6,7,8";		//求和列
$From=$From==""?"m":$From;
ChangeWtitle("$SubCompany 报废记录审核列表");
$funFrom="ck_bf";
$Th_Col="选项|40|序号|40|报废日期|70|配件|45|配件名称|250|历史<br>订单|40|在库|60|可用库存|60|报废数量|60|单价|50|单位|45|小计|60|单据|50|报废原因|320|分类|65|状态|50|操作|50";
...
if($From!="slist"){
	$SearchRows="";
	$SearchRows=" and F.Estate=1 AND F.OutSign =1";
...
WHERE 1 $SearchRows  AND F.OutSign =1 ORDER BY F.Id DESC";
//echo $mySql;
...
if($myRow = mysql_fetch_array($myResult)){
	do{
        ...
        if($Qty >$tStockQty || $Qty>$oStockQty){
	        ...
	        $LockRemark = "库存不足，不能报废，请相关人员修改报废数量";
        }else{
