...
if($inAction && mysql_affected_rows()>0){
       ...
       for($i=0;$i<$Count;$i++){
	        ...
	        $StockId=0;//采购单流水号
	        $StuffId=0;	//配件ID
			$StuffName=$valueTemp[0];
	        $Price=$valueTemp[1];//单价
			$Qty=$valueTemp[2];	//数量
			$SheetRemark=$valueTemp[3];//扣款原因
	        $Amount=sprintf("%.2f",$Qty*$Price);
	        $addRecodes="INSERT INTO $DataIn.cw15_gyskksheet (Id, Mid, PurchaseID, StockId, StuffId, StuffName,Qty, Price, Amount,Remark,GoodsId,Kid) VALUES (NULL,'$Mid','$PurchaseID','$StockId','$StuffId','$StuffName','$Qty','$Price','$Amount','$SheetRemark','0','0')";
