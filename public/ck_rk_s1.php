...
//步骤2：需处理
$Th_Col="采购单号|60|采购|50|供应商|80|选项|40|序号|40|需求流水号|90|配件名称|190|英文代码|120|需求数|60|增购数|60|实购数|60|未收数|60";
$ColsNumber=16;
$tableMenuS=600;
...
$sSearch.=$Jid==""?"":" and P.CompanyId='$Jid'";
$sSearch.=$Bid==""?"":" and B.Number='$Bid'";
...
$mySql="SELECT M.PurchaseID,M.Date,M.Id,S.StuffId,S.StockId,S.AddQty,S.FactualQty,P.Forshort,B.Name,D.StuffCname,D.StuffEname
	...
	WHERE 1 and S.rkSign>0 and S.Mid>0 $sSearch 
	";
//echo $mySql;
	...
	do{
		...
		if($StuffId!=""){			
			$StuffCname=$mainRows["StuffCname"];
                        $StuffCname=strtr($StuffCname,"'"," "); 
     		$StuffEname=$mainRows["StuffEname"];       
			...
			$ValueSTR="$StockId^^$StuffId^^$StuffCname^^$FactualQty^^$AddQty^^$CountQty^^$Unreceive";
			if($Unreceive==0){
				$chooseStr="&nbsp;";
				}
			else{
			...
			if($midDefault!="" && $midDefault==$Mid){//同属于一个主ID，则依然输出明细表格
				...
				$m=$m+2;
				echo"<td width='$Field[$m]'>$StuffEname</td>";
				$m=$m+2;
