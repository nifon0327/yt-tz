...
$SealCompanyStr="上海研砼治筑建筑科技有限公司";
$SealCompanyId=$SealCompanyId==""?1:$SealCompanyId;
if($SealCompanyId==2){
	$SealCompanyStr="上海研砼治筑建筑科技有限公司";
...
LEFT JOIN $DataIn.staffmain S ON M.BuyerId=S.Number WHERE M.Id='$Id' ",$link_id);
...
if($myRow = mysql_fetch_array($Result)){
	...
	do{
		...
		$FilePath="../download/PurchasePDF/";
		if(!file_exists($FilePath)){
		    dir_mkdir($FilePath);
		}

		@$barcodeFile=$FilePath.$CodeTemp.'.jpg';
...
if($InvoiceTax>0){
	//include "PurchaseToPDF_tax.php";  //含税PDF
}
...
$PurchaseFile="<a href=\"$donwloadFileaddress?d=$tmpd&f=$PurchaseFile&Type=&Action=6\" target=\"download\" ><img src='../images/down.gif' style='background:#F00' alt='下载采购单(PDF)' width='18' height='18'>下载采购单</a>";
echo "$PurchaseFile &nbsp; &nbsp; $taxPurchaseFile ";
