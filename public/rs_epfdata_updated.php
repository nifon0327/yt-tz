...
//步骤2：
$Log_Item="公积金资料";		//需处理
$upDataSheet="$DataPublic.epfdata";	//需处理
$Log_Funtion="更新";
...
switch($ActionId){
	...
	default:
		...
		$payResult =mysql_query("
			...
			UNION 
			SELECT MIN(Month) AS minM,MAX(Month) AS minT FROM $DataOut.sbpaysheet WHERE 1 AND Number=$Number) M
			",$link_id);
		*/
		$payResult =mysql_query("
			SELECT MIN(minM) AS minTemp,MAX(minT) AS maxTemp FROM(
			SELECT MIN(Month) AS minM,MAX(Month) AS minT FROM $DataIn.sbpaysheet WHERE 1 AND TypeId=2 AND Number=$Number
			) M
			",$link_id);
		...
		if($Log==""){
			$Sql = "UPDATE $DataPublic.epfdata SET Type='$Type',Locks='0',Note='$Note',Date='$Date',Operator='$Operator' $eMonthSTR  $sMonthSTR WHERE Id=$Id and Number=$Number LIMIT 1";
