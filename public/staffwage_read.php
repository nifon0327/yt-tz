...
$funFrom="staffwage";
if($From!="slist"){
	$sumCols="22,23,24,25,26,27";		//求和列
	$Th_Col="选项|40|序号|30|分类|30|所属公司|60|工作</br>地点|40|部门|50|职位|50|小组|60|员工<br>姓名|50|工龄<br>Y(M)|40|支付<br>货币|40|底薪|40|加班费|40|工龄<br>津贴|40|岗位<br>津贴|40|生活/住宿<br>补助|60|交通<br>补助|40|就学<br>补助|40|购房<br>补助|40|绩效<br>奖金|40|其他<br>奖金|40|考勤<br>扣款|40|取消<br>津贴|40|小计|55|社保|50|公积金|50|个税|40|实付|55|状态|40|备注|40";
	$swidth=270;
...
else
{
	...
	$sumCols="23,24,25,26,27,28";		//求和列
	$Th_Col="选项|40|序号|30|分类|30|所属公司|60|月份|60|工作</br>地点|40|部门|50|职位|50|小组|60|员工<br>姓名|50|工龄<br>Y(M)|40|支付<br>货币|40|底薪|40|加班费|40|工龄<br>津贴|40|岗位<br>津贴|40|生活/住宿<br>补助|60|交通<br>补助|40|就学<br>补助|40|购房<br>补助|40|绩效<br>奖金|40|其他<br>奖金|40|考勤<br>扣款|40|取消<br>津贴|40|小计|55|社保|50|公积金|50|个税|40|实付|55|状态|40|备注|40";
	$swidth=330;
...
//步骤6：需处理数据记录处理
//初始化
$sumDx=$sumGljt=$sumGwjt=$sumJj=$sumShbz=$sumZsbz=$sumJtbz=$sumStudybz=$sumHousebz=$sumJbf=$sumYxbz=$sumtaxbz=$sumJz=$sumSb=$sumGjj=$sumCt=$sumKqkk=$sumdkfl=$sumRandP=$sumOtherkk=$sumAmount=$sumTotal=0;$sumJbjj=0;$sumYwjj=0;
$i=1;
...
$mySql="SELECT 
S.Id,S.Month,S.KqSign,S.Number,S.Dx,S.Gljt,S.Gwjt,S.Jj,S.Jbjj,S.Ywjj,S.Shbz,S.Zsbz,S.Jtbz,S.Studybz,S.Housebz,
S.Jbf,S.Yxbz,S.taxbz,S.Jz,S.Sb,S.Gjj,S.Ct,
S.Kqkk,S.dkfl,S.RandP,S.Otherkk,S.Amount,S.Estate,S.Remark,S.Locks,M.Name,M.ComeIn,M.Estate AS mEsate,M.WorkAdd,M.Id As PID,B.Name AS Branch,J.Name AS Job,M.GroupId,E.GroupName,S.Currency,C.Symbol,C.Rate,S.cSign  
...
if($myRow = mysql_fetch_array($myResult)){
	do{
		...
		// $reduceMoney = 0;
		// if($APP_FACTORY_CHECK && $KqSign == '1' && $chooseMonth == '2015-10'){
		...
		$checkResult = mysql_query("SELECT Amount FROM $DataIn.hdjbsheet WHERE Number='$Number' and Month='$SMonth'",$link_id);
		//echo "SELECT Amount FROM $DataIn.hdjbsheet WHERE Number=$Number and Month='$SMonth' <br>"; 
		if($checkRow = mysql_fetch_array($checkResult)){
			$Jbjj=sprintf("%.0f",$checkRow["Amount"]);
	    }
		
		$Total=$Dx+$Jbf+$Gljt+$Gwjt+$Jj+$Ywjj+$Shbz+$Jtbz+$Studybz+$Housebz+$Yxbz+$taxbz-$Kqkk-$dkfl;//+$Jbjj
		if ($SMonth<'2015-09'){$Total+=$Jbjj;}
		...
		$sumDx+=$Dx*$Rate;			$sumGljt+=$Gljt*$Rate;		$sumGwjt+=$Gwjt*$Rate;		$sumJj+=$Jj*$Rate;  
		$sumJbjj+=$Jbjj*$Rate;
		...
		if($From!="slist"){	
			...
			$ValueArray=array(
				...
				array(0=>$Jj,			1=>"align='center'"),
				array(0=>$Ywjj,			1=>"align='center'"),
				array(0=>"<div class='redB'>".$Kqkk."</div>A", 1=>"align='center'"),
				array(0=>"<div class='redB'>".$dkfl."</div>", 1=>"align='center'"),
				array(0=>$Total, 1=>"align='center'"),
