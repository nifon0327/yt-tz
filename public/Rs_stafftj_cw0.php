...
二合一已更新
*/
$mainData="$DataIn.cw17_tjmain";
$sheetData="$DataIn.cw17_tjsheet";
$Pagination=$Pagination==""?0:$Pagination;	//默认分页方式:1分页，0不分页
$Page_Size = 100;							//每页默认记录数量	
//步骤1：
include "../model/subprogram/cw0_model1.php";
$mySql="SELECT 
	M.PayDate,M.PayAmount,M.Payee,M.Receipt,M.Checksheet,M.Remark AS PayRemark,M.Locks AS MLocks,P.ComeIn,S.CheckT,S.HG,
	S.Id,S.Mid,S.Number,S.Month,S.Amount,S.Date,S.Estate,S.Locks,P.Name,B.Title,S.Remark,BD.Name AS BranchName,J.Name AS JobName,S.Attached ,S.tjType,M.cSign
	FROM $mainData M
...
$GDnumber= array("①","①","②","③","④","⑤","⑥","⑦","⑧","⑨","⑩");
	do{
		...
		if($Keys & mUPDATE || $Keys & mLOCK){
			$upMian="<img location.href=\"#\"' style='CURSOR: hand' onclick='upMainData(\"rs_stafftj_upmain\",$Mid)' src='../images/edit.gif' title='更新结付单资料!' width='13' height='13'>";	
			}
		else{
			$upMian="&nbsp;";
			}
		...
		if($midDefault!="" && $midDefault==$Mid){//同属于一个主ID，则依然输出明细表格
			...
			$m=$m+2;
			echo"<td class='A0001' width='$Field[$m]' align='center'>$tjType</td>";
			$m=$m+2;
			...
			$m=$m+2;
			echo"<td class='A0001' width='$Field[$m]' align='center'>$HG</td>";
			$m=$m+2;
