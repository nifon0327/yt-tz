...
<tr><td colspan='27' class='A0011'>当天是：$weekInfo (直落工时的薪酬计算与加班工时一致)<span style='color:#ff0033'>(Number号红色为数据已作修改,黄色为数据未保存)</span></td></tr>";
	...
	$mySql="SELECT M.Number,M.Name,J.Name AS Job,G.GroupName FROM $DataIn.staffmain M LEFT JOIN $DataIn.jobdata J ON J.Id=M.JobId 
...
//echo $mySql;
	...
	while($sqlRows = mysql_fetch_assoc($result)){
		...
		$NumberT=$number;
			...
			if(skipStaff($NumberT) && $APP_FACTORY_CHECK==true){
				continue;
		}

		$staff = new AttendanceAvatar($number, $DataIn, $DataIn, $link_id);
		...
		$workDayOverTime = $dayAttendanceInfomation["workOtHours"] + $dayAttendanceInfomation["workZlHours"];
		...
		if($staff->hasZlhours != 'N'){
			$zlInfomation = explode('|', $staff->hasZlhours);
			$ZL_Hours = $zlInfomation[1];
			switch ($zlInfomation[0]) {
				case 'G':
					$hasZlStateG = " title='包括直落：$ZL_Hours 工时'  style='CURSOR: pointer;color: #009900;font-weight: bold;' ";
				break;
				case 'X';
					$hasZlStateX = " title='包括直落：$ZL_Hours 工时'  style='CURSOR: pointer;color: #009900;font-weight: bold;' ";
				break;
				case 'F':
					$hasZlStateH = " title='包括直落：$ZL_Hours 工时'  style='CURSOR: pointer;color: #009900;font-weight: bold;' ";
				break;
			}
		}
		...
		$weekDayOverTime = $dayAttendanceInfomation["weekOtTime"] + $dayAttendanceInfomation["weekZlHours"];
		...
		$holidayOverTime = $dayAttendanceInfomation["holidayOtHours"] + $dayAttendanceInfomation["holidayZlHours"];
