...
$sumCols="";		//求和列
$From=$From==""?"read":$From;
ChangeWtitle("$SubCompany 配件图存档");
$funFrom="stufffile";
$Th_Col="选项|40|序号|40|图档分类|80|图档说明|220|图档下载|80|供应商|80|配件类型|80|状态|40|更新日期|70|操作|60";
//必选，分页默认值
...
if($From!="slist"){
	...
	$result =  mysql_query("SELECT S.FileType,T.Name 
			FROM $DataIn.doc_stuffdrawing S 
			LEFT JOIN $DataPublic.doc_type T ON T.Id=S.FileType
			WHERE 1 AND T.Id IN (3,4) GROUP BY S.FileType ORDER BY T.Id",$link_id);
	...
	//添加客户检索选项
	$result =  mysql_query("
			SELECT S.CompanyId,C.Forshort FROM $DataIn.doc_stuffdrawing S 
	...
	if($myrow = mysql_fetch_array($result)){
			...
			do{
				...
				if ($Company==$ComId){
					...
					$SearchRows.=" AND S.CompanyId='$ComId'";
					}
				else{
...
$mySql="SELECT S.Id,S.FileRemark,S.FileName,S.Estate,S.Locks,S.Date,S.Operator,
T.Name AS FileType,C.Forshort AS Company,P.TypeName
FROM $DataIn.doc_stuffdrawing S 
LEFT JOIN $DataPublic.doc_type T ON T.Id=S.FileType 
LEFT JOIN $DataIn.trade_object C ON C.CompanyId=S.CompanyId
LEFT JOIN $DataIn.stufftype P ON P.TypeId=S.StuffType
WHERE 1 $SearchRows 
ORDER BY S.Id ASC";
$myResult = mysql_query($mySql." $PageSTR",$link_id);
if($myRow = mysql_fetch_array($myResult)){
	$d=anmaIn("download/stuffdrawing/",$SinkOrder,$motherSTR);
	do{
		$m=1;
		$Id=$myRow["Id"];
