...
$tableMenuS=600;
$sumCols="15";		//求和列
ChangeWtitle("$SubCompany 员工工伤报销费用");
$funFrom="Rs_staffHurt";
$nowWebPage=$funFrom."_read";
$Th_Col="选项|40|序号|40|所属公司|60|员工姓名|60|部门|60|职位|60|入职日期|70|总金额|60|备注|250|工伤日期|80|工伤凭证|30|社保凭证|30|社保金额(公)|60|社保金额(私)|60|费用凭证|30|实报金额|60|结付状态|60|登记日期|70|操作员|60";
...
if($From!="slist"){
   ...
   $cSignTB="S";$SelectFrom=5;
	...
	$date_Result = mysql_query("SELECT Month FROM $DataIn.cw18_workhurtsheet WHERE 1 GROUP BY Month order by Month DESC",$link_id);
...
$mySql="SELECT S.Id,S.cSign,S.Number,S.Month,S.Amount,S.SSecurityAmout,S.PSecurityAmout,S.AllAmout,
S.Locks,S.Date,S.Operator,S.Estate,S.Mid,P.Name,B.Name AS BranchName,J.Name AS JobName,S.Remark,S.Attached ,P.ComeIn,S.CheckT,S.HurtDate,S.HostpitalInvoice,S.SSecurityInvoice
...
//$GDnumber= array("①","①","②","③","④","⑤","⑥","⑦","⑧","⑨","⑩");
	do{
			...
			$PSecurityAmout=$myRow["PSecurityAmout"];  //社保局支付费用
		    ...
		    include"../model/subselect/cSign.php";
 			$Estate=$myRow["Estate"];
			if ($Amount ==0){
				$Estate="<span class='greenB'>无须支付</span>";
		...
		$ValueArray=array(
			...
			array(0=>$PSecurityAmout,	1=>"align='right'"),
