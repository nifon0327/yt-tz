...
//步骤2：
ChangeWtitle("$SubCompany 更新维护人员");//需处理
$nowWebPage =$funFrom."_upUser";	
...
$Parameter="fromWebPage,$fromWebPage,funFrom,$funFrom,From,$From,CompanyId,$CompanyId,Id,$Id,ActionId,$ActionId,OMTypeId,$OMTypeId,TypeId,$TypeId,BranchId,$BranchId,UserMCID,$UserMCID";
...
?>
    ...
    <table width='<?php  echo $tableWidth?>' border="0" cellspacing="0" bgcolor="#FFFFFF">
		...
		<tr class="">
            ...
            <td scope="col" align="right">维护人：</td>
            <td scope="col">
             <input name="UserName" type="text" id="UserName" style="width:200px;"  dataType="Require"   msg="未填写" >
              <!--  <input type="button" name="Submit" value="..." title="维护人" onclick="SearchData(1,0)">-->
		...
		<tr>
			<td width="10" class="A0010">&nbsp;</td>
			<td height="25" colspan="2" valign="bottom"><span class="redB">◆历史维护人明细</span></td>
			<td width="10" class="A0001">&nbsp;</td>
		</tr>
	</table>
    <!--
	...
	<table width='<?php  echo $tableWidth?>' border="0" cellspacing="0" bgcolor="#FFFFFF">
		...
		<div style="width:100%;height:100%;overflow-x:hidden;overflow-y:auto">
                ...
                <table width='100%' cellpadding="0" cellspacing="0" bgcolor="#FFFFFF" >
                  <tr>  
                    ...
                    <td width="80" class="A0001" align="center">维护人</td>
		...
		//子分类列表
		/*
		$StockResult = mysql_query("SELECT S.Name,F.MCId,F.SDate as Date,F.Remark,F.Operator FROM $DataPublic.fixed_userdata F
								    left join $DataPublic.staffmain S ON S.Number=F.User
								   	where F.Mid='$Id' AND F.UserType=2 order by F.SDate",$link_id);
		*/
...
include "../model/subprogram/add_model_b.php";
  $StaffSql = mysql_query("SELECT M.Id,M.Number,M.Name, B.Name AS Branch
	...
	WHERE 1 AND M.Estate='1' AND M.cSign=$cSign ORDER BY M.BranchId,M.JobId,M.ComeIn,M.Number",$link_id);
	...
	if(TableTemp==ListTable){	//新增需求单列表
		...
		if (message==true){
			//输入删除原因
				...
				myurl="fixed_assets_updated.php?SubName="+delSubName+"&ActionId=delMaintainer";
				/*
	...
	if(TableTemp==ListTable){	//新增需求单列表
		...
		if ((BackData!=null) &&  (BackData!="") ){
			//输入删除原因
				...
				myurl="fixed_assets_updated.php?SubName="+BackData+"&Id="+SubId+"&ActionId=UpdateSubMaintainer";
				/*
				retCode=openUrl(myurl);
				if (retCode!=-2){//标记删除成功，不直接删除需求单，而是做标记
					//TableTemp.deleteRow(rowIndex);
...
function CheckForm(){
	...
	document.form1.action="fixed_assets_updated.php"+"&ActionId=AddMaintainer";
