...
for ($pi=1;$pi<=$Counts;$pi=$pi+1){  //循环输出
	$eurTableNo="eurTableNo".strval($pi);
     if($addSign==0){  //新加一页
		...
		$pageNo=$pageNo+1;
		$pdf->FloorSign=0;		
		if($CompanyId=='1075'){//卡登仕
		     $Com_Pack_PI="出 货 单";
		     include "invoicetopdf/invoicepublicHeaderS_1075.php";  //页头
		    }
		else{
		     $Com_Pack_PI="COMMERCIAL INVOICE";
		     include "cw_invoicetopdf/invoicepublicHeaderS.php";  //页头
		    }
	...
	*/
	...
	if($pi==$Counts) //说明最后一条记录情况,有了统计，则要计算统计的高度。
	{
		...
		if(($$eurTableNo)!=""){   //输出表的内容。
			...
			$$eurTableNo="<table  border=1 > <tr>
				...
				<td  width=38 bgcolor='#999999' >小计</td>
			  ...
			  <tr>
				<td  width=38 bgcolor='#999999'>合计</td>
				<td  width=19 align='right'>$Total</td>
			  </tr>
			   <tr>
			  ...
			  <tr  >
			  <td  height=8 colspan=3 align='left' valign=middle >币种  :$Symbol</td>
			  </tr>
			  ...
			  */
				...
				$tmp_str="Notes:";
				include "cw_invoicetopdf/invoicepublicText.php";  //英文输出
			...
			$pdf->htmltable($tmp_Total);
			 ...
			 if($CompanyId=='1075'){//卡登仕
				$pdf->SetXY($CurMargin+0.4,$NowSY+2.6);
				$pdf->Text($CurMargin+0.4,$NowSY+2.6,"付款方式:"); 

				$pdf->SetXY($CurMargin,$NowSY);				
				$pdf->SetFont('uGB','',$InvoiceHeadFontSize); //设回字体大小
			    $tmp_Terms="<table  border=1 >
						<tr>
							<td $colspan  $width  height=17  align='left' valign='top'>&nbsp;        <br>      下单前30%定金,出货后7天付70%余款</td>
						</tr>
						</table>";	
				...
				$NowSY=$pdf->GetY(); 
				$pdf->SetFont('uGB','B',$InvoiceHeadFontSize); //设回字体大小
				$pdf->Text($CurMargin+1,$NowSY+5,"币  种    :"); 
				$pdf->SetXY($CurMargin,$NowSY);
				$pdf->SetFont('uGB','',$InvoiceHeadFontSize); //设回字体大小
			...
			*/
			...
			$tmp_BANK="<table  border=1 >
			  <tr>
				<td $colspan  $width   align='left' height=30  valign=middle >&nbsp;<br>Beneficary: $Beneficary<br>Bank      : $Bank<br>Bank Add  : $BankAdd<br>Swift ID  : $SwiftID<br>A/C NO    : $ACNO</td>
			  </tr>
			</table>";
	...
	if($addSign==0)		
	{
		include "cw_invoicetopdf/invoicepublicFooterS.php";  //页尾  
	...
	if($isSeal==1){
		$sealX=$pdf->GetX()+130;
		$sealY=$pdf->GetY()-36;
		//include "cw_invoicetopdf/invoicepublicSeal.php";  //封印
	 }
}
	...
	$eurplNo="plTableNo".strval($pi); 
     if($addSign==0){  //新加一页
		...
		//表标题
		{
			...
			<td width=25 align=center valign=middle style=bold>PO#</td>
			<td width=35 align=center valign=middle style=bold>产品代码</td>
			<td width=38 align=center valign=middle style=bold>产品描述</td>
			<td width=15 align=center valign=middle style=bold>数量/箱</td>	
			<td width=25 align=center valign=middle style=bold>外箱尺寸(CM)</td>
			<td width=15 align=center valign=middle style=bold>合计</td>
			<td width=13 align=center valign=middle style=bold>净重KG</td>
