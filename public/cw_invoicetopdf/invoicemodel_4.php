...
for ($pi=1;$pi<=$Counts;$pi=$pi+1){  //循环输出
	$eurTableNo="eurTableNo".strval($pi);
     if($addSign==0){  //新加一页
		...
		include "cw_invoicetopdf/invoicepublicHeader.php";  //页头
	...
	*/
	...
	if($addSign==0)		
	{
		include "cw_invoicetopdf/invoicepublicFooter.php";  //页尾  
	...
	if($isSeal==1){
		//$sealX=$pdf->GetX()+130;
		//$sealY=$pdf->GetY()-36;
		//$pdf->Text(0,$sealY,"4:$sealY");  //输出当前值
...
$eurEmptyField="<table  border=1 >
<tr >
<td width=16 align=center height=$RowsHight valign=middle style=bold> </td>	
	...
	$eurplNo="eurplNo".strval($pi);   //每一条记录都是一个表格
     if($addSign==0){  //新加一页
		...
		//表标题
		{
			...
			<td width=16 align=center height=$RowsHight valign=middle style=bold>Ln.</td>
			<td width=98 align=center valign=middle style=bold>Item</td>
			<td width=15 align=center valign=middle style=bold>Unit/<br>carton</td>
			...
			<td width=13 align=center valign=middle style=bold>GW (KG)</td>
			</tr></table>" ;//$eurTableList;
			$pdf->htmltable($eurTableField);
