...
for ($pi=1;$pi<=$Counts;$pi=$pi+1){  //循环输出
	$eurTableNo="eurTableNo".strval($pi);
     if($addSign==0){  //新加一页
		...
		include "cw_invoicetopdf/invoicepublicHeader.php";  //页头
		{
			...
			<td width=20  bgcolor=#CCCCCC align=center valign=middle style=bold>Shipment:</td>			
	...
	if($pi==$Counts) //说明最后一条记录情况,有了统计，则要计算统计的高度。
	{
		...
		if(($$eurTableNo)!=""){   //输出表的内容。
			...
			*/
			/*	$pdf->SetFont('Arial','B',$InvoiceHeadFontSize); //设回字体大小
				$pdf->Text($CurMargin+160,$NowSY+3.5,"Currency:"); 
				$pdf->SetFont('Arial','',$InvoiceHeadFontSize); //设回字体大小
...
$pdf->Image($mc_officialseal,$pdf->GetX()+130,$pdf->GetY()-54,0,0,"JPG");	
*/
?>