...
$ColsNumber=17;
$tableMenuS=500;
ChangeWtitle("$SubCompany 未做检验标准图");
$funFrom="productdata_ts";
$From=$From==""?"ajax":$From;
...
//步骤4：需处理-可选条件下拉框
if($TestStandard==1){
       $StandardStr=" AND P.TestStandard!=$TestStandard";
      }
else{
       $StandardStr=" AND P.TestStandard=$TestStandard";
    }
if($From!="slist"){
	...
	WHERE 1 AND Y.Estate>0 AND C.Estate=1  $StandardStr
	...
	WHERE 1 AND Y.Estate>0 AND T.Estate=1 and P.Estate=1 $StandardStr   $SearchRows GROUP BY P.TypeId ORDER BY T.TypeId
	",$link_id);
	if($myrow = mysql_fetch_array($result)){
...
$mySql= "SELECT P.Id,P.ProductId,P.cName,P.eCode,P.Price,P.Unit,P.Moq,P.CompanyId,P.Description,P.Remark,P.pRemark,
	P.TestStandard,P.Date,P.PackingUnit,S.Estate,P.Locks,P.Code,P.Operator,T.TypeName
	FROM $DataIn.yw1_ordersheet Y 
    LEFT JOIN $DataIn.productdata P ON Y.ProductId=P.ProductId 
	LEFT JOIN $DataIn.producttype T ON T.TypeId=P.TypeId
    LEFT JOIN $DataIn.productstandimg S ON S.ProductId=P.ProductId 
	where 1 AND Y.Estate>0 $SearchRows  $StandardStr GROUP BY Y.ProductId ORDER BY  Estate DESC,Id DESC";
//echo $mySql;
...
if($myResult && $myRow = mysql_fetch_array($myResult)){
	do{
		...
		/*
		$Remark2 = "";
		$remarksql=mysql_query("SELECT ProductId,Remark
						FROM $DataIn.test_remark 
						WHERE ProductId= '$ProductId'",$link_id);
		
		
		if($remarkRow=mysql_fetch_array($remarksql)){
			do{
			    $ProductIdRm=$remarkRow["ProductId"];
				$RemarkResult=$remarkRow["Remark"];
				$Remark2="<a href='productRemark.php?f=$ProductIdRm&Remark=$RemarkResult' target='_blank'>查看</a>";
		...
		*/
		...
		if($StuffmyRow=mysql_fetch_array($StuffResult)) {//如果设定了产品配件关系
			...
			/*	array(0=>$PackingUnit,		1=>"align='center'"),
				...
				//array(0=>$Remark2,			1=>"align='center'")
				);
		$checkidValue=$Id;
