...
$ColsNumber=8;				
$tableMenuS=550;
ChangeWtitle("$SubCompany 配件QC检验标准图");
$funFrom="stuff_qcstandard";
$nowWebPage=$funFrom."_read";
$Th_Col="选项|60|序号|30|所属类别|80|标准说明|300|连接配件数量|80|内容|50|有效状态|60|更新日期|80|操作员|50";
...
if($myRow = mysql_fetch_array($myResult)){
	do{
		...
		if($Estate==2){
			...
			$EstateSTR="查阅";$ClassColor="yellowB";}
		...
		if ($IsType==1){
			...
			$QtyResult =mysql_fetch_array(mysql_query("SELECT count(*) as Qty FROM $DataIn.stuffqcimg  Q
LEFT JOIN $DataIn.stuffdata D ON D.StuffId=Q.StuffId
WHERE 1 AND D.Estate=1 AND  Q.QcId='$Id'",$link_id));
		...
		//动态读取
		$showPurchaseorder="<img onClick='ShowQC(QCList$i,showtable$i,QCList$i,\"$Id|$TypeId|$IsType\",$i);' name='showtable$i' src='../images/showtable.gif' alt='显示此QC标准图的产品.' width='13' height='13' style='CURSOR: pointer'>";
		$QCTB="
...
function ShowQC(e,f,Order_Rows,QCId,RowId){
	...
	if (yy.indexOf("showtable")==-1){
		...
		//动态加入采购明细
		if(QCId!=""){			
			var url="../public/stuff_qcstandard_ajax.php?QCId="+QCId+"&RowId="+RowId; 
