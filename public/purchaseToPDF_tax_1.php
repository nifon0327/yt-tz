...
//include "../Admin/invoicetopdf_blue/config.php";  //字体颜色，行高等配置文
...
LEFT JOIN $DataIn.providersheet X ON X.CompanyId=M.CompanyId
...
$CompanyShortName=$_SESSION["Login_cSign"]==3?"鼠 宝":"黑 云";
...
if($myRow = mysql_fetch_array($Result)){
	...
	do{
		...
		/*if($InvoiceTax>0){
			$taxThisAmount=$Qty*$Price;
			$taxThisAmount=$taxThisAmount+$taxThisAmount*$InvoiceTax/100;
			$Price=$taxThisAmount/$Qty;
...
$filename="../download/PurchasePDF/".$PurchaseID."_tax.pdf";
if(file_exists($filename)){unlink($filename);}

include "Purchase_Blue/Purchasemodel_2.php";

$pdf->Output("$filename","F");
//$Log.="<br>$PurchaseID 采购单重置完成!";
//echo "<br>$PurchaseID 采购单重置完成!";

			$PurchaseFile=$PurchaseID.'_tax.PDF';
			...
			$taxPurchaseFile="<a href=\"$donwloadFileaddress?d=$tmpd&f=$PurchaseFile&Type=&Action=6\" target=\"download\" ><img src='../images/down.gif' style='background:#F00' alt='下载采购单(含税PDF)' width='18' height='18'>下载采购单</a>";
			//echo "$PurchaseFile";
