...
$tableMenuS=600;
$sumCols="8,9,10";		//求和列
ChangeWtitle("$SubCompany 商业险缴费记录");
$funFrom="Rs_Commer";
$nowWebPage=$funFrom."_read";
$Th_Col="选项|40|序号|40|所属公司|60|类型|60|员工姓名|60|部门|60|职位|60|缴费月份|70|个人缴费|60|公司缴费|60|小计|60|结付|80|单据|60|登记日期|100|操作员|80";
...
$mySql="SELECT 
	S.Id,S.BranchId,S.JobId,S.Number,S.Month,S.mAmount,S.cAmount,S.Locks,S.Date,S.Operator,S.Estate,S.Mid,M.Name,S.TypeId,P.Picture,M.Estate AS mEstate,S.ValidMonth,S.cSign
FROM $DataIn.sbpaysheet S
LEFT JOIN $DataIn.rs_casualty_picture P ON P.Mid=S.Id
LEFT JOIN $DataIn.staffmain M ON S.Number=M.Number
WHERE 1 $SearchRows ORDER by S.id  "; //ORDER BY convert(M.Name using gbk) asc ";
//echo $mySql;
...
WHERE R.NewNumber='$Number' AND R.Month='$chooseMonth'",$link_id));
        ...
        if($mEstate!=1)$Name= "<div class='redB' >$Name</div>";
        $Picture=$myRow["Picture"];
        if($Picture!=""){
		    $Dir=anmaIn("download/Casualty/",$SinkOrder,$motherSTR);
			$Bill=anmaIn($Picture,$SinkOrder,$motherSTR);
			$Picture="<span onClick='OpenOrLoad(\"$Dir\",\"$Bill\",\"\",\"Limit\")'  style='CURSOR: hand;color:#FF6633'>View</span>";
            }
       else $Picture="&nbsp;";
		$ValueArray=array(
