<?php 
//电信-zxq 2012-08-01
//二合一已更新
include "../model/modelhead.php";
//$AuthResult=mysql_query("SELECT Number FROM $DataIn.authority WHERE Estate=1",$link_id);
//$Auth_Number=mysql_result($AuthResult,0,"Number");
$tableMenuS=600;				//必选参数,功能项列出的起始位置
$From=$From==""?"cw":$From;		//必选参数：是否来自查询结果浏览
$funFrom="ch_express";			//必选参数：功能模块
$nowWebPage=$funFrom."_cw";		//必选参数：功能页面
$Log_Item="快递费";
$MergeRows=0;
switch($Estate){
	case "0":			//已结付处理		
		ChangeWtitle($SubCompany.$Log_Item."已结付记录");$ColsNumber=14;					//必选参数,需处理
		$sumCols="7";			//求和列,需处理
		$MergeRows=8;
		$Th_Col="更新|35|结付日期|75|结付凭证|35|结付回执|35|结付备注|35|结付总额|70|结付银行|100|所属公司|60|选项|40|序号|40|寄件日期|80|快递公司|80|提单号码|100|件数|40|重量|50|金额|60|寄/到付|60|经手人|60|备注|50";
		$EstateSTR0="selected";
		$ActioToS="1,15,16";
		include $funFrom."_cw0.php";
		break;
	default:			//未结付处理
		ChangeWtitle($SubCompany.$Log_Item."未结付记录");$sumCols="8";			//求和列,需处理
		$ColsNumber=13;					//必选参数,需处理
		$Th_Col="选项|50|序号|40|所属公司|60|寄件日期|80|快递公司|80|提单号码|100|件数|40|重量|50|金额|60|寄/到付|60|经手人|60|备注|250|状态|40|操作|60";
		$Estate=3;
		$EstateSTR3="selected";
		if($Login_P_Number=='10871'||$Login_P_Number=='10006'){$ActioToS="1,18,15";}
		else{$ActioToS="1,18";}
		include $funFrom."_cw3.php";
		break;
	}
?>