<?php 
//步骤1 $DataIn.errorcasedata 二合一已更新$DataIn.电信---yang 20120801
...
$upDataSheet="$DataIn.stuffqcstandard";	//需处理
$Log_Funtion="更新";
...
switch($ActionId){
	...
	case 82:
		...
		if($_POST['ListId']){//如果指定了操作对象
			...
			//将productqcimg表中以前上传的记录，删除。
			 $delSql="delete from stuffqcimg where QcId='$Id' OR StuffId IN ($Ids)";
			 $delResult=mysql_query($delSql);
                         
		     $inRecode="INSERT INTO $DataIn.stuffqcimg SELECT NULL,StuffId,'$Id','$Date','$Operator','1','0','0','$Operator','$DateTime',null,null FROM $DataIn.Stuffdata WHERE 1 $TypeIdSTR";
	...
	default:
		if($Attached!=""){//有上传文件
			...
			$OldFile=$Attached;
			$FilePath="../download/stuffQCstandard/";
