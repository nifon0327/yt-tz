<?php
$mainData="$DataIn.cw21_housefeemain";
$sheetData="$DataIn.cw21_housefeesheet";
...
$mySql="SELECT 
	...
	S.Id,S.Mid,S.Month,S.Amount,S.Date,S.Estate,S.Locks,S.Operator,P.Name,B.Title,S.Remark,BD.Name AS BranchName,J.Name AS JobName,S.Attached,S.Number,M.cSign
	FROM $mainData M
...
if($mainRows = mysql_fetch_array($mainResult)){
	...
	do{
		...
		$ImgDir="download/staffhouseinfo/";
		...
		if($Attached!="" ){
			 ...
			 $d1=anmaIn("download/staffhouseinfo/",$SinkOrder,$motherSTR);
		...
		if($Keys & mUPDATE || $Keys & mLOCK){
			$upMian="<img location.href=\"#\"' style='CURSOR: pointer' onclick='upMainData(\"staffhouse_subsidy_upmain\",$Mid)' src='../images/edit.gif' title='更新结付单资料!' width='13' height='13'>";
			}
		else{
			$upMian="&nbsp;";
			}
		...
		if($midDefault!="" && $midDefault==$Mid){//同属于一个主ID，则依然输出明细表格
			...
			$m=$m+2;
			echo"<td class='A0000' width='' align='center'>$Date</td>";
			echo"</tr></table>";
			$i++;
			}
		else{
