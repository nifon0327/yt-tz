...
//end of changes by mmp
    ...
    function getExcelResultBySheetNo($sheetNo) {
        $result = array();
        for ($i = 1; $i <= $this->sheets[$sheetNo]['numRows']; $i++) {
            for ($j = 1; $j <= $this->sheets[$sheetNo]['numCols']; $j++) {
                $result[$i][$j] = $this->sheets[$sheetNo]['cells'][$i][$j];
            }
        }
        return $result;
    }
