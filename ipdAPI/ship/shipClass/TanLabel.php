...
class TanLabel{
    ...
    public function getLabel($originalLabel, $labelInfomation, $DataIn, $DataPublic){
        ...
        $shipType = '';
        $printType = $labelInfomation['printType'];
        if($printType == 'emu'){
            $typeArray = explode('-', $labelInfomation['InvoiceNO']);
            if(strtoupper($typeArray[1])=='AIR'){
                $shipType = 'AIR';
            }
        }else if($printType == 'ship'){
            $invoiceWiseSql = "SELECT Wise FROM $DataIn.ch1_shipmain Where InvoiceNO = '".$labelInfomation['InvoiceNO']."' Limit 1";
            $invoiceWiseResult = mysql_query($invoiceWiseSql);
            $invoiceWise = mysql_fetch_assoc($invoiceWiseResult);

            $shipType = (strpos(strtoupper($invoiceWise["Wise"]), "AIR") != '')?"AIR":"";  
        }
