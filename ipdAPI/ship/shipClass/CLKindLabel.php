...
class CLKindLabel{
    ...
    public function getLabel($originalLabel, $labelInfomation, $DataIn, $DataPublic){
        ...
        //先搜索lotto码
        $companyId = $labelInfomation['companyIdStr'];
        $getLottoCodeSql = "SELECT lotto from ch_initallotto where companyId = $companyId";
        $getLottoResult = mysql_query($getLottoCodeSql);
        $lottoResult = mysql_fetch_assoc($getLottoResult);
        $this->defaultLotto = $lottoResult['lotto'];
