<?php
	...
	$leaveList = array();
	$today = date("Y-m-d");
	$checkLeaveSql = "SELECT B.Number, B.Name, A.StartDate, A.EndDate, B.cSign, A.bcType, A.Type, C.GroupName, D.Name as WorkAdd
	...
	while($checkLeaveRow = mysql_fetch_assoc($checkLeaveResult))
	{
		...
		$leaveList[] = array("Name"=>"$name", "Number"=>"$number", "State"=>"$leaveState", "Type"=>"$type", "GroupName"=>"$workAdd-$groupName");
