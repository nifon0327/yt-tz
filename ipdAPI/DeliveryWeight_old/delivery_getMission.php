<?php
    ...
    $myResultSql="SELECT R.StuffId, Sum(R.cgQty) as cgQty, Sum(R.Qty) as Qty, Sum(R.restQty) as restQty, R.StuffCname, R.Forshort, R.CompanyId, R.FrameCapacity FROM (
                ...
                P.Forshort, P.CompanyId,D.StuffCname,D.Picture,D.TypeId,YEARWEEK(G.DeliveryDate,1) AS Weeks,H.DateTime,(S.Qty - IFNULL(SUM(C.Qty), 0)) as restQty
                ...
                WHERE  S.Estate=2  AND M.Floor='$Floor'  AND S.SendSign IN(0,1)  AND  H.LineId='$LineId' GROUP BY S.Id Having restQty > 0
                ORDER BY Estate,QcDate,shDate,H.DateTime,Weeks,ReduceWeeks) AS R Group By R.StuffId";
    //echo $myResultSql;
    ...
    while($myRow = mysql_fetch_assoc($myResult)){
        ...
        $missionList[] = array('StuffId'=>"$StuffId", 'StuffName'=>"$stuffName", 'shQty'=>"$shQty", 'recordQty'=>"$DjQty", 'frameCapacity'=>"$FrameCapacity", 'forShort'=>"$Forshort", 'companyId'=>"$CompanyId", 'week'=>"$thisWeek", 'property'=>$propertis, 'positions'=>$states);
    }
