<?php
	...
	//$CheckSign = '0';
	$CheckPosition = $CheckSign=='1'?'(3)':'(6,12)';
	
	$checkBill = array();
	$checkBillSql = "SELECT S.Id,S.StockId,S.Qty,S.StuffId,S.SendSign,D.StuffCname,D.Picture,T.AQL,(G.AddQty+G.FactualQty) AS cgQty,M.Date,D.TypeId,Y.ProductId,G.POrderId,M.CompanyId,P.Forshort, M.BillNumber, PI.Leadtime, G.DeliveryDate
					 ...
					 WHERE S.Estate=2  
					 And D.SendFloor in $CheckPosition
					 ORDER BY S.StuffId";
	//echo $checkBillSql;	
	...
	if(mysql_num_rows($billResult)>0)
	{
		...
		while($billRows = mysql_fetch_assoc($billResult))
		{
			...
			$shDateSql = mysql_query("SELECT * FROM $DataIn.gys_shdate WHERE Sid = $Id Order by Id DESC Limit 1");
			$shDateResult = mysql_fetch_assoc($shDateSql);
			$shDate = $shDateResult["shDate"];
...
*/
	    	...
	    	if(mysql_num_rows($checkExpress) > 0 && $POrderId != ""){
	    		//echo "SELECT Type FROM $DataIn.yw2_orderexpress WHERE POrderId='$POrderId' AND Type=2 LIMIT 1 <br>";
	    		//echo 
				continue;		    
			}
            ...
            $tmpLine[] = array("stockId"=>"$StockId", "stuffCname"=>"$StuffCname", "cgQtyCount"=>"$cgQtyCount", "noQtyCount"=>"$noQtyCount", "qtyCount"=>"$QtyCount", "note"=>"$Remark", "AQL"=>"$AQL", "Id"=>"$Id", "stuffId"=>"$StuffId", "productId"=>"$ProductId", "hasError"=>"$hasError", "lockMark"=>"$lockMark", "picture"=>"$Picture", "shDate"=>"$shDate", "isLast"=>"$isLastBgColor", "history"=>"$historyOrderCount", "companyId"=>"$companyId", "companyName" => "$companyName", "billNumber"=>"$billNumber", "addBuyTag" => "$addBuyTag", "TypeId" => "$TypeId", "piDate"=>"$piDate", "piWeek"=>"$leadTime", "property" => "$sProperty", "LastBgColor"=>"$LastBgColor", 'recordCount'=>"$recordQty");
