<?php
	...
	//$POrderId = '201409122506';
	$products = array();
	
	$checkProduct=mysql_query("SELECT Y.ProductId,Y.OrderPO,Y.Qty as PQty,Y.PackRemark,Y.sgRemark,Y.ShipType,PI.Leadtime,C.Forshort AS Client,P.cName,P.TestStandard,P.eCode,P.Weight,U.Name as Unit    
    ...
    {    	
	    ...
	    $eCode = $checkProductRows["eCode"];
	    $weight = $checkProductRows["Weight"];
	    $Unit = $checkProductRows["Unit"];
	    ...
	    $products[] = array("client" =>"$Client:$cName", "headInfomation"=>$lineTwoText, "productId"=>"$ProductId", "eCode"=>"$eCode", "testStandard"=>"$TestStandard", "netWeight"=>"$weight", "boxPcs"=>"$boxPcs ".$Unit, "extraWeight"=>"$extraWeight");
	...
	M.Date,A.StuffCname,A.Picture,A.Gfile,A.Gstate,A.TypeId,B.Name,C.Forshort,C.Currency,MP.Remark AS Position,ST.mainType,MT.TypeColor ,K.tStockQty 
	    ...
	    while($StockRows = mysql_fetch_assoc($sListResult))
	    {
            ...
            else
            {
       		...
       		$products[] = array("State"=>"$Date", "StockId"=>"$StockId", "StuffCname"=>"$StuffCname", "OrderQty"=>"$OrderQty", "rkQty"=>"$rkQty", "tStockQty"=>"$tStockQty", "llQty"=>"$llQty", "scQty"=>"$scQty",  "StuffId"=>"$StuffId", "Picture"=>"$Picture", "TypeId"=>"$TypeId", 'supplier'=>"$Forshort");
	   }
