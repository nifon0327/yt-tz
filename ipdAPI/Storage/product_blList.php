<?php
	...
	$mySql="SELECT A.*,C.Forshort, C.PickNumber,P.cName,P.TestStandard, P.Weight, P.eCode,PI.Leadtime FROM
			(
				SELECT M.CompanyId,M.OrderPO,M.OrderDate,S.Id,S.POrderId,S.ProductId, S.sgRemark, S.PackRemark,S.Qty,S.Price,SUM(G.OrderQty) AS blQty,IFNULL(SUM(L.Qty),0) AS llQty, SUM(L.Estate) as llEstate,Count(S.POrderId) as count
	...
	while($registrRow = mysql_fetch_assoc($registrListResult))
	{
		...
		else if($llEstate > 0 && $llEstate == $count)
		{
			$canUsed = "yes";	
		}
		else
		...
		$products[] = array("OrderPO"=>"$orderPO", "CName"=>"$productName", "ECode"=>"$productEcode", "Qty"=>"$qty", "Note"=>"$note", "OrederDate"=>"$orderDate", "TestStandar"=>"$TestStandardIpad", "ProductId"=>"$productId", "POrderId"=>"$POrderId", "companyId"=>"$companyId", "companyName"=>"$companyShort", "ProductType"=>"$productType", "BLDate"=>"$BlDate", "LockState" => "$lockState", "NetWeight"=>"$weight", "boxPcs"=>"$boxPcs"." PCS", "printFormatter" => "$paramters", "printValue" => "$printValue", "qrFormat"=> "$qrFormat", "pickerNumber"=>"$pickNumber", "Processes" => $procsses, "extraWeight"=>"$extraWeight", "packRemark" => "$packRemark", "OverQty" => "$overQty", "PI" => "$piDate", "mission"=>"$mission", "canUsed"=>"$canUsed", "PIWeek"=>"$piWeek");
