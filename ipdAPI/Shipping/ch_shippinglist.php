...
M.Id,M.CompanyId,M.Number,M.InvoiceNO,M.InvoiceFile,M.CompanyId,M.Wise,M.Date,M.Estate,M.Locks,M.Sign,M.Operator,C.Forshort,C.PayType,S.InvoiceModel,B.Name
	...
	while($shipRow = mysql_fetch_assoc($shippingResult))
	{
		...
		//获取该送货单下的产品
		$shipProductSql = "SELECT S.Id,S.POrderId,O.OrderPO,P.cName,P.eCode,S.Qty,S.Price,S.Type,S.YandN,P.Weight AS Weight,M.Date,E.Leadtime,P.TestStandard,P.MainWeight,P.Code,N.OrderDate AS OrderDate ,P.ProductId,N.ClientOrder, H.EndPlace, H.StartPlace,O.PackRemark as PackRemark,P.Description,M.Ship
						   ...
						   AND S.Type='1'
						   UNION ALL
						   SELECT S.Id,S.POrderId,O.SampPO AS OrderPO,O.SampName AS cName,O.Description AS eCode,S.Qty,S.Price,S.Type,S.YandN,O.Weight AS Weight,M.Date,'' AS Leadtime,'' AS TestStandard,'' AS MainWeight, '' as Code,O.Date AS OrderDate  ,'' AS ProductId,'' AS ClientOrder, H.EndPlace, H.StartPlace,''as PackRemark,'' as Description,'' as Ship
						   FROM $DataIn.ch1_shipsheet S 
						   LEFT JOIN $DataIn.ch1_shipmain M ON M.Id=S.Mid 
						   ...
						   AND S.Type='2'
						   UNION ALL
						   SELECT S.Id,S.POrderId,'' AS OrderPO,O.Description AS cName,O.Description AS eCode,S.Qty,S.Price,S.Type,S.YandN,'0' AS Weight ,M.Date,'' AS Leadtime,'' AS TestStandard,'' AS MainWeight, '' as Code,O.Date AS OrderDate,'' AS ProductId,'' AS ClientOrder, H.EndPlace, H.StartPlace, '' as PackRemark, '' as Description, '' as Ship
						   FROM $DataIn.ch1_shipsheet S 
						   LEFT JOIN $DataIn.ch1_shipmain M ON M.Id=S.Mid 
		...
		while($shipProductRow = mysql_fetch_assoc($shipProductResult))
		{
			...
			$shipType = strtoupper($shipTypeRow["name"] == "SEA")?strtoupper($shipTypeRow["name"]):"";		
			...
			*/
			...
			{ 
				...
				if($CompanyId == "1089" || $CompanyId == "1066" || $CompanyId == "1064" || $CompanyId == "1024" || $CompanyId == "1081" || $CompanyId == "1080" || $CompanyId == "1077" || $CompanyId == "1073" || $CompanyId == "1092" || $CompanyId == "1013" || $CompanyId == "1046" || $CompanyId == "1084" || $CompanyId == "100032" || $CompanyId == "1049" || $CompanyId == '1076' || $CompanyId == '1098' || $CompanyId == '100057' || $CompanyId == '1093'){
					$labelName = $CompanyId;
				}
				...
				if($CompanyId == '1049' || $CompanyId == '1098'){
					...
					$printLabel = str_replace("*DeviceType", $pMark[2], $printLabel);
				}
			...
			{
				$totleWeightAverage = number_format(($productInBox*$Weight)/1000+1,2);
			}
