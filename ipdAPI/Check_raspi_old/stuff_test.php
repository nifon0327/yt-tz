<?php
    ...
    $myResultSql="SELECT Sum(A.Qty) as Qty, SUM(A.recordQty) as recordQty, A.QcDate, A.shDate, A.FrameCapacity, A.CompanyId, A.Forshort, A.StuffCname, A.Name, A.uploadtime From
            ...
            M.CompanyId,P.Forshort,D.StuffCname,D.Picture,D.TypeId,D.Date as uploadtime,
            Max(IFNULL(C.Date,Now())) AS QcDate,Max(T.shDate) AS shDate,D.FrameCapacity, staff.Name
    ...
    $result = array('name'=>"$stuffname", 'stuffId'=>"$stuffId", 'supplier'=>"$supplier", 'buyer'=>"$buyer", 'uploadtime'=>$uploadtime, 'storeIn'=>$storeIn, 'frameCapacity'=>"$FrameCapacity", 'timelap'=>"$timelap", 'shQty'=>"$shQty", 'recordQty'=>"$recordQty", 'state'=>$states);
