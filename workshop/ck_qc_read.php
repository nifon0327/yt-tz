...
$myResult=mysql_query("SELECT  S.Id,S.Mid,S.StuffId,S.StockId,(G.AddQty+G.FactualQty) AS cgQty,S.Qty,S.SendSign,G.POrderId, 
		   ...
		   Max(IFNULL(C.Date,Now())) AS QcDate,IFNULL(W.ReduceWeeks,1) AS ReduceWeeks,H.Estate,L.LineNo,Max(T.shDate) AS shDate    
			...
			WHERE  S.Estate=2  AND M.Floor='$Floor'  AND S.SendSign IN(0,1)  AND C.Sid>0 GROUP BY S.Id ORDER BY Estate,QcDate,shDate,H.DateTime,Weeks,ReduceWeeks",$link_id);
while($myRow = mysql_fetch_array($myResult)) {
         ...
         if ($m<10){
			     ...
			     $shDate=$myRow["shDate"];
			     $DateChars="到";
			     $DateStr=GetDateTimeOutString($shDate,$myRow["QcDate"],1);
				 ...
				 $ListSTR.="<table id='ListTable$m' name='ListTable[]' class='$tableClass'>
					<tr>
					    ...
					    <td colspan='4' width='920' class='title'>$Forshort-$StuffCname</td>
					    <td class='time' width='40'><div>$LineNo</div></td>
				   </tr>
				   <tr>
					    ...
					    <td width='240' class='time $shColors'>$DateStr</td>
...
$WaitQty=number_format($WaitQty);
 ...
 $QtyResult =mysql_fetch_array(mysql_query("SELECT SUM(S.Qty) AS Qty   FROM $DataIn.qc_cjtj S  
			WHERE  DATE_FORMAT(S.Date,'%Y-%m-%d')='$curDate' AND S.LineId IN ($LineId)
",$link_id));
...
$curMonth=date("Y-m");
$QtyResult =mysql_fetch_array(mysql_query("SELECT SUM(S.Qty) AS Qty   FROM $DataIn.qc_cjtj S  
			WHERE  DATE_FORMAT(S.Date,'%Y-%m')='$curMonth' AND S.LineId IN ($LineId)  
",$link_id));
$MonthQty=$QtyResult["Qty"]==""?0:number_format($QtyResult["Qty"]);

	
$WeekName="<img src='image/qc.png' style='width:160px;height:160px;margin-bottom:15px;'>";
...
<div id='headdiv' style='height:260px;'>
   ...
   <ul id='quantity3' class='float_right' style='padding-top:50px;'>
             <li class='text_left'><span style='color:#007C2D'><?php echo $MonthQty; ?></span></li>
   ...
   <ul id='count' class='border3'>
           <li style='width:540px;'>待处理 <div></div><br><span class='red_color'><?php echo $QcedQty; ?> </span><span><?php echo  "($QcedCount)"; ?> </span></li>
	       <li style='width:540px;'>品检中 <br><span><?php echo $CurQty; ?></span><span><?php echo "($CurCount)"; ?></span></li>
	 </ul>
</div>
