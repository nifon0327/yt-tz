...
//获取拉线组长
$leaderNumberSql = "SELECT Name, Number FROM $DataIn.staffmain WHERE GroupId = 604 and JobId = 39";
...
$WaitQty=0; $WaitCount=0;   
 //品检任务
$mySql = "SELECT * From (SELECT  S.Id,S.Mid,S.StuffId,S.StockId,(G.AddQty+G.FactualQty) AS cgQty,S.Qty,S.SendSign,G.POrderId, 
           ...
           Max(IFNULL(C.Date,Now())) AS QcDate,IFNULL(W.ReduceWeeks,1) AS ReduceWeeks,H.Estate,L.LineNo,S.shDate , D.CheckSign
            ...
            WHERE  S.Estate=2  AND M.Floor = $Floor AND D.ComboxSign!=1 AND L.LineNo is null GROUP BY S.Id 
            Union ALL
            SELECT  S.Id,S.Mid,S.StuffId,S.StockId,(G.AddQty+G.FactualQty) AS cgQty,S.Qty,S.SendSign,G.POrderId, 
            IF(G.StockId>0,G.DeliveryDate,CG.DeliveryDate) AS  DeliveryDate,
            M.CompanyId,P.Forshort,D.StuffCname,D.Picture,D.TypeId,YEARWEEK(IF(G.StockId>0,G.DeliveryDate,CG.DeliveryDate),1) AS Weeks,H.DateTime,
           Max(IFNULL(C.Date,Now())) AS QcDate,IFNULL(W.ReduceWeeks,1) AS ReduceWeeks,H.Estate,L.LineNo,S.shDate , D.CheckSign
            FROM $DataIn.gys_shsheet S 
            INNER JOIN $DataIn.gys_shmain M ON S.Mid=M.Id 
            LEFT JOIN cg1_stuffcombox co ON co.StockId=S.StockId
            LEFT JOIN $DataIn.cg1_stocksheet  G ON G.StockId=co.mStockId 
            LEFT JOIN  $DataIn.cg1_stockmain GM ON GM.Id=G.Mid 
            LEFT JOIN $DataIn.stuffdata D ON D.StuffId=S.StuffId 
            LEFT JOIN $DataIn.trade_object P ON P.CompanyId=M.CompanyId 
            LEFT JOIN $DataIn.qc_mission H ON H.Sid=S.Id 
            ...
            ORDER BY AA.Estate,AA.QcDate,AA.shDate,AA.DateTime,AA.Weeks,AA.ReduceWeeks";
            //
...
while($myRow = mysql_fetch_array($myResult)) {
        ...
        if ($m<12){
            ...
            if($shDate == ""){
                ...
                $DateChars="到";
                $DateStr=$shDate!=''?GetDateTimeOutString($shDate,""):'';
            ...
            if($CheckSign == 0){
            $checktypeName = 'partCheck';
            }else{
            $checktypeName = 'allCheck';
            }
            $checkTypeImg = "<img style='margin-top:10px;margin-right:-30px;width:35px;height:35px;' src='image/$checktypeName.png'>";
            ...
            <tr height='40' style='vertical-align:top;'>
                ...
                <td class='time static_qty_ac' width='300' style='padding-top:-5px;padding-right:55px;'>$Qty $checkTypeImg</td>
            </tr>";
            // <tr>
            ...
            // </tr>
             ...
             //备注 
             $Remark="";
             $RemarkResult=mysql_query("SELECT Remark  FROM $DataIn.gys_shremark WHERE  Sid='$Id' ORDER BY Date DESC LIMIT 1",$link_id);
            ...
            if ($bpRemark!="" || $Remark!=""){
                    ...
                    $ListSTR.="<tr height='40'>
                                    <td colspan='4' class='remark_qc' style='margin-top:-10px;padding-left:15px;'><img class='float_left' src='image/remark.png'/><div class='float_left' style='padding-bottom:0px;'>$bpRemark $Remark</div></td>
                                    <td></td>
                                    <td class='time_qc float_right'><span class='$typeColor' style='font-size:30px;margin-right:60px;'>$DateStr</span></td>
                                </tr>";
            }
            else{
                    $DateStr= $factoryCheck=='on'?'':$DateStr;
                    $ListSTR.="<tr><td colspan='4' style='height:30px;line-height:40px;'>&nbsp;</td>
                                    <td  class='remark_qc text_right'><span class='$QcDateColor'>$QcDateStr</span>&nbsp;</td>
                                    <td class='time_qc float_right'><span class='$typeColor' style='font-size:30px;margin-right:60px;'>$DateStr</span></td>
                                </tr>";
            }                        
...
//部门人数统计
$staffStaticSql = "SELECT count(*) as count, '1' as sign FROM $DataIn.staffmain WHERE workAdd=1 AND GroupId=604 AND Estate =1 
                   Union ALL
                   ...
                   AND B.workAdd = 1 AND B.GroupId = 604 AND B.Estate =1
                  ";
...
$staffCount -= $leaveCount;

$WeekName="<img src='../download/staffPhoto/P11825.png' style='height:125px;margin-bottom:0px;'>";
