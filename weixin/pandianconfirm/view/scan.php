...
<head>
  ...
  #exception{border: solid 1px gray; display: inline-block; width: 70%;line-height: 45px; font-size:16px;
    border-radius: 3px; background-color: teal; color: white; margin-top: 10px;margin-bottom: 50px;}
  </style>
</head>
<body>
  <header>扫描垛牌</header>
  <article>
    ...
    <button id="exception">点击查看所有异常构件</button>
  ...
  (function($,POP,SERVICE,undefined){
    var serviceUrl = '<?php $_SERVER['HTTP_HOST']?>/weixin/pandianconfirm/controller/index.php';
    var module = {
      doms: {
        scan:$('#scan'),
        jump:$('#jump')
