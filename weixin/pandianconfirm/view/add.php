...
<body>
  ...
  (function($,POP,SERVICE,undefined){
    var stackId, stackNo, selectedRow =[], serviceUrl = '<?php $_SERVER['HTTP_HOST']?>/weixin/pandianconfirm/controller/index.php',
    ...
    var module = {
      ...
      setupSearchOption:function(id,value){
        if(!value) return false

        $('#' + id + ' option').each(function(){
          if($(this).attr('param') == value) {
      ...
      setupLastSearch:function(lastSearch){
        ...
        if(lastSearch.lastSearch.ProductName){
          $('#productCode').val(lastSearch.lastSearch.ProductName)
        }
      },
      retrieveLastSearch:function(){
        var _self = this
        SERVICE.sendSHR(serviceUrl,{action:'getLastSearch',stackId:stackId}, function(oData){
          if(oData.result && oData.result.lastSearch.TradeId){
      ...
      retrieveStackData:function(){
        ...
        SERVICE.sendSHR(serviceUrl,{action:'getStackInfo',stackId:stackId}, function(oData){
          if(oData.result && oData.result.length>0){
            _self.updateStackData(oData.result[0].Creator,oData.result[0].DoubleCheckUser)
          }
        })
      },
            ...
            POP.ShowConfirm('请确认是否保存已选构件', '确定', '取消', function(){
              ...
              SERVICE.sendSHR(serviceUrl,{action:'addProductsToStack',stackId:stackId,productIds:JSON.stringify(oArray)}, function(oData){
                POP.ShowAlert('添加成功','确定',function(){
                  window.location.href = './stack.php?stackId=' + stackId + '&stackNo=' + stackNo
                })
              })
            })
          }
