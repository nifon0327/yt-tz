...
<footer>
    <button id="seat-save">盘点移库</button>
...
<script type="text/javascript">
    (function ($, POP, SERVICE, undefined) {
        var stackId,
            ...
            serviceUrl = '<?php $_SERVER['HTTP_HOST']?>/weixin/inventoryConfirm/controller/index.php',
            pandianResult = {}
        var module = {
            doms: {
                table: $('table'),
            ...
            generateStackSelector: function (stacklist) {
                ...
                if (stacklist && stacklist.length > 0) {
                    for (var i = 0; i < stacklist.length; i++) {
                        _html += '<option param="' + stacklist[i].StackId + '">' + stacklist[i].StackId + '</option>'
                    }
                }
            ...
            saveStackChange: function () {
                ...
                SERVICE.sendSHR(serviceUrl, {
                    action: 'moveProduct',
                    originStackId: seatSelectedIdMove,
                    stackId: stackIdSelected,
                    productIds: JSON.stringify(oArray)
                }, function (oData) {
            ...
            generateSeatSelector: function (seatlist, selectedSeatId) {
                ...
                this.doms.btnSeatSave.show().off('click').on('click', function () {
                    if (!seatSelectedId) {
                        ...
                        POP.ShowConfirm('确定整垛移动库位？', '确定', '取消', function () {
                            _self.updateSeatId();
                        })
                    }
                })
            ...
            retrieveStackSeat: function () {
                var _self = this
                SERVICE.sendSHR(serviceUrl, {action: 'getStackSeat', stackId: stackNo}, function (oData) {
            ...
            retrieveStackData: function () {
                var _self = this
                SERVICE.sendSHR(serviceUrl, {action: 'getStackInfo', stackId: stackNo}, function (oData) {
            ...
            updateStackData: function (data) {
                ...
                for (i = 0; i < data.length; i++) {
                    temp.push({
                        ...
                        floor: data[i].BuildingNo + '#' + data[i].FloorNo,
                        name: data[i].CmptNo
                    })
                }
                list = temp;
