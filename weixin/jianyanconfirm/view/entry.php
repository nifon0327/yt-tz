...
<body>
  ...
  (function($,POP,SERVICE,undefined){
    var stackId, serviceUrl = '<?php $_SERVER['HTTP_HOST']?>/weixin/jianyanconfirm/controller/index.php',
    ...
    var module = {
      ...
      checkRecord: function(recordNumber,item){
        SERVICE.sendSHR(serviceUrl,{action:'confirm',recordNumber:recordNumber,item:item,productionLine:lineSelected}, function(oData){
          if(oData.result){
            if(oData.result.exist === 1){
              POP.ShowConfirm('记录编号已存在', '重新输入', '查看', null, function(){
                window.location.href = './produce.php?id=' + oData.result.id
              })
            }else{
