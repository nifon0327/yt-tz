...
<body>
  ...
  (function($,POP,SERVICE,undefined){
    var serviceUrl = '<?php $_SERVER['HTTP_HOST']?>/weixin/inspectionconfirm/controller/index.php',
      ...
      productSelector, productSelected;
    var module = {
      doms: {
      ...
      initProductName: function(list) {
        console.log(list)
        var _self = this
        productSelector = new MobileSelect({
          trigger: '#name',
          title: '产品名称',
          wheels: [{data:list}],
          keyMap: {
          ...
          callback:function(indexArr, data){
            productSelected = data[0].NameRule
          }
        });
      ...
      checkRecord: function(recordNumber){
        SERVICE.sendSHR(serviceUrl,{
          ...
          recordName:productSelected?productSelected:'',
          status:pageType,
          ...
          if(oData.result){
            if(oData.result[1].exist === 1){
              POP.ShowConfirm('记录编号已存在', '重新输入', '查看', null, function(){
                window.location.href = './finished.php?recordId=' + oData.result[0].Id
              })
            }else{
