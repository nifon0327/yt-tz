  ...
  (function($,POP,SERVICE,undefined){
    ...
    var module = {
      ...
      retrieveStatusName: function(status){
        ...
        if(!!status && status == 1){
          text = '未审核'
        } else if (!!status && (status == 2 || status == 0)){
          text = '合格'
      ...
      addProductByName: function(productname){
        var _self = this
        SERVICE.sendSHR(serviceUrl,{
          action:'insertInspectionProductByQrCode',
          inspectionRecordId:recordId,
          workShopId:lineId,
      ...
      bind:function(){
        ...
        this.doms.btnAdd.on('click',function(){
          window.location.href = './finished_add.php?recordId=' + recordId + '&lineId=' + lineId
        })
        ...
        //   _self.addProductByName('5-24-YLB-C-2b-776')
        // })
