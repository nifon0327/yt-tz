...
<body>
  ...
  (function($,POP,SERVICE,undefined){
    var recordId, lineId, selectedRow =[], serviceUrl = '<?php $_SERVER['HTTP_HOST']?>/weixin/inspectionconfirm/controller/index.php',
      pageType = 1, // 1 - 成品
    ...
    var module = {
      ...
      init: function(){
        ...
        lineId = this.getUrlParameter('lineId')
        if(!recordId || !lineId) {
      ...
      getLastSearch: function(){
        ...
        SERVICE.sendSHR(serviceUrl,{
          ...
          if(oData.result && oData.result.lastSearch && oData.result.trade){
            _self.setupLastSearch(oData.result)
            _self.search()
          } else {
            _self.retrieveOptionData({action:'getCompanyForShort'},'generateProjectOptions')
          }
        })
      },
      setupLastSearch:function(lastSearch){
        if(lastSearch.lastSearch.TradeId){
          tradeId = lastSearch.lastSearch.TradeId
          this.generateProjectOptions(lastSearch.trade,lastSearch.lastSearch.TradeId)
          if(lastSearch.lastSearch.BuildingNo){
            buildingNo = lastSearch.lastSearch.BuildingNo
            this.generateBuildingOptions(lastSearch.building,lastSearch.lastSearch.BuildingNo)
            if(lastSearch.lastSearch.FloorNo){
              floorNo = lastSearch.lastSearch.FloorNo
              this.generateFlourOptions(lastSearch.floor,lastSearch.lastSearch.FloorNo)
              if(lastSearch.lastSearch.CmptTypeId){
                typeId = lastSearch.lastSearch.CmptTypeId
                this.generateTypeOptions(lastSearch.type,lastSearch.lastSearch.CmptTypeId)
              } else if(lastSearch.type){
                this.generateTypeOptions(lastSearch.type)
              }
            } else if(lastSearch.floor) {
              this.generateFlourOptions(lastSearch.floor)
            }
          } else if(lastSearch.building){
            this.generateBuildingOptions(lastSearch.building)
          }
        } else if (lastSearch.trade){
          this.generateProjectOptions(lastSearch.trade)
        }
        if(lastSearch.lastSearch && lastSearch.lastSearch.ProductName){
          $('#productCode').val(lastSearch.lastSearch.ProductName)
        }
      },
        ...
        if(list){
          for(var i=0;i<list.length;i++){
            if(!!target && list[i].BuildingNo === target){
              _html += '<option selected="true" param="' + list[i].BuildingNo +'">' + list[i].BuildingNo + '</option>'
            } else {
              _html += '<option param="' + list[i].BuildingNo +'">' + list[i].BuildingNo + '</option>'
            }
          }
        }
        ...
        if(list){
          for(var i=0;i<list.length;i++){
            if(!!target && list[i].FloorNo === target){
              _html += '<option selected="true" param="' + list[i].FloorNo +'">' + list[i].FloorNo + '</option>'
            } else {
              _html += '<option param="' + list[i].FloorNo +'">' + list[i].FloorNo + '</option>'
            }
          }
        }
        ...
        if(list){
          for(var i=0;i<list.length;i++){
            if(!!target && list[i].TypeId === target){
              _html += '<option selected="true" param="' + list[i].TypeId +'">' + list[i].CmptType + '</option>'
            } else {
              _html += '<option param="' + list[i].TypeId +'">' + list[i].CmptType + '</option>'
            }
          }
        }
      ...
      updateList: function(list){
        ...
        searchResult = list
        for(var i=0; i<list.length;i++){
          temp.push({ProductId: list[i].ProductId,project: list[i].Forshort, name: list[i].cName, CjtjId: list[i].CjtjId})
        }
      ...
      triggerSelectionClick: function(type,param){
        ...
        if (type === 'project'){
          ...
          floorNo = null
          typeId = null
          this.retrieveOptionData({action:'getCompanyBuilding',tradeId:tradeId},'generateBuildingOptions')
        } else if (type === 'building'){
          buildingNo = param
          floorNo = null
          typeId = null
          this.retrieveOptionData({action:'getBuildingFloor',tradeId:tradeId,buildingNo:buildingNo},'generateFlourOptions')
        } else if (type === 'floor'){
          floorNo = param
          typeId = null
          this.retrieveOptionData({action:'getCmptType',tradeId:tradeId,buildingNo:buildingNo,floorNo:floorNo},'generateTypeOptions')
        } else if (type === 'type'){
          typeId = param
        }
      },
        ...
        $('#save').on('click',function(){
          if(_self.selectedRowCheck()){
            POP.ShowConfirm('请确认是否保存已选构件', '确定', '取消', function(){
              ...
              SERVICE.sendSHR(serviceUrl,{
                action:'insertInspectionRecord',
                inspectionRecordId:recordId,
                product:JSON.stringify(oArray),
                status:pageType
              }, function(oData){
                POP.ShowAlert('添加成功','确定',function(){
                  window.location.href = './finished.php?recordId=' + recordId
                })
              })
            })
          }
